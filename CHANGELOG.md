...
- An ABI 19 one-byte construction candidate with exact `u8` boundary behavior, independently verified arena/descriptor/store shape, scalar/result alias rejection, and exact compiler preflight advanced to `Bytesˉfromˉu16ˉlittle`; Windows Standard and pinned-QEMU evidence pass while cross-host qualification remains pending.
