...
- Native compiler execution, Windvale-native platform memory authority, complete execution-support lifetime ownership, the shared JIT/AOT backend, .NET retirement, and a general Windvale OS runtime remain active or planned milestones rather than completed releases; the current OS candidate interprets one fixed WVB in a two-process proof, not a scheduler, general loader/JIT, or complete kernel runtime.
