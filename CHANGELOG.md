...
- Native compiler execution, Windvale-native platform memory authority, complete execution-support lifetime ownership, the shared JIT/AOT backend, .NET retirement, and a general Windvale OS runtime remain active or planned milestones rather than completed releases; the current OS candidate discovers sections in one embedded WVB and interprets a bounded profile in a two-process proof, not a runtime-supplied loader, scheduler, general interpreter/JIT, or complete kernel runtime.
