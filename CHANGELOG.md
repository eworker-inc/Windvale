...
- Nominal record and enum identities throughout native machine IR plus a deterministic record-storage planner; exact compiler liveness projects a 1,489-cell worst frame under the unchanged ABI-20 2,048-cell ceiling without yet changing record allocation or claiming full native reproduction.
...
- A standalone `/playground/wasm-demo/` route that checks and executes the pinned 1,923-byte profile-5 Windvale-generated artifact without loading Blazor or .NET, while keeping Stage 0 artifact-production provenance explicit.
