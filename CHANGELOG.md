...
- Windvale-authored WebAssembly backend profiles: a portable `.wv` selector emits deterministic import-free modules from bounded WVB; execution ABI 1 preserves checked arithmetic, ABI 2 adds exact instruction limits for structured control and bounded calls, and ABI 3 adds fixed disjoint 4 MiB linear-memory buffers for exact bytes/text identity profiles with strict guest-side UTF-8. Exact result or output validity, Windvale failure status, and instruction accounting are preserved without an engine trap.
