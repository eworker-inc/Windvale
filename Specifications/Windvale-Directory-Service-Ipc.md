...
The cross-host `os-services` verification owner compiles the directory core, hosted bridge, snapshot verifier, snapshot service, and hosted snapshot bridge through explicit Project 2 manifests. Its portable behavior WVB verifies a successful exact chunk, missing name, invalid offset, invalid name and limit non-invocation, and malformed snapshot rejection. The former managed differential, 512-input containment, and transport-lifecycle results are preserved as historical qualification evidence in the recovery release rather than claimed as live `main` tests.
