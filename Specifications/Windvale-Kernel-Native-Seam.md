...
The builder independently decodes the assembled WVA objects, paging/process objects, both bridge objects, and all portable native objects and requires their exact architecture, section, symbol, code, and relocation shapes before linking. On normal probe 28, the loader emits final success and shutdown only after admission, Windvale selection, immutable grant, init block, section-derived user-space interpretation, client send/exit, terminal borrow cleanup, init wake/receive/exit, the retained probe, and Main all return. The two kernel-fault paths retain terminal WVA normalization. The user-fault path instead contains vector 13 in the interpreter process, performs the same cleanup, and still wakes init. The ABI-16 selector and fragment verifier independently validate every portable module, borrowed-byte descriptors and reads where present, empty service lists, and unused zero-length runtime resources before each WVO is admitted.
...
The retained native probe is compiled and AOT-linked on the host; its WVB is not interpreted in the guest. Separately, probe 28 validates the exact 174-byte `Embeddedˉwvbˉprogram` identity, grants its init-owned page, discovers its WVB section payloads, executes the bounded code through the user-space Windvale interpreter, and removes the terminal alias. Reaching `resource-grant=pass`, `resource-revoked=pass`, and `wvb-runtime=interpreted` proves that sequence and result `29`; `native-wvb=pass` still owns the later retained AOT computation. None of these markers claims a general loader, capability-transfer facility, or JIT.
