...
Probe 29's retained WVA version-8 object includes process-specific normalized entries for vectors 6, 13, and 14. The init and client WVA objects use the exact `syscall` instruction statement. Init links to its Windvale `Main`, passes ordered two-resource token `131073` to fixed grant syscall `4`, then receives and exits; the client links to the Windvale bytecode-interpreter export inside its bounded 33-page RX image.

The kernel memory object calls the exception installer and paging installer after switching to the measured four-page kernel-owned stack, then calls `Windvale_kernel_wva_main`. The paging installer constructs the unchanged checked six-page hierarchy and calls the two WVA paging functions. The inbound WVA Main tail transfer reaches admission bridge version 2, which retains the 8,944/2 admission context and token 73. The protected-process path then runs Windvale policy token `97`, reconstructs and revalidates `WVKMEM06` after that generated call, constructs separate init and interpreter roots, places the WVB and four-byte budget in distinct init RO/NX pages, and enters init. Windvale `Initˉresourceˉservice.Main` selects ordered set `(1,2)`; the WVA shim requests one atomic grant and blocks. The client discovers and validates both typed resources, enforces budget `4`, and interprets result `29`. After client exit or contained fault, Stage 0 clears both aliases and the complete private publication, then reloads init's CR3. Result 29 wakes init and permits the retained native bridge. [Windvale-Os-Wvb-Admission.md](Windvale-Os-Wvb-Admission.md), [Windvale-Os-Bytecode-Interpreter.md](Windvale-Os-Bytecode-Interpreter.md), and [Windvale-Protected-Process.md](Windvale-Protected-Process.md) own the exact identities, ordering, and failure rules.
...
The retained native probe is compiled and AOT-linked on the host; its WVB is not interpreted in the guest. Separately, probe 29 validates the exact 174-byte `Embeddedˉwvbˉprogram`, grants its page together with the separate four-byte budget page, executes four opcodes through the user-space interpreter, and removes both terminal aliases. Reaching `resource-grant=pass`, `typed-resources=pass`, `resource-revoked=pass`, and `wvb-runtime=interpreted` proves that sequence and result `29`; `native-wvb=pass` still owns the later retained AOT computation. None claims a general loader, capability-transfer facility, or JIT.
