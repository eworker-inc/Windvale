...
The builder independently decodes the assembled WVA objects, paging/process objects, both bridge objects, and all portable native objects and requires their exact architecture, section, symbol, code, and relocation shapes before linking. On normal probe 25, the loader emits final success and shutdown only after admission, init block, section-derived user-space interpretation, client send/exit, init wake/receive/exit, the retained probe, and Main all return. The two kernel-fault paths retain terminal WVA normalization. The user-fault path instead contains vector 13 in the interpreter process and still wakes the service. The ABI-16 selector and fragment verifier independently validate every portable module, borrowed-byte descriptors and reads where present, empty service lists, and unused zero-length runtime resources before each WVO is admitted.
...
The retained native probe is compiled and AOT-linked on the host; its WVB is not interpreted in the guest. Separately, probe 25 validates the exact 174-byte `Embeddedˉwvbˉprogram` identity, discovers its WVB section payloads, and executes the bounded code through the user-space Windvale interpreter. Reaching `wvb-runtime=interpreted` proves that path returned `29`; `native-wvb=pass` still owns the later retained AOT computation. Neither marker claims a runtime-supplied loader or JIT.
