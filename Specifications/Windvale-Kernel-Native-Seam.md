...
Kernel native seam version 16, WVA seam version 7, admission bridge version 1, and retained bridge 10 are cross-host qualified at exact commit `860c69c`. Probe 22 retains ABI 16 and retained bridge 10 while advancing the candidate composition to WVA seam version 8, admission bridge version 2, and protected-process version 1. [Decision 0091](../Documents/Decisions/0091-First-Protected-Windvale-Process.md) owns that extension; focused Windows and pinned-QEMU evidence pass, while cross-host qualification remains pending. Historical ownership through Decision 0090 remains unchanged. [Decision 0056](../Documents/Decisions/0056-Windvale-Owned-Post-Memory-Evidence.md) records the qualified version-2 bidirectional WVA/WV boundary.
...
Probe 22's WVA version-8 object retains those exports and adds process-specific normalized entries for vectors 6, 13, and 14. The user WVA objects use the new exact `syscall` instruction statement. The kernel WVA object has 14 symbols and eight relative relocations; the process user object links its one call relocation to `Windvale_kernel_embedded_main` inside a one-page user image.

The kernel memory object calls the exception installer and paging installer after switching to the kernel-owned stack, then calls `Windvale_kernel_wva_main`. The paging installer constructs a checked six-page hierarchy and calls the two WVA paging functions. The inbound WVA Main tail transfer reaches admission bridge version 2. That bridge constructs a service-free context with exact budgets 8,944/2, calls AOT Windvale export `Windvale_kernel_wvb_admit`, accepts only token 73, then calls the protected-process entry. The process path runs Windvale policy token 91, constructs the separate root and descriptor state, and executes `Windvale_kernel_embedded_main` at CPL3 behind the user WVA entry. Only exact process result 29 permits a tail transfer to retained native bridge 10. [Windvale-Os-Wvb-Admission.md](Windvale-Os-Wvb-Admission.md) and [Windvale-Protected-Process.md](Windvale-Protected-Process.md) own the exact identities, ordering, and failure rules.
...
The builder independently decodes the assembled WVA objects, paging/process objects, both bridge objects, and all portable native objects and requires their exact architecture, section, symbol, code, and relocation shapes before linking. On normal probe 22, the loader emits final success and shutdown only after admission, CPL3 send/receive/exit, the retained probe, and Main all return. The two kernel-fault paths retain terminal WVA normalization. The user-fault path instead takes vector 13 through the process WVA stub, records the fault, and resumes the saved kernel continuation. The ABI-16 selector and fragment verifier independently validate every portable module, borrowed-byte descriptors and reads where present, empty service lists, and unused zero-length runtime resources before each WVO is admitted.
...
The retained probe is compiled and AOT-linked on the host. The guest does not retain, decode, verify, or JIT that probe's WVB bytes. Probe 22 retains and validates the exact 174-byte `Embeddedˉwvbˉprogram` identity before its derived AOT object executes at CPL3. Reaching `native-wvb=pass` still proves the later retained preverified computation returned the expected packed result; `wvb-admission=pass`, `process=isolated`, and `ipc=pass` own the fixed admission/process evidence. None claims a general loader or JIT.
