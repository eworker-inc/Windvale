...
Kernel native seam version 16, WVA seam version 7, admission bridge version 1, and retained bridge 10 are cross-host qualified at exact commit `860c69c`. Probe 23 retains ABI 16, WVA seam version 8, admission bridge version 2, memory/paging version 2, and retained bridge 10 while advancing the candidate protected-process contract to version 2. [Decision 0092](../Documents/Decisions/0092-First-Windvale-Init-Resource-Service.md) owns the two-root init-service extension; focused Windows and pinned-QEMU evidence pass, while cross-host qualification remains pending. Historical ownership through Decision 0091 remains unchanged.
...
Probe 23's retained WVA version-8 object includes process-specific normalized entries for vectors 6, 13, and 14. The service and client WVA objects use the exact `syscall` instruction statement. Each role-specific object links one call relocation to its Windvale `Main` inside a one-page user image.

The kernel memory object calls the exception installer and paging installer after switching to the kernel-owned stack, then calls `Windvale_kernel_wva_main`. The paging installer constructs a checked six-page hierarchy and calls the two WVA paging functions. The inbound WVA Main tail transfer reaches admission bridge version 2. That bridge constructs a service-free context with exact budgets 8,944/2, calls AOT Windvale export `Windvale_kernel_wvb_admit`, accepts only token 73, then calls the protected-process entry. The process path runs Windvale policy token 92, constructs separate init and client roots, blocks init, runs the admitted client, transfers one message through reduced endpoints, restores init's ABI-16 context, and executes `Initˉresourceˉservice.Main` at CPL3. Only exact result 29 permits a tail transfer to retained native bridge 10. [Windvale-Os-Wvb-Admission.md](Windvale-Os-Wvb-Admission.md) and [Windvale-Protected-Process.md](Windvale-Protected-Process.md) own the exact identities, ordering, and failure rules.
...
The builder independently decodes the assembled WVA objects, paging/process objects, both bridge objects, and all portable native objects and requires their exact architecture, section, symbol, code, and relocation shapes before linking. On normal probe 23, the loader emits final success and shutdown only after admission, init block, client send/exit, init wake/receive/exit, the retained probe, and Main all return. The two kernel-fault paths retain terminal WVA normalization. The user-fault path instead contains vector 13 in the client and still wakes the service. The ABI-16 selector and fragment verifier independently validate every portable module, borrowed-byte descriptors and reads where present, empty service lists, and unused zero-length runtime resources before each WVO is admitted.
...
The retained probe is compiled and AOT-linked on the host. The guest does not retain, decode, verify, or JIT that probe's WVB bytes. Probe 23 retains and validates the exact 174-byte `Embeddedˉwvbˉprogram` identity before its derived AOT object executes at CPL3. Reaching `native-wvb=pass` still proves the later retained preverified computation returned the expected packed result; `wvb-admission=pass`, `processes=isolated`, `init-service=pass`, and `ipc=cross-process` own the fixed admission/process evidence. None claims a general loader or JIT.
...
Candidate firmware probe 23 retains those ABI, WVA, admission, memory, paging, and bridge contracts while advancing protected processes to version 2. The Windvale init-service WVB is 371 bytes with SHA-256 `478dfcd36fed7c8063cfb3f53a6a1362bda5353656339b730be573a1be8f95b0`; its linked user image is 2,302 bytes. The normal process object is 7,988 bytes with SHA-256 `3607d66b7e633027062a9fbb963dbd1a2723c2f7ee77fab235443c7ddd266809`. All 25 focused OS tests and all four pinned-QEMU scenarios pass on Windows. The protected-process and boot-probe specifications record complete artifact identities; cross-host promotion remains pending.
