...
Kernel native seam version 17, WVA seam version 8, admission bridge version 2, retained bridge 10, paging version 3, and interpreter profile number 4 are active in qualified probe 30. The composition advances memory to version 7, protected processes to version 9, resource records to version 4, and retains the OS-private `WVBR002` directory. [Decisions 0099](../Documents/Decisions/0099-Bounded-Native-Frame-Admission.md) and [0100](../Documents/Decisions/0100-First-Reclaimed-And-Reused-Process-Root.md) own the qualified boundaries at exact implementation commit `4a077ab9ebaf2108201927eef3095e87ef2ed907`; Probe 29 remains qualified history.
...
Probe 30's retained WVA version-8 object includes process-specific normalized entries for vectors 6, 13, and 14. The init and client WVA objects use the exact `syscall` instruction statement. Init links to its Windvale `Main`, performs grant/receive twice with ordered token `131073`, then exits; each rebuilt client links to the same Windvale bytecode-interpreter export inside its bounded 33-page RX image.

The kernel memory object calls the exception installer and paging installer after switching to the measured four-page kernel-owned stack, then calls `Windvale_kernel_wva_main`. The paging installer constructs the unchanged checked six-page hierarchy and calls the two WVA paging functions. The inbound WVA Main tail transfer reaches admission bridge version 2, which retains the 8,944/2 admission context and token 73. The protected-process path then runs Windvale policy token `97`, reconstructs and revalidates `WVKMEM07`, constructs init plus client generation 1, and enters init. After the first typed grant, interpretation, and generation-matched cleanup, Stage 0 zeroes/releases the exact 42-page tail, reallocates the identical root, and rebuilds generation 2 from immutable inputs. Init grants again; generation 2 independently interprets result `29`; the second cleanup preserves grant history `2`; and init exits after receiving both results. [Windvale-Os-Wvb-Admission.md](Windvale-Os-Wvb-Admission.md), [Windvale-Os-Bytecode-Interpreter.md](Windvale-Os-Bytecode-Interpreter.md), and [Windvale-Protected-Process.md](Windvale-Protected-Process.md) own the exact identities, ordering, and failure rules.
...
The builder independently decodes the assembled WVA objects, paging/process objects, both bridge objects, and all portable native objects and requires their exact architecture, section, symbol, code, and relocation shapes before linking. On normal probe 29, final success and shutdown require admission, Windvale set selection, atomic typed grant, init block, two resource reads, budgeted interpretation, client send/exit, two-alias cleanup, init wake/receive/exit, retained probe, and Main. The two kernel-fault paths retain terminal WVA normalization. The user-fault path contains vector 13 in the interpreter process, performs the same two-resource cleanup, and still wakes init.
...
The retained native probe is compiled and AOT-linked on the host; its WVB is not interpreted in the guest. Separately, probe 30 validates the exact 174-byte `Embeddedˉwvbˉprogram`, grants its page together with the separate four-byte budget page twice, executes four opcodes through each user-space interpreter generation, and removes both terminal alias sets. Reaching `resource-grant=pass`, `typed-resources=pass`, `resource-revoked=pass`, `process-reuse=pass`, and `wvb-runtime=interpreted` proves that sequence and both results `29`; `native-wvb=pass` still owns the later retained AOT computation. None claims a general loader, capability-transfer facility, or JIT.
...
ABI 17 retains ABI 16's portable service semantics and 64-parameter internal call convention while raising the verified frame envelope to 2,048 cells. The kernel still passes zero service-table, record/text-arena, argument, output, file-input, and file-output fields because its current module uses none of those facilities. Moving the raw allocator/release emitter and future exception policy into ordinary `.wv` still requires:
...
WVA remains semantically checked assembly. Executable version 8 adds only the three process-normalization exports while retaining paging, `push_i32`, and shutdown mechanics. The WVA grammar also adds the exact no-operand `syscall` statement used by the separate user shims. It does not add arbitrary executable-byte directives, generic MSR/control-register access, local branch labels, or memory operands. The current native-probe bridge, admission bridge, memory tail-release emitter, and process machine exist because WVA lacks stack/register moves, comparisons, conditional branches, loops, and checked state mutation required by these transitions. Every seam has exact independently decoded shape and a named future replacement owner.

The seam does not move the UEFI loader, memory-map scanner, arena initializer, allocator/release machine implementation, page-table/process-root constructor, descriptor/MSR publication, syscall dispatcher, terminal COM1 policy, external-symbol adaptation, PE32+ packaging, or native compiler out of C#. The raw objects establish verified link positions through which those pieces can be replaced without changing the loader-to-source evidence chain. WVB admission and process generation/lifecycle policy are Windvale-owned. Q35 poweroff, normalized trap-entry sequences, user syscall bytes, NX/WP activation, and CR3 activation are WVA-authored; Stage 0 only assembles, validates, links, and packages them.
