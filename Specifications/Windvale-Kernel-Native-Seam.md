...
Kernel native seam version 7 defines how the shared portable-WVB native backend, reference host tools, Windvale Assembly, and system-profile Windvale source divide responsibility while the native runtime grows. Decisions 0064 through 0068 qualify the shared consumer through ABI 9, bridge 4, and firmware probe 11. [Decision 0069](../Documents/Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md) advances the implemented candidate to ABI 10, bridge 5, and firmware probe 12; cross-host and pinned-QEMU qualification are pending. [Decision 0056](../Documents/Decisions/0056-Windvale-Owned-Post-Memory-Evidence.md) records the qualified version-2 bidirectional WVA/WV boundary.
...
| --- | --- | --- |
| C# reference/recovery host | Compile WVB, select and verify ABI-10 native code, assemble, link, package PE32+, provide host oracles, and retain bounded loader/memory/bridge emitters while replacements are unavailable. | Remains an independent recovery and comparison path; does not define kernel policy. |
...
## Executable WVA version 3, bridge version 5, and portable probe version 5 seam
...
3. passes the context pointer in `RDX` and calls the ABI-10 portable export `Main`;
...
The builder independently decodes the assembled WVA object and the bridge object and requires their exact architecture, section, symbol, code, and relocation shapes before linking. The ABI-10 selector and fragment verifier independently validate the portable module, borrowed-byte descriptors and reads, empty service list, and unused zero-length record arena before its WVO is admitted.
...
ABI 10 retains those portable semantics while adding host-side bounded dynamic text, descriptor returns, void calls, and diagnostics. The kernel still passes a zero service-table pointer and a zero-length record arena because its current module uses none of those facilities. Moving the allocator and future exception policy into ordinary `.wv` still requires:
...
The ABI-10 candidate rebuilds the portable WVO to 8,010 bytes with SHA-256 `f3d0d2aec5b7fb81d02e4188fb6ba48b6a21dc91c89bdf7f00daaf7b0a981038`. Its unchanged 315-byte bridge object retains SHA-256 `949c5fdd641722c541e2ed6583a5c28bd681b77d91f28ea5cda2999026d75a23`. Firmware probe 12 remains 15,872 bytes with candidate SHA-256 `3010bc72b9c26386f062f78481c900cac841321b040b41447a0bbb65a9e392fe`; all 15 focused OS tests pass on Windows. Cross-host and pinned-QEMU qualification remain pending.
