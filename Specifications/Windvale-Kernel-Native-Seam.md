...
Kernel native seam version 16, WVA seam version 7, admission bridge version 1, and retained bridge 10 are cross-host qualified at exact commit `860c69c`. Candidate probe 23 retains ABI 16, advances WVA to version 8 and admission bridge to version 2, and adds memory/paging version 2 plus the two-root init-service composition. Candidate probe 24 retains those seams while advancing protected processes to version 3 and placing the first bounded Windvale interpreter at CPL3. [Decision 0093](../Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) owns that runtime extension; Linux OS-test execution remains pending for the candidate seams.
...
Probe 24's retained WVA version-8 object includes process-specific normalized entries for vectors 6, 13, and 14. The service and client WVA objects use the exact `syscall` instruction statement. The service links to its Windvale `Main`; the client links to the Windvale bytecode-interpreter export inside its bounded eight-page RX image.

The kernel memory object calls the exception installer and paging installer after switching to the kernel-owned stack, then calls `Windvale_kernel_wva_main`. The paging installer constructs a checked six-page hierarchy and calls the two WVA paging functions. The inbound WVA Main tail transfer reaches admission bridge version 2. That bridge constructs a service-free context with exact budgets 8,944/2, calls AOT Windvale export `Windvale_kernel_wvb_admit`, accepts only token 73, then calls the protected-process entry. The process path runs Windvale policy token 93, constructs separate init and interpreter roots, blocks init, interprets the admitted WVB at CPL3, transfers result 29 through the reduced endpoints, restores init's ABI-16 context, and executes `Initˉresourceˉservice.Main`. Only exact result 29 permits a tail transfer to retained native bridge 10. [Windvale-Os-Wvb-Admission.md](Windvale-Os-Wvb-Admission.md), [Windvale-Os-Bytecode-Interpreter.md](Windvale-Os-Bytecode-Interpreter.md), and [Windvale-Protected-Process.md](Windvale-Protected-Process.md) own the exact identities, ordering, and failure rules.
...
The builder independently decodes the assembled WVA objects, paging/process objects, both bridge objects, and all portable native objects and requires their exact architecture, section, symbol, code, and relocation shapes before linking. On normal probe 24, the loader emits final success and shutdown only after admission, init block, user-space interpretation, client send/exit, init wake/receive/exit, the retained probe, and Main all return. The two kernel-fault paths retain terminal WVA normalization. The user-fault path instead contains vector 13 in the interpreter process and still wakes the service. The ABI-16 selector and fragment verifier independently validate every portable module, borrowed-byte descriptors and reads where present, empty service lists, and unused zero-length runtime resources before each WVO is admitted.
...
The retained native probe is compiled and AOT-linked on the host; its WVB is not interpreted in the guest. Separately, probe 24 validates the exact 174-byte `Embeddedˉwvbˉprogram` identity and executes those bytes through the user-space Windvale interpreter. Reaching `wvb-runtime=interpreted` proves that bounded path returned `29`; `native-wvb=pass` still owns the later retained AOT computation. Neither marker claims a general loader or JIT.
...
Candidate firmware probe 22 retains ABI 16/context 7 and bridge 10, advances WVA to version 8, admission bridge to version 2, memory/paging to version 2, and adds protected-process version 1. The 1,123-byte WVA object has SHA-256 `8a6f54950f15c7331107a5bfa7bd2d863f64b25d395b7cfd9983c31130599363`; the 484-byte admission bridge has SHA-256 `7b53fc11e4e99966386994c247c3a2a19f99ef8da751dbd9dc53f5575871a00d`. All 25 focused OS tests and all six focused assembler tests pass on Windows. [Windvale-Protected-Process.md](Windvale-Protected-Process.md) and [Windvale-Os-Boot-Probe.md](Windvale-Os-Boot-Probe.md) record the exact process objects, four EFI identities, and pinned-QEMU scenarios. Cross-host promotion remains pending.

Firmware probe 23 retains those ABI, WVA, admission, memory, paging, and bridge contracts while advancing protected processes to version 2. The Windvale init-service WVB is 371 bytes with SHA-256 `478dfcd36fed7c8063cfb3f53a6a1362bda5353656339b730be573a1be8f95b0`; its linked user image is 2,302 bytes. Exact commit `22e350b` passes Windows/Linux build and Seed qualification; Linux OS-test execution remains pending.

Candidate probe 24 advances protected processes to version 3 while retaining those machine seams. Its 3,211-byte interpreter WVB has SHA-256 `639e191af1844b6660750978854f5e168c25f4949f1d9282ca5777d65f617083`; the 30,270-byte normal interpreter image executes under eight RX pages, two RW/NX stack pages, and one RW/NX context page. The normal process object is 38,332 bytes with SHA-256 `44559a001988e503374c2b83bc8056d928e075381ca3cd93e155040a2f63fd10`. All 25 focused OS tests and all four pinned-QEMU scenarios pass on Windows; cross-host qualification remains pending.
