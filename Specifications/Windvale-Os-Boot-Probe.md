...
Firmware boot probe version 15 is the current ABI-13/context-5 qualification candidate for the executable Windvale OS evidence slice: UEFI entry, structural table validation, bounded memory-map acquisition, bounded termination of boot services, a compiler-generated Windvale kernel-entry handoff, one kernel-owned physical arena, a zeroing page allocator, a copied handoff, an owned kernel stack, one ordinary portable-WVB module AOT-compiled through the shared native backend and its versioned execution context, borrowed-byte decoding, post-firmware serial observation, and guest-controlled QEMU completion. All 15 OS tests pass on Windows. Version 14 remains the latest cross-host and pinned-QEMU-qualified probe until exact-candidate qualification completes. This is not a functioning kernel or an operating-system qualification.

Decisions 0045 through 0068 own the qualified probe progression through version 11 and ABI 9. [Decision 0069](../Documents/Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md) owns the qualified version-12 ABI-10 rebuild at exact commit `7979933`. [Decision 0071](../Documents/Decisions/0071-Native-Text-Arena-And-Core-Text-Services.md) owns qualified version 13 / ABI 11 at exact commit `8888951`. [Decision 0073](../Documents/Decisions/0073-Native-Argument-Table-And-Process-Input-Services.md) owns qualified version 14 / ABI 12 at exact commit `328e455`. [Decision 0074](../Documents/Decisions/0074-Native-Windows-And-Linux-Output-Services.md) owns the version-15 / ABI-13 candidate. PE32+ construction remains owned by [Windvale-Uefi-Application.md](Windvale-Uefi-Application.md), the special system subset by [Windvale-X64-Kernel-Target.md](Windvale-X64-Kernel-Target.md), the internal call boundary by [Windvale-Kernel-Handoff.md](Windvale-Kernel-Handoff.md), memory ownership by [Windvale-Kernel-Memory.md](Windvale-Kernel-Memory.md), native implementation roles by [Windvale-Kernel-Native-Seam.md](Windvale-Kernel-Native-Seam.md), the execution context by [Windvale-Native-Execution-Context.md](Windvale-Native-Execution-Context.md), and emulator inputs by [Windvale-Os-Boot-Environment.md](Windvale-Os-Boot-Environment.md).
...
Version 9 does not recompute either table CRC and therefore calls this structural validation, not complete table authentication.
...
The `memory-map`, `boot-services`, memory, allocator, stack, Hello World, native-context, native-WVB, source-pass, and success lines are all emitted only after `ExitBootServices` returns success. `memory-owned=pass`, `allocator=pass`, `kernel-stack=pass`, and Hello World are selected from typed WIR and emitted through relocatable calls in the special compiler-generated Main after the memory transition and stack switch. Each byte crosses the WVA output shim. `native-context=pass` and `native-wvb=pass` come from the loader only after the ABI-13 module consumes the exact context, validates its borrowed bytes, returns packed 29, and the special Main returns zero; `windvale-source=pass` then records aggregate source success. A failure after serial initialization emits `status=fail` and writes value 1 to QEMU test port `0xF4`. Success writes zero. QEMU's `isa-debug-exit` therefore returns host code 3 for probe failure and 1 for success. Port `0xF4` remains test transport rather than a Windvale OS device contract. The complete serial marker is required because a QEMU startup error can also return 1.
...
```text
windvale-os-boot-report 15
status=pass
architecture=x86-64
application-format=pe32-plus-uefi-application-v3
probe-version=15
efi-bytes=15872
efi-sha256=d716b77a91646da6b423bacb1faa6d70f5a097241c610fe49291b068f33d5f29
serial-marker=windvale-os-boot-15-entry-system-table-memory-map-boot-services-exited-memory-owned-allocator-kernel-stack-hello-native-context-native-wvb-windvale-source-status-pass
qemu-exit-code=1
```
