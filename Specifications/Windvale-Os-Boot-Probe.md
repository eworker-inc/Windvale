...
12. releases the terminal generation-two client allocation, first-fits its root
    ...
    binds the exact launch-request digest, publishes a durable `1/81/1`
    filesystem domain ledger plus provider-only endpoint and fresh ready thread
    and process, but does not publish a consumer, provider entry, or request; and
