...
This is cross-host qualification of the parser and hosted native shell, not a standalone executable or native build driver. The current WVO still requires its verified fragment metadata and Stage 0 publication owner. Project-relative resolution, source reads for compilation, compiler invocation, and atomic WVB publication therefore remain in the C# `windvale build` path.
