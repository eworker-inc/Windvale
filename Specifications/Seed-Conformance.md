...
The comparison intentionally ignores operating-system description, architecture label, and installed runtime description. It requires equal module-, object-, assembly-, and link-format versions, complete golden module, object, flat-image, and map SHA-256 values, results, and normalized program output. The golden contract covers the portable integer-data example, hosted text example, portable Foundation byte-header example, Windvale-written hosted `wvdump`, the complete normalized `wvdump` line report for the real integer-data module, the Windvale-written WVO core and its exact 189-byte object, both Stage 0 and Windvale-written encodings of the canonical WVA example, the Windvale linker core plus its canonical hosted WVO scan and two-object resolution/layout analysis, and the complete Stage 0 two-object link map. Each host verifier also exercises native assembly, Windvale assembly, Windvale WVO scanning and link analysis, no-partial-output, independent object verification, Stage 0 resolution/layout/relocation, and the hosted file adapters.
