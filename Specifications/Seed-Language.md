...
The C# Stage 0 compiler and reference runtime implement the complete surface below. The Windvale-written compiler implements the same source and canonical WVB contracts for its current `i32`, `u8`, `u32`, `bool`, `text`, `bytes`, nominal, variant, and bounded-collection value subset; `i64` and `u64` remain Stage 0-only there. Baseline native, WebAssembly, and Windvale OS consumers remain explicit narrower WVB subsets until their separately verified lowering contracts advance.
...
`let` locals and parameters are immutable. `var` locals may be assigned after initialization. `+=`, `-=`, and `*=` are statement-only compound assignments to a simple mutable-local name. They read the target once before evaluating the right operand, apply the underlying checked same-type operation, and store only after that operation succeeds; they are not expressions and cannot be chained. Stage 0 admits the same `i32`, `i64`, `u32`, and `u64` types as the underlying operators, while the current WVB 1.6 Windvale-written compiler admits `i32` and `u32` compound assignment.
...
The Stage 0 reference compiler implements constants of every admitted Seed scalar width. The current WVB 1.6 Windvale-written compiler implements the `i32`, `u8`, `u32`, `bool`, and enum subset; `i64` and `u64` constant lowering advances with that compiler's separately verified WVB 1.7 adoption.
