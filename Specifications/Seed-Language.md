...
The Windvale-written compiler and current native WVB runner implement the scalar source and canonical WVB surface below, including `i64`, `u64`, `i8`, `i16`, `u16`, `rune`, `f32`, `f64`, the admitted edition-1 `unit` and `never` checkpoint, edition-1 named variant construction and destructuring through WVB 1.16, contextual immutable fixed-array construction/access through WVB 1.17, and exact Language 1.0 Vector/Sequence type metadata through WVB 1.18. The 1.18 checkpoint does not yet execute collection values. The retained Stage 0 recovery compiler remains a WVB 1.11 differential boundary and does not define the fixed-integer, rune, floating-point, unit, never, multi-field variant, fixed-array, Vector, or Sequence extensions. Direct native array or collection lowering, browser packaging, WebAssembly distribution, and Windvale OS consumers remain explicit narrower subsets until their separately verified contracts advance.
...
- A descriptorless Seed enum contains 1 through 256 uniquely named members with
  ...
  current WVB writer emits exact signed `i32` and exact `u8` backing and rejects
  a retained use of any other backing as `Unsupportedˉshape` without narrowing
...
- Descriptorless Seed `windvale run` looks for exported `Main() -> i32`.
  ...
  `Main() -> i32`. WVB 1.23 through WVB 1.26 require the budget entry and
  permit affine budget/result locals only in `Main` when executable Split,
  fallible Vector construction, recoverable append, or owned Vector calls are
  present.
...
The Windvale-written compiler implements constants of every admitted scalar width. It lowers `i64` and `u64` constants to canonical WVB 1.11, `i8`, `i16`, and `u16` constants to the WVB 1.12 fixed-integer family, rune constants to WVB 1.13, the guaranteed floating literal forms to WVB 1.14, edition-1 unit or never evidence to WVB 1.15, edition-1 multi-field variant metadata or named field extraction to WVB 1.16, fixed-array metadata, construction, and checked access to WVB 1.17, and exact Vector or Sequence type metadata to WVB 1.18, rejecting checked overflow, underflow, a non-scalar, or an invalid exact collection type before publication. The retained Stage 0 recovery compiler covers only its frozen WVB 1.11 surface.
