...
An imported module may contain imports, capability declarations, records, enums, and functions; module data remains root-only. Every imported function must be declared `export`. Each dependency is semantically checked against only its own declarations and transitive imports; flattening cannot make a root or sibling function, nominal type, or capability visible without an import. Composition internalizes dependency records, enums, and functions into one ordinary WVB module; only the root module's exported functions remain WVB exports. All composed nominal types share the existing type namespace, and all composed functions share one ordinal function namespace; duplicates are rejected. Dependency records and enums form visible static source contracts but are not separately exported from the resulting WVB. Runtime WVB linkage, package lookup, aliases, private dependency declarations, qualified type names, and dependency module data are not part of this contract.
