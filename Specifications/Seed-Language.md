...
An imported module must use `profile portable`, may contain only imports, records, enums, and functions, and must declare every function `export`. Each dependency is semantically checked against only its own declarations and transitive imports; flattening cannot make a root or sibling function, nominal type, or capability visible without an import. Composition internalizes dependency records, enums, and functions into one ordinary WVB module; only the root module's exported functions remain WVB exports. All composed nominal types share the existing type namespace, and all composed functions share one ordinal function namespace; duplicates are rejected. Dependency records and enums form visible static source contracts but are not separately exported from the resulting WVB. Imported capabilities, data, runtime WVB linkage, package lookup, aliases, private dependency declarations, and qualified type names are not part of this contract.

This portable, capability-free dependency rule is the current Seed implementation boundary, not the durable application-library policy. [Decision 0140](../Documents/Decisions/0140-Per-Module-Platform-Scope-And-Filesystem-Capabilities.md) accepts per-part platform scope and capability-bearing static libraries as later architecture. A future source/module decision must specify their syntax, transitive approval, canonical lowering, and validation before this section changes behavior.
