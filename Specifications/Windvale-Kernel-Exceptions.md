...
Version 2 is cross-host qualified at exact commit `12e9e2e` through the pre-paging firmware probe-20 baseline, retained unchanged through qualified probe 28, and unchanged in candidate probe 29. It retains vector 6, adds general protection vector 13, and moves both entry-normalization stubs into WVA while one bounded Stage 0 object still owns descriptor publication and terminal policy. [Decision 0086](../Documents/Decisions/0086-First-Wva-Owned-Normalized-X64-Trap-Entries.md) and [kernel trap frame version 1](Windvale-Kernel-Trap-Frame.md) own the boundary. Probes 22 through candidate 29 add a process-private IDT extension for vectors 6, 13, and 14 plus CPL3 containment without changing the qualified ring-0 terminal contract.
...
The process common entry inspects the saved `CS`. A CPL0 origin tail-transfers unchanged to the qualified terminal handler. A CPL3 origin records vector/error and returns to the saved kernel continuation with the process and thread faulted. Candidate probe 29 retains deterministic interpreter-process `CLI` delivery as `(13, 0)` after the atomic typed WVB/budget grant, budgeted interpretation, and one valid send, then cleans both terminal borrows before waking init; vector 6 and vector 14 are structurally installed but are not yet claimed as live user-fault evidence.

## Deterministic scenarios

Candidate probe 29 supports four explicit construction scenarios after page-table activation, WVB admission, protected-process construction, the init-owned typed two-resource grant, and terminal cleanup:
...
The qualified version-2 terminal contract still provides no page-fault `CR2`, double-fault containment, IST, NMI, IRQ, PIC/APIC, interrupt enablement, register-save frame, nested-fault policy, `IRETQ`, recovery, unwinding, SMP, scheduler integration, or WVR-to-CPU mapping. Candidate probe 29's process-private extension retains one TSS, bounded interpreter-process fault containment, and atomic post-fault cleanup of both resource aliases but no general dispatcher, page-fault policy, resumption, signal model, or process-facing exception ABI. The shared terminal handler remains Stage 0 C#-emitted machine code because current WVA lacks comparisons, conditional branches, memory reads, and its polled serial loop; system-profile `.wv` policy still requires bounded unsafe memory plus a specified kernel call convention.
