...
The IDT contains exactly 14 addressable 16-byte entries. Vectors other than 6 and 13 are zero and not present. The IDTR operand is outside the admitted table limit, and bytes 234 through 4095 remain zero. Maskable interrupts stay disabled; this version does not install an interrupt controller or claim absent vectors are safely handled.
...
The process common entry inspects the saved `CS`. A CPL0 origin tail-transfers unchanged to the qualified terminal handler. A CPL3 origin records vector/error and returns to the saved kernel continuation with the process and thread faulted. Qualified probe 29 retains deterministic interpreter-process `CLI` delivery as `(13, 0)` after the atomic typed WVB/budget grant, budgeted interpretation, and one valid send, then cleans both terminal borrows before waking init; vector 6 and vector 14 are structurally installed but are not yet claimed as live user-fault evidence.

## Deterministic scenarios

Qualified probe 29 supports four explicit construction scenarios after page-table activation, WVB admission, protected-process construction, the init-owned typed two-resource grant, and terminal cleanup:

- `normal` completes init resource-set selection/grant, init block, budgeted bytecode interpretation, client send/exit, two-resource cleanup, init wake/receive/exit, both later Main paths, success markers, and the WVA Q35 shutdown adapter;
...
The version-3 installer object exports only the installer and imports the two WVA entry symbols. Its typed relocations are the only address transfer from descriptor construction to WVA. The ordinary build now constructs that exact object through the focused [Windvale-native producer](Windvale-X64-Exception-Object-Producer.md); Stage 0 retains the historical recovery recipe. The WVA object exports both entries and the common terminal handler and retains its serial writer as a local function. Before PE publication, focused tests lock:
...
The locally verified version-3 candidate produces a 1,894-byte WVA object with SHA-256 `845d45d6787ec819ca300ffc81a9ffe3e86c7b3998f3dd2a50a017a353d86193`. It contains 333 code bytes, 162 marker bytes, an exported 164-byte terminal at code offset 121, and one 48-byte local serial writer at offset 285. The installer-only object is 483 bytes with SHA-256 `9caeb7ce353bca33e3bbac729ecca0423d59f8ce6b65ccd6b54fa53c381d617c`; its complete 222-byte code has SHA-256 `b7fd72dbc78d6f02511f9769a727421c7cb91409eef2164b4a51aced4dc2df14`. The native producer reproduces that identity and independently admits the complete WVO before publication.
