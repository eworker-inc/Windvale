...
- Status: Implemented locally through experimental profile 15 and cross-host qualified through profile 10, but not an accepted permanent target
...
Range, invalid-UTF-8, u16-narrowing, per-value, and aggregate-heap failures return guest statuses `3008`, `3014`, `3016`, `3015`, and `3018`. Invariant scalar formatting, `text.quote`, `bytes.sha256`, records, enums, capabilities, reclaiming allocation, and general nonempty-stack joins remain outside profile 15. Empty output after successful complete verification continues to mean “outside interpreter profile.”
...
- recursion, function values, indirect calls, `break`, `continue`, arbitrary or unbounded instruction streams, invariant scalar formatting, text quoting, byte hashing, executable records or enums, reclaiming allocation, collection, or browser capability imports;
...
The next measured slice should complete the interpreter operations needed by the Windvale compiler: invariant scalar formatting, `text.quote`, `bytes.sha256`, then direct records and enums, while retaining the complete verifier as a separate first stage and the profile-15 dynamic-value charging contract. Capability execution remains separately authorized by the worker. Once that runtime path covers the compiler's required values, execute the Windvale compiler's verified WVB in the same guest and connect source input to canonical WVB output. Recursion, indirect calls, `break`, `continue`, reclaiming allocation, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. The verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
