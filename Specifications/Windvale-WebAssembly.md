...
Further extraction should follow measured ownership boundaries rather than move the whole interpreter state through opaque packets. The existing direct-AOT selector is not yet a complete shortcut: compiler static data now fits the bounded borrowed-descriptor contract, but nominal values, recursion, the sixteen-function ceiling, and general verified call ordering remain unsupported there. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract.
