...
This selector is not itself a general Windvale-native WVB semantic verifier. Profile 11 can execute a Windvale-written verifier that completely consumes the seven bounded section payloads, profile 12 can decompose a larger Windvale-written verifier across descriptor-bearing private functions, and profile 13 supplies capacity for the complete compiler-aligned consumer. That retained ten-function consumer proves canonical metadata and references plus typed local, operand-stack, call, capability, record, enum, return, reachability, and declared-stack contracts for the source compiler's empty-stack control boundaries. Profile 14 separately lowers the first bounded WVB scalar interpreter, profile 15 adds bounded immutable text and bytes values, invariant formatting, and deterministic quoting, profile 16 adds import-free SHA-256, and profile 17 admits the canonical two-function root-first composition of that interpreter with its portable Windvale SHA implementation. The ordinary composed WVB therefore contains root `Main` plus one call-free SHA leaf. The interpreter executes bounded direct record and enum values, supports a versioned byte-array guest entry and return, and admits compiler-scale function, frame, stack, and instruction metadata. A four-artifact compiler-capacity bundle derived from the same canonical verifier sources admits the exact evolved hosted and portable compilers under independent metadata/reference, typed-first, typed-second, and control/reachability meters; the typed phases partition all function indices. Bounded reclamation for generated values plus stable-slot, conservatively traced guest records carries the portable compiler through an ordinary 100,000-instruction `WVXO 2` guest-budget result. The separate 64 KiB guest text/bytes heap remains bounded and monotonic. General WVB graphs with nonempty stack joins and executable capability authorization remain outside this claim. A future default browser execution path must compose the verifier, explicit worker authorization, and the selected interpreter profile before removing the Stage 0 path.
...
### Profile 17: expanded descriptor calls and root-first leaf composition

Profile 17 composes the already-bounded descriptor-call and profile-16 scalar operation sets without changing execution ABI 3, fixed memory, metering, status propagation, arena ownership, or generated private-function representation. It admits two through sixteen `bytes -> bytes` functions, at most 8,191 nonparameter locals and 131,072 code bytes per function, 100,000 decoded instructions per function, 131,072 aggregate code bytes, 400,000 aggregate decoded instructions, and declared stack depth one through four.

One of two statically acyclic orderings is required. The retained decreasing-ordinal form keeps `Main` last and permits only calls to lower ordinals. The new static-composition form has exactly two functions: exported `Main` at ordinal zero and one call-free leaf at ordinal one. Only `Main` may call, every call targets the leaf, and at least one call is required. The second form therefore has dynamic call depth exactly two and rejects self-calls, leaf-to-root calls, cycles, recursion, additional functions, and any other forward edge before publication.
...
The retained root-first consumer is the project composition of `Wvb-Scalar-Interpreter-Main.wv` with `Foundation/Sha256.wv`. The Foundation leaf implements SHA-256 in Windvale source as one focused `bytes -> bytes` function and returns exactly 64 lowercase hexadecimal ASCII bytes. This avoids a host hash service, new assembly, duplicate interpreter source, or a general forward-call graph.

### Retained profile-16 interpreter: direct records and enums

The retained interpreter additionally consumes the WVB nominal-type section and admits value shapes and guest opcodes for records and enums. This is a guest-interpreter expansion, not another outer target profile: the composed outer source retains profile 17's exact two-function shape, execution ABI 3 does not change, and the scalar `WVXI 1` / `WVXO 1` contract remains available.
...
The exact portable compiler completes preflight under these bounds. With canonical `Function-Only.wv` WVSS input and call depth 64, guest budgets one, 1,229, 1,230, 1,511, and 100,000 return normal `WVXO 2` status `3011` with their exact charged guest counts after 66,187,926, 67,126,106, 67,126,629, 67,256,958, and 118,006,709 outer instructions. Bounded generated-value and guest-record reclamation therefore clear the former allocation and instruction-37,085 `WVR3017` boundaries without increasing the fixed arenas. Complete compiler execution and result publication are not yet claimed.
...
Profile 17 additionally requires preservation of every earlier generated Wasm identity; exact independent reconstruction of the root-first function indices and direct call; reference-runtime agreement for the Windvale SHA known answers; rejection of root self-call, leaf-to-root call, recursion, cycles, a third function, and every other forward edge; enforcement of the expanded local, per-function code, aggregate code, instruction, stack, and output bounds; and independent WebAssembly validation and execution of the composed interpreter artifact.
