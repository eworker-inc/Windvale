...
Further extraction should follow measured ownership boundaries rather than move the whole interpreter state through opaque packets. The existing direct-AOT selector is not yet a complete shortcut: compiler static data, nominal declarations, and the 417-function inventory now fit explicit bounds, but its 308 signature families, nominal values, recursive and backward calls, general control, and broader opcode set remain unsupported in direct emission. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract.
