...
Every candidate receives one deterministic frame width. Frames contain at least 128 eight-byte cells and grow by powers of two to the smallest width that contains the candidate's largest combined parameter/local declaration, capped at 2,048 cells. This preserves the compact retained path while the exact portable compiler uses a 2,048-cell frame. All functions in one candidate share the selected width, and every saved frame carries the existing function, return-PC, code-offset, and code-length header.

The exact portable compiler completes preflight under these bounds. With canonical `Function-Only.wv` WVSS input, guest budget one, and call depth 64, it returns a normal `WVXO 2` status `3011` with one charged guest instruction. A 20,000,000-instruction request instead reaches enclosing execution-ABI status `3018` with empty output because immutable whole-frame replacement exhausts the generated Wasm module's 4 MiB monotonic value arena. This outer failure is not a guest compiler diagnostic and does not carry `WVXO 2`.
...
The next measured slice must replace repeated immutable whole-frame reconstruction with bounded reusable or reclaiming interpreter-owned local and saved-frame storage, including exact alias, reset, range, exhaustion, and malformed-input behavior. Merely increasing the 4 MiB monotonic arena is insufficient because allocation scales with local stores rather than live frame bytes. A direct-AOT portable compiler remains an alternative only if measured implementation evidence makes it smaller than a coherent safe interpreter-storage contract. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, guest record reclamation, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. After compiler execution succeeds from bounded WVSS to `WVCO 1`, the verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
