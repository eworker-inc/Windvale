...
Record collection uses those same verified shapes. Current locals are visited at eight-byte cell boundaries and only kind-7 record cells become roots. Saved frames are walked backward from their terminal caller function index; the collector derives each exact frame start and appends only its record-shaped locals, so packed return PCs, function indices, scalar locals, and descriptor locals cannot retain record storage. Operand-stack cells, fields under construction, and marked records' nested fields remain conservatively recognized at eight-byte boundaries. The arena therefore remains bounded and partially conservative rather than exact-precision.
