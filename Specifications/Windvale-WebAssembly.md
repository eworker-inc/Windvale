...
The exact portable compiler completes preflight under these bounds. With canonical `Function-Only.wv` WVSS input and call depth 64, guest budgets one, 1,511, 1,512, and 100,000 return normal `WVXO 2` status `3011` with their exact charged guest counts after 45,177,347, 46,155,795, 46,156,318, and 96,797,247 outer instructions. Bounded guest-record tracing therefore clears the former instruction-37,085 `WVR3017` boundary without increasing the 4 KiB arena. Complete compiler execution and result publication are not yet claimed.
...
- function values, indirect calls, unbounded recursion, `break`, `continue`, arbitrary or unbounded instruction streams, unbounded record allocation, exact-precision garbage collection, or browser capability imports;
