...
- Status: Implemented locally through experimental profile 17, a compiler-capacity verifier bundle, and bounded reclamation for generated values and guest records; cross-host qualified through profile 10, but not an accepted permanent target
...
The direct runtime profiles additionally admit at most 256 immutable `text` and `bytes` data declarations whose concatenated UTF-8 and opaque-byte payload is at most 15,360 bytes. `text.const` and `bytes.const` must reference an in-range declaration of the exact corresponding kind. The encoder concatenates declaration payloads in canonical WVB order into one active WebAssembly data segment at byte 50,176 and lowers each constant to a borrowed `(pointer, length)` descriptor. Static descriptors are outside the reclaiming arena and are therefore ignored by retain/release without copying or acquiring ownership. A module with zero static payload retains the previous no-data WebAssembly identity and omits section 11.

The direct runtime profiles also completely validate and consume at most 1,024 nominal declarations even when the retained executable subset does not use them. Records have one through 64 fields, enums have one through 256 members, all type and member names have one through 255 bytes, and every nested record or enum shape index is in range. Unknown kinds, empty member lists, truncated or oversized extents, and trailing type-payload bytes are rejected before function selection. This is declaration admission only: nominal function signatures, locals, values, and operations remain outside the direct lowering subset, and unused nominal declarations do not change emitted WebAssembly bytes.

Before executable-profile selection, the direct backend additionally consumes a bounded general function inventory: one through 512 functions, at most 64 parameters and 8,191 nonparameter locals per function, at most 131,072 code bytes per function and 1,048,576 aggregate code bytes, and declared stack depth one through 256. Names occupy one through 255 bytes; primitive and record/enum shapes are structurally complete with in-range nominal indices; code ranges are contiguous, ordered, nonoverlapping, and exactly cover the Code payload. Inventory admission does not imply code admission: general signature agreement, calls, control flow, reachability, recursion, nominal values, and instruction lowering remain separate selectors.

The following code-inventory pass decodes each admitted function independently through the complete current WVB 1.11 opcode-width table. It admits at most 200,000 aggregate instructions and 4,096 direct calls; requires canonical Boolean constants, in-range local operands, function-relative jump and false-branch extents, and in-range direct-call targets; and imposes no forward-only call ordering. This is still structural inventory rather than executable admission: instruction-boundary control targets, terminator rules, typed stacks, call signature agreement, reachability, recursion budgets, nominal value storage, and general emission remain separate proof obligations.
...
The retained root-first consumer composes `Wvb-Scalar-Interpreter-Main.wv`, `Wvb-Scalar-Interpreter-Envelope.wv`, and `Foundation/Sha256.wv`. The focused envelope function validates request framing and bounded WVB section offsets, identifies the unique `Main` export, and returns a versioned preflight summary. Versions 1 and 2 retain their portable capability-free selection; version 3 also recognizes the exact bounded browser console profile below. The root retains the complete executable verifier and interpreter. It classifies each static opcode effect through one immutable 256-byte WVB data declaration lowered to the bounded page-zero static range; module-dependent effects remain explicit corrections. The Foundation function implements SHA-256 in Windvale source and returns exactly 64 lowercase hexadecimal ASCII bytes. The composed outer source remains capability-free and the generated module retains no host imports.
...
### Retained profile-17 interpreter: bounded browser console envelope

The interpreter additionally accepts `WVXI 3` for either a verified portable, capability-free `Main() -> i32` candidate or a verified hosted candidate that declares exactly `console.write_line(text) -> void` and exports `Main() -> i32`. The containing WebAssembly module remains import-free. The host grants capability bit zero in the request; a declaration without that grant returns `WVR3010` with zero charged guest instructions. No other capability identity or signature is admitted.
...
| 12 | 4 | Maximum guest call depth |
| 16 | 4 | Candidate WVB length |
| 20 | 4 | Capability grants; only values zero and one are valid |
| 24 | 4 | Standard-output byte limit, at most 65,536 |
| 28 | 4 | Reserved diagnostic-output limit, currently zero |
| 32 | Candidate WVB length | Exact verified WVB bytes |

`console.write_line` consumes one text descriptor and appends its exact strict UTF-8 bytes followed by one LF. A complete line must fit within the request limit; otherwise the call appends nothing and returns `WVR3013`. Each call remains one guest instruction even though the enclosing outer meter charges the bounded interpreter work independently.

Every well-formed version-3 execution result uses this exact `WVXO 3` layout:
...
Record collection uses those same verified shapes. Current locals are visited at eight-byte cell boundaries and only kind-7 record cells become roots. Saved frames are walked backward from their terminal caller function index; the collector derives each exact frame start and appends only its record-shaped locals, so packed return PCs, function indices, scalar locals, and descriptor locals cannot retain record storage. The current construction and every newly marked record are walked against their verified field declarations, and only kind-7 field cells extend the worklist. A separate two-word record mask identifies record values across the bounded 64-cell operand stack, including local loads, construction, field loads, calls, and returns. Every admitted root domain therefore has type evidence under the current immutable, acyclic record model.

The exact portable compiler completes preflight under these bounds. The earlier WVB baseline returned normal `WVXO 2` status `3011` through a 100,000-instruction guest budget. After the WVB 1.11 compiler and the root-first SHA composition evolved, typed record tracing and a measured 768-cell record arena restored complete compilation. The retained Decision-0294 baseline used the 919,577-byte compiler with SHA-256 `2bf84dc2a8cbb80c52ec7fb6cb2e29eef27def1707f398a276c61063d73df06e`, the 828,165-byte interpreter Wasm with SHA-256 `f3226906f1848cee60d4b25fe0ed4cf3710bd79bb55b12fe16620fc382756c72`, and its 110,700-byte interpreter WVB source with SHA-256 `325556e1a6bff318818877265207d4c0c5be1374439d5d021f38703805db076f`. That baseline compiled the pinned 100-byte source in 1,183,292 guest and 1,404,070,227 outer instructions, producing a canonical `WVCO 1` kind-zero payload byte-identical to the 183-byte Stage 0 WVB oracle. These identities are historical evidence, not the current browser interpreter package identity. The separate direct backend now fully consumes the compiler's 104 static declarations, 82 nominal declarations, 417-function directory, 157,844 instruction encodings, and 2,991 direct-call targets, then stops without output at typed executable semantics. It does not yet lower the compiler itself.
...
Profile 8 emits an import-free WebAssembly binary version-1 module with one fixed, non-growable 129-page memory. Page zero remains target-private: bytes 0 through 50,175 hold bounded scratch and reclaiming-allocation metadata, while bytes 50,176 through 65,535 are the optional immutable static-data range. The host owns a 4 MiB input window beginning at byte 65,536; the guest owns a separate 4 MiB output window beginning at byte 4,259,840. The regions are disjoint, checked before access, and exactly fill the remaining memory.
...
The profile-12 descriptor-call fixture has 764-byte WVB SHA-256 `a44c8bdbf9983a7929a769d5ca2e0b60323d72cf96b04e31450d9757bb15729a`. Its nested calls and conditional route map `[9, 8]` to `[9, 9, 8]` after 127 instructions; budget 126 returns `WVR3011` without output. With the shared reclaiming allocator and reference helpers it emits a 5,421-byte Wasm module with SHA-256:

```text
7e489e3150ed5a3457fc3cefa3cfa97b3736cf394ca8fd199984ecc0bd7e95dc
```

The static-descriptor fixture has 364-byte WVB SHA-256 `6e606418a136b5e426279efd03f10c8ed640f752747b18777390703c5c946090`. It retains `"Windvale"` text followed by bytes `[33, 42]`, maps input `[1, 2, 3]` to the exact UTF-8/byte concatenation after 26 instructions, and emits 2,339 import-free Wasm bytes with SHA-256 `2246c38d2cbc765271926c5f709e8a13cd062d82ee529e5a22dd346206a1772c`.
...
The root-first graph and first cohesive extraction restore ordinary optimizing-tier engine evidence without changing execution ABI 3. The minimal portable source now completes from bounded `WVSS 1` to a byte-exact `WVCO 1` result. Decisions 0270, 0273, and 0275 compose verification, compilation, and result execution inside one disposable browser worker and make that static worker the normal website playground without a .NET browser runtime. Decision 0277 pins the standalone native compiler packages and native publisher path that regenerate the exact portable compiler WVB without .NET. Decision 0278 adds the separate native WebAssembly compiler that lowers the native-built interpreter WVB to the exact packaged Wasm. Normal website artifact production is therefore .NET-free; Stage 0 remains only the explicit recovery route for reconstructing the two native compiler package families.

Further extraction should follow measured ownership boundaries rather than move the whole interpreter state through opaque packets. The existing direct-AOT selector is not yet a complete shortcut: compiler static data, nominal declarations, the 417-function directory, every instruction width, and all direct-call targets now fit explicit bounds, but typed agreement across its 308 signature families, control-flow proof, nominal values, recursion policy, and general operation emission remain unsupported. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract.
