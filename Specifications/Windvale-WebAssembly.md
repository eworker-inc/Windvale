...
The exact portable compiler completes preflight under these bounds. The earlier WVB baseline returned normal `WVXO 2` status `3011` through a 100,000-instruction guest budget. After the WVB 1.11 compiler and the root-first SHA composition evolved, typed record tracing and a measured 768-cell record arena restore complete compilation: the current 919,577-byte compiler with SHA-256 `2bf84dc2a8cbb80c52ec7fb6cb2e29eef27def1707f398a276c61063d73df06e` running in the 828,165-byte interpreter Wasm with SHA-256 `f3226906f1848cee60d4b25fe0ed4cf3710bd79bb55b12fe16620fc382756c72` compiles the pinned 100-byte source in 1,183,292 guest and 1,404,070,227 outer instructions. That Wasm is generated from the 110,700-byte interpreter WVB with SHA-256 `325556e1a6bff318818877265207d4c0c5be1374439d5d021f38703805db076f`. Its canonical `WVCO 1` kind-zero payload is byte-identical to the 183-byte Stage 0 WVB oracle. The normal browser host may first execute the digest-pinned 292-byte scalar warmup WVB for exactly 20,000 instructions on the same instance. That call returns status `3011`, exactly 20,000 guest and 15,154,202 outer instructions, and a zero scalar result; it does not alter the exact compiler counters or establish a portable engine-tier threshold. The separate direct backend now fully consumes the compiler's 104 static declarations, 82 nominal declarations, 417-function directory, 157,844 instruction encodings, and 2,991 direct-call targets, then stops without output at typed executable semantics. It does not yet lower the compiler itself.
