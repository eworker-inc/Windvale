...
- Status: Implemented locally through experimental profile 16, a compiler-capacity verifier bundle, and bounded reclaiming value storage; cross-host qualified through profile 10, but not an accepted permanent target
...
The exact portable compiler completes preflight under these bounds. With canonical `Function-Only.wv` WVSS input and call depth 64, guest budgets one, 1,511, and 1,512 return normal `WVXO 2` status `3011` with their exact charged guest counts. A 100,000-instruction guest request reaches guest instruction 37,085 and returns normal `WVXO 2` guest status `3017` after 63,171,965 outer instructions. This is exhaustion of the interpreter's separate 4 KiB guest-record arena, not generated-Wasm value storage and not a compiler diagnostic.
...
The next measured slice must add ownership and reclamation for the interpreter's 4 KiB guest-record store, including nominal identity, aliases through locals/stack/calls/returns, reset, range, exhaustion, and stale-reference behavior. The exact compiler reaches that store's `WVR3017` at guest instruction 37,085; merely increasing it would again change only the distance to failure. The separate 64 KiB guest heap is expected to follow and must be measured rather than conflated with generated-Wasm value storage. The existing direct-AOT selector is not a current shortcut: compiler static data, nominal values, recursion, and general verified call ordering remain unsupported there. Reconsider direct AOT only after those gaps have a measured smaller implementation path. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. After compiler execution succeeds from bounded WVSS to `WVCO 1`, the verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
