...
The exact portable compiler completes preflight under these bounds. The earlier WVB baseline returned normal `WVXO 2` status `3011` through a 100,000-instruction guest budget. After the WVB 1.11 compiler and the root-first SHA composition evolved, typed record tracing and a measured 768-cell record arena restore complete compilation: the current 919,577-byte compiler with SHA-256 `2bf84dc2a8cbb80c52ec7fb6cb2e29eef27def1707f398a276c61063d73df06e` running in the 839,104-byte interpreter Wasm with SHA-256 `f65c4e203d4b244ec52e0619f9d1a99ce1d2809296313cb154bba8316c6d916c` compiles the pinned 100-byte source in 1,183,292 guest and 1,513,523,789 outer instructions. Its canonical `WVCO 1` kind-zero payload is byte-identical to the 183-byte Stage 0 WVB oracle. The normal browser host may first execute the digest-pinned 292-byte scalar warmup WVB for exactly 20,000 instructions on the same instance. That call must return status `3011`, the exact guest count, and a zero scalar result; it does not alter the exact compiler counters or establish a portable engine-tier threshold.
