...
The next measured slice should execute the Windvale compiler's verified WVB in the same guest and connect bounded source input to canonical WVB output. That measurement must identify which current selector ceilings, call frames, record lifetime, dynamic heap, instruction budget, and capability seams are real compiler blockers before any bound is raised. Capability execution remains separately authorized by the worker. Recursion, indirect calls, `break`, `continue`, reclaiming allocation, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. The verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
