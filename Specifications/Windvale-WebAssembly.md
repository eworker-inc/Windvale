...
This selector is not itself a general Windvale-native WVB semantic verifier. Profile 11 can execute a Windvale-written verifier that completely consumes the seven bounded section payloads, profile 12 can decompose a larger Windvale-written verifier across descriptor-bearing private functions, and profile 13 supplies capacity for the complete compiler-aligned consumer. That retained ten-function consumer proves canonical metadata and references plus typed local, operand-stack, call, capability, record, enum, return, reachability, and declared-stack contracts for the source compiler's empty-stack control boundaries. Profile 14 separately lowers the first bounded WVB scalar interpreter, profile 15 adds bounded immutable text and bytes values, invariant formatting, and deterministic quoting, and profile 16 adds import-free SHA-256. The retained profile-16 interpreter now also executes bounded direct record and enum values, supports a versioned byte-array guest entry and return, and admits compiler-scale function, frame, stack, and instruction metadata. A three-artifact compiler-capacity bundle derived from the same canonical verifier sources admits both the exact 599,868-byte hosted compiler and exact 597,545-byte portable compiler under independent metadata/reference, typed-execution, and control/reachability meters. Exact per-function frames let the portable compiler execute 1,511 guest instructions; budget 1,512 is the first enclosing Wasm monotonic-value-arena failure, before a compiler result is published. General WVB graphs with nonempty stack joins and executable capability authorization remain outside this claim. A future default browser execution path must compose the verifier, explicit worker authorization, and the selected interpreter profile before removing the Stage 0 path.
...
Preflight records each function's parameter count and exact frame length `(parameters + locals) * 8` bytes. One bounded zero backing is large enough for the candidate's largest declaration, but every active local frame is a slice with its function's exact length. A saved call frame contains the return PC, exact caller locals, and caller function index; return reconstructs the caller's frame length, code offset, and code length from verified metadata. The 2,048-cell per-function ceiling remains unchanged.

The exact portable compiler completes preflight under these bounds. With canonical `Function-Only.wv` WVSS input and call depth 64, guest budgets one and 1,511 return normal `WVXO 2` status `3011` with their exact charged guest counts. Budget 1,512 is the first enclosing execution-ABI status `3018`; a 20,000,000-instruction request reaches the same exact outer boundary. Immutable `local.store` frame reconstruction still exhausts the generated Wasm module's 4 MiB monotonic value arena. This outer failure is not a guest compiler diagnostic and does not carry `WVXO 2`.
...
- The retained profile-16 interpreter additionally accepts WVB record/enum shapes and operations under the format's 1,024-type, 64-field, and 256-member bounds, plus a separate 4,096-byte immutable record-field arena with `WVR3017` exhaustion. Its version-2 protocol transports a bounded guest `bytes` parameter and `bytes` result.
- Its compiler-scale execution-entry expansion admits 4,096 functions, 64 parameters, 2,048 combined parameter/local cells, stack depth 64, 400,000 aggregate instructions, a 20,000,000 guest budget, and call depth 64. Candidate-adaptive power-of-two frames retain a 128-cell minimum.
...
- function values, indirect calls, unbounded recursion, `break`, `continue`, arbitrary or unbounded instruction streams, unbounded or reclaiming record allocation, collection, or browser capability imports;
...
The next measured slice must replace repeated immutable frame reconstruction with bounded reusable or reclaiming interpreter-owned local and saved-frame storage, including exact alias, reset, range, exhaustion, and malformed-input behavior. Merely increasing the 4 MiB monotonic arena is insufficient because allocation scales with local stores rather than live frame bytes. The existing direct-AOT selector is not a current shortcut: compiler static data, nominal values, recursion, and general verified call ordering remain unsupported there. Reconsider direct AOT only after those gaps have a measured smaller implementation path. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, guest record reclamation, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. After compiler execution succeeds from bounded WVSS to `WVCO 1`, the verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
