...
The retained root-first consumer composes `Wvb-Scalar-Interpreter-Main.wv`, `Wvb-Scalar-Interpreter-Stack-Effect.wv`, and `Foundation/Sha256.wv`. The focused stack-effect function accepts one little-endian opcode and returns the consumed-cell count plus produced-value flags; the root retains operation-specific dynamic corrections. The Foundation function implements SHA-256 in Windvale source and returns exactly 64 lowercase hexadecimal ASCII bytes. Both helpers remain capability-free `bytes -> bytes` functions and the generated module retains no host imports.
