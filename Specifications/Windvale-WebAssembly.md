...
The next measured slice is structural: widen the current root-first descriptor call-module contract beyond its special `Main` plus SHA leaf composition, extract allocator/reference and record-reclamation ownership from the 5,740-local interpreter root, and restore ordinary optimizing-tier engine evidence without changing execution ABI 3. Only then should the exact portable compiler resume beyond the retained 500,000-instruction probe toward a complete `WVCO 1` result. The record collector remains conservative and bounded; false retention requires explicit pressure evidence if it becomes observable. The existing direct-AOT selector is not a current shortcut: compiler static data, nominal values, recursion, and general verified call ordering remain unsupported there. Reconsider direct AOT only after those gaps have a measured smaller implementation path. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. After compiler execution succeeds from bounded WVSS to `WVCO 1`, the verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
