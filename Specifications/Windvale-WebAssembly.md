...
The next measured slice must complete ownership transitions for the interpreter's 64 KiB guest text/bytes heap: descriptor-consuming stack operations, saved calls and returns, discarded results, record reclamation, exact exhaustion, and stale-reference behavior. The exact current compiler has been probed through 500,000 guest instructions without reaching a guest-heap failure, so continued work must combine focused ownership fixtures with eventual complete-compiler evidence rather than assume its next operation. Extracting the allocator into a separate Windvale module also requires widening the current root-first descriptor call-module contract beyond its special `Main` plus SHA leaf composition; that compiler change remains separate from this interpreter slice. The record collector remains conservative and bounded; false retention requires explicit pressure evidence if it becomes observable. The existing direct-AOT selector is not a current shortcut: compiler static data, nominal values, recursion, and general verified call ordering remain unsupported there. Reconsider direct AOT only after those gaps have a measured smaller implementation path. Capability execution remains separately authorized by the worker. Indirect calls, `break`, `continue`, and browser capability imports remain outside the profile until each has an explicit resource and evidence contract. After compiler execution succeeds from bounded WVSS to `WVCO 1`, the verifier, interpreter, source compiler, and result execution must all move into one disposable worker before the editable playground becomes the default .NET-free path.
