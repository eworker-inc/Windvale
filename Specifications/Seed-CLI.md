...
- `assemble` currently reads strict UTF-8 WVA 1 source, validates and encodes it through the frozen Stage 0 recovery/differential assembler, verifies the generated WVO, and writes deterministic bytes. The default output replaces `.wva` with `.wvo`; input and output paths must differ. The command remains on this path until the native `WVHS 1` candidate passes its dual-host cutover gate; normal use then moves to the digest-bound Windvale-native application while Stage 0 remains explicitly available for recovery and differential evidence.
