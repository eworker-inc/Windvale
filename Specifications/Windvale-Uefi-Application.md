...
Windvale UEFI application format version 1 defines the first deterministic adapter from a verified Windvale flat link to a PE32+ x86-64 EFI application. It is intentionally a narrow bootstrap target. [Decision 0045](../Documents/Decisions/0045-First-Uefi-Application-And-Boot-Probe.md) owns that boundary.
...
The adapter consumes one successful `Linkˉresult` already validated and independently reconstructed by `flat-x86-64-v1`. Version 1 requires:

- link base address zero;
- exactly one non-empty section, which must be code because the link entry is an exported function;
- no imports and no WVO relocations;
- an entry offset strictly inside the code bytes; and
- at most 4,194,304 code bytes.

The accepted bytes must be address-independent. Version 1 cannot discover opaque absolute addresses embedded manually in machine code. Native code at this system boundary is unsafe input supplied by the bootstrap implementation, not portable Windvale code.
...
| PE32+ optional header | `0x098` | magic `0x20B`, writer 1.0, image base `0x400000`, section alignment `0x1000`, file alignment `0x200` |
...
`.text` begins at RVA `0x1000`, is readable and executable, and is not writable. `.reloc` begins at the next 4 KiB RVA, is readable, initialized, and discardable. Its directory size is 12 bytes. The block names the `.text` page, has block size 12, and contains two zero `IMAGE_REL_BASED_ABSOLUTE` entries. Those entries are padding and cause no load-address fixup.
...
| `WVU1002` | The link shape, base, imports, relocations, code range, or entry is unsupported. |
...
Version 1 has no general PE section mapping, import table, export table, resources, debug data, authenticode signature, exception/unwind table, TLS, WVO-to-PE relocation translation, Secure Boot signature, or compatibility mode. It does not make PE an implicit definition of Windvale semantics. Generalization requires a concrete native-backend or boot-stage case and an independently verifiable rule.
