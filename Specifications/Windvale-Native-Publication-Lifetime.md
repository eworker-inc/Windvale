...
`Compiler/Windvale/Native-Publication-Lifetime-Core.wv` is portable and owns request validation, status selection, and canonical transition construction. `Compiler/Windvale/Native-Publication-Lifetime-Bridge.wv` is a hosted wrapper whose sole `file.read_bytes` capability reads one immutable in-memory resource named `native-publication-lifetime-request.bin` and returns the response as `bytes`.

The bridge runs under the Stage 0 reference interpreter before allocation, so it does not depend on the executable image whose lifetime it plans. The host independently reconstructs every header field and all nine transitions, then passes only that verified immutable plan to one internal executable-image owner. The owner keeps the raw address private to the runtime assembly, tracks actual state, checks the accepted transition before each operation, and releases from every post-allocation partial state.
