...
| `Unsupportedˉmodule` | Capabilities, nominal types, or static data outside the accepted bounded scalar/i32-data profile are present. |
...
- parameterless `Main() -> i32`, zero through four `i32`, `bool`, `u8`, or `u32` helper parameters, helper returns and declared locals using those same four scalar types, no more than 1,024 combined parameters and declared locals per function, a declared maximum stack depth from one through 1,024, at most 8,192 code bytes per function, adjacent exact code ranges, and no unclaimed function-section bytes;
- one control-flow graph per function of at most 1,024 instructions drawn from `i32.const`, `bool.const`, `u8.const`, `u32.const`, `local.load`, `local.store`, `data.length`, `data.load.i32`, checked `i32.add`, `i32.subtract`, `i32.multiply`, `i32.negate`, checked `u32.add`, all six signed `i32` comparisons, `u32.less`, `u8.equal`, `bool.equal`, `bool.not_equal`, `bool.not`, `jump`, `branch.false`, `call`, and `return`; every data operation names the accepted declaration, and every direct call names any in-range function, consumes that function's exact declared typed arguments, and produces its declared scalar result; and
