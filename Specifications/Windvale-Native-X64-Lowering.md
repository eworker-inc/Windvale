...
`Wvb-To-Wvo-Static-Descriptors.wv` returns 42 through one text, one bytes value, one i32 array, descriptor locals, UTF-8 conversion, length, slice, and all four admitted reads. The memory adapter, hosted tool, and generated native tool reproduce Stage 0's exact 7,626-byte WVO with two sections, three data symbols, and three relocations. `Wvb-To-Wvo-Text-Services.wv` exercises text concatenation, UTF-8 validation/conversion, and quoting and reproduces Stage 0's exact 5,140-byte WVO. Additional malformed cases reject an out-of-range descriptor data index, a text constant naming bytes data, and a runtime text operation receiving the wrong descriptor type before publication.
