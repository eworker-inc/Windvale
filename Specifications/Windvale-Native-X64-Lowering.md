...
### WVB 1.16 nominal values

The current-source native candidate accepts WVB 1.16 without rewriting its
version. The complete WVB verifier runs before native lowering, followed by
the same scalar, nominal, direct-call, and resource bounds listed above.
Callable descriptors, function references, and closures remain unavailable in
this version. Metadata normalization and the hosted enum-request reader also
recognize 1.16; neither metadata reader replaces complete verification.

The [paired-host checkpoint](../Documents/Evidence/2026-09-08-Native-Wvb-16-Nominal-Admission.json)
executes a canonical `Option<u64>` return containing the maximum `u64`, compares
repeated native objects, and rejects unsupported-version, truncated, trailing,
oversized-section, and reserved-field mutations. This is not qualification of
every 1.16 operation or promotion of the installed lowerer/build driver.
...
and ABI identities, affine availability, and the same length, alignment,
allocation, and teardown contract used by the scalar oracle before emitting
native code.

WVB 1.34 additionally maps shape `36` to one internal `u64` cell while retaining
identity `36` separately from the shape-`25` owner. Shape `36` is admitted only
as a helper parameter or compiler-generated local, never as a result or nominal
shape. Every local view is constructed from an available shape-`25` owner and
used by the adjacent bounded local-load sequence leading to a direct call whose
corresponding parameter is shape `36`. The view cannot be taken or loaded from
a parameter. Native affine analysis therefore preserves the owner rather than
turning the shared scalar representation into a copy. Six independent
shape/version corruptions reject before WVO publication.

WVB 1.35 additionally admits shape `28` as an exact immutable borrowed
`Foreignˉscratch<Abi>` helper parameter or compiler-generated view. The native
function model retains the borrow kind separately from its nominal record type,
so record layout and ownership mode cannot be conflated. Every local view is
created from an available matching owner and confined to its adjacent direct
call. Opcode `DD` independently validates the scratch local, canonical nominal,
enum ABI, construction relation, and affine availability, then reads private
record field zero and stores its complete `u64` length. The observation emits
29 bytes including the common instruction-budget charge, performs no allocation
or backing copy, and leaves the owner available. Eight independent WVB
corruptions reject before WVO publication.

WVB 1.36 retains scratch field zero as a private packed descriptor: its low
32 bits contain the bounded owner length and its high 32 bits contain the
construction alignment. `Scratchˉlength` zero-extends only the low word. Exact
opcode `DE` consumes ordered `u64` start, length, and required-alignment values,
names the direct scratch local and canonical region Result, and consumes that
scratch through every successor and function exit. The backend independently
rechecks the canonical scratch, region, pointer-failure, and ABI relation before
emitting code.

The metered `DE` sequence is exactly 340 bytes: the common ten-byte instruction
charge plus 330 operation-specific bytes. It checks zero length, unsigned
`start + length` carry, exclusive end against the bounded owner length, then a
nonzero power-of-two required alignment no greater than the construction
alignment and satisfied by the logical start. It constructs exact
`Outˉofˉrange`, `Addressˉoverflow` with width 64, or `Misaligned` Failure data.
Success stores only a private packed logical start/length descriptor in the
...
callable may be created, stored in an exact local, loaded, and consumed by
`call.indirect`; it cannot be a function parameter or result, occur inside an
aggregate, cross a control-flow block edge, or survive the frame that created
its environment. Each descriptor must resolve to exactly one physical target
in the module. This unique-target restriction keeps the initial indirect call
fully checked without introducing a general dispatch table.

One callable value occupies one 16-byte ABI cell: a `u32` physical target, a
`u32` callable Types identity, and a `u64` environment address. `D3` writes a
zero environment for a noncapturing target. `D5` copies one through 64 admitted
scalar or enum captures into caller-frame-owned 16-byte cells and points the
callable at that exact range. A function may reserve at most 1,024 aggregate
environment cells, and callable cells, environments, capture scratch, ordinary
locals, and ordinary temporaries all remain inside the existing 2,048-cell
frame bound.

`D4` revalidates the target and callable identity at runtime. A noncapturing
value must retain a zero environment. A capturing value must point inside the
current frame's exact environment range; the call copies its immutable capture
prefix into caller-owned argument cells, appends the public arguments, and uses
the ordinary direct-call ABI, instruction meter, and call-depth meter. There is
no heap closure, host function pointer, ambient authority, hidden retain/release,
or second native calling convention in this slice. Callable operations are
accepted only in a one-block function; wider control flow, same-signature
dynamic target selection, callable parameters/results, retained descriptor or
aggregate captures, and escaping environments require separately versioned
work.
...
descriptor, enum, or copied-record payload, and its ordinary value cell holds
only the backing address. `variant.is_case` reads the tag; typed control-flow
proves the selected case before `variant.payload` returns its payload
representation. Variant construction, local movement, calls, and returns
preserve caller-owned backing so no callee-frame pointer escapes.
...
When the final nonempty value is followed only by canonical zero-length
publication regions, the producer consumes those internal transitions through
the exact Complete cursor and object extent without emitting another chunk.
Any zero-length result that does not end at that exact terminal state is an
output-limit failure. Manifest entries therefore remain nonempty even when a
relocation-free object ends at its symbol chunk.
...
Each resource is constructed and written inside one private helper invocation
that returns only scalar status, cursor, position, index, and length evidence.
The ABI 22 function-entry checkpoint reclaims that invocation's dynamic text
and byte allocations when it returns, so completed resource values do not
accumulate in the caller while a compiler-scale object is staged. The caller
retains only bounded scalar manifest entries. A pure plan step immediately
beyond a coalesced code boundary may be recomputed when the next helper begins;
the grouping, bytes, positions, cursor progression, write count, and limits
remain exact. [Decision 0908](../Documents/Decisions/0908-Bound-Compiler-Scale-Staging-Arena-Per-Resource.md)
records the allocation-lifetime correction and byte-identity evidence.
...
including checked `u64`, bounded variants, nested immutable records, ABI-23
provider calls, frame-owned callable values, and immutable unsafe-scratch
observation, now builds as a 678,601-byte WVB at SHA-256
`c552c6ca542a60de8140c78e4d978be75a70f8baf50cf7ae5661008c9259b823`.
The same native segmented path reconstructs its 9,754,112-byte Windows and
9,752,576-byte Linux applications exactly after rebuilding and repinning the
embedded staging producer. The fixed Return-42 WVB/WVO pair remains unchanged,
and the 369-byte independent-metadata fixture lowers to an exact 1,151-byte WVO.
