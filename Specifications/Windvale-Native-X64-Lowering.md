...
| `Unsupportedˉmodule` | Capabilities, nominal types, or static data outside the accepted bounded immutable profile are present. |
...
- portable profile, no capabilities or nominal types, and zero through eight immutable text, bytes, or `[i32]` declarations; text contains at most 1 MiB of valid UTF-8, bytes contains at most 4 MiB, and each i32 array contains at most 262,144 elements;
...
- parameterless `Main() -> i32`, zero through four `i32`, `bool`, `u8`, or `u32` helper parameters and scalar helper returns, declared locals using those four scalar types plus `text` and `bytes`, no more than 1,024 combined parameters and declared locals per function, a declared maximum stack depth from one through 1,024, at most 8,192 code bytes per function, adjacent exact code ranges, and no unclaimed function-section bytes;
- one control-flow graph per function of at most 1,024 instructions drawn from `i32.const`, `bool.const`, `text.const`, `u8.const`, `u32.const`, `bytes.const`, `local.load`, `local.store`, `data.length`, `data.load.i32`, `bytes.length`, `bytes.slice`, `bytes.read_u8`, `bytes.read_u16_little`, `bytes.read_u32_little`, `bytes.read_i32_little`, `bytes.concat`, `text.concat`, `text.utf8_is_valid`, `text.from_utf8`, `text.quote`, `text.to_utf8`, checked `i32.add`, `i32.subtract`, `i32.multiply`, `i32.negate`, checked `u32.add`, all six signed `i32` comparisons, all six unsigned `u32` comparisons, `u8.equal`, `u8.not_equal`, `bool.equal`, `bool.not_equal`, `bool.not`, `jump`, `branch.false`, `call`, and `return`; every data operation names a declaration of the exact required kind, and every direct call names any in-range function, consumes that function's exact declared typed scalar arguments, and produces its declared scalar result; and
...
Every read is preceded by a checked range or exact payload-size test. Unknown versions, reordered/truncated/extended sections, inconsistent lengths, alternate function metadata, unknown opcodes, invalid local, data, or call indices, wrong descriptor data kinds, type or stack mismatches, malformed branch targets, unreachable control blocks, and mismatched maximum-stack declarations fail closed. Forward, backward, self-recursive, mutually recursive, and cyclic call edges share the same exact signature checks. Every function entry consumes ABI 22's shared call-depth budget before its frame executes, and every emitted WVB instruction, including every data operation, backward jump, and call, consumes the shared instruction budget before its operation. Recursion and nonterminating cycles therefore reach `WVR3004` or `WVR3011` rather than escaping resource accounting. Stack-passed or descriptor call parameters, descriptor returns, mutable or other data declarations, capabilities, and nominal identities remain outside the subset.
...
The mixed-scalar call code and WVO SHA-256 identities remain `1a0a541d2bd59378b4fa6df53248c3c359e909a0b7446198ebb1a58ca5a79721` and `cb7d2c74edb7aa3443e1e23cf0d762d4c15b79c39ea4f363531b2ec80633c13f`. Existing retained objects remain byte-identical. The current WVB identities are `79e4ddac4eac5e85aa4a10b14b91de9ca41a8263a3bf811bb38ec3eed5ed6f1c` for the data module, `93eb0f25ddebd70b2c56edfdd7604ebd31297bdc5306d023cd8c84d63faafbb2` for the layout module, `3aaf9719cd776de2b7d5dd8fabb470cb0c3b0bceaf1932540845bdd1c0962e2c` for the object module, `57ba4340976a218defaad9b2b343f4a9d8a8abf76cd9962b62c02d10aa946e79` for the core closure, `ecc4ade816ea4f040684ba6bde5ce0770745f571527ef2e66a960a9f2e4f0c43` for the memory adapter, and `31a1fc990d11952ad13790dded7b7e88fa953eeaea929975333b04c1a264b8c3` for the 167,172-byte hosted tool. The hosted-tool WVB lowers through the complete Stage 0 backend to 2,257,076 code bytes and a 2,263,326-byte WVO; those sizes are evidence, not a permanent optimization promise.

This slice does not yet transfer stack-passed or descriptor parameters, descriptor returns, dynamic byte builders beyond concatenation, the remaining `u8`/`u32` instruction family, more than eight functions, capabilities, nominal values, general relocations, fragment verification, W^X publication, PE/ELF construction, or the complete compiler. It does not satisfy the native-retirement gate by itself.
