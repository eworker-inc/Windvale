...
Sparse materialization remains available as a Stage 0 recovery operation. The native candidate now performs the ordinary bounded materialization logic in Windvale, although its first complete application value is limited to 4 MiB while the recipe itself supports slightly larger maximum containers. Arbitrary completed PE/ELF input is verified in portable Windvale through the bounded segmented contract. C# remains required for independent target parsing, recovery provenance, candidate construction, and broader host evidence until the retirement gate.
