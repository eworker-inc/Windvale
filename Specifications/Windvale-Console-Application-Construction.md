...
The Stage 0 materializer treats the recipe as untrusted. It checks exact identity, version, total size, status, request evidence, target-specific segment count, every descriptor, ordering, non-overlap, application bounds, literal extent, native extent, and complete payload consumption before copying anything into the zero-initialized result.

For the current transition, each C# target adapter separately constructs its recovery image from the independently checked Windvale layout. The materializer compares every completed byte with that recovery image and rejects any disagreement. The portable [console-application verifier](Windvale-Console-Application-Verification.md) then verifies the completed executable and recovers the original native image and entry; the existing detailed C# PE and ELF verifiers repeat that recovery as independent differential oracles.
...
Sparse materialization remains a Stage 0 C# operation over the validated recipe. Arbitrary completed PE/ELF input is now verified in portable Windvale through the bounded segmented contract, while C# remains required for host evidence validation, independent target parsing, recovery provenance, and final host-array/publication operations.
