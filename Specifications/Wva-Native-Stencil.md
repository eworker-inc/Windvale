...
`Compiler/Windvale/Native-Stencil-Bridge.wv` embeds both retained objects, calls the exact consumer, and returns their instantiated leaves as one immutable 75-byte bundle. Decision 0080 first made that bundle the live process-input source through a verified descriptor-return entry. Decision 0366 retains the bridge and its WVB as source-reproducible qualification, differential, and recovery evidence while the normal runtime embeds and digest-checks the exact generated 5-byte and 70-byte leaves directly. Ordinary process-input service assembly therefore no longer loads, lowers, publishes, invokes, copies, or splits the generator WVB. This is a supply-boundary change, not a language-semantics or stencil-format change.
