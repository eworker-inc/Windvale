...
The later [compiler build-driver profile](Windvale-Compiler-Build-Driver.md) deliberately reuses this exact authority/runtime/startup family but carries distinct `WVHB 1` metadata and outer container format 5. The [native WVA assembler profile](Windvale-Native-Wva-Assembler.md) reuses the same bounded startup and service implementation with distinct `WVHS 1` metadata and outer container format 6. Each constructor and parser selects its expected profile explicitly; the qualified format-3 compiler bytes and identities remain unchanged.
