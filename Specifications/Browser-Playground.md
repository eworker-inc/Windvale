...
The reusable engine also exposes the bounded WebAssembly lowerer as a separate immutable result. It embeds the portable backend and hosted shell as `.wv` source, compiles the composition once through Stage 0, requires backend WVB SHA-256 `e3cb41c7724130ed18c20af7e823a80f76ca7fafd3a0befef0cecb790c9e305f`, and publishes no Wasm on selector failure. The browser adapter transfers successful bytes to a new module worker, which applies `WebAssembly.validate`, rejects all imports, checks the exact ABI-0, ABI-1, ABI-2, or ABI-3 exports, and is terminated after success, failure, or timeout. ABI 3 additionally validates its fixed memory extent and non-growth, disjoint 4 MiB regions, input length, output descriptor, and strict UTF-8 output before returning a bounded copied buffer; earlier ABIs return only scalar evidence. The lowerer now reaches profile 15's bounded scalar/text/bytes interpreter capacity and retains the 722,837-byte complete compiler-aligned verifier plus the separate 253,707-byte interpreter. The verifier covers structure, identities, typed executable flow, reachability, and exact stack contracts; the interpreter adds scalar calls/control, static data, descriptor calls, immutable text/bytes operations, strict UTF-8, and explicit allocation failures. The static direct page deliberately remains on its smaller profile-8 text artifact; these deeper artifacts do not yet change that page or its worker lifetime.
