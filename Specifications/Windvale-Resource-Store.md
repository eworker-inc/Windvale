...
The next integration boundary requires a versioned bounded IPC request/reply descriptor, a resource-service capability, independently lived resource borrows, and terminal or explicit cleanup. A later block driver may supply store bytes, but its device and DMA authority must remain outside this format. Writable files, directories, replacement, allocation, crash recovery, caches, and an on-disk filesystem require separate measured contracts.
