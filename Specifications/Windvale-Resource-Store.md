...
The current candidate constructs the exact canonical three-entry image as a 1,196-byte object with SHA-256 `624ece2d2e032f6f0929675a8f79ceb223538d84bccace264ecbbfdce5eca4ad`. `WVRES005` attaches its one-page RO/NX mapping to init, publishes a bounded descriptor in init's RW/NX data page, and leaves it absent from both client roots. Each client sends `boot:main.configuration`; the init WVA seam scans directory names, selects resource 3 dynamically, and copies its metadata and bytes into the reply. Both QEMU client generations complete this lookup before terminal cleanup; the WVB 1.11 rebind requires new cross-host qualification before it inherits the earlier Probe 34 claim.

Probe 35 retains those exact store bytes and lookup semantics while advancing the surrounding attached-resource record set to `WVRES006`. The store remains resource 4 and separate from the new resource-5 `WVDS 1` directory snapshot. Both rebuilt clients complete this package-resource lookup before their distinct directory request; neither protocol nor namespace is reinterpreted as the other.
