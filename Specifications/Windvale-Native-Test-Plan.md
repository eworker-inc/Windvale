...
`WVNT 1` is the fixed first repository test inventory executed entirely through
...
[`Tests/Native/Plan.txt`](../Tests/Native/Plan.txt) is exactly 268 UTF-8/LF bytes
with SHA-256
`d04f77c41bbae2c98541b3a0e6dec0ee0c725106dae72e5bb128d52c4abf3fc5`.
...
Success prints `Tests: 2, Passed: 2, Failed: 0` plus LF and returns `0`. The first
...
the runner's accepted scalar subset, including calls, loops, branches, signed and
unsigned arithmetic, comparisons, booleans, and `u8`. Tests outside that subset
remain in the explicit Stage 0 lane until the relevant native runtime, tool, fixture,
and oracle are available. Qualification of this plan advances Decision 0057's native
test condition but does not complete it.
