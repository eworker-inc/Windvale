...
The protocol is composed with the capacity-one block-exchange lifecycle and the
immutable block-image provider. Their 59-case owner returns 47 on Windows and
pins deterministic Windows/Linux images.

The architecture-neutral exchange loop and an immutable-memory provider are
implemented separately. This wire contract does not yet prove a privileged
endpoint adapter, hardware driver, DMA, media-change notification, partition
discovery, or directory data interpretation.
