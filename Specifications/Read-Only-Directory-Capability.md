...
The Stage 0 reference runtime accepts an explicit `IReadˉonlyˉdirectory` instance. It does not resolve host paths. Windows and Linux launchers may later construct a conforming immutable snapshot behind that interface. Windvale OS must bind the same semantic capability to a checked runtime adapter and isolated provider endpoint; the kernel remains responsible only for capability identity, rights, generation, IPC bounds, peer lifecycle, and cleanup.
