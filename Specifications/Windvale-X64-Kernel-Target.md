...
Identical source and target version produce identical WVO bytes. The canonical probe-21 `Hello-World.wv` input produces a 3,524-byte WVO object with SHA-256 `7dd04428af5860a977cdbe6ce875784a68691dee5b39ca0f97c9d5da9b9a6101`, 104 relative relocations, and no absolute relocation. Its additional source-owned evidence is `wvb-admission=pass`; compiler target semantics and version do not change. Candidate firmware probe version 21 links this special object with the independent memory and paging layers, bidirectional WVA seam, fixed Windvale-owned WVB admission path, admitted AOT object, retained ABI-16 portable-WVB-derived object, both native bridges, normalized vector-6/vector-13 exception destinations, and WVA Q35 shutdown adapter. Main executes on the kernel-owned stack under the new page-table root only after verifier token 73 and both exact native results 29 following successful `ExitBootServices`. The pre-paging probe-20 baseline at `12e9e2e` remains the latest cross-host-qualified predecessor until Decisions 0088 through 0090 complete qualification.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Probe 21 retains AOT execution of a broader shared-ABI portable module and adds paging, fixed WVB admission, and target-specific exception/shutdown mechanics outside this compiler target; none of these make the special target general or give it a WVB loader. The pre-paging probe-20 composition remains the cross-host-qualified compiler-target baseline. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
