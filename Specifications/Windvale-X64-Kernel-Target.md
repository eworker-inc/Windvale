...
Identical source and target version produce identical WVO bytes. The canonical probe-28 `Hello-World.wv` input produces a 7,154-byte WVO object with SHA-256 `e22f79d3ed9aad92c9827cc89e711eb78f2e81ed0f207b755f9da9d70be6d66d` and no absolute relocation. Its source-owned evidence includes `wvb-admission=pass`, `processes=isolated`, `resource-grant=pass`, `resource-revoked=pass`, `wvb-runtime=interpreted`, `init-service=pass`, and `ipc=cross-process`; compiler target semantics and version do not change. Candidate firmware probe 28 links this special object with memory/paging/process layers, the bidirectional WVA seam, fixed Windvale-owned WVB admission and two-process policy, separate interpreter and init CPL3 AOT images, the retained ABI-16 portable-WVB-derived object, both native bridges, normalized kernel/process exception destinations, and the WVA Q35 adapter. Main executes on the kernel stack only after verifier token 73, process-policy token 96, the init resource grant, terminal cleanup, interpreted/client/init results 29, and the retained native result 29. Qualified probe 27 retains the preceding special-object identity.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Probe 28 retains AOT execution of broader shared-ABI portable modules and adds paging, fixed WVB admission, two-process isolation, one init-owned resource grant with terminal cleanup, section-derived user-space interpretation, cross-process IPC, and target-specific exception/shutdown mechanics outside this compiler target; none make the special target general or give it a general WVB loader. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
