...
Identical source and target version produce identical WVO bytes. The canonical `Hello-World.wv` input produces a 2,564-byte WVO object with SHA-256 `f2c28eb5f020f59b8acb480fc8dc62e393ebb14405b3c12ecb05076176d44420`, 72 relative relocations, and no absolute relocation. Firmware probe version 8 links that object with the independent memory layer, bidirectional WVA machine seam, exact native bridge, and one ordinary portable-WVB-derived ABI-6 object. Main executes on the kernel-owned stack after that portable module consumes its exact versioned context and returns its accepted result following successful `ExitBootServices`.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, a general native calling convention, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Firmware probe version 8 separately exercises AOT execution of a shared-ABI portable module; it does not make this target general and does not load or verify WVB inside the guest. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
