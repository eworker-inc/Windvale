...
Identical source and target version produce identical WVO bytes. The canonical probe-25 `Hello-World.wv` input produces a 5,894-byte WVO object with SHA-256 `44be3b850c7da68788ccff6a86779b4b9a5ad7d2b2b92d24e7a4d7315901921c` and no absolute relocation. Its source-owned evidence includes `wvb-admission=pass`, `processes=isolated`, `wvb-runtime=interpreted`, `init-service=pass`, and `ipc=cross-process`; compiler target semantics and version do not change. Candidate firmware probe 25 links this special object with memory/paging/process layers, the bidirectional WVA seam, fixed Windvale-owned WVB admission and two-process policy, separate interpreter and service CPL3 AOT images, the retained ABI-16 portable-WVB-derived object, both native bridges, normalized kernel/process exception destinations, and the WVA Q35 adapter. Main executes on the kernel stack only after verifier token 73, process-policy token 94, interpreted/client/service results 29, and the retained native result 29. Focused Windows evidence passes; exact commit `190174a` retains the latest cross-host-qualified probe-24 form.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Probe 25 retains AOT execution of broader shared-ABI portable modules and adds paging, fixed WVB admission, two-process isolation, section-derived user-space interpretation, cross-process IPC, and target-specific exception/shutdown mechanics outside this compiler target; none make the special target general or give it a runtime-supplied WVB loader. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
