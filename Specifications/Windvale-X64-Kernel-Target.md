...
Identical source and target version produce identical WVO bytes. The canonical probe-22 `Hello-World.wv` input produces a 2,954-byte WVO object with SHA-256 `61df8691c2b1c6eff31a6782cca144669aad32c26294e60fb97b8d5b15ff4de4`, 85 relative relocations, and no absolute relocation. Its source-owned evidence now includes `wvb-admission=pass`, `process=isolated`, and `ipc=pass`; compiler target semantics and version do not change. Candidate firmware probe 22 links this special object with memory/paging/process layers, the bidirectional WVA seam, fixed Windvale-owned WVB admission and process policy, the admitted CPL3 AOT object, the retained ABI-16 portable-WVB-derived object, both native bridges, normalized kernel/process exception destinations, and the WVA Q35 adapter. Main executes on the kernel stack only after verifier token 73, process-policy token 91, exact process result 29, and the retained native result 29. Focused Windows evidence passes; exact commit `860c69c` retains the latest cross-host-qualified probe-21 form.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Probe 22 retains AOT execution of broader shared-ABI portable modules and adds paging, fixed WVB admission, process isolation, IPC, and target-specific exception/shutdown mechanics outside this compiler target; none make the special target general or give it a WVB loader. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
