...
Identical source and target version produce identical WVO bytes. The canonical probe-27 `Hello-World.wv` input produces a 6,494-byte WVO object with SHA-256 `77bfe58344ad82c3ef9255ee31e860c8df132fe8e71880d7683feba81f71e43f` and no absolute relocation. Its source-owned evidence includes `wvb-admission=pass`, `processes=isolated`, `resource-grant=pass`, `wvb-runtime=interpreted`, `init-service=pass`, and `ipc=cross-process`; compiler target semantics and version do not change. Candidate firmware probe 27 links this special object with memory/paging/process layers, the bidirectional WVA seam, fixed Windvale-owned WVB admission and two-process policy, separate interpreter and init CPL3 AOT images, the retained ABI-16 portable-WVB-derived object, both native bridges, normalized kernel/process exception destinations, and the WVA Q35 adapter. Main executes on the kernel stack only after verifier token 73, process-policy token 95, the init resource grant, interpreted/client/init results 29, and the retained native result 29. Qualified probe 26 retains the preceding special-object identity.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Probe 27 retains AOT execution of broader shared-ABI portable modules and adds paging, fixed WVB admission, two-process isolation, one init-owned resource grant, section-derived user-space interpretation, cross-process IPC, and target-specific exception/shutdown mechanics outside this compiler target; none make the special target general or give it a general WVB loader. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
