...
Identical source and target version produce identical WVO bytes. The canonical `Hello-World.wv` input produces a 2,564-byte WVO object with SHA-256 `f2c28eb5f020f59b8acb480fc8dc62e393ebb14405b3c12ecb05076176d44420`, 72 relative relocations, and no absolute relocation. Candidate firmware probe version 20 links that unchanged special object with the independent memory layer, bidirectional WVA seam, ABI-15 portable-WVB-derived object, native bridge, normalized vector-6/vector-13 exception destinations, and separate WVA Q35 shutdown adapter. Main executes on the kernel-owned stack after the portable module consumes context version 7, validates borrowed bytes, and returns its accepted result following successful `ExitBootServices`. Probe 17 remains the latest qualified predecessor until Decisions 0085 through 0087 complete qualification.

This special target does not support general expressions, locals, calls, branches, Unicode console output, static-data addressing, multiple functions, optimization, unwind information, Windows or Linux executable production, or compiler self-hosting. Probe 20 retains AOT execution of a broader shared-ABI portable module and target-specific exception/shutdown mechanics outside this compiler target; neither makes the special target general nor loads or verifies WVB inside the guest. Probe 17 remains the cross-host-qualified compiler-target baseline. Each expansion requires focused semantics, encoding, relocation, and differential evidence rather than silent fallback.
