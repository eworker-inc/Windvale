...
[Decision 0140](../Documents/Decisions/0140-Per-Module-Platform-Scope-And-Filesystem-Capabilities.md) accepts a filesystem capability family with per-part platform scope, typed rights-limited instances, exact partial-progress and durability semantics, and optional extensions. [Decision 0153](../Documents/Decisions/0153-First-Versioned-Read-Only-Directory-Capability.md) implements the first separate read-only directory operation. It does not retroactively turn `file.read_bytes` or `file.write_bytes` into general application filesystem APIs; these leaves retain the bounded tool-oriented behavior specified here.
