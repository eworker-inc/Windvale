...
| `Badˉversion` | 3 | The version is not WVB 1.4. |
...
| `Unknownˉopcode` | 16 | Function code contains an opcode outside WVB 1.4. |
...
With useful `wvdump` complete, the next toolchain boundary is deterministic byte construction plus the smallest section, symbol, and relocation contracts required by a Windvale-written assembler and linker.
