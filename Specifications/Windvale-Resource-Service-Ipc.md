...
The capacity-one boundary rejects a second send, receive-before-ready, reply-before-request consumption, replay, close-before-terminal, wrong endpoint, missing right, empty message, and a message above 4 KiB. Peer exit clears retained bytes and moves to `peer-exited`; explicit close then completes hosted cleanup. The transport does not parse magic, request identifiers, names, kinds, or store bytes. Version 1 defines deterministic failure rather than an unbounded queue.

Probe 34's `WVCHAN03` adopts the terminal boundary in the guest. Client exit or fault clears message state, sender, receiver, waiter, byte length, and both registered destination/capacity pairs before recording peer status, process reference, and close count. A checked reopen clears that evidence before generation 2; generation 2 ends closed. The guest retains its fixed synchronous coordinator rather than claiming a general scheduler or peer-close result ABI.
...
Probe 35 retains the exact `WVRQ 1` / `WVRY 1` bytes while moving replies to dedicated response pages and renaming syscall rights, wait reasons, and operations 5 through 7 as service-generic. Numeric syscall values and the format-blind `WVCHAN03` transport do not change. Init re-registers its receive window after the resource reply so the same checked channel can carry the distinct `WVDQ 1` / `WVDR 1` directory exchange without making the kernel aware of either protocol.

The complete portable `WVRS 1` verifier remains the general oracle. The measured guest seam accepts only the exact canonical three-entry boot profile and validates structural coverage, metadata, ordering, and digest-text grammar. It does not recompute per-entry payload SHA-256; Stage 0 binds the complete immutable store page to its SHA-256 identity before construction. The next filesystem slice requires a measured namespace/provider contract—such as path components and lookup authority or a block-backed read-only provider—not more implicit resource-name semantics in the kernel.
