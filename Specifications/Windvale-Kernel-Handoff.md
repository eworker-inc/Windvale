...
The memory layer calls WVA export `Windvale_kernel_wva_main` only after owned-state initialization, handoff copy, stack switch, exception installation, and [kernel paging version 3](Windvale-Kernel-Paging.md) activation. WVA tail-transfers to admission bridge version 2. Its exact 8,944/2 context accepts only Windvale admission token 73, then calls [protected process version 7](Windvale-Protected-Process.md). Only exact init selection/grant, section-derived user-space interpretation, terminal resource cleanup, interpreter-process completion, and init completion reach the retained 271/2 portable-probe bridge; only that bridge's packed result 29 reaches compiler export `Windvale_kernel_main`. Normal return powers off through WVA. Separate images prove terminal CPL0 `(6, 0)` and `(13, 0)` plus contained interpreter CPL3 `(13, 0)`. Probe 28 does not prove general WVB loading, physical/virtual-memory management, capability transfer, page/root reclamation, JIT publication, scheduling, interrupts, recovery, or a general service runtime.
