...
The [portable construction recipe](Windvale-Console-Application-Construction.md) consumes this plan and determines every final byte through exact literal/native spans and implicit zeros. The [portable verifier](Windvale-Console-Application-Verification.md) regenerates the same canonical recipe, checks every owned byte and zero gap, and recovers the native image and entry. C# remains the independent recovery construction and untrusted-executable oracle.
