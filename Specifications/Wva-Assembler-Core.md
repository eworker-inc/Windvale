...
The lexical scanner was first cross-host qualified at `e5fd109`, and the complete semantic inspector at `cc57bf9`. The object encoder and hosted write path described below are an implementation candidate until their exact revision completes Windows/Debian qualification with byte-for-byte Stage 0 equality.
...
The module currently duplicates small WVO record-construction patterns from `Wvoˉobjectˉcore` because Seed has no module import/composition contract. A later refactor may split scanner, semantic, encoding, and shared object facilities once that contract exists. The next independent architecture boundary is the linker: it owns multi-object resolution, layout, relocation application, map evidence, and final images rather than extending this assembler.
