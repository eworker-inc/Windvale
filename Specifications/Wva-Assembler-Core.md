...
The module consumes the shared Foundation contracts for alignment, machine names, ordinal byte-span comparison, and bounded unsigned decimal accumulation. It still owns WVA token spans, signed numbers, width rules, exact diagnostics, scanner, semantics, object measurement, and encoding; further extraction requires another demonstrated consumer rather than a broad split by file size. The separately owned linker consumes completed WVO bytes and owns multi-object resolution, layout, relocation application, map evidence, and final images rather than extending this assembler.
