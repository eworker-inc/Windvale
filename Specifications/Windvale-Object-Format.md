...
- A complete object is limited to 4 MiB in the Seed object tools.
...
Sections are strictly ordered by kind, then ordinal name, and names are unique. Alignment is a power of two from 1 through 4,096. Code, read-only-data, and writable-data sections require `memory size == data length`. A zero-fill section has no encoded data and a nonzero memory size. Total encoded section data is limited to 4 MiB; any one object's materialized plus zero-fill memory is limited to 16 MiB. Version 1.0 derives access policy from the kind and has no arbitrary section flags.
