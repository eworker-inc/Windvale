...
This does not give Windvale OS a runtime service table, output/file table, record or text allocator, general WVB loader/verifier, JIT, or hosted capability implementation. Exact commit `860c69c` proves that the ABI-16 compiler supplies service-free generated code through explicit versioned contexts, including one fixed Windvale-owned verify-before-execute path; the qualified predecessor proves the retained portable path under ABI 15.
