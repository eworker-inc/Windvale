...
The [Windvale service-table constructor](Windvale-Native-Service-Table-Construction.md) owns the exact version, size, slot order, and required/absent target presence from already published opaque service targets. The host derives those targets from the independently accepted publication plan, verifies the complete response, and owns allocation, copying, invocation, and teardown.
...
places the UTF-8 and integer-format core/bridge projects plus the shared
service-code-builder project beside their `Runtime/Windvale` sources. The
paired native front-door helper reproduces and inspects those five WVBs exactly;
...
[Decision 0513](../Documents/Decisions/0513-Native-Fixed-Service-And-Publication-Build-Inspection-Transfer.md)
adds component-local projects for the text-concatenation, text-quote,
enum-name, enum-metadata, and native-publication core/bridge pairs. The
service-bundle-materialization core and bridge remain genuine root aggregates
because they span Compiler, Foundation, and Runtime. The paired native helper
reproduces all twelve WVBs and inspects eleven public ownership surfaces; the
