...
Windvale OS still has no general runtime service registry, output filesystem, record or text allocator, general WVB loader/verifier, JIT, or dynamic hosted-capability implementation. The fixed grant proves that ABI-16 generated hosted code can consume one Windvale-selected immutable OS resource. Stage 0 still writes the table, PTE, and verified WVA stencil bytes after validating Windvale-owned records; those are named replacement seams rather than portable semantics.
