...
The experimental `/playground/webassembly-compiler/` route is a static, .NET-free source-to-result probe. Its disposable worker loads the package by manifest, compiles canonical `WVSS 1` source to `WVCO 1` WVB, resubmits the untrusted WVB for verification and scalar execution, and terminates after success, failure, or a ten-minute timeout. Current browser execution is substantially slower than the focused Node.js optimizing-tier check; do not treat this probe as the normal playground route until its bounded browser run is qualified. `verify:wasm-compiler-demo` checks the static containment boundary; `verify:wasm-compiler-core` executes the exact compile/verify/run pipeline under Node.js's optimizing WebAssembly tier.
