...
The site follows the visitor's operating-system light or dark preference through `prefers-color-scheme`. The browser playground lives below the same origin at <https://windvale.ca/playground/>, so navigation and the saved theme remain continuous. The normal playground, its focused compiler proof, and its direct-artifact demo are static applications over Windvale-authored WebAssembly; none starts Blazor or a .NET runtime. During local development, Vite proxies `/playground/` to an independent static Vite server at `http://127.0.0.1:5174/` while the browser stays on the website's `http://127.0.0.1:5173/` origin.
...
The playground's npm build copies only those manifest-owned files into `wwwroot/webassembly-compiler/artifacts/`. The generated directory is ignored locally and becomes part of the static playground publication; no compiler or .NET process runs during this packaging step.

The normal `/playground/` route now uses the static, .NET-free source-to-result worker. It loads the package by manifest, compiles canonical single-module `WVSS 1` source to `WVCO 1` WVB, resubmits the untrusted WVB for verification and scalar execution, and terminates after success, failure, or a ten-minute timeout. The focused `/playground/webassembly-compiler/` route retains the pinned proof presentation. Real Chromium produces the exact 183-byte WVB and result `42` in 85.9 seconds on the focused route and 101.3 seconds through the complete normal editor route, with zero framework requests. This is sufficient for the deliberately narrow development profile, not a general interactive-performance or language-surface claim. `verify:wasm-compiler-demo` checks both static containment boundaries; `verify:wasm-compiler-core` executes the exact compile/verify/run pipeline under Node.js's optimizing WebAssembly tier.
