...
`npm --prefix Website run verify:wasm-compiler-package` independently checks the pinned `Artifacts/WebAssembly-Playground` compiler package without .NET. It validates all three manifest identities, the interpreter's import-free ABI-3 and fixed-memory contract, and exact one-instruction admission of the packaged compiler WVB. Normal website publication verifies and copies this package. Its WVB and Wasm artifacts have digest-pinned native regeneration routes, while reconstructing the native tool packages remains an explicit Stage 0 recovery operation.
