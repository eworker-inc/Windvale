...
The site follows the visitor's operating-system light or dark preference through `prefers-color-scheme`. The browser playground lives below the same origin at <https://windvale.ca/playground/>, so navigation and the saved theme remain continuous. The playground is a static application over Windvale-authored WebAssembly and starts neither Blazor nor a .NET runtime. During local development, Vite proxies `/playground/` to an independent static Vite server at `http://127.0.0.1:5174/` while the browser stays on the website's `http://127.0.0.1:5173/` origin.
...
The 1731×909 `preview.png` is the approved editorial source for the default social preview. Public metadata uses the versioned 1200×630 `preview-2026-08-22.jpg` as its first Open Graph, X, and generic preview candidate, followed by the 512×512 `preview-square-2026-08-06.png` mark for clients that require or strongly prefer a square thumbnail. The current wide card presents Windvale as an inspectable computing stack and carries a restrained E-Worker stewardship mark plus the canonical `windvale.ca` address. Versioned derivatives are immutable, and prior derivatives remain published, so a later refresh receives a new URL without breaking an older cached reference. `support-og.png` remains the support-page wide preview and uses the same square fallback. Refresh these assets deliberately when their public visual direction changes, not automatically with every comic, and keep their declared dimensions, media types, hashes, and alt text synchronized with the pages and focused verifier that use them. Support-tier cards use lightweight code-native system diagrams so their visual language stays technical, responsive, and independent of decorative character art. `assets/favicon.png` and `assets/apple-touch-icon.png` use the Windvale wind-and-butterfly mark on a dark background so the mark remains visible in browser and device chrome.
...
The `Deploy verified website snapshot` workflow checks four times per day at 00:17, 06:17, 12:17, and 18:17 UTC. It normally publishes only when the exact current `main` commit has a successful push-triggered `Verify` run and differs from `deployment.json` on the live site. An unchanged or unverified commit stops before checkout, dependency installation, compilation, or Cloudflare publication. A manual dispatch can explicitly force a rebuild of an already-live commit.

During the bounded compiler 0.1-to-1.0 migration, a manual dispatch may set `allow_unverified_compiler_migration=true` when aggregate compiler verification is expected to fail. This does not turn failed evidence into passing evidence and never applies to scheduled deployment. The workflow still builds the complete website, verifies the published playground and pinned WebAssembly compiler package, and publishes only if those website-specific steps pass. Each use remains visible in the workflow inputs and warning log. Remove this exception when the migration reaches its restored aggregate-verification checkpoint.
