...
- `Database/` contains reusable capability-free database algorithms. `Storage-Geometry.wv` owns checked zero-based `u64` page-range arithmetic, `Storage-Page.wv` owns immutable read plans and exact response admission, `Durable-Superblock.wv` owns `WVDS 1` encoding and dual-slot recovery selection, `Durable-Page.wv` owns `WVPG 1` physical pages, `Durable-Commit-Record.wv` owns `WVCR 1` commit linkage, `Commit-Publication.wv` owns the durable-before-publish state machine, `Storage-Publication.wv` maps that state to bounded storage actions, `Storage-Recovery.wv` selects and repairs an unpublished tail without uncertain mutation replay, and `Wvdb-Reader.wv` remains the bounded experimental snapshot reader.
