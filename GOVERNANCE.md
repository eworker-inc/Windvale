...
The official GitHub repository is the private `eworker-inc/Windvale` repository under the existing [E-Worker Inc organization](https://github.com/eworker-inc), which can contain Windvale and other E-Worker projects. The administrative service account is [`EWorkerAI`](https://github.com/EWorkerAI); its login identity remains separate from source authorship. The repository is inspected privately before public visibility. Individual or service accounts administer it through least-privilege roles; the organization is not named after one project or AI model.
