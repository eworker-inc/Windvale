...
The grammar follows the implemented Windvale Seed lexical contract in `Specifications/Seed-Language.md` and `Specifications/Compiler-Source-Lexer.md`. It recognizes declarations, control flow, profiles, built-in types, Boolean and numeric literals, strict string escapes, `//` comments, operators, calls, member access, and Windvale's U+02C9 macron-separated identifiers.
