...
pinned_analyzer_wvb="$repository_root/Artifacts/Language-1.0-Target-Aware-Emission-Bootstrap/Wvb/wvanalyze.wvb"
pinned_emitter_wvb="$repository_root/Artifacts/Language-1.0-Target-Aware-Emission-Bootstrap/Wvb/wvemit.wvb"
export WINDVALE_SPLIT_COMPILER_ACTIVITY=0
...
compare_split_reports() {
    ...
    local right=$2
    local status
    grep -v '^INFO  split compiler active ' -- "$left" \
        >"$work/Deterministic-Left.out"
    status=$?
    [[ $status -le 1 ]] || return 1
    grep -v '^INFO  split compiler active ' -- "$right" \
        >"$work/Deterministic-Right.out"
    status=$?
    [[ $status -le 1 ]] || return 1
    cmp -s -- "$work/Deterministic-Left.out" \
        "$work/Deterministic-Right.out"
}
...
expect_analysis_failure() {
    ...
    node "$script_directory/Verify-Source-Analysis-Diagnostic.mjs" \
        "$work/$prefix.err" wir "$expected_status"
}
...
expect_profiled_symbol_failure() {
    ...
    [[ ${#analysis_lines[@]} -eq 1 ]] || return 1
    node "$script_directory/Verify-Source-Analysis-Diagnostic.mjs" \
        "$work/$prefix.err" symbols "$expected_status"
}
...
printf '%s  %s\n' \
    ...
    "$pinned_analyzer_wvb" | sha256sum --check --strict --quiet || exit $?
[[ $(wc -c < "$pinned_emitter_wvb") -eq 1556434 ]] || exit 1
printf '%s  %s\n' \
    d16cc44f65a788a8c2dc45d423686dde095cac63e8f2fd8305d1246b29c168f9 \
    "$pinned_emitter_wvb" | sha256sum --check --strict --quiet || exit $?
"$script_directory/Package-Segmented-Compiler-Wvb.sh" 8 \
    "$pinned_analyzer_wvb" "$work/Pinned-Analyzer.elf" \
    --development-cache || exit $?
"$script_directory/Package-Segmented-Compiler-Wvb.sh" 8 \
    "$pinned_emitter_wvb" "$work/Pinned-Emitter.elf" \
    --development-cache || exit $?
node "$script_directory/Write-Split-Compiler-Producer-Identity.mjs" \
    analyzer "$work/Pinned-Analyzer.elf" \
    "$work/Pinned-Analyzer.identity" || exit $?
node "$script_directory/Write-Split-Compiler-Producer-Identity.mjs" \
    emitter "$work/Pinned-Emitter.elf" \
    "$work/Pinned-Emitter.identity" || exit $?
node "$script_directory/Build-Cached-Split-Project-Wvb.mjs" \
    ...
    "$work/Admitter.wvb" \
    "$work/Pinned-Analyzer.elf" "$work/Pinned-Analyzer.identity" \
    "$work/Pinned-Emitter.elf" "$work/Pinned-Emitter.identity" || exit $?
node "$script_directory/Build-Cached-Split-Project-Wvb.mjs" \
...
node "$script_directory/Build-Cached-Segmented-Hosted-Wvb.mjs" 7 \
    "$work/Validator.wvb" "$work/Validator.elf" || exit $?
"$script_directory/Package-Segmented-Compiler-Wvb.sh" 8 \
    "$work/Analyzer.wvb" "$work/Analyzer.elf" --development-cache || exit $?
node "$script_directory/Write-Split-Compiler-Producer-Identity.mjs" \
    analyzer "$work/Analyzer.elf" "$work/Analyzer.identity" || exit $?
node "$script_directory/Verify-Source-Wir-Incremental-Generics.mjs" \
    "$work/Analyzer.elf" "$work" || exit $?
...
analyze_authenticated \
    ...
    2>"$work/Memory-Budget-Split-Analysis.err" || exit $?
[[ ! -s $work/Memory-Budget-Split-Analysis.err ]] || exit 1
grep -Fq 'source analysis status=Published ' \
    "$work/Memory-Budget-Split-Analysis.out" || exit 1
mkdir -- "$work/Memory-Budget-Split-Malformed" || exit $?
node "$script_directory/Verify-Language-1.0-Memory-Budget-Split-Wir.mjs" \
    "$work/Emitter.elf" \
    "$work/Memory-Budget-Split.wvss" \
    "$work/Memory-Budget-Split.wvca" \
    "$work/Memory-Budget-Split.wvlb" \
    "$work/Memory-Budget-Split.wvir" \
    "$work/Memory-Budget-Split-Malformed" || exit $?
expect_profiled_analysis_failure_with_dependencies \
    "$repository_root/Tests/Fixtures/Language-1.0/Memory-Budget-Split-Immutable-Borrow.wv" \
    Memory-Budget-Split-Immutable-Borrow Invalidˉborrow \
    "$repository_root/Libraries/Foundation/Memory/Memory.wv" \
    "$repository_root/Libraries/Foundation/Values/Result.wv" || exit $?
expect_profiled_analysis_failure_with_dependencies \
    "$repository_root/Tests/Fixtures/Language-1.0/Memory-Budget-Split-Wrong-Limit.wv" \
    Memory-Budget-Split-Wrong-Limit Invalidˉargument \
    "$repository_root/Libraries/Foundation/Memory/Memory.wv" \
    "$repository_root/Libraries/Foundation/Values/Result.wv" || exit $?
expect_profiled_analysis_failure_with_dependencies \
    "$repository_root/Tests/Fixtures/Language-1.0/Memory-Budget-Split-Wrong-Result.wv" \
    Memory-Budget-Split-Wrong-Result Genericˉresolution \
    "$repository_root/Libraries/Foundation/Memory/Memory.wv" \
    "$repository_root/Libraries/Foundation/Values/Result.wv" || exit $?
expect_profiled_analysis_failure_with_dependencies \
    "$repository_root/Tests/Fixtures/Language-1.0/Memory-Budget-Split-Wrong-Allocation-Failure.wv" \
...
compare_split_reports "$work/Compile-A.out" "$work/Compile-B.out" || exit 1
...
"$script_directory/Package-Segmented-Compiler-Wvb.sh" 5 \
    "$work/Floating-Runner.wvb" "$work/Floating-Runner.elf" \
    --development-cache \
    >/dev/null || exit $?
...
compare_split_reports "$work/Multi-Field-Variant-A.out" \
...
compare_split_reports "$work/Foundation-Generic-A.out" \
...
node "$script_directory/Run-Split-Compiler.mjs" "$work/Admitter.elf" "$work/Validator.elf" "$work/Analyzer.elf" "$work/Emitter.elf" \
    ...
    --source-profile "$source_profile" \
    --target-descriptor "$target_descriptor" \
    "$repository_root/Tests/Fixtures/Language-1.0/Generic-Multiple-Specializations-Executable.wv" \
...
compare_split_reports "$work/Generic-Specializations-A.out" \
