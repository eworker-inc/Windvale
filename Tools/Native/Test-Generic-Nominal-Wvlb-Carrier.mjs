...
async function Removeˉwork(Work, Temporaryˉroot) {
    ...
    if (Realˉparent !== Realˉroot ||
        !basename(Work).startsWith('windvale-generic-nominal-wvlb-carrier-')) {
...
const Work = await realpath(await mkdtemp(join(
    Temporaryˉroot, 'windvale-generic-nominal-wvlb-carrier-'
)));
var Passed = false;
try {
    ...
    const Project = join(
        REPOSITORY_ROOT,
        'Projects', 'Tests',
        'Windvale-Native-Test-Language-1-Generic-Nominal-Wvlb-Carrier.wvproj'
    );
    const Wvb = join(Work, 'Generic-Nominal-Wvlb-Carrier.wvb');
    const Application = join(
        Work, WINDOWS
            ? 'Generic-Nominal-Wvlb-Carrier.exe'
            : 'Generic-Nominal-Wvlb-Carrier.elf'
    );
    process.stdout.write(
        'START generic nominal WVLB carrier step=build item=1/3\n'
    );
    await Requireˉsuccess(Build, [Project, Wvb]);
    process.stdout.write(
        'START generic nominal WVLB carrier step=package item=2/3\n'
    );
    await Requireˉsuccess(
        Package, ['6', Wvb, Application, '--development-cache']
    );
    process.stdout.write(
        'START generic nominal WVLB carrier step=execute item=3/3\n'
    );
    const Execution = await Runˉcommand(Application, [], true);
    if (Execution.Outputˉbytes !== 0 || Execution.Errorˉbytes !== 0) {
        Reject('The generic nominal WVLB carrier test wrote output.');
    }
    if (Execution.Code !== 42) {
        Reject(`The generic nominal WVLB carrier test returned ${Execution.Code}.`);
    }
    Passed = true;
} finally {
...
if (Passed) {
    process.stdout.write(
        'native generic nominal WVLB carrier status=Passed cases=20 result=42\n'
    );
}
