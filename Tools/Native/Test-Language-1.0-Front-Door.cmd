...
for %%F in ("%Work%\Analyzer.wvb") do if not "%%~zF"=="1098751" goto :cleanup
certutil -hashfile "%Work%\Analyzer.wvb" SHA256 | findstr /I /C:"4e24d6312b01efbd8caeb155ed1a0ce4339f4debe3cf2d77e300798e11ccd68b" >nul || goto :cleanup
...
for %%F in ("%Work%\Emitter.wvb") do if not "%%~zF"=="1013482" goto :cleanup
certutil -hashfile "%Work%\Emitter.wvb" SHA256 | findstr /I /C:"3fb526c3298406a3ba71df5e074d58d000532e80640421fc4d665389d7a0ea0d" >nul || goto :cleanup
call "%Native%\Package-Segmented-Compiler-Wvb.cmd" 7 ^
...
call "%Native%\Run-Wvb.cmd" "%Work%\Borrow-Call.wvb" ^
    >"%Work%\Borrow-Call-Run.out" ^
    2>"%Work%\Borrow-Call-Run.err" || goto :cleanup
for %%F in ("%Work%\Borrow-Call-Run.err") do if not "%%~zF"=="0" goto :cleanup
call :expect_result_42 "%Work%\Borrow-Call-Run.out" || goto :cleanup
call :expect_analysis_failure ^
...
set "FailureStep=compiler-generic-wir"
echo START language 1 front door step=generic-wir-split
node "%Native%\Build-Cached-Split-Project-Wvb.mjs" ^
    "%RepositoryRoot%\Projects\Tests\Windvale-Native-Test-Language-1-Generic-Wir.wvproj" ^
...
for %%F in ("%Work%\Generic-Wir-A.wvb") do if not "%%~zF"=="1257466" goto :cleanup
certutil -hashfile "%Work%\Generic-Wir-A.wvb" SHA256 | findstr /I /C:"d170b8bcf3c41c746d25e623e5253e6c4232275119cec9ffe13091276db697dd" >nul || goto :cleanup
...
set "FailureStep=compiler-generic-wir-current-native-verifier"
"%Work%\Verifier.exe" "%Work%\Generic-Wir-A.wvb" ^
    >"%Work%\Verify-Generic-Wir.out" 2>"%Work%\Verify-Generic-Wir.err" || goto :cleanup
for %%F in ("%Work%\Verify-Generic-Wir.err") do if not "%%~zF"=="0" goto :cleanup
findstr /c:"wvb status=Valid profile=compiler-aligned" ^
    "%Work%\Verify-Generic-Wir.out" >nul || goto :cleanup
echo PASS  language 1 front door step=generic-wir-split wvb-bytes=%GenericWirWvbBytes% verification=current-native
...
fc /b "%Work%\Fixed-Array-A.out" "%Work%\Fixed-Array-B.out" >nul || goto :cleanup
...
if not "%Result%"=="0" (
    ...
    if exist "%Work%\Verify-Generic-Wir.out" type "%Work%\Verify-Generic-Wir.out" >&2
    if exist "%Work%\Verify-Generic-Wir.err" type "%Work%\Verify-Generic-Wir.err" >&2
...
echo native language 1 front door status=Passed cases=380 frozen-inputs=251 source-fixtures=86 descriptor-cases=33 profile-cases=4 value-front-end-cases=39 generic-front-end-cases=4 generic-resolution-cases=1 generic-type-catalog-cases=1 generic-specialization-cases=4 generic-wir-cases=4 generic-nominal-pipeline-cases=26 generic-nominal-function-body-cases=33 generic-nominal-declaration-dependency-cases=33 generic-nominal-variant-cases=97 compiler-cases=36 borrow-cases=9 fixed-integer-cases=22 rune-cases=20 floating-cases=27 fixed-array-cases=6 vector-sequence-type-cases=6 vector-sequence-runtime-cases=9 unit-never-cases=21 multi-field-variant-cases=25 typed-failure-cases=5 foundation-generic-cases=6 compiler-result=42 compiler-wvb-bytes=221 generic-wir-wvb-bytes=%GenericWirWvbBytes% generic-type-catalog-wvb-bytes=%GenericTypeCatalogWvbBytes% generic-nominal-variant-wvb-bytes=%GenericNominalVariantWvbBytes% value-if-wvb-bytes=%ValueIfWvbBytes% value-match-wvb-bytes=%ValueMatchWvbBytes% value-match-never-wvb-bytes=%ValueMatchNeverWvbBytes% unit-wvb-bytes=%UnitWvbBytes% never-wvb-bytes=%NeverWvbBytes% record-update-wvb-bytes=1116 fixed-integer-wvb-bytes=5335 rune-wvb-bytes=%RuneWvbBytes% floating-wvb-bytes=%FloatingWvbBytes% fixed-array-wvb-bytes=%FixedArrayWvbBytes% vector-sequence-type-wvb-bytes=%VectorSequenceTypesWvbBytes% vector-sequence-runtime-wvb-bytes=971 multi-field-variant-wvb-bytes=%MultiFieldVariantWvbBytes% typed-failure-wvb-bytes=%ResultTryWvbBytes% foundation-generic-wvb-bytes=%FoundationGenericWvbBytes% generic-specializations-wvb-bytes=%GenericSpecializationsWvbBytes%
exit /b 0
...
if not "!AnalysisLine!"=="source analysis status=Sourceˉwir symbol-status=Valid binding-status=Valid wir-status=%~3" exit /b 1
exit /b 0
