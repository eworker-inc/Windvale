...
async function Removeˉwork(Work, Temporaryˉroot) {
    ...
    if (Realˉparent !== Realˉroot ||
        !basename(Work).startsWith('windvale-generic-nominal-declarations-')) {
...
const Work = await mkdtemp(join(
    Temporaryˉroot, 'windvale-generic-nominal-declarations-'
));
var Passed = false;
try {
    ...
    const Project = join(
        REPOSITORY_ROOT,
        'Projects', 'Tests',
        'Windvale-Native-Test-Language-1-Generic-Nominal-Declaration.wvproj'
    );
    const Wvb = join(Work, 'Generic-Nominal-Declarations.wvb');
    const Application = join(
        Work, WINDOWS
            ? 'Generic-Nominal-Declarations.exe'
            : 'Generic-Nominal-Declarations.elf'
    );
    process.stdout.write(
        'START generic nominal declarations step=build item=1/3\n'
    );
    await Requireˉsuccess(Build, [Project, Wvb]);
    process.stdout.write(
        'START generic nominal declarations step=package item=2/3\n'
    );
    await Requireˉsuccess(
        Package, ['6', Wvb, Application, '--development-cache']
    );
    process.stdout.write(
        'START generic nominal declarations step=execute item=3/3\n'
    );
    const Execution = await Runˉcommand(Application, [], true);
    if (Execution.Outputˉbytes !== 0 || Execution.Errorˉbytes !== 0) {
        Reject('The generic nominal declaration test wrote output.');
    }
    if (Execution.Code !== 42) {
        Reject(`The generic nominal declaration test returned ${Execution.Code}.`);
    }
    Passed = true;
} finally {
...
if (Passed) {
    process.stdout.write(
        'native generic nominal declarations status=Passed cases=12 result=42\n'
    );
}
