...
pass 'retained candidate inventory'

if "$script_directory/Verify-Compiler-Convergence.sh" unexpected >/dev/null 2>&1; then
    fail
elif [[ $? -ne 64 ]]; then
    fail
fi
...
pass 'retained-to-current compiler differential smoke'
