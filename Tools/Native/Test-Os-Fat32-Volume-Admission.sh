...
[[ $? -eq 47 ]] || exit 1
"$script_directory/Build-Wvb.sh" "$repository_root/Projects/Tests/Windvale-Native-Test-Os-Fat32-Directory-Admission.wvproj" "$work/Directory-Test.wvb" >/dev/null || exit $?
verify "$work/Directory-Test.wvb" 12922 adfd7e04136874ab60157cf5a718565a8aa73f19cebe61a32d2c219dfb6c0dc7 || exit $?
"$script_directory/Lower-Wvb-To-Wvo.sh" "$work/Directory-Test.wvb" "$work/Directory-Test.wvo" >/dev/null || exit $?
verify "$work/Directory-Test.wvo" 132089 88ace5e3672136f1834b81ca54c16141c7583aa7f5461f2bb0a9716b593a393e || exit $?
"$script_directory/Link-Wvo.sh" 0 Main "$work/Directory-Test.bin" "$work/Directory-Test.wvo" >/dev/null || exit $?
verify "$work/Directory-Test.bin" 130449 eeafe20120fcce8e657c0f3b70c57306436fe1d548c72536fb12720fe3f724b9 || exit $?
"$script_directory/Package-Console.sh" windows-x64-console-v1 "$work/Directory-Test.bin" 5611 "$work/Directory-Test.exe" >/dev/null || exit $?
verify "$work/Directory-Test.exe" 132608 1debd01031f349f444aa66f09f7bd248fdf65333cbaa279336701503f88651c0 || exit $?
"$script_directory/Package-Console.sh" linux-x64-console-v1 "$work/Directory-Test.bin" 5611 "$work/Directory-Test.elf" >/dev/null || exit $?
