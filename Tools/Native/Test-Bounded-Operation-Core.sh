...
if [[ $# -ne 0 ]]; then
    echo 'Usage: ./Tools/Native/Test-Bounded-Operation-Core.sh' >&2
    exit 64
fi
...
cleanup() {
    case "$work" in
        "$temporary_root"/windvale-bounded-operation.*)
            rm -f -- "$work"/*
            rmdir -- "$work"
            ;;
        *)
...
library_project_resource=${repository_root//\\//}/Projects/Libraries/Windvale-Library-Bounded-Operation-Core.wvproj
test_project_resource=${repository_root//\\//}/Projects/Tests/Windvale-Native-Test-Bounded-Operation-Core.wvproj
...
verify_file "$lowerer" 7598080 a58fd44c8c19da19a1699b33392996a673e291f6d9f951eb578f829c4b2b5452 || exit $?
...
"$script_directory/Link-Wvo.sh" 0 Main "$work/Operation-Image.chunk-0" "$work/Test-A.wvo" >"$work/Link.txt" || exit $?
...
"$script_directory/Build-Cached-Hosted-Application.sh" 6 "$work/Test-A.wvb" "$work/Operation-Image" 1 "$entry" "$work/Operation.elf" linux >/dev/null || exit $?
"$script_directory/Build-Cached-Hosted-Application.sh" 6 "$work/Test-A.wvb" "$work/Operation-Image" 1 "$entry" "$work/Operation.exe" windows >/dev/null || exit $?
set +e
"$work/Operation.elf" >/dev/null
application_result=$?
set -e
