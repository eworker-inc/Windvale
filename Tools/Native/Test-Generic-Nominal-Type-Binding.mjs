...
async function Removeˉwork(Work, Temporaryˉroot) {
    ...
    if (Realˉparent !== Realˉroot ||
        !basename(Work).startsWith('windvale-generic-nominal-type-binding-')) {
...
const Work = await mkdtemp(join(
    Temporaryˉroot, 'windvale-generic-nominal-type-binding-'
));
var Passed = false;
try {
    ...
    const Project = join(
        REPOSITORY_ROOT,
        'Projects', 'Tests',
        'Windvale-Native-Test-Language-1-Generic-Nominal-Type-Binding.wvproj'
    );
    const Wvb = join(Work, 'Generic-Nominal-Type-Binding.wvb');
    const Application = join(
        Work, WINDOWS
            ? 'Generic-Nominal-Type-Binding.exe'
            : 'Generic-Nominal-Type-Binding.elf'
    );
    process.stdout.write(
        'START generic nominal type binding step=build item=1/3\n'
    );
    await Requireˉsuccess(Build, [Project, Wvb]);
    process.stdout.write(
        'START generic nominal type binding step=package item=2/3\n'
    );
    await Requireˉsuccess(
        Package, ['6', Wvb, Application, '--development-cache']
    );
    process.stdout.write(
        'START generic nominal type binding step=execute item=3/3\n'
    );
    const Execution = await Runˉcommand(Application, [], true);
    if (Execution.Outputˉbytes !== 0 || Execution.Errorˉbytes !== 0) {
        Reject('The generic nominal type-binding test wrote output.');
    }
    if (Execution.Code !== 42) {
        Reject(`The generic nominal type-binding test returned ${Execution.Code}.`);
    }
    Passed = true;
} finally {
...
if (Passed) {
    process.stdout.write(
        'native generic nominal type binding status=Passed cases=45 result=42\n'
    );
}
