...
set "SelectedCases=32"
if /I not "%DevelopmentTarget%"=="all" set "SelectedCases="
for %%T in (
    tree-node logical-record typed-row transaction-mutations transaction-leaf-rewrite transaction-leaf-pages query-ir sql-lowerer json-value json-protocol local-service collection-catalog bootstrap single-leaf
...
echo native database storage status=Passed cases=41 local-results=0 cross-host-images=Verified
exit /b 0
