...
async function Removeˉwork(Work, Temporaryˉroot) {
    ...
    if (Realˉparent !== Realˉroot ||
        !basename(Work).startsWith('windvale-generic-nominal-development-')) {
...
const Work = await realpath(await mkdtemp(join(
    Temporaryˉroot, 'windvale-generic-nominal-development-'
)));
var Passed = false;
try {
    ...
    const Wvb = join(Work, 'Generic-Nominal-Development-Bundle.wvb');
    const Application = join(
        Work, WINDOWS
            ? 'Generic-Nominal-Development-Bundle.exe'
            : 'Generic-Nominal-Development-Bundle.elf'
    );
    process.stdout.write(
        'START generic nominal development bundle step=build item=1/3\n'
    );
    await Requireˉsuccess(Build, [Project, Wvb]);
    process.stdout.write(
        'START generic nominal development bundle step=package item=2/3\n'
    );
    await Requireˉsuccess(
        Package, ['6', Wvb, Application, '--development-cache']
    );
    process.stdout.write(
        'START generic nominal development bundle step=execute item=3/3\n'
    );
    const Execution = await Runˉcommand(Application, [], true);
    if (Execution.Outputˉbytes !== 0 || Execution.Errorˉbytes !== 0) {
        Reject('The generic nominal development bundle wrote output.');
    }
    if (Execution.Code !== 42) {
        Reject(
            `The generic nominal development bundle returned ${Execution.Code}.`
        );
    }
    Passed = true;
} finally {
