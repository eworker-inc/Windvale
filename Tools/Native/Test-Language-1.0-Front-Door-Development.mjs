...
async function Runˉcommand(Tool, Arguments, Deadline, StreamOutput = false) {
    const Remaining = Deadline - Date.now();
    if (Remaining <= 0) throw Object.assign(new Error('Front-end development timed out.'), { exitCode: 124 });
    ...
    return new Promise((Complete, Reject) => {
        ...
        const Timer = setTimeout(() => Stop('Front-end development timed out.', 124), Remaining);
        const Activity = setInterval(() => process.stdout.write(
            `INFO front-end development step=active tool=${basename(Tool)}\n`), 30_000);
        for (const [Stream, Chunks] of [[Child.stdout, Output], [Child.stderr, Errors]]) {
            Stream.on('data', Chunk => {
                Bytes += Chunk.length;
                if (Bytes > MAXIMUM_OUTPUT_BYTES) Stop('Front-end diagnostic limit exceeded.', 2);
                else {
                    Chunks.push(Chunk);
                    if (StreamOutput && Stream === Child.stdout) process.stdout.write(Chunk);
                }
            });
        }
