...
function Add-Os-Suite {
    param([Parameter(Mandatory)][string]$Path)
    if ($Path -match 'X64-(?:Code|Process-(?:Entry|Privileged-Entry|Thread-Timer-State|Timer-Activation|Init-Return-Program-Validation|Provider-(?:User-Transfer|Return-Init-Transfer)|Coordinator|Endpoint|Memory-Allocation|Record|Paging|Image|Client-(?:Reservation|Record|Paging|Image|Program-Resource|Budget-Resource|Store-Resource|Directory-Resource|Store-Validation|Directory-Validation)|Directory-(?:Allocation|Record|Paging|Image)))-Emission') {
