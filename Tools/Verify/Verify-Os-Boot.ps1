...
$EXPECTED_SERIAL_MARKER =
    "windvale-os-boot 15`nentry=pass`nsystem-table=pass`nmemory-map=pass`nboot-services=exited`nmemory-owned=pass`nallocator=pass`nkernel-stack=pass`nHello from Windvale`nnative-context=pass`nnative-wvb=pass`nwindvale-source=pass`nstatus=pass`n"
...
try {
    ...
    $Report = [pscustomobject][ordered]@{
        ...
        EfiSha256 = $EfiSha256
        SerialMarker = 'windvale-os-boot-15-entry-system-table-memory-map-boot-services-exited-memory-owned-allocator-kernel-stack-hello-native-context-native-wvb-windvale-source-status-pass'
    ...
    if ($PassThru) {
        ...
        Write-Output 'windvale-os-boot-report 15'
