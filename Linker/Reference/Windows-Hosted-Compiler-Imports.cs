...
internal static class Windowsˉhostedˉcompilerˉimports
{
    ...
    internal const int GET_FILE_SIZE_IAT_OFFSET = KERNEL_IAT_OFFSET + 32;
    internal const int GET_LAST_ERROR_IAT_OFFSET = KERNEL_IAT_OFFSET + 40;
    internal const int GET_STD_HANDLE_IAT_OFFSET = KERNEL_IAT_OFFSET + 48;
    internal const int LOCAL_FREE_IAT_OFFSET = KERNEL_IAT_OFFSET + 56;
    internal const int MULTI_BYTE_TO_WIDE_CHAR_IAT_OFFSET = KERNEL_IAT_OFFSET + 64;
    internal const int READ_FILE_IAT_OFFSET = KERNEL_IAT_OFFSET + 72;
    internal const int WIDE_CHAR_TO_MULTI_BYTE_IAT_OFFSET = KERNEL_IAT_OFFSET + 80;
    internal const int WRITE_FILE_IAT_OFFSET = KERNEL_IAT_OFFSET + 88;
    ...
    internal static ImmutableArray<byte> Build(uint importˉaddress)
    {
        if (importˉaddress % PAGE_BYTES != 0)
        {
            throw new ArgumentOutOfRangeException(
                ...
                "The Windows hosted-compiler import page must be page aligned.");
        }
    ...
    internal static void Verify(ReadOnlySpan<byte> bytes, uint importˉaddress)
    {
        ...
        {
            throw Invalid("The Windows hosted-compiler import page has an invalid extent.");
        }
        Verifyˉdescriptor(
            bytes,
            DIRECTORY_OFFSET,
        ...
        {
            throw Invalid("The Windows hosted-compiler import page has noncanonical padding.");
        }
    }
    ...
    private static void Requireˉascii(ReadOnlySpan<byte> bytes, int offset, string value)
    {
        ...
        {
            throw Invalid($"The Windows hosted-compiler import name '{value}' is invalid.");
        }
    }
