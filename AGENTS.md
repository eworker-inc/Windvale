...
- Prefer `Tools/Verify/Verify-Changed.ps1` for the Windows inner loop. Its change classifier uses a lightweight scope for ordinary documentation and editor-package-only work, a website scope for static site, browser packaging, Cloudflare function, and website-tool changes, and qualification for specifications, implementation, unrecognized configuration, and mixed higher-risk changes. The website scope runs `Tools/Verify/Verify-Website.ps1` instead of Seed qualification. Use `Tools/Verify/Verify-Seed.ps1 -Level Fast -TestArea <area> [-TestFilter '<displayed-name substring>'] -FailFast` when selecting Seed tests explicitly; Fast and the no-argument Development suite omit explicitly extended compiler-closure, AOT-transport, full-stage, and golden contracts. Add `-IncludeExtended` to an explicit Fast selection only when an extended contract is the narrowest affected check. Run `Tests/Windvale.Os.Tests` directly for focused OS work. Development includes every regular Seed test plus the bounded OS in-process suite. Use `-Level Standard` for every regular and extended Seed in-process contract plus the OS suite, and request `-Level Qualification` explicitly for the complete native CLI gate.
