...
- C# and .NET remain the Stage 0 bootstrap and recovery path, not permanent product dependencies. Under Decision 0213, the C# source compiler becomes feature-frozen at the next qualified WVB 1.11 baseline: new source-language semantics belong in `Compiler/Windvale`, while bounded Stage 0 corrections remain permitted for recovery, security, qualification, and independent evidence. Retire .NET from the normal build, test, packaging, and execution path only after the documented native-retirement gate is qualified on Windows and Linux; preserve recovery provenance explicitly.
...
- Prefer `Tools/Verify/Verify-Changed.ps1` for the Windows inner loop. Its change classifier uses a lightweight scope for ordinary documentation and editor-package-only work, a website scope for static site, browser packaging, Cloudflare function, and website-tool changes, and qualification scope for higher-risk changes. Qualification-scoped changed-file verification maps maintained boundaries to focused native retirement suites in canonical order and refuses explicit uncovered gaps; it does not fall back to .NET or the complete unfiltered native gate. The website scope runs `Tools/Verify/Verify-Website.ps1`. Use the managed `Tools/Verify/Verify-Seed.ps1 -Level Fast -TestArea <area> [-TestFilter '<displayed-name substring>'] -FailFast` only as an explicit recovery/differential diagnostic while its remaining native evidence gap is being closed. Fast and the no-argument Development suite omit explicitly extended compiler-closure, AOT-transport, full-stage, and golden contracts. Add `-IncludeExtended` to an explicit Fast selection only when that managed recovery evidence is specifically required. Run focused native OS owners through their retirement-suite filters. Use local Standard or Qualification only when the changed boundary or final gate explicitly requires managed comparison evidence.
