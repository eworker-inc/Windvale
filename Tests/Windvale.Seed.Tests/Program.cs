...
internal static class Program
{
    ...
    private const string WEBASSEMBLY_CORE_SHA256 = "1cc90cbe50b2605096c7002e52c5075423dbf3f567ee4bd70d141dee52e61008";
    private const string WEBASSEMBLY_TOOL_SHA256 = "851f8122acc9314b1061db5a907841dbb4f9aa68641aedc6ef73f3d94f949cd3";
    private const string WEBASSEMBLY_DEMO_SHA256 = "083222ad86052dc39e3415850ff67dc09a52b6f144aa2dad329b860ad0a5c5d9";
    ...
    private const string WEBASSEMBLY_RUNTIME_U32_GUARD_WVB_SHA256 = "d6ba02dfe12efdcb7c2f8ed6664551a776e79e3ff2c30134dc7e3642ee7ce743";
    ...
    private const string WEBASSEMBLY_RUNTIME_CALLS_WVB_SHA256 = "a44c8bdbf9983a7929a769d5ca2e0b60323d72cf96b04e31450d9757bb15729a";
    private const string WEBASSEMBLY_RUNTIME_CALLS_SHA256 = "5ee04d5b3b33399dce61709135709f0d0ebb7d6374e14759d83986859806eadd";
    ...
    private const string WEBASSEMBLY_WVB_SCALAR_INTERPRETER_WVB_SHA256 = "f0f51936fec70d64f5d8021733b8d8312bce14ba411d90e4896ef19e317fb7a4";
    private const string WEBASSEMBLY_WVB_SCALAR_INTERPRETER_SHA256 = "683410069c64d0143f748d34cb63f16b7d36c130662c282c003b981b24d37580";
    ...
    private static readonly string SOURCE_WVB_SCALAR_INTERPRETER_I32_OVERFLOW_SOURCE = Readˉembeddedˉsource(
        "Windvale.Seed.Tests.Source-Wvb-Scalar-Interpreter-I32-Overflow.wv");

    private static readonly string SOURCE_WVB_SCALAR_INTERPRETER_U32_OVERFLOW_SOURCE = Readˉembeddedˉsource(
        "Windvale.Seed.Tests.Source-Wvb-Scalar-Interpreter-U32-Overflow.wv");
    ...
    private static readonly string WEBASSEMBLY_RUNTIME_U32_GUARD_SOURCE = Readˉembeddedˉsource(
        "Windvale.Seed.Tests.WebAssembly-Runtime-U32-Guard-Main.wv");
    private static readonly string WEBASSEMBLY_RUNTIME_CALLS_SOURCE = Readˉembeddedˉsource(
        "Windvale.Seed.Tests.WebAssembly-Runtime-Calls-Main.wv");
    ...
    private static void Windowsˉconsoleˉapplicationˉruns()
    {
        ...
        Rejectˉwindowsˉapplication(
            Mutateˉbyte(
                First.Imageˉbytes,
                0x200 + Windowsˉconsoleˉapplicationˉcontract.STARTUP_BYTES),
            "WVW2007");
        ...
        foreach (var Case in Portableˉprocessˉresultˉcases())
        {
            var Application = Windowsˉconsoleˉapplicationˉwriter.Write(
                Compileˉnativeˉprocessˉresult(Case.Windvaleˉresult));
            True(Application.Success, "The Windows target rejected a process-result boundary fixture.");
            Equal(Case.Processˉresult, Executeˉwindowsˉapplication(Application.Imageˉbytes));
        }
    }
    ...
    private static void Linuxˉconsoleˉapplicationˉruns()
    {
        ...
        foreach (var Case in Portableˉprocessˉresultˉcases())
        {
            var Application = Linuxˉconsoleˉapplicationˉwriter.Write(
                Compileˉnativeˉprocessˉresult(Case.Windvaleˉresult));
            True(Application.Success, "The Linux target rejected a process-result boundary fixture.");
            Equal(Case.Processˉresult, Executeˉlinuxˉapplication(Application.Imageˉbytes));
        }
    }

    private static (int Windvaleˉresult, int Processˉresult)[] Portableˉprocessˉresultˉcases() =>
    [
        (-1, 1),
        (0, 0),
        (1, 1),
    ...
    private static void Nativeˉconsoleˉapplicationˉpublicationˉisˉatomic()
    {
        ...
        var Outputˉpath = System.IO.Path.Combine(Directoryˉpath, "Application.elf");
        ...
        try
        {
            ...
            try
            {
                Windvale.Tool.Atomicˉfileˉpublisher.Publish(
                    Outputˉpath,
                    [5, 6, 7],
                    _ => throw new IOException("Injected prepublication failure."));
                throw new InvalidOperationException("Atomic publication accepted the injected failure.");
            }
            catch (IOException Exception)
            {
                True(
                    Exception.Message.Contains("Injected prepublication failure", StringComparison.Ordinal),
                    "Atomic publication returned the wrong injected failure.");
            }

            Sequenceˉequal(Original, File.ReadAllBytes(Outputˉpath));
            Equal(
                0,
                Directory.GetFiles(Directoryˉpath, ".Application.elf.*.tmp").Length);

            byte[] Replacement = [8, 9, 10];
            Action<string>? Prepareˉtemporary = null;
            if (OperatingSystem.IsLinux())
            {
                Prepareˉtemporary = Prepareˉlinuxˉtestˉexecutable;
            }
            Windvale.Tool.Atomicˉfileˉpublisher.Publish(
                Outputˉpath,
                Replacement,
                Prepareˉtemporary);
            Sequenceˉequal(Replacement, File.ReadAllBytes(Outputˉpath));
            if (OperatingSystem.IsLinux())
            {
                Equal(
                    ...
                    File.GetUnixFileMode(Outputˉpath));
            }
        }
        finally
        {
            foreach (var Path in Directory.GetFiles(Directoryˉpath))
            {
                File.Delete(Path);
            }
            Directory.Delete(Directoryˉpath);
        }
    }

    private static void Prepareˉlinuxˉtestˉexecutable(string path)
    {
        if (!OperatingSystem.IsLinux())
        {
    ...
    private static void Compilerˉwebassemblyˉruns()
    {
        ...
        var U32ˉguardˉwvb = Compileˉsuccess(
            WEBASSEMBLY_RUNTIME_U32_GUARD_SOURCE);
        Equal(
            WEBASSEMBLY_RUNTIME_U32_GUARD_WVB_SHA256,
            Moduleˉdigest.Calculateˉsha256(U32ˉguardˉwvb));
        var U32ˉguardˉverified = Moduleˉcodec.Readˉandˉverify(U32ˉguardˉwvb);
        var U32ˉguardˉinput = ImmutableArray.Create<byte>(
            42, 0, 0, 0,
            1, 0, 0, 0,
            1, 0, 0, 0);
        var U32ˉguardˉreference = new Referenceˉruntime(
            U32ˉguardˉverified,
            new Referenceˉcapabilityˉhost(new StringWriter()),
            Runtimeˉoptions.Portableˉdefaults).Runˉmainˉbytes(U32ˉguardˉinput);
        Sequenceˉequal(
            ImmutableArray.Create<byte>(42, 0, 0, 0),
            U32ˉguardˉreference.Bytes);
        Equal(57L, U32ˉguardˉreference.Executedˉinstructions);
        var U32ˉguardˉlowered = Runˉwebassemblyˉtool(Tool, U32ˉguardˉwvb);
        Equal(0, U32ˉguardˉlowered.Exitˉcode);
        Equal(
            "webassembly status=Valid module-bytes=1893 execution-abi=3\n",
            U32ˉguardˉlowered.Output);
        Equal(
            WEBASSEMBLY_RUNTIME_U32_GUARD_SHA256,
            Moduleˉdigest.Calculateˉsha256(U32ˉguardˉlowered.Writtenˉbytes.AsSpan()));
        Validateˉruntimeˉwebassembly(
            U32ˉguardˉlowered.Writtenˉbytes.AsSpan(), U32ˉguardˉverified);
        var U32ˉguardˉrepeat = Runˉwebassemblyˉtool(Tool, U32ˉguardˉwvb);
        Sequenceˉequal(
            U32ˉguardˉlowered.Writtenˉbytes,
            U32ˉguardˉrepeat.Writtenˉbytes);

        var Runtimeˉcallsˉwvb = Compileˉsuccess(
            WEBASSEMBLY_RUNTIME_CALLS_SOURCE);
        Equal(
            WEBASSEMBLY_RUNTIME_CALLS_WVB_SHA256,
            Moduleˉdigest.Calculateˉsha256(Runtimeˉcallsˉwvb));
        var Runtimeˉcallsˉverified = Moduleˉcodec.Readˉandˉverify(
            Runtimeˉcallsˉwvb);
        Equal(3, Runtimeˉcallsˉverified.Functions.Length);
        True(
            Runtimeˉcallsˉverified.Functions.All(Function =>
                Function.Declaration.Parameterˉtypes.Length == 1 &&
                Function.Declaration.Parameterˉtypes[0].Kind == Valueˉtype.Bytes &&
                Function.Declaration.Returnˉtype.Kind == Valueˉtype.Bytes),
            "The runtime-call fixture does not have the bytes-to-bytes shape.");
        True(
            Runtimeˉcallsˉverified.Functions.Any(Function =>
                Function.Instructions.Any(Instruction =>
                    Instruction.Opcode == Opcode.Call)) &&
            Runtimeˉcallsˉverified.Functions.Any(Function =>
                Function.Instructions.Any(Instruction =>
                    Instruction.Opcode == Opcode.Branchˉfalse)),
            "The runtime-call fixture did not retain calls composed with control.");
        var Runtimeˉcallsˉinput = ImmutableArray.Create<byte>(9, 8);
        var Runtimeˉcallsˉreference = new Referenceˉruntime(
            Runtimeˉcallsˉverified,
            new Referenceˉcapabilityˉhost(new StringWriter()),
            Runtimeˉoptions.Portableˉdefaults).Runˉmainˉbytes(
                Runtimeˉcallsˉinput);
        Sequenceˉequal(
            ImmutableArray.Create<byte>(9, 9, 8),
            Runtimeˉcallsˉreference.Bytes);
        Equal(127L, Runtimeˉcallsˉreference.Executedˉinstructions);
        var Runtimeˉcallsˉlowered = Runˉwebassemblyˉtool(
            Tool, Runtimeˉcallsˉwvb);
        Equal(0, Runtimeˉcallsˉlowered.Exitˉcode);
        Equal(
            "webassembly status=Valid module-bytes=4086 execution-abi=3\n",
            Runtimeˉcallsˉlowered.Output);
        Equal(
            WEBASSEMBLY_RUNTIME_CALLS_SHA256,
            Moduleˉdigest.Calculateˉsha256(
                Runtimeˉcallsˉlowered.Writtenˉbytes.AsSpan()));
        Validateˉruntimeˉcallˉwebassembly(
            Runtimeˉcallsˉlowered.Writtenˉbytes.AsSpan(),
            Runtimeˉcallsˉverified);
        var Runtimeˉcallsˉrepeat = Runˉwebassemblyˉtool(
            Tool, Runtimeˉcallsˉwvb);
        Sequenceˉequal(
            Runtimeˉcallsˉlowered.Writtenˉbytes,
            Runtimeˉcallsˉrepeat.Writtenˉbytes);
        var Forwardˉruntimeˉcall = Runtimeˉcallsˉwvb.ToArray();
        var Runtimeˉcallsˉcode = Findˉsectionˉpayload(
            Forwardˉruntimeˉcall, Sectionˉkind.Code);
        var Callingˉfunctionˉindex = Array.FindIndex(
            Runtimeˉcallsˉverified.Functions.ToArray(),
            Function => Function.Instructions.Any(Instruction =>
                Instruction.Opcode == Opcode.Call));
        True(
            Callingˉfunctionˉindex > 0,
            "The runtime-call fixture has no private caller to corrupt.");
        var Firstˉruntimeˉcall = Runtimeˉcallsˉverified
            .Functions[Callingˉfunctionˉindex].Instructions
            .First(Instruction => Instruction.Opcode == Opcode.Call);
        BinaryPrimitives.WriteUInt32LittleEndian(
            Forwardˉruntimeˉcall.AsSpan(
                Runtimeˉcallsˉcode +
                Runtimeˉcallsˉverified.Functions[Callingˉfunctionˉindex]
                    .Declaration.Codeˉoffset +
                Firstˉruntimeˉcall.Offset + 1,
                4),
            (uint)Callingˉfunctionˉindex);
        var Forwardˉruntimeˉcallˉresult = Runˉwebassemblyˉtool(
            Tool, Forwardˉruntimeˉcall);
        Equal(1, Forwardˉruntimeˉcallˉresult.Exitˉcode);
        Equal(
            "webassembly status=Unsupportedˉcode\n",
            Forwardˉruntimeˉcallˉresult.Diagnostics);
        Equal(0, Forwardˉruntimeˉcallˉresult.Writeˉcount);

        var Envelopeˉverifierˉwvb = Compileˉsuccess(
            WEBASSEMBLY_WVB_ENVELOPE_VERIFY_SOURCE);
        Equal(
            WEBASSEMBLY_WVB_ENVELOPE_VERIFY_WVB_SHA256,
            Moduleˉdigest.Calculateˉsha256(Envelopeˉverifierˉwvb));
        var Envelopeˉverifierˉverified = Moduleˉcodec.Readˉandˉverify(
            Envelopeˉverifierˉwvb);
        var Envelopeˉreference = new Referenceˉruntime(
            Envelopeˉverifierˉverified,
            new Referenceˉcapabilityˉhost(new StringWriter()),
            Runtimeˉoptions.Portableˉdefaults).Runˉmainˉbytes(
                ImmutableArray.Create(Envelopeˉverifierˉwvb));
        Sequenceˉequal(ImmutableArray.Create<byte>(1), Envelopeˉreference.Bytes);
        Equal(2_206L, Envelopeˉreference.Executedˉinstructions);
        foreach (var Malformed in new byte[][]
        {
            Envelopeˉverifierˉwvb[..11],
            [.. Envelopeˉverifierˉwvb, 0],
        })
        {
        ...
        var Envelopeˉlowered = Runˉwebassemblyˉtool(
            Tool, Envelopeˉverifierˉwvb);
        Equal(0, Envelopeˉlowered.Exitˉcode);
        Equal(
            "webassembly status=Valid module-bytes=14902 execution-abi=3\n",
            Envelopeˉlowered.Output);
        Equal(
            WEBASSEMBLY_WVB_ENVELOPE_VERIFY_SHA256,
            Moduleˉdigest.Calculateˉsha256(
                Envelopeˉlowered.Writtenˉbytes.AsSpan()));
        Validateˉruntimeˉwebassembly(
            Envelopeˉlowered.Writtenˉbytes.AsSpan(),
            Envelopeˉverifierˉverified);
        var Envelopeˉrepeat = Runˉwebassemblyˉtool(
            Tool, Envelopeˉverifierˉwvb);
        Sequenceˉequal(
        ...
        Equal(134_443_344L, Semanticˉlowered.Executedˉinstructions);
        Equal(
        ...
        Equal(222_376_689L, Executableˉsemanticˉlowered.Executedˉinstructions);
        Equal(
        ...
        Equal(25_568, Scalarˉinterpreterˉwvb.Length);
        Equal(
        ...
        Equal(1_572, Scalarˉinterpreterˉmain.Declaration.Localˉtypes.Length);
        Equal(23_823, Scalarˉinterpreterˉmain.Declaration.Codeˉlength);
        Equal(3, Scalarˉinterpreterˉmain.Declaration.Maximumˉstackˉdepth);
        Equal(5_208, Scalarˉinterpreterˉmain.Instructions.Length);

        ImmutableArray<byte> Buildˉscalarˉrequest(
            IEnumerable<byte> Candidate,
            uint Guestˉbudget,
            uint Maximumˉcallˉdepth)
        {
            var Candidateˉbytes = Candidate.ToArray();
            var Request = new byte[16 + Candidateˉbytes.Length];
            BinaryPrimitives.WriteUInt32LittleEndian(Request.AsSpan(0, 4), 0x4958_5657u);
            BinaryPrimitives.WriteUInt16LittleEndian(Request.AsSpan(4, 2), 1);
            BinaryPrimitives.WriteUInt16LittleEndian(Request.AsSpan(6, 2), 0);
            BinaryPrimitives.WriteUInt32LittleEndian(Request.AsSpan(8, 4), Guestˉbudget);
            BinaryPrimitives.WriteUInt32LittleEndian(
                Request.AsSpan(12, 4),
                Maximumˉcallˉdepth);
            Candidateˉbytes.CopyTo(Request, 16);
            return ImmutableArray.Create(Request);
        }

        WebAssemblyˉexecutionˉresult Readˉscalarˉresponse(
            ImmutableArray<byte> Response)
        {
            Equal(20, Response.Length);
            Equal(0x4F58_5657u, BinaryPrimitives.ReadUInt32LittleEndian(Response.AsSpan(0, 4)));
            Equal((ushort)1, BinaryPrimitives.ReadUInt16LittleEndian(Response.AsSpan(4, 2)));
            Equal((ushort)0, BinaryPrimitives.ReadUInt16LittleEndian(Response.AsSpan(6, 2)));
            return new(
                checked((int)BinaryPrimitives.ReadUInt32LittleEndian(Response.AsSpan(8, 4))),
                BinaryPrimitives.ReadInt32LittleEndian(Response.AsSpan(16, 4)),
                BinaryPrimitives.ReadUInt32LittleEndian(Response.AsSpan(12, 4)));
        }

        Runtimeˉbytesˉresult Runˉscalarˉinterpreter(
            ImmutableArray<byte> Request,
            long Maximumˉinstructions = 1_000_000)
        {
            ...
            var Result = Runtime.Runˉmainˉbytes(Request);
            Equal(
                Result.Executedˉinstructions,
                Runtime.Readˉfunctionˉsteps()
                    .Sum(Function => Function.Executedˉinstructions));
            return Result;
        }

        long Runˉscalarˉoverflow(ImmutableArray<byte> Request)
        {
            var Runtime = new Referenceˉruntime(
                Scalarˉinterpreterˉverified,
                new Referenceˉcapabilityˉhost(new StringWriter()),
            ...
            try
            {
                _ = Runtime.Runˉmainˉbytes(Request);
            }
            catch (Runtimeˉexception Exception) when (Exception.Code == "WVR3007")
            {
                return Runtime.Readˉfunctionˉsteps()
                    .Sum(Function => Function.Executedˉinstructions);
            }

            throw new InvalidOperationException(
                "The scalar WVB interpreter did not preserve checked overflow.");
        }

        var Functionˉonlyˉwvb = Compileˉsuccess(SOURCE_WVB_FUNCTION_ONLY_SOURCE);
        Equal(
            SOURCE_WVB_FUNCTION_ONLY_SHA256,
            Moduleˉdigest.Calculateˉsha256(Functionˉonlyˉwvb));
        Equal(
            new WebAssemblyˉexecutionˉresult(0, 6, 199),
            Runˉreferenceˉwebassemblyˉi32(Functionˉonlyˉwvb));
        var Functionˉonlyˉrequest = Buildˉscalarˉrequest(
            Functionˉonlyˉwvb,
            1_000,
            8);
        var Functionˉonlyˉinterpreted = Runˉscalarˉinterpreter(
            Functionˉonlyˉrequest);
        Equal(121_003L, Functionˉonlyˉinterpreted.Executedˉinstructions);
        Equal(
            new WebAssemblyˉexecutionˉresult(0, 6, 199),
            Readˉscalarˉresponse(Functionˉonlyˉinterpreted.Bytes));

        var Scalarˉguestˉwvb = Compileˉsuccess(
            SOURCE_WVB_SCALAR_INTERPRETER_GUEST_SOURCE);
        Equal(2_366, Scalarˉguestˉwvb.Length);
        Equal(
            SOURCE_WVB_SCALAR_INTERPRETER_GUEST_SHA256,
            Moduleˉdigest.Calculateˉsha256(Scalarˉguestˉwvb));
        Equal(
            new WebAssemblyˉexecutionˉresult(0, 42, 351),
            Runˉreferenceˉwebassemblyˉi32(Scalarˉguestˉwvb));
        var Scalarˉguestˉinterpreted = Runˉscalarˉinterpreter(
            Buildˉscalarˉrequest(Scalarˉguestˉwvb, 1_000, 8));
        Equal(270_950L, Scalarˉguestˉinterpreted.Executedˉinstructions);
        Equal(
            new WebAssemblyˉexecutionˉresult(0, 42, 351),
            Readˉscalarˉresponse(Scalarˉguestˉinterpreted.Bytes));

        var Guestˉlimited = Runˉscalarˉinterpreter(
            Buildˉscalarˉrequest(Functionˉonlyˉwvb, 198, 8));
        Equal(120_641L, Guestˉlimited.Executedˉinstructions);
        Equal(
            new WebAssemblyˉexecutionˉresult(3011, 0, 198),
            Readˉscalarˉresponse(Guestˉlimited.Bytes));
        var Depthˉlimited = Runˉscalarˉinterpreter(
            Buildˉscalarˉrequest(Functionˉonlyˉwvb, 1_000, 1));
        Equal(48_861L, Depthˉlimited.Executedˉinstructions);
        Equal(
            new WebAssemblyˉexecutionˉresult(3004, 0, 27),
            Readˉscalarˉresponse(Depthˉlimited.Bytes));

        var I32ˉoverflowˉwvb = Compileˉsuccess(
            SOURCE_WVB_SCALAR_INTERPRETER_I32_OVERFLOW_SOURCE);
        Equal(
            SOURCE_WVB_SCALAR_INTERPRETER_I32_OVERFLOW_SHA256,
            Moduleˉdigest.Calculateˉsha256(I32ˉoverflowˉwvb));
        ...
        Equal(
            SOURCE_WVB_SCALAR_INTERPRETER_U32_OVERFLOW_SHA256,
            Moduleˉdigest.Calculateˉsha256(U32ˉoverflowˉwvb));
        ...
        Equal(145_469, Scalarˉinterpreterˉlowered.Writtenˉbytes.Length);
        Equal(82_657_852L, Scalarˉinterpreterˉlowered.Executedˉinstructions);
        Equal(
        ...
        var Firstˉruntimeˉbranch = Envelopeˉverifierˉverified.Functions[0]
            .Instructions.First(Instruction => Instruction.Opcode == Opcode.Branchˉfalse);
        BinaryPrimitives.WriteUInt32LittleEndian(
            Invalidˉruntimeˉtarget.AsSpan(
                Envelopeˉcode +
                Envelopeˉverifierˉverified.Functions[0].Declaration.Codeˉoffset +
                Firstˉruntimeˉbranch.Offset + 1,
                4),
            (uint)Firstˉruntimeˉbranch.Offset + 1u);
        var Invalidˉruntimeˉtargetˉresult = Runˉwebassemblyˉtool(
            Tool, Invalidˉruntimeˉtarget);
        Equal(1, Invalidˉruntimeˉtargetˉresult.Exitˉcode);
        Equal(
            "webassembly status=Unsupportedˉfunction\n",
            Invalidˉruntimeˉtargetˉresult.Diagnostics);
        Equal(0, Invalidˉruntimeˉtargetˉresult.Writeˉcount);
    ...
    private static void Assemblerˉencodesˉexpandedˉx64()
    {
        ...
        const string Registerˉsource = """
            ...
            """;
        var Registers = Objectˉcodec.Readˉandˉverify(Assembleˉsuccess(Registerˉsource)).Value;
        Sequenceˉequal<byte>(
            [
                0x41, 0xB8, 0xFF, 0xFF, 0xFF, 0xFF,
                0x41, 0xBF, 0xFF, 0xFF, 0xFF, 0xFF,
                0x4C, 0x89, 0xC0,
                0x41, 0x89, 0xC1,
                0x48, 0x01, 0xC8,
                0x4D, 0x29, 0xF8,
                ...
                0x39, 0xD0,
                0x4D, 0x85, 0xFE,
                0x50, 0x41, 0x50, 0x41, 0x5F,
                0xFF, 0xD0, 0x41, 0xFF, 0xD1,
                0xFF, 0xE3, 0x41, 0xFF, 0xE4,
                0xC3,
            ],
            Registers.Sections.Single().Data);

        var Registersˉ32 = new[]
        {
            "eax", "ecx", "edx", "ebx", "esp", "ebp", "esi", "edi",
            "r8d", "r9d", "r10d", "r11d", "r12d", "r13d", "r14d", "r15d",
        };
        var Registersˉ64 = new[]
        {
            "rax", "rcx", "rdx", "rbx", "rsp", "rbp", "rsi", "rdi",
            "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15",
        };
        for (var Index = 0; Index < Registersˉ32.Length; Index++)
        {
            var Source = $$"""
                windvale-assembly 1
                symbol export function Main in .text
                section code .text align 16
                define Main
                move_u32 {{Registersˉ32[Index]}} {{Index}}
                move {{Registersˉ32[Index]}} {{Registersˉ32[Index]}}
                move {{Registersˉ64[Index]}} {{Registersˉ64[Index]}}
                push {{Registersˉ64[Index]}}
                pop {{Registersˉ64[Index]}}
                return
                end define
                end section
                """;
            var Register = Objectˉcodec.Readˉandˉverify(Assembleˉsuccess(Source)).Value;
            var Low = (byte)(Index & 7);
            var Expected = new List<byte>();
            if (Index >= 8)
            {
                Expected.Add(0x41);
            }
            Expected.Add((byte)(0xB8 + Low));
            Expected.AddRange([(byte)Index, 0, 0, 0]);
            if (Index >= 8)
            {
                Expected.Add(0x45);
            }
            Expected.AddRange([0x89, (byte)(0xC0 + Low * 9)]);
            Expected.Add((byte)(Index >= 8 ? 0x4D : 0x48));
            Expected.AddRange([0x89, (byte)(0xC0 + Low * 9)]);
            if (Index >= 8)
            {
                Expected.Add(0x41);
            }
            Expected.Add((byte)(0x50 + Low));
            if (Index >= 8)
            {
                Expected.Add(0x41);
            }
            Expected.Add((byte)(0x58 + Low));
            Expected.Add(0xC3);
            Sequenceˉequal<byte>(
                Expected,
                Register.Sections.Single().Data);
        }

        const string Labelˉsource = """
            windvale-assembly 1
            symbol export function Main in .text
            section code .text align 16
            define Main
            ...
            """;
        var Labels = Objectˉcodec.Readˉandˉverify(Assembleˉsuccess(Labelˉsource)).Value;
        Sequenceˉequal<byte>(
            [0x39, 0xD0, 0x0F, 0x84, 0x05, 0, 0, 0, 0xE9, 0xF3, 0xFF, 0xFF, 0xFF, 0xC3],
            Labels.Sections.Single().Data);
        Equal(0, Labels.Relocations.Length);

        const string Scopedˉlabelˉsource = """
            windvale-assembly 1
            symbol local function Other in .text
            symbol export function Main in .text
            section code .text align 16
            define Main
            jump_label Again
            ...
            """;
        var Scopedˉlabels = Objectˉcodec.Readˉandˉverify(Assembleˉsuccess(Scopedˉlabelˉsource)).Value;
        Sequenceˉequal<byte>(
            [0xE9, 0, 0, 0, 0, 0xC3, 0xE9, 0, 0, 0, 0, 0xC3],
            Scopedˉlabels.Sections.Single().Data);

        var Conditions = new[]
        {
            "overflow", "not_overflow", "below", "above_equal",
            "equal", "not_equal", "below_equal", "above",
            "sign", "not_sign", "parity", "not_parity",
        ...
        for (var Index = 0; Index < Conditions.Length; Index++)
        {
            var Source = $$"""
                ...
                section code .text align 16
                define Main
                branch {{Conditions[Index]}} Done
                label Done
                return
                end define
                end section
                """;
            var Condition = Objectˉcodec.Readˉandˉverify(Assembleˉsuccess(Source)).Value;
            Sequenceˉequal<byte>(
        ...
        const string Ripˉrelativeˉsource = """
            ...
            """;
        var Rip = Objectˉcodec.Readˉandˉverify(Assembleˉsuccess(Ripˉrelativeˉsource)).Value;
        Sequenceˉequal<byte>(
            [
                0x8B, 0x05, 0, 0, 0, 0,
                ...
                0x89, 0x0D, 0, 0, 0, 0,
                0x4C, 0x89, 0x0D, 0, 0, 0, 0,
                0x4C, 0x8D, 0x15, 0, 0, 0, 0,
                0xC3,
            ],
            Rip.Sections[0].Data);
        Sequenceˉequal<uint>([2, 9, 16, 22, 29, 36], Rip.Relocations.Select(Item => Item.Offset));
        True(Rip.Relocations.All(Item =>
            Item.Kind == Objectˉrelocationˉkind.Relativeˉi32 &&
            Item.Symbolˉindex == 0 &&
            Item.Addend == -4), "RIP-relative operations must emit canonical relative-i32 relocations.");

        var Linked = Linkˉsuccess(
            [Assembleˉsuccess(Ripˉrelativeˉsource)],
            new(Linkˉcontract.DEFAULT_BASE_ADDRESS, "Main"));
        Equal(48, Linked.Imageˉbytes.Length);
        Sequenceˉequal<int>(
            [38, 31, 24, 18, 11, 4],
            Rip.Relocations.Select(Item =>
                BinaryPrimitives.ReadInt32LittleEndian(
    ...
    private static void Wvaˉassemblerˉmatchesˉoracle()
    {
            ...
            string source)
        {
            ...
            var Result = new Referenceˉruntime(
                ...
                new Referenceˉcapabilityˉhost(new Hostedˉresourceˉcontext(
                    ...
                    Writer)),
                new(Authorized, Maximumˉinstructions: 50_000_000)).Runˉmain();
            return (Result, Output.ToString(), Diagnostics.ToString(), Writer);
        }
        ...
        const string Allˉregistersˉsource = """
            windvale-assembly 1
            symbol export function Main in .text
            section code .text align 16
            define Main
        ...
        const string Expandedˉx64ˉsource = """
            windvale-assembly 1
            symbol local data Value in .data
            symbol export function Main in .text
            section code .text align 16
            ...
            jump_register r12
            label Flow
            branch overflow Done
            branch not_overflow Done
            branch below Done
            branch above_equal Done
            branch equal Done
            branch not_equal Done
            branch below_equal Done
            branch above Done
            branch sign Done
            branch not_sign Done
            branch parity Done
            branch not_parity Done
            branch less Done
            branch greater_equal Done
            branch less_equal Done
            branch greater Done
            jump_label Flow
            label Done
            load_u32 eax Value
        ...
        Equal(6, Expandedˉobject.Relocations.Length);
        True(
            Expandedˉobject.Relocations.All(Item => Item.Kind == Objectˉrelocationˉkind.Relativeˉi32),
        ...
        foreach (var (Source, Code) in Cases)
        {
            ...
            if (Windvale.Result.Exitˉcode != 2)
            {
                throw new InvalidOperationException(
                    $"The Windvale assembler accepted the {Code} differential rejection fixture: {Source}");
            }
    ...
    private static void Validateˉruntimeˉcallˉwebassembly(
        ReadOnlySpan<byte> module,
        Verifiedˉmodule source)
    {
        True(
        ...
        Reader.Readˉheader();
        var Typeˉend = Reader.Readˉsection(1);
        Reader.Require(Reader.Readˉuleb32() == 2, "The runtime-call type count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x60, "The runtime-call wrapper type is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 2, "The runtime-call wrapper parameter count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7F, "The runtime-call budget type is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7F, "The runtime-call input-length type is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 1, "The runtime-call wrapper result count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7F, "The runtime-call status type is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x60, "The runtime-call private type is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 1, "The runtime-call private parameter count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7E, "The runtime-call descriptor parameter is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 1, "The runtime-call private result count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7E, "The runtime-call descriptor result is invalid.");
        Reader.Require(Reader.Position == Typeˉend, "The runtime-call type section has trailing bytes.");

        var Functionˉend = Reader.Readˉsection(3);
        Reader.Require(
            Reader.Readˉuleb32() == (uint)source.Functions.Length + 1u,
            "The runtime-call Wasm function count is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 0, "The runtime-call wrapper type index is invalid.");
        for (var Functionˉindex = 0; Functionˉindex < source.Functions.Length;
            Functionˉindex++)
        {
            Reader.Require(Reader.Readˉuleb32() == 1, "A runtime-call private type index is invalid.");
        }
        Reader.Require(Reader.Position == Functionˉend, "The runtime-call function section has trailing bytes.");

        var Memoryˉend = Reader.Readˉsection(5);
        Reader.Require(Reader.Readˉuleb32() == 1, "The runtime-call memory count is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 1, "The runtime-call memory is not fixed.");
        Reader.Require(Reader.Readˉuleb32() == 129, "The runtime-call memory minimum is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 129, "The runtime-call memory maximum is invalid.");
        Reader.Require(Reader.Position == Memoryˉend, "The runtime-call memory section has trailing bytes.");

        var Globalˉend = Reader.Readˉsection(6);
        Reader.Require(Reader.Readˉuleb32() == 11, "The runtime-call global count is invalid.");
        Reader.Readˉglobal(0, 3);
        Reader.Readˉglobal(0, 65_536);
        Reader.Readˉglobal(0, 4_194_304);
        Reader.Readˉglobal(0, 4_259_840);
        ...
        Reader.Readˉglobal(1, 0);
        Reader.Readˉglobal(1, 0);
        Reader.Readˉglobal(1, 4_259_840);
        Reader.Readˉglobal(1, 0);
        Reader.Require(Reader.Position == Globalˉend, "The runtime-call global section has trailing bytes.");

        var Exportˉend = Reader.Readˉsection(7);
        Reader.Require(Reader.Readˉuleb32() == 10, "The runtime-call export count is invalid.");
        Reader.Readˉexport("Windvale.run", 0, 0);
        Reader.Readˉexport("Windvale.abi", 3, 0);
        Reader.Readˉexport("Windvale.memory", 2, 0);
        Reader.Readˉexport("Windvale.input_offset", 3, 1);
        ...
        Reader.Readˉexport("Windvale.output_kind", 3, 6);
        Reader.Readˉexport("Windvale.instructions", 3, 7);
        Reader.Require(Reader.Position == Exportˉend, "The runtime-call export section has trailing bytes.");

        var Codeˉend = Reader.Readˉsection(10);
        Reader.Require(
            Reader.Readˉuleb32() == (uint)source.Functions.Length + 1u,
            "The runtime-call body count is invalid.");
        var Wrapperˉlength = Reader.Readˉuleb32();
        Reader.Require(Wrapperˉlength <= int.MaxValue, "The runtime-call wrapper is oversized.");
        var Wrapperˉend = Reader.Position + (int)Wrapperˉlength;
        Reader.Require(Reader.Readˉuleb32() == 2, "The runtime-call wrapper local groups are invalid.");
        Reader.Require(Reader.Readˉuleb32() == 2, "The runtime-call wrapper i32 local count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7F, "The runtime-call wrapper i32 local type is invalid.");
        Reader.Require(Reader.Readˉuleb32() == 1, "The runtime-call wrapper descriptor count is invalid.");
        Reader.Require(Reader.Readˉbyte() == 0x7E, "The runtime-call wrapper descriptor type is invalid.");
        Reader.Skip(Wrapperˉend - Reader.Position);

        foreach (var Function in source.Functions)
        {
            var Bodyˉlength = Reader.Readˉuleb32();
            Reader.Require(Bodyˉlength <= int.MaxValue, "A runtime-call body is oversized.");
            var Bodyˉend = Reader.Position + (int)Bodyˉlength;
            Reader.Require(
                Reader.Readˉuleb32() == (uint)Function.Declaration.Localˉtypes.Length + 2u,
                "A runtime-call private local group count is invalid.");
            foreach (var Type in Function.Declaration.Localˉtypes)
            {
                Reader.Require(Reader.Readˉuleb32() == 1, "A runtime-call local is not singular.");
                Reader.Require(
                    Reader.Readˉbyte() == (Type.Kind == Valueˉtype.Bytes ? 0x7E : 0x7F),
                    "A runtime-call private local type changed.");
            }
            Reader.Require(Reader.Readˉuleb32() == 6, "The runtime-call i32 scratch count is invalid.");
            Reader.Require(Reader.Readˉbyte() == 0x7F, "The runtime-call i32 scratch type is invalid.");
            Reader.Require(Reader.Readˉuleb32() == 3, "The runtime-call descriptor scratch count is invalid.");
            Reader.Require(Reader.Readˉbyte() == 0x7E, "The runtime-call descriptor scratch type is invalid.");
            Reader.Skip(Bodyˉend - Reader.Position);
        }
        Reader.Require(Reader.Position == Codeˉend, "The runtime-call code section has trailing bytes.");
        Reader.Require(Reader.Position == Reader.Length, "The runtime-call module has trailing bytes.");
    }
        ...
        Verifiedˉmodule source)
    {
        ...
        True(
            Function.Instructions.Length is >= 1 and <= 100_000 &&
                ...
                Function.Instructions[^1].Opcode is
                    Opcode.Jump or Opcode.Branchˉfalse or Opcode.Return,
        ...
        True(
            Function.Instructions.All(Instruction => Instruction.Opcode is
                ...
                Opcode.I32ˉadd or Opcode.I32ˉsubtract or Opcode.I32ˉmultiply or
                Opcode.I32ˉnegate or Opcode.I32ˉequal or Opcode.I32ˉnotˉequal or
                Opcode.I32ˉless or Opcode.I32ˉlessˉequal or
                Opcode.I32ˉgreater or Opcode.I32ˉgreaterˉequal or
                ...
                Opcode.U32ˉadd or Opcode.U32ˉsubtract or Opcode.U32ˉmultiply or
                Opcode.U32ˉequal or Opcode.U32ˉnotˉequal or
                Opcode.U32ˉless or Opcode.U32ˉlessˉequal or
                Opcode.U32ˉgreater or Opcode.U32ˉgreaterˉequal or
                Opcode.U8ˉequal or Opcode.U8ˉnotˉequal or
                Opcode.Bytesˉconcat or Opcode.Bytesˉfromˉu8 or
                Opcode.Bytesˉfromˉu16ˉlittle or Opcode.Bytesˉfromˉu32ˉlittle or
                Opcode.Bytesˉfromˉi32ˉlittle or Opcode.Jump or
                Opcode.Branchˉfalse or Opcode.Pop or Opcode.Return),
            "The runtime-value source opcode is outside the profile.");
        if (Hasˉcontrol)
        {
            var Boundaries = Function.Instructions
                .Select(Instruction => (uint)Instruction.Offset)
                .ToHashSet();
            foreach (var Control in Function.Instructions.Where(Instruction =>
                Instruction.Opcode is Opcode.Jump or Opcode.Branchˉfalse))
            {
                True(
                    Boundaries.Contains(Control.Unsignedˉoperand),
                    "A runtime-value control target is not an instruction boundary.");
                if (Control.Unsignedˉoperand != 0)
                {
                    var Previous = Function.Instructions.Last(Instruction =>
                        (uint)(Instruction.Offset + Instruction.Size) ==
                            Control.Unsignedˉoperand);
                    True(
                        Previous.Opcode is Opcode.Jump or Opcode.Branchˉfalse or
                            Opcode.Return,
                        "A runtime-value control target does not follow a terminator.");
                }
            }
        }
        ...
        Reader.Require(
            Reader.Readˉuleb32() == (Hasˉcontrol ? 6u : 5u),
            "The runtime-value i32 scratch count is invalid.");
        ...
        var Elseˉcount = 0;
        var Branchˉcount = 0;
        var Programˉcounterˉaccesses = 0;
        var Programˉcounter =
            (uint)Function.Declaration.Allˉlocalˉtypes.Length + 7u;
        ...
        while (Reader.Position < Bodyˉend)
        {
            ...
            {
                case 0x03:
                    Reader.Require(Reader.Readˉbyte() == 0x40, "A runtime-value loop type is invalid.");
                    Loopˉcount++;
                    Depth++;
                    break;
                case 0x04:
                    ...
                    break;
                case 0x05:
                    Reader.Require(Depth > 0, "A runtime-value else appears outside a block.");
                    Elseˉcount++;
                    break;
                case 0x0B:
                    if (Depth == 0)
                ...
                case 0x21:
                case 0x22:
                    if (Reader.Readˉuleb32() == Programˉcounter)
                    {
                        Programˉcounterˉaccesses++;
                    }
                    break;
                case 0x0C:
                    Reader.Readˉuleb32();
        ...
        Reader.Require(
            !Hasˉcontrol ||
                (Loopˉcount == 1 &&
                    Elseˉcount == Function.Instructions.Count(Instruction =>
                        Instruction.Opcode == Opcode.Branchˉfalse) &&
                    Branchˉcount >= Function.Instructions.Count(Instruction =>
                        Instruction.Opcode is Opcode.Jump or Opcode.Branchˉfalse) &&
                    Programˉcounterˉaccesses >= Branchˉcount),
            "The runtime-value control dispatcher evidence changed.");
