...
4. Generalize exactly one measured boundary after qualified probe 29: add page/root reclamation only when reuse is real, broaden the interpreter for a real module, separate resource lifetimes only for a real consumer, or add a third runnable only when it creates scheduler pressure. Keep JIT publication, broader traps, virtual memory, and lifecycle adapters separate.
