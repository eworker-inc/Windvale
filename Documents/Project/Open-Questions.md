...
- Probe 38 implements Decision 0173's fixed third directory-service process, separate resource and directory endpoints, and smallest ready/wait dispatcher locally. Which process/thread states, cursor rules, wake causes, counters, and malformed-state checks should survive the first timer-preemption slice before becoming a more general run-queue contract?
...
2. Cross-host qualify Probe 38's third service, second endpoint, and state-driven ready/wait dispatcher, then add one monotonic timer and bounded single-CPU preemption before general memory objects, the first flat resource domain, dynamic launch, supervision, or driver isolation. Keep each later mechanism a separate evidence claim.
