...
- Which parts of candidate Decision 0098's fixed typed resource pair, atomic cleanup, saved context, and wait/wake coordinator should survive independently lived resources, a reusable address space, third runnable, or general loader case?
...
Decisions 0058 through qualified 0097 and candidate 0098 establish reproducible bytecode compiler convergence, the bounded shared ABI-16 native path with all 12 current service leaves and calls through 64 parameters, live Windvale-produced service leaves, Windvale-owned executable-image layout and lifetime, WVA-owned Q35 poweroff, normalized no-error/error-code trap entries, the first kernel-owned W^X root, fixed in-guest WVB admission, protected processes, the first Windvale init/resource service, the first user-space Windvale bytecode interpreter, section-derived validation, a typed WVB/execution-budget pair, an atomic Windvale-selected grant, per-opcode budget enforcement, and automatic terminal cleanup. The recommended next decisions are:
...
4. Cross-host qualify candidate probe 29. Then generalize exactly one measured boundary: add page/root reclamation only when reuse is real, broaden the interpreter for a real module, separate resource lifetimes only for a real consumer, or add a third runnable only when it creates scheduler pressure. Keep JIT publication, broader traps, virtual memory, and lifecycle adapters separate.
