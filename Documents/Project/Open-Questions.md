...
Decisions 0058 through 0090 establish reproducible bytecode compiler convergence, the bounded shared ABI-16 native path with all 12 current service leaves and calls through 64 parameters, live Windvale-produced service leaves, Windvale-owned executable-image layout and lifetime, WVA-owned Q35 poweroff, normalized no-error/error-code trap entries, the first kernel-owned W^X root, fixed in-guest WVB admission, and the durable capability-oriented OS boundary. The recommended next decisions are:
...
4. Design the first protected address-space/thread/capability-table/resource-budget/IPC slice on top of qualified probe 21, retaining pinned-QEMU evidence before Hyper-V qualification and treating broader traps, virtual memory, and lifecycle adapters as separate contracts.
