...
- Which parts of Decision 0091's first internal process/thread/capability/syscall/IPC encoding should survive once a second process and init/resource service supply broader evidence?
...
Decisions 0058 through 0091 establish reproducible bytecode compiler convergence, the bounded shared ABI-16 native path with all 12 current service leaves and calls through 64 parameters, live Windvale-produced service leaves, Windvale-owned executable-image layout and lifetime, WVA-owned Q35 poweroff, normalized no-error/error-code trap entries, the first kernel-owned W^X root, fixed in-guest WVB admission, the first protected process/capability/IPC experiment, and the durable capability-oriented OS boundary. The recommended next decisions are:
...
4. Use candidate probe 22's protected address-space/thread/capability/resource/IPC slice to start one minimal init/resource service, retaining pinned-QEMU evidence before Hyper-V qualification and treating scheduling, broader traps, virtual memory, and lifecycle adapters as separate contracts.
