...
4. Qualify probe 26, then generalize exactly one measured boundary: move immutable resource ownership/transfer toward init or package service, broaden the interpreter for a real module, or add a third runnable only when it creates real scheduler pressure. Keep JIT publication, broader traps, virtual memory, and lifecycle adapters separate.
