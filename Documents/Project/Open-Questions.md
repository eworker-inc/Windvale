...
- After Probe 34's immutable guest store and peer cleanup, which measured boundary should come first: portable path-component and directory lookup semantics, a namespace/provider capability with explicit service lifetime, or a read-only block-backed store? Which real consumer will justify that choice without combining writable allocation, caching, and crash recovery prematurely?
