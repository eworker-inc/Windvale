...
- Which fields and invariants from qualified `WVPROC17` process, thread, capability, endpoint, directory-snapshot, and ready/wait records must survive the first timer-preemption slice without freezing the current offsets, slot numbers, or one-thread shape?
- On the selected physical and virtual x86-64 environments, which TSC, HPET, and local-APIC measurements qualify the first clocksource, clockevent, and private experimental quantum?
...
2. Add one monotonic timer and bounded single-CPU preemption to Probe 38's qualified three-process ready/wait baseline before general memory objects, the first flat resource domain, dynamic launch, supervision, or driver isolation. Keep each later mechanism a separate evidence claim.
