...
4. Generalize exactly one measured boundary after Probe 32: support a non-tail lifetime only when it creates allocator pressure, broaden the interpreter for a real module, separate resource lifetimes only for a real consumer, or add a third runnable only when it creates scheduler pressure. Keep JIT publication, broader traps, virtual memory, and lifecycle adapters separate.
