...
- Decision 0153 selects one immutable directory snapshot and exact bounded read as the first rights-limited filesystem operation. What is the smallest Probe-35 runtime/service binding that preserves `WVDR 1`, typed revocation/stale/peer-exit results, generation safety, and format-blind kernel IPC without repurposing the package-resource `WVRS 1` store?
