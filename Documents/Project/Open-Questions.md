...
Decisions 0058 through 0092 establish reproducible bytecode compiler convergence, the bounded shared ABI-16 native path with all 12 current service leaves and calls through 64 parameters, live Windvale-produced service leaves, Windvale-owned executable-image layout and lifetime, WVA-owned Q35 poweroff, normalized no-error/error-code trap entries, the first kernel-owned W^X root, fixed in-guest WVB admission, protected processes, and the first Windvale init/resource service with cross-process IPC. The recommended next decisions are:
...
4. Cross-host-qualify candidate probe 23, then choose the smallest measured step-6 case: either a general admitted boot resource/module or a third runnable that creates real scheduler pressure. Keep broader traps, virtual memory, and lifecycle adapters separate.
