...
4. Cross-host-qualify candidate probe 25, then generalize exactly one runtime boundary: carry a bounded boot-resource/module input into the now section-derived interpreter, or add a third runnable only when it creates real scheduler pressure. Keep JIT publication, broader traps, virtual memory, and lifecycle adapters separate.
