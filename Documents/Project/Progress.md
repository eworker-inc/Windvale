...
| 10. Native host tools and .NET retirement | 🚧 In progress | Exact commit `524e84afb6e5bab6bbd95ebc0b9eeaf886af834b` cross-host qualifies the Decision 0213 WVB 1.11 semantic-freeze baseline, paired verifier/build-driver applications, direct no-.NET execution, and current bounded Windvale-owned ABI-22 lowering evidence. Exact commit `9d36387867ebff80ee94c6f9f7996da4ef32a4a3` then qualifies Decision 0214's first Windows/Linux publisher profile over one shared verifier, transaction core, SHA-256, and platform adapters; the extended native fault/concurrency matrix remains hardening work. The current cutover candidate distributes the pinned build-driver/publisher inventory, adds explicit Stage 0 reconstruction, and makes their composition the documented ordinary project source-to-WVB route. Forward language semantics advance only through `Compiler/Windvale`; C# retains its recovery and independent-evidence role. | Qualify the composed native front door on both hosts, then continue with native execution/inspection, test orchestration, backend transfer, publisher hardening, remaining tools, and the final recovery archive. |
