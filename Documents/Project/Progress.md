...
| 9. Shared native backend | 🎯 Current transfer | Qualified ABI 19 constructs one native byte under the retained arena and typed-slot contracts; exact compiler preflight now reaches `Bytesˉfromˉu16ˉlittle`. | Implement only the next measured opcode/runtime contract. |
...
- 🎯 Qualified compiler WVB → qualified ABI-19 one-byte construction → exact preflight reaches `Bytesˉfromˉu16ˉlittle`
