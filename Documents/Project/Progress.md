...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0169 package and directly run the exact ABI-22 compiler. Implemented Decision 0185 adds deterministic Windows/Linux packages for the Windvale-written compiler-aligned WVB verifier with five exact capabilities, one read-only snapshot, no file-output binding, independent outer verification, and direct .NET-free child evidence; cross-host qualification is pending. | Qualify the verifier pair on both hosts, then move the build driver and remaining verify, test, assemble, link, inspect, package, and run tools off Stage 0 before changing normal automation. |
