...
| Offline package lifecycle | 🎯 | [Decision 0568](../Decisions/0568-Offline-Package-Lifecycle-And-Generation-Activation-1.md) selects Milestone 4 without assigning `v0.2.0`. Portable Generation 1 / Activation 1 semantics pass, and the first fast host owner proves compare-before-write activation, atomic replacement, stale rejection, interruption recovery, and rollback publication. | Route commands through the active generation, then prove one offline directory with two real packages on both hosts. |
...
rollback transition. Active-generation command dispatch and the two-real-package
end-to-end demonstration remain. Networking, OS-1, and database breadth stay
independent.
