...
The latest Phase 10 local increment is [Decision 0455](../Decisions/0455-Native-Probe-40-Process-Policy-Source-Path.md), which composes the existing native builder, lowerer, and export renamer for portable process-policy source and reduces the frozen seed to one object. The retirement coordinator now owns 30 suites and 3,145 cases. Earlier transferred families remain indexed by their decisions and the native retirement-suite specification. Internal `WVL1011`, hostile unsafe-bytecode value limits, large-native segmented-object transfer, broader native host-container reconstruction, additional dynamic WVA vectors, complete Windows/Linux execution, promotion, and the grouped retirement gate remain.
...
owns 30 suites and 3,145 fixed cases without .NET. The latest two cases verify
exact process-policy source/WVB/WVO construction and destination preservation;
the Probe 40 cases preserve
