...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds all 16 bundles and 72 source fixtures and admits the exact `en@1` chain only through hash-pinned Project 3 inputs. Slice 2 now has vertical compiler/verifier/runtime checkpoints for storage-free `unit` return/control, named record update, WVB 1.12 fixed-width integers, WVB 1.13 exact runes, and WVB 1.14 strict `f32`/`f64`. The focused 91-case owner includes 20 rune and 27 floating cases; deterministic floating output is 2,809 bytes and the source-built runner returns `42`. Project 2 and descriptorless Seed retain their prior behavior; complete unit/`never`, remaining aggregate/control semantics, localized token execution, paired-host evidence, and `zh-Hans@1` qualification remain open. | Continue Slice 2 with complete unit/`never` semantics over the same compiler, verifier, and runtime path. |
