...
- 🚧 Probe 40 fixed ResourceDomain1 + versioned executable-publication/machine admission → checked start syscall and dynamic allocation → generation-2 reconstruction, resumed validation, and source-owned client/result return transfers → supervised live isolated provider.
