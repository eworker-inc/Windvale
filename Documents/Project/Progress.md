...
| Unsafe Foundation slice | Candidate | Canonical unsafe value types and `Constructˉscratch` lower to typed WVIR and WVB 1.33 with explicit memory authority, size, alignment, ABI, effect, failure shape, complete compiler-aligned verification, and a bounded scalar provider. [Decision 0905](../Decisions/0905-Transfer-Affine-Memory-Budgets-Through-Ordinary-Calls.md) transfers an entry or split child budget by value through an ordinary direct call, preserves its limit, rejects duplicate transfer, and executes exact child-budget exhaustion. | Encode borrowed-budget calls in WVB, then add native containment, pointer and write-region borrowing, authenticated Foreign calls, a migrated real boundary, Linux reproduction, and paired-host qualification. |
