...
The latest implemented Phase 10 local increment is [Decision 0517](../Decisions/0517-Native-Source-Semantic-Build-And-Inspection-Transfer.md). The paired native helper now builds 88 exact artifacts and owns 144 build/verify/inspect/run cases. This slice reproduces the source-set, source-graph, and source-symbol core/demo/tool WVBs and natively inspects all three core surfaces. Its complete Windows contract passes in 580.1 seconds, and the three focused regular/extended compiler owners pass 3/3 in 28.950 test seconds. The nine aggregate manifests remain at repository root because their complete closures span Compiler or Examples plus Foundation; their dependencies use canonical module-name order as a compatibility workaround for the pinned native Project driver's documented order-sensitivity defect. This removes twelve additional managed invocations from each broad host script, 150 cumulatively. All three semantic demos and the three capability-bearing hosted-tool executions remain managed because the native runner stops the demos with code `3004` and does not bind those tool capabilities; the broad harness and later qualification phases also still use .NET, so T2 remains managed-normal. Independent Linux execution, the remaining broad transfers, grouped qualification, promotion, and the final Stage 0 recovery release remain open.

[Decision 0516](../Decisions/0516-Native-Source-Parser-Build-And-Inspection-Transfer.md) previously transferred the first three source-compiler construction and inspection phases. Decision 0517 preserves that evidence while advancing through canonical source sets, import graphs, and declaration/signature symbols.
