...
| 9. Shared native backend | 🎯 Current transfer | Implemented Decision 0133 consumes the qualified record maps in ABI 21, independently decodes frame-owned direct records, and runs the exact compiler fixture with zero record-arena use; full Windows Seed and OS evidence passes, with cross-host qualification pending. | Qualify ABI 21 on both hosts, measure the new `WVR3018` dynamic-value boundary, then prove native Stage 1/Stage 2 equality. |
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale constructs live service leaves and owns image layout/lifecycle; the exact compiler produces the exact 815-byte Stage 0 WVB through Windows and Linux native W^X paths. Paired `windows-x64-console-v1` and `linux-x64-console-v1` slices package the same capability-free ABI-21 scalar fragment, share portable `0…255` process-result semantics, publish atomically, use WVA-authored startups, consume a portable Windvale layout, and derive every final PE/ELF byte from fixed sparse Windvale recipes with complete C# recovery agreement; Windows direct execution passes while fresh Linux evidence is pending. | Qualify ABI 21 and both narrow containers cross-host, then transfer untrusted PE/ELF structural verification into portable Windvale before satisfying the larger .NET-retirement gate. |
| 11. Boot path and kernel | 🚧 In progress | The ABI-21 Probe-32 candidate retains `WVPROC11` behavior while `WVKMEM11` bounds a 141-page arena, 120-page client root, exact 24,240-byte stack path, and zero record-arena use. Implemented-candidate `WVRS 1` plus one-page `WVRQ 1` / `WVRY 1` prove strict dynamic lookup through a format-blind capacity-one lifecycle and a live hosted Windvale service without changing the guest ABI. | Cross-host qualify the shared ABI-21/`WVKMEM11` rebuild, then version checked one-page user-buffer IPC and an immutable store capability into the guest; prove service/client death cleanup and one configuration lookup in QEMU. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector now adds profile 10 over execution ABI 3: checked unsigned scalar operations and compiler-produced nested control compose with bounded byte values through a validated basic-block dispatcher. A real Windvale-written WVB 1.6 envelope verifier executes as deterministic import-free Wasm. Profile 8's editable .NET-free page remains intact; profile 5 is the latest cross-host-qualified boundary. | Extend envelope verification into complete WVB payload and semantic verification, then add measured text/record/enum needs; obtain cross-host profile-6–10 and cross-browser evidence. |
...
- 🚧 ABI-21 direct records → deterministic frame backing → caller-owned returns → zero record-arena use in both the exact compiler and rebuilt Probe 32 (cross-host qualification pending)
...
- 🚧 Typed byte/word WVA + exact C# differential oracle → WVA-owned exception terminal and one bounded COM1 byte loop; the integrated Windows Qualification passes 77/77 Seed and 31/31 OS tests, and all four pinned-QEMU scenarios pass, with cross-host qualification pending
