...
| 10. Native host tools and .NET retirement | 🚧 In progress | Exact commit `524e84afb6e5bab6bbd95ebc0b9eeaf886af834b` cross-host qualifies the Decision 0213 WVB 1.11 semantic-freeze baseline, paired standalone verifier applications, project-aware format-5 native build driver, direct no-.NET child execution, and current Windvale-owned bounded ABI-22 lowering evidence. Decision 0214 now implements the shared Windvale transaction outcomes for the separate atomic publisher without changing `file.write_bytes`. Forward language semantics advance only through `Compiler/Windvale`; C# retains its recovery and independent-evidence role. | Implement the narrow Windows/Linux publication adapters and raw packages, qualify their fault and indeterminate-completion evidence, distribute pinned artifacts and provenance, then move ordinary source-to-verified-WVB automation to the native front door. |
