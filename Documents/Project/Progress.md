...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector now composes bounded acyclic direct calls with sequential loops and conditionals under one exact budget. The editable playground compares worker evidence with its .NET oracle, while a separate static route executes the pinned 2,729-byte profile-7 artifact without loading .NET. Profile 5 remains the latest cross-host-qualified boundary. | Define the versioned linear-memory text/buffer ABI, obtain cross-host profile-6/7 and cross-browser evidence, and move the remaining Stage 0 pipeline off the UI thread. |
