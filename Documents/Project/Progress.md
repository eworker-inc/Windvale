...
| 11. Boot path and kernel | 🚧 In progress | Probe 37 remains the cross-host-qualified endpoint baseline. Locally implemented Probe 38 splits the directory provider into a third protected process, adds a second `WVENDP01`/`WVCHAN04` pair, and routes initial entries and explicit wakes through a three-record ready/wait dispatcher. All 38 focused OS tests and five pinned Windows QEMU scenarios pass locally; independent cross-host qualification is pending. | Add the first monotonic timer and bounded single-CPU preemption without turning the fixed dispatcher into a public scheduling ABI. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 16 over execution ABI 3. The complete 722,837-byte verifier and separate 412,367-byte WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs verify-then-interpret scalar/text/bytes/record/enum workloads plus `WVXI 2` byte-array entry. A capability-free 597,545-byte compiler adapter accepts WVSS, passes all three compiler-capacity phases, completes enlarged interpreter preflight, and executes its first guest instruction. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Replace immutable whole-frame reconstruction with bounded reusable/reclaiming interpreter storage; the current full compiler run reaches the enclosing 4 MiB Wasm arena. Continue until it publishes canonical WVB, then package the complete static-worker pipeline. |
...
- 🔵 Capability-bearing hosted library → explicit transitive application approval → canonical WVB requirement → separate runtime grant → live immutable `WVRS 1` lookup → versioned `WVDR 1` directory read → cross-host-qualified Probe-35 guest service → Probe-36 contained service peer loss → Probe-37 kernel-owned endpoint identity → local Probe-38 split providers and two endpoints; names and discovery remain
