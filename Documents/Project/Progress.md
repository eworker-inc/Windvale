...
| Libraries 1.0 | Active | Foundation memory-budget, collection, byte-buffer, and builder contracts have focused implementations and fixtures. Canonical Option/Result case predicates and direct-owner immutable payload borrowing reach WVIR 1.33/1.34. Candidate WVB 1.39 now publishes all three immutable payload projections with explicit borrowed view and payload-local shapes. The focused Windows owner passes all 184 cases, including 12 deterministic structural publication cases; no database suite was part of that run. The current database remains a useful bounded byte-oriented consumer and recompiles through the current compiler. | Encode borrowed-payload identity across direct call parameters, add complete WVB verification and runtime execution, then reproduce on Linux. Complete Option/Result exclusive borrow, take, and mapping operations before moving through primitive ordering, collection mutation/slicing, and bounded byte construction. Migrate and qualify required real consumers afterward; the current database's passing storage suite does not prove those APIs or unsafe-region adoption. |
...
- The Libraries 1.0 track has a source-publication candidate at WVB 1.39 for
  ...
  consumers reject 1.39 until cross-call borrowed-payload identity, typed-stack
  and lifetime verification, and runtime support land.
