...
| Unsafe Foundation slice | Candidate | Canonical unsafe value types and `Constructˉscratch` lower to typed WVIR and WVB 1.33 with explicit memory authority, size, alignment, ABI, effect, failure shape, complete compiler-aligned verification, and a bounded scalar provider. [Decision 0904](../Decisions/0904-Execute-Wvb-1.33-Unsafe-Scratch-In-A-Bounded-Scalar-Provider.md) adds exact validation failures, zeroed private backing, a non-address-like opaque carrier, and invocation teardown without a host capability. | Add callable budget transfer, native containment, pointer and write-region borrowing, authenticated Foreign calls, a migrated real boundary, Linux reproduction, and paired-host qualification. |
