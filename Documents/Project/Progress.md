...
The latest Phase 10 local increment is [Decision 0450](../Decisions/0450-Native-Probe-40-Native-Bridge-And-Support-Producer.md), which adds the two-section native bridge/support object to the focused producer, reduces the frozen seed to six objects, and expands its permanent lane to six cases. The retirement coordinator now owns 28 suites and 3,133 cases. Earlier transferred families remain indexed by their decisions and the native retirement-suite specification. Internal `WVL1011`, hostile unsafe-bytecode value limits, large-native segmented-object transfer, broader native host-container reconstruction, additional dynamic WVA vectors, complete Windows/Linux execution, promotion, and the grouped retirement gate remain.
...
owns 28 suites and 3,133 fixed cases without .NET. The latest six cases verify
all three exact OS Probe object recipes, independent admission, destination
