...
Filesystem slice 4 now has its first implemented format boundary. The strict
FAT32 volume-admission policy checks a 512-byte boot sector against a separately
admitted 70,000-sector device extent, derives 68,890 legal data clusters, proves
both FATs can address them, and bounds the root and reserved metadata sectors.
All 25 focused cases pass; FAT entry/chain and directory/file reads remain the
next provider work.
