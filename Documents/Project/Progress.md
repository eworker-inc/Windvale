...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 15 over execution ABI 3. The complete 722,837-byte verifier and separate 253,707-byte WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs the real verify-then-interpret composition with exact scalar calls/control plus bounded static data, descriptor calls, immutable text/bytes operations, strict UTF-8, dual budgets, call depth, and allocation failures. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Add invariant formatting, text quoting, byte hashing, and record/enum values to the interpreter, then execute the compiler and package the complete static-worker pipeline before cross-host and cross-browser qualification. |
