...
| 9. Shared native backend | 🎯 Current transfer | Cross-host-qualified Decision 0133 consumes the record maps in ABI 21, independently decodes frame-owned direct records, and runs the exact compiler fixture with zero record-arena use. Local Decision 0141 reduces Decision 0136's 902,262,268 constructed flat bytes to an exact ideal peak of 9,030,829 bytes across 17 live backings. | Replay the exact lifetime trace through a concrete bounded allocator, including metadata and fragmentation, then implement the smallest verified ownership/reclamation model before native Stage 1/Stage 2 equality. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector now reaches local profile 12 over execution ABI 3. Real private `bytes -> bytes` Wasm calls share status, instruction budget, and the bounded arena across control, enabling the semantic verifier to be split into understandable Windvale functions. Profile 11's 113,385-byte structural verifier and profile 8's editable .NET-free page remain intact; profile 10 is cross-host qualified. | Build the modular canonical semantic WVB verifier over profile 12; then add measured text/record/enum execution and obtain cross-host plus cross-browser evidence. |
