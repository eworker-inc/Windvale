...
The latest Phase 10 local increment is [Decision 0453](../Decisions/0453-Native-Probe-40-Loader-Object-Producer.md), which reconstructs the normal loader object through a small Windvale producer and a pinned architecture fixture, reduces the frozen seed to three objects, and expands the permanent producer lane to nine cases. The retirement coordinator now owns 28 suites and 3,136 cases. Earlier transferred families remain indexed by their decisions and the native retirement-suite specification. Internal `WVL1011`, hostile unsafe-bytecode value limits, large-native segmented-object transfer, broader native host-container reconstruction, additional dynamic WVA vectors, complete Windows/Linux execution, promotion, and the grouped retirement gate remain.
...
owns 28 suites and 3,136 fixed cases without .NET. The latest nine cases verify
all six exact OS Probe object recipes, independent admission, destination
