...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration Slice 1 evidence](Windvale-Language-1.0-Migration-Evidence.md) now binds all 16 bundles and 72 source fixtures, runs 37 bounded descriptor/resolver cases, admits only the pinned bootstrap `en@1`, exposes the post-descriptor source as a no-copy view, parses the standalone Core header, and deterministically compiles and executes one 221-byte edition-1 WVB through the existing compiler. Seed remains the temporary descriptorless path; external profile artifacts, most Language 1.0 semantics, Linux performance evidence, and `zh-Hans@1` qualification remain open. | Make exact source-profile lock/artifact inputs part of the build plan, validate their chain, and route English token resolution through the admitted profile for Slice 2. |
