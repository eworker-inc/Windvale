...
| 9. Shared native backend | 🎯 Current transfer | Accepted ABI 20 adds checked two-byte construction; exact compiler preflight now completes selection and measures 4,556,121 bytes against the 1,048,576-byte fragment limit. | Analyze exact size pressure, then choose bounded compaction, function-granular publication, or a justified limit revision. |
...
| 11. Boot path and kernel | 🚧 In progress | Probe 32 through ABI 19 / `WVKMEM10` is cross-host qualified with the same `WVPROC11` behavior, a 134-page arena, and a 113-page client root; all four local pinned-QEMU scenarios pass. | Let non-tail lifetime pressure, broader traps, Hyper-V, or a third runnable choose the next kernel slice. |
...
| WebAssembly interoperability | 🚧 In progress | A cross-host-qualified portable `.wv` selector emits deterministic Wasm from a bounded straight-line WVB `i32` stream. The playground runs accepted output in a disposable worker and compares ABI-1 result and instruction evidence with its .NET oracle. | Move the remaining Stage 0 pipeline off the UI thread and add structured control flow before considering a broader target. |
...
- 🎯 Qualified compiler WVB → accepted ABI-20 fixed-width construction → exact native selection → measured code-size boundary
