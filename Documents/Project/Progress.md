...
| 7. Foundation modules | 🔵 Ongoing | Machine contracts, byte ordering, decimal parsing, and byte construction are shared by real Windvale tools. Decisions 0145 and 0153 qualify explicit transitive platform-library approval plus the first versioned, rights-limited immutable directory read. Probe 35 now exercises the same semantics through an isolated guest service. | Add another library contract only when a real host or OS consumer requires it; keep provider and platform metadata explicit. |
...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 34 remains the baseline. Implemented-candidate Probe 35 adds `WVKMEM14`, `WVPROC14`, `WVRES006`, dedicated service-response pages, an init-only `WVDS 1` mapping, and two maximal guest directory reads while retaining ABI 22, `WVCHAN03`, 110 RX pages, and the six-page stack proof. All 37 OS tests and four Windows pinned-QEMU scenarios pass locally. | Obtain fresh cross-host qualification, then separate service discovery/endpoints or migrate another checked machine-policy leaf into Windvale only when a concrete next consumer requires it. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 15 over execution ABI 3. The complete 722,837-byte verifier and separate 306,560-byte WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs the real verify-then-interpret composition with exact scalar calls/control, bounded static data and descriptors, immutable text/bytes operations, strict UTF-8, invariant formatting, UTF-16-compatible quoting, dual budgets, call depth, and allocation failures. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Add import-free SHA-256 and record/enum values to the interpreter, then execute the compiler and package the complete static-worker pipeline before cross-host and cross-browser qualification. |
...
- 🚧 Capability-bearing hosted library → explicit transitive application approval → canonical WVB requirement → separate runtime grant → live immutable `WVRS 1` lookup → versioned `WVDR 1` directory read → Probe-35 guest service; fresh cross-host qualification and independent platform metadata remain
...
- 🚧 Three typed resources → canonical `WVRS 1` lookup → separately attached `WVDS 1` snapshot → exact `WVDQ 1` / maximal `WVDR 1` exchange in both rebuilt clients → all four local Windows Probe-35 QEMU scenarios; fresh cross-host qualification remains
