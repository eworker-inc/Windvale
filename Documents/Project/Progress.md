...
The latest Phase 10 local increment is [Decision 0464](../Decisions/0464-Native-WVHV-Evidence-Process.md). The existing native metadata-request process now hashes immutable verifier and six-service resources into exact `WVVE 1` evidence, and a second focused native tool projects it through Decision 0463's constructor into exact `WVVR 1`. Both WVBs build through the native compiler and execute as native x64 for Windows and Linux verifier resources; their final requests match the frozen oracle byte for byte. The original ten-service mode also passes with repinned exact Windows/Linux package identities. Decisions 0461 and 0462 separately own admitted `WVHV` metadata and the exact 4,096-byte runtime header. The qualified ordinary verifier artifacts remain pinned while retained request-tool containers, six-service bundle materialization/orchestration, verifier startup, layout/plan, platform bytes, publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next are retained request-tool containers and the native verifier service-bundle process, then verifier startup/layout composition, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
