...
The latest Phase 10 local increment is [Decision 0463](../Decisions/0463-Native-WVHV-Metadata-Request-Ownership.md). Portable Windvale now projects target, entry, the shared six-service publication plan, and seven SHA-256 identities into exact `WVVR 1` verifier metadata requests. Its 15,070-byte native-built WVB executes identically under Windvale interpretation and native lowering, matches the frozen Windows/Linux C# request oracle byte for byte, and rejects thirteen malformed evidence families. Decisions 0461 and 0462 separately own admitted `WVHV` metadata and the exact 4,096-byte runtime header; none of these modules uses the frozen C# source compiler. Decision 0460's repinned 57-artifact toolset and exact package smoke remain stable. The qualified ordinary verifier artifacts remain pinned while native immutable-resource hashing, six-service bundle orchestration, verifier startup, layout/plan, platform bytes, publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next is the native verifier evidence/service-bundle process, then verifier startup/layout composition, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
