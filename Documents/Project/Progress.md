...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 35 adds `WVKMEM14`, `WVPROC14`, `WVRES006`, dedicated service-response pages, an init-only `WVDS 1` mapping, and two maximal guest directory reads while retaining ABI 22, `WVCHAN03`, 110 RX pages, and the six-page stack proof. All 37 OS tests pass on Windows and Debian; four pinned-QEMU scenarios pass on Windows. | Separate service discovery/endpoints or migrate another checked machine-policy leaf into Windvale only when a concrete next consumer requires it. |
...
- 🔵 Capability-bearing hosted library → explicit transitive application approval → canonical WVB requirement → separate runtime grant → live immutable `WVRS 1` lookup → versioned `WVDR 1` directory read → cross-host-qualified Probe-35 guest service; independent platform metadata remains
...
- 🔵 Three typed resources → canonical `WVRS 1` lookup → separately attached `WVDS 1` snapshot → exact `WVDQ 1` / maximal `WVDR 1` exchange in both rebuilt clients → cross-host-qualified Probe 35 with all four Windows QEMU scenarios
