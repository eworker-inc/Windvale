...
The latest implemented Phase 10 local increment is [Decision 0513](../Decisions/0513-Native-Fixed-Service-And-Publication-Build-Inspection-Transfer.md). The paired native helper now builds 43 exact artifacts and owns 76 build/verify/inspect/run cases. The fixed-service/publication slice reproduces twelve core/bridge products and natively inspects eleven ownership surfaces while preserving independent retained-WVB, leaf, and fragment comparisons. Its Windows contract passes in 39.3 seconds, and the five directly affected frozen behavioral owners pass 5/5 in 12.158 test seconds. Ten manifests are component-local; only the genuine cross-component service-bundle aggregates remain at root, and four obsolete root manifests are removed. This removes twenty-three additional managed invocations from each broad host script, eighty-two cumulatively. Capability-bearing execution, the broad harness, and later qualification phases still use .NET, so T2 remains managed-normal. Independent Linux execution, the remaining broad transfers, grouped qualification, promotion, and the final Stage 0 recovery release remain open.

[Decision 0512](../Decisions/0512-Native-Io-Service-Build-And-Inspection-Transfer.md) previously moved the complete output/file-output/file-input closure. Decision 0513 preserves that evidence while extending the same paired helper to fixed service leaves, enum metadata, publication layout, and service-bundle materialization.
