...
The latest implemented Phase 10 local increment is [Decision 0514](../Decisions/0514-Native-Runtime-Table-Build-And-Inspection-Transfer.md). The paired native helper now builds 59 exact artifacts and owns 100 build/verify/inspect/run cases. The runtime-table slice reproduces sixteen output, file-output, file-input, service, execution-context, argument, entry, and byte-result-admission products and natively inspects all eight public bridges while preserving independent retained-WVB and fragment comparisons. Its Windows contract passes in 40.9 seconds, and the eight directly affected frozen behavioral owners pass 8/8 in 7.981 test seconds. All sixteen manifests are component-local and eight obsolete root manifests are removed. This removes twenty-four additional managed invocations from each broad host script, 106 cumulatively. Capability-bearing execution, the broad harness, and later qualification phases still use .NET, so T2 remains managed-normal. Independent Linux execution, the remaining broad transfers, grouped qualification, promotion, and the final Stage 0 recovery release remain open.

[Decision 0513](../Decisions/0513-Native-Fixed-Service-And-Publication-Build-Inspection-Transfer.md) previously moved fixed service leaves, enum metadata, publication layout, and service-bundle materialization. Decision 0514 preserves that evidence while extending the same paired helper through the runtime-table and entry-metadata layer.
