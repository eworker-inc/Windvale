...
The latest Phase 10 local increment is [Decision 0465](../Decisions/0465-Retained-Native-WVHV-Request-Containers.md). The small `WVVE` to `WVVR` wrapper now has exact retained Windows and Linux format-3 containers. Its WVB builds through the native compiler; both containers have fixed identities; the current-host artifact rebuilds only through `recovery-aot`, executes as a real native process after Decision 0464's immutable hashing process, and reproduces the exact request for both verifier targets. New C# is limited to deletion-bound target and identity wiring. Decisions 0461 through 0464 own request evidence/projection, admitted `WVHV` metadata, and the exact 4,096-byte runtime header. The qualified ordinary verifier artifacts remain pinned while native reconstruction of the wrapper containers, six-service bundle materialization/orchestration, verifier startup, layout/plan, platform bytes, publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next are native request-container reconstruction and the verifier service-bundle process, then verifier startup/layout composition, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
