...
| Libraries 1.0 | Active | The package parser and lock consumer use `Option<u64>` and immutable borrowing through Project 4 builds, native publication, and Windows/Debian execution. Focused WVB 1.40/1.41 checks cover Vector payloads and scalar indexing. | The next owned-resource consumer is the typed Package-Lock directory. It needs record-element Vector storage/tracing, budget threading, and scanner ownership. Automatic CI execution of indexed borrowing, native lowering for minors 40/41, installed promotion, wider payloads, exclusive borrow, Take, mapping, and remaining Foundation/Data/Backend APIs are separate gates. See the [completion matrix](Compiler-Tools-And-Libraries-1.0-Matrix.md#current-checkpoint-and-next-action). |
