...
| 9. Shared native backend | 🎯 Current transfer | Cross-host-qualified Decision 0133 consumes the record maps in ABI 21. Decisions 0143 and 0147 prove the complete allocation capacity and publish 186,557 independently reconstructed ownership actions. Local Decision 0148 adds a live WVA-owned first-fit/reference-count allocator leaf and projects the exact compiler to 180,190 allocator calls without changing ABI 21. | Integrate owner tokens and allocator operations into a successor ABI for one small descriptor fixture, independently verify the selected instructions, then broaden to the exact compiler and shared host/OS consumers. |
...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified ABI-21 Probe 32 remains the baseline. Implemented-candidate Probe 34 advances to `WVKMEM12`, `WVPROC13`, `WVCHAN03`, and `WVRES005`: init alone owns a 1,195-byte RO/NX `WVRS 1` mapping, dynamically selects `boot:main.configuration` in both rebuilt clients, and terminal peer cleanup clears retained IPC state. All four Windows pinned-QEMU scenarios pass. | Use the new capability-bearing library boundary to specify one rights-limited read-only filesystem/provider operation with typed recoverable results; keep writable state, crash consistency, broader scheduling, and traps separate. |
