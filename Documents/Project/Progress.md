...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact replacement source design, and [migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds its bundles and fixtures. Slices 2 and 3 now cover the scalar/value surface, typed failure, generic Foundation `Option`/`Result`, canonical generic syntax and resolution, and direct bounded generic-function specialization. Slice 4 has a validated WVGT generic-nominal catalog, recursive nominal binding, substituted layouts, materialization plans, WVLB 1.3 carriage, main WVB materialization, and executable generic-record construction and field reads. [Decision 0810](../Decisions/0810-Use-The-Split-Compiler-For-Compiler-Scale-Development.md) keeps the immutable Seed as recovery provenance while the evolving analyzer/emitter becomes the Windvale 1.0 compiler. The focused Language 1.0 owner passes 186 cases. | Connect generic nominal types inside generic-function bodies, then migrate the remaining Foundation special planning and continue the remaining slices. Keep broad storage, OS, paired-host, and complete Qualification gates for final integration unless a changed boundary directly requires them. |
