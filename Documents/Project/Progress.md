...
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale constructs live service leaves and owns image layout/lifecycle; the exact compiler now produces the exact 815-byte Stage 0 WVB through Windows and Linux native W^X paths. An implemented `windows-x64-console-v1` slice passes the complete local Windows gate, packages capability-free scalar programs as import-free PE32+, and runs `Sum-Data.wv` directly to result `29`; cross-host construction remains pending. | Qualify the first Windows container cross-host, then add measured hosted/runtime ownership, native compiler reproduction, and standalone Windows/Linux tools before satisfying the documented .NET-retirement gate. |
...
- 🚧 Portable scalar `.wv` → verified WVB/native fragment → deterministic import-free Windows `.exe` → process result
