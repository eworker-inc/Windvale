...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds all 16 bundles and 72 source fixtures and admits the exact `en@1` chain only through hash-pinned Project 3 inputs. Slice 2 now has vertical compiler/verifier/runtime checkpoints for named record update, WVB 1.12 fixed-width integers, WVB 1.13 exact runes, WVB 1.14 strict `f32`/`f64`, WVB 1.15 ordinary `unit` plus return-only `never`, and WVB 1.16 zero-through-64-field variants in the source-built native scalar runner. The variant path shares one bounded 768-cell record/variant arena, rejects mismatched cases with `WVR3017`, and reclaims 900 sequential replacements in its pressure oracle. Project 2 and descriptorless Seed retain their prior behavior; value-producing control, localized token execution, paired-host evidence, direct native/browser/OS WVB 1.16 consumers, and `zh-Hans@1` qualification remain open. | Implement value-producing `if` and `match` over the same compiler, verifier, and scalar runtime architecture. |
