...
The latest Phase 10 local increment is [Decision 0460](../Decisions/0460-Native-Hosted-Enum-Directory-For-Variants.md). A separate Windvale hosted nominal-shape reader now admits WVB 1.11 records and variants for bounded package metadata without pretending that the ordinary native lowerer can execute variants. The Windvale request and service processes preserve records and variants as zero-member type-directory entries, including modules with no enum members, and the refreshed source-exact retained Windvale bridge passes focused process and artifact-identity checks. This closes Decision 0459's measured enum-request rejection. The 57-artifact candidate toolset is repinned and its established exact Windows, cross-target Linux, and rejection-preservation package smoke passes. Recovery export plus native linking supplies the enlarged verifier's exact image and entry; image-mode construction passes every native process, including enum request/service, but the diagnostic cannot be promoted because the packager exposes only compiler-family profiles and profile 2 is the `WVHB` build driver. The qualified ordinary artifacts remain pinned while a distinct read-only verifier profile, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next are the explicit verifier container profile and startup/metadata admission, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
...
owns 31 suites and 3,147 fixed cases without .NET. The latest two cases verify
