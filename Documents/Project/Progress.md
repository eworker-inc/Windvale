...
| 9. Shared native backend | 🎯 Current transfer | Qualified Decision 0112 measures 1,480,096 record bytes and completes exact compiler execution under a bounded 2 MiB host arena without changing ABI 20. | Resolve the separate 4 MiB WVO/object and flat-linker AOT boundaries from measured output pressure. |
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale constructs live service leaves and owns image layout/lifecycle; the exact compiler now produces the exact 815-byte Stage 0 WVB through Windows and Linux native W^X paths. | Prove native compiler reproduction and standalone Windows/Linux tools, then satisfy the documented .NET-retirement gate. |
