...
The latest Phase 10 local increment is [Decision 0470](../Decisions/0470-Native-WVHV-Container-Composition.md). Windvale now admits the complete digest-bound verifier service bundle and joins the previously native-owned runtime, platform, startup, and bundle responses into exact format-4 PE/ELF applications. The new 53,900-byte WVB and paired native packages extend the hosted candidate to 72 artifacts; no managed product writer or target was added. Decisions 0461 through 0469 own admitted `WVHV` metadata, the exact runtime header, immutable hashing, exact `WVVR` projection, retained request containers, native reconstruction, the complete six-service bundle, startup relocation, and platform bytes. New C# remains test/differential evidence plus deletion-bound recovery identity wiring. The qualified ordinary verifier artifacts remain pinned while durable pipeline wiring, native execution, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next is wiring this constructor to the existing durable publisher, then native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
