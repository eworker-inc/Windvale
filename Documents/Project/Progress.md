...
| 7. Foundation modules | 🔵 Ongoing | Machine contracts, byte ordering, decimal parsing, and byte construction are shared by real Windvale tools. Decision 0145 adds explicit transitive platform-library approval; candidate Decision 0153 adds the first versioned, rights-limited immutable directory read with typed results. | Qualify the directory contract cross-host, then bind the same operation to a Windvale OS runtime/service endpoint without exposing native paths or changing the kernel's format-blind role. |
...
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale owns version-1 startup/layout/construction/verification. Implemented-candidate version 2 adds the first standalone `console.write_line` capability with `WVHC 1` metadata, WVA startups, exact Windows/Linux output leaves, and local Windows execution. ABI 22 also has cross-host-qualified in-memory compiler reproduction. | Qualify version 2 cross-host, then lift the separate 4 MiB WVO/link boundary deliberately, package the compiler, and replace the remaining native tools and repository workflow. |
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 34 retains `WVPROC13`, `WVCHAN03`, and `WVRES005`; the ABI-22 rebuild advances kernel memory to `WVKMEM13`, a 144-page arena, a 121-page client root, and 110 RX pages while preserving the six-page stack proof. Decision 0153 now supplies the next application-facing directory contract without changing the probe. | Design the smallest format-blind IPC/runtime binding for the exact `WVDR 1` read operation. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 14 over execution ABI 3. The complete 722,837-byte verifier and separate 145,469-byte scalar WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs the real verify-then-interpret composition with exact scalar calls/control, guest and outer budgets, call depth, and checked overflow. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Add text/bytes and record/enum values to the interpreter, then move source compilation and the complete worker pipeline into static assets before cross-host and cross-browser qualification. |
...
- 🚧 Capability-bearing hosted library → explicit transitive application approval → canonical WVB requirement → separate runtime grant → live immutable `WVRS 1` lookup → candidate versioned `WVDR 1` directory read; independent platform metadata and OS binding remain
...
- 🚧 Hosted `.wv` with `console.write_line` → verified service requirement → `WVHC 1` metadata and exact output leaf → standalone Windows/Linux console application; cross-host qualification pending
...
- ✅ Canonical WVB → complete Windvale verifier Wasm → bounded scalar interpreter Wasm → exact result and dual-budget evidence under Node.js
...
- ✅ Three typed resources → canonical `WVRS 1` image → checked guest request/reply → independently mapped RO/NX init store → dynamic guest lookup → peer-death cleanup in cross-host-qualified Probe 34; the separate `WVDR 1` directory binding remains
