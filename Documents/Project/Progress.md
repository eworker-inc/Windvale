...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 13 over execution ABI 3. Its bounded acyclic call graph now admits sixteen functions, 131,072 aggregate WVB code bytes, 400,000 decoded instructions, and 1 MiB of generated Wasm while preserving profile-12 artifacts. A derived nine-function, 440,333-byte semantic verifier proves both former capacity boundaries. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Use the new capacity for executable typed-stack/local/control-flow verification, then measured interpreter/compiler execution and cross-host plus cross-browser evidence. |
