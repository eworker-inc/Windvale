...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 40 adds `WVKMEM17`, fixed generation-safe memory objects, and WVA-owned first-fit non-tail client release/zero/reuse while the later directory object remains live, without changing `WVPROC17`, paging 5, ABI 22, or Probe 39's timer proof. Historical qualification passes all 87 Seed and 39 OS tests on Windows/Debian and all five pinned Windows QEMU scenarios. Decision 0435 removes Stage 0 from supplied-image boot execution; the repaired normal image again passes its exact digest-bound boot contract. Decisions 0436 and 0437 add portable UEFI v3 construction, independent verification, native Project 1 front doors, and the real native-link-output handoff to a hosted Windvale packager. | Retain and verify native host containers, pass the other four digest-bound supplied-image scenarios, and reconstruct all five images without the managed Probe 40 coordinator. |
...
owns 24 suites and 3,118 fixed cases without .NET. Hostile value limits and
