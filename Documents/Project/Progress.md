...
The latest Phase 10 local increment is [Decision 0487](../Decisions/0487-Native-WVHV-Publisher-Promoter-Applications.md). A separate non-circular Windvale promoter now has exact paired Windows/Linux applications constructed by the same native metadata, identity, structure, target, object, import, and PE/ELF stages as the hosted-verifier publisher. Exact role fields preserve every original publisher record and final byte while allowing the digest-bound promoter to install both publisher subjects through the existing immutable-snapshot atomic transaction. The installed current-host publisher then installs an exact verifier without loading .NET. The focused native owners pass 12 construction/install cases and four rejection/preservation cases; the 32-suite retirement coordinator now owns 3,161 fixed cases. Independent Linux execution, native build-driver self-convergence, remaining runtime/backend gaps, GitHub cutover, the grouped dual-host gate, promotion, and the final Stage 0 recovery release remain.
