...
The latest Phase 10 local increment is [Decision 0449](../Decisions/0449-Native-Probe-40-Admission-Bridge-Producer.md), which consolidates the focused native object producer, moves the WVB admission bridge out of the frozen seed, and expands its permanent lane to five cases. The retirement coordinator now owns 28 suites and 3,132 cases. Earlier transferred families remain indexed by their decisions and the native retirement-suite specification. Internal `WVL1011`, hostile unsafe-bytecode value limits, large-native segmented-object transfer, broader native host-container reconstruction, additional dynamic WVA vectors, complete Windows/Linux execution, promotion, and the grouped retirement gate remain.
...
owns 28 suites and 3,132 fixed cases without .NET. The latest five cases verify
both exact OS Probe object recipes, independent admission, destination
