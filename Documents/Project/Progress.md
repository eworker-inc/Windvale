...
| 11. Boot path and kernel | 🚧 In progress | Qualified probe 28 cleans one borrow; candidate probe 29 atomically grants and cleans a typed WVB/budget pair and measures a four-page kernel stack. | Cross-host qualify probe 29, then let actual page/root reuse, independent lifetime, broader input, or a third runnable measure the next boundary. |
| 12. Runtime on Windvale OS | 🚧 In progress | Candidate process `2` reads typed WVB and execution-budget resources, charges four opcodes, and loses both aliases at terminal state. | Qualify the pair, then broaden semantics only for a measured program; keep JIT publication outside the kernel. |
...
- 🚧 Probe 29 → atomic typed WVB/budget set → exact WVA lookup and Windvale opcode charging → 25/25 local OS tests plus four Windows pinned-QEMU scenarios; cross-host qualification pending
