...
| Development installers | 🚧 | [Decision 0562](../Decisions/0562-First-Deterministic-Development-Installers.md) constructs pinned `0.1.0-dev.1` Windows ZIP and Linux tarball candidates containing the seven native core tools, bounded `wv` client, offline payload doctor, immutable per-user generation, command shims, and safe uninstall. The eight-case Windows owner passes locally. | Retain the exact focused owner on both permanent hosts, then consume these identities from the signed release envelope rather than widening the installer. |
...
Promote the first deterministic per-user Windows and Linux development
installers under Decision 0562. Their base payload is Windvale plus the bounded
