...
| 7. Foundation modules | 🔵 Ongoing | Machine contracts, byte ordering, decimal parsing, and byte construction are shared by real Windvale tools. Candidate Decision 0145 moves portable `WVRS 1` policy into `Libraries/Foundation` and adds the first capability-bearing `Libraries/Platform` adapter with explicit transitive approval. | Qualify the library slice cross-host, then add platform/filesystem facilities only from a measured consumer and exact provider contract. |
| 8. Self-hosted compiler | ✅ Qualified | The committed 12-module inventory produces byte-identical 599,868-byte Stage 1 and Stage 2 compilers on Windows and Debian. Integrated Decision 0150 runs the exact native Stage 1 over the complete inventory and reproduces those Stage 2 bytes locally. | Cross-host qualify complete native reproduction on the integrated ownership-plan baseline. |
| 9. Shared native backend | 🎯 Current transfer | Cross-host-qualified Decision 0133 supplies frame-owned records. Decision 0147 publishes and independently reconstructs 186,557 descriptor-ownership actions; Decision 0148 adds the live WVA first-fit/reference-count leaf; integrated Decision 0150 verifies generation-owned byte buffers and return checkpoints and completes native reproduction without implicitly calling that leaf. | Qualify ABI 22 on both hosts, then add caller-liveness evidence before compacting descriptor-bearing aggregate returns or broadening allocator integration. |
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale owns startup templates, layout, every final PE/ELF byte, and completed-container verification. ABI 22 has hosted in-memory compiler reproduction evidence, while bounded Stage 0 publication and recovery oracles remain. | Serialize hosted capability/runtime requirements, lift the separate 4 MiB WVO/link boundary deliberately, package the compiler, and replace the remaining native tools and repository workflow. |
| 11. Boot path and kernel | 🚧 In progress | Probe 34 retains `WVPROC13`, `WVCHAN03`, and `WVRES005`; the combined ABI-22 rebuild advances kernel memory to `WVKMEM13`, a 144-page arena, a 121-page client root, and 110 RX pages while preserving the six-page stack proof. | Rebuild and qualify the integrated Probe 34 locally and across hosts before the next filesystem/provider operation. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 13 over execution ABI 3. A ten-function, 722,837-byte import-free Wasm verifier now proves canonical WVB metadata plus compiler-aligned typed local/stack, call, capability, record, enum, return, reachability, and exact maximum-stack flow. Node.js accepts three representative modules and rejects nine Stage-0-matched executable mutations. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Execute verified WVB with a Wasm-hosted Windvale interpreter, then move source compilation and the complete worker pipeline into static assets before cross-host and cross-browser qualification. |
...
- 🚧 Capability-bearing hosted library → explicit transitive application approval → canonical WVB requirement → separate runtime grant → live immutable `WVRS 1` lookup; independent platform metadata and filesystem instances remain
...
- 🚧 Native Stage 1 compiler → complete 12-source inventory → byte-identical Stage 2 compiler
...
- 🚧 Verified descriptor ownership → exact WVA allocator leaf → live W^X differential execution; successor-ABI selection remains open
- 🚧 ABI-22 dynamic values → generation-owned byte buffers → verified return checkpoints → complete native Stage 2 reproduction (cross-host qualification pending)
