...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0169 carry the exact ABI-22 compiler through bounded large-native transport, service manifests, RW/NX runtime state, deterministic PE/ELF containers, public `compile`/`aot` publication, and direct Stage 2 reproduction on both hosts without loading .NET. The recovery runbook records exact Stage 0 provenance. | Move the remaining build, verify, test, link, package, and run tools off Stage 0 before changing normal automation. |
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 37 adds kernel-owned `WVENDP01`: process capabilities resolve through one generation-safe endpoint, rebind across client reuse, and close exactly once on service exit or fault. All 87 Seed and 38 OS tests pass on Windows/Debian; five pinned Windows QEMU scenarios pass. | Apply Decision 0173's selected pressure: split the immutable directory provider into a fixed third process, add the second endpoint, and replace the exact coordinator with the smallest state-driven ready/wait dispatcher. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 16 over execution ABI 3. The complete 722,837-byte verifier and separate 410,698-byte WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs verify-then-interpret scalar/text/bytes/record/enum workloads plus the new `WVXI 2` byte-array entry. A capability-free 597,545-byte compiler adapter accepts WVSS and returns versioned WVB-or-diagnostic bytes; all three compiler-capacity phases admit it. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Expand the interpreter past its measured sixteen-function rejection to the portable compiler's 326-function graph, then address frames, recursion, budgets, and dynamic-value lifetime until it produces canonical WVB; package the complete static-worker pipeline afterward. |
