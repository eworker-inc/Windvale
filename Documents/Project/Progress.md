...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0169 package and directly run the exact ABI-22 compiler. Implemented Decisions 0185–0187 add paired standalone verifier applications plus a paired format-5 Windvale-native driver that consumes explicit sources or Project 1, compiles each source snapshot once, invokes the shared portable verifier, and publishes accepted WVB without loading .NET; direct current-host and malformed-container evidence pass, while cross-host qualification is pending. | Qualify the verifier/driver pairs on both hosts, transfer the shared x64 lowering backend into Windvale, and add a named atomic-replacement or external native publication step before moving native packaging and normal automation off Stage 0. |
