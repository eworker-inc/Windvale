...
| Development installers | ✅ | [Decision 0562](../Decisions/0562-First-Deterministic-Development-Installers.md) qualifies pinned `0.1.0-dev.1` Windows ZIP and Linux tarball artifacts containing the seven native core tools, bounded `wv` client, manifest-pinned offline doctor, immutable per-user generation, command shims, and safe uninstall. [Verify run 31881681424](https://github.com/eworker-inc/Windvale/actions/runs/31881681424) passes the eight-case owner on both target hosts. | Preserve the exact inputs and consume these identities from the signed release envelope rather than widening the installer. |
...
Consume the qualified deterministic per-user Windows and Linux development
installers from Decision 0562. Their base payload is Windvale plus the bounded
launcher/client, offline payload verifier, compiler, assembler, linker, runtime,
...
implemented under Decisions 0563 and 0564. Their 13 release-envelope and 8
approval-record cases pass on both hosts in Verify run 31883543587. The
remaining sequence is project-owner root custody, one selected-state complete
