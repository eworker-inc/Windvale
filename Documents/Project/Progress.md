...
The latest Phase 10 local increment is [Decision 0488](../Decisions/0488-Native-WVB-Publisher-Applications.md). The current general WVB publisher now rebuilds and lowers through the native front doors, then uses the same exact role-aware metadata, identity, structure, target, object, import, and PE/ELF pipeline as the hosted-verifier publisher and promoter. Its paired Windows/Linux applications match the independent Stage 0 writer byte for byte; the current-host candidate durably publishes a canonical portable WVB without loading .NET. The focused native owner now covers 15 construction/admission/execution cases, and the 32-suite retirement coordinator owns 3,164 fixed cases. The retained and candidate publishers expose the same existing current compiler/build-driver semantic-verifier rejection, so independent Linux execution, native build-driver self-convergence, GitHub cutover, the grouped dual-host gate, promotion, and the final Stage 0 recovery release remain.
