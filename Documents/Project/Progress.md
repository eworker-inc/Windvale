...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0169 package and directly run the exact ABI-22 compiler. Implemented Decisions 0185–0186 add paired standalone verifier applications plus a paired format-5 Windvale-native driver that compiles source, invokes the shared portable verifier, and publishes accepted WVB without loading .NET; direct current-host and malformed-container evidence pass, while cross-host qualification is pending. | Qualify the verifier/driver pairs on both hosts, add a named atomic-replacement capability or external native publication step, then move project planning, tests, assemble, link, inspect, package, and run off Stage 0 before changing normal automation. |
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 38 splits the directory provider into a third protected process, adds a second `WVENDP01`/`WVCHAN04` pair, and routes initial entries and explicit wakes through a three-record ready/wait dispatcher. All 87 Seed and 38 OS tests pass on Windows/Debian; five pinned Windows QEMU scenarios pass. | Add the first monotonic timer and bounded single-CPU preemption without turning the fixed dispatcher into a public scheduling ABI. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 16 over execution ABI 3. The complete 722,837-byte verifier and separate 412,498-byte WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs verify-then-interpret scalar/text/bytes/record/enum workloads plus `WVXI 2` byte-array entry. A capability-free 597,545-byte compiler adapter accepts WVSS, passes all three compiler-capacity phases, and executes 1,511 guest instructions through exact per-function frames. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Replace immutable frame reconstruction with bounded reusable/reclaiming interpreter storage; guest budget 1,512 is the exact first enclosing 4 MiB Wasm-arena failure. Continue until the compiler publishes canonical WVB, then package the complete static-worker pipeline. |
...
- 🔵 Capability-bearing hosted library → explicit transitive application approval → canonical WVB requirement → separate runtime grant → live immutable `WVRS 1` lookup → versioned `WVDR 1` directory read → cross-host-qualified Probe-35 guest service → Probe-36 contained service peer loss → Probe-37 kernel-owned endpoint identity → qualified Probe-38 split providers and two endpoints; names and discovery remain
...
- 🔵 Explicit ordered `.wv` inputs → Windvale-native compiler → shared portable compiler-aligned verifier → accepted WVB publication through format-5 Windows/Linux driver packages; current-host direct evidence passes, cross-host qualification and atomic source-visible replacement remain
