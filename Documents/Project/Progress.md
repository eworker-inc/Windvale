...
| 9. Shared native backend | 🎯 Current transfer | Cross-host-qualified Decision 0133 consumes the record maps in ABI 21, independently decodes frame-owned direct records, and runs the exact compiler fixture with zero record-arena use. Local Decision 0136 profiles the successful complete bootstrap as 902,262,268 flat dynamic bytes, approximately 99.65% from `bytes.concat`. | Preserve dynamic-backing identity, measure live roots across caller replacement and return, then implement the smallest general ownership/construction model before native Stage 1/Stage 2 equality. |
