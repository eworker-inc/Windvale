...
The latest Phase 10 local increment is [Decision 0452](../Decisions/0452-Native-Probe-40-Memory-Object-Producer.md), which moves the normal memory object into a separate focused Windvale producer, reduces the frozen seed to four objects, and expands the permanent producer lane to eight cases. The retirement coordinator now owns 28 suites and 3,135 cases. Earlier transferred families remain indexed by their decisions and the native retirement-suite specification. Internal `WVL1011`, hostile unsafe-bytecode value limits, large-native segmented-object transfer, broader native host-container reconstruction, additional dynamic WVA vectors, complete Windows/Linux execution, promotion, and the grouped retirement gate remain.
...
owns 28 suites and 3,135 fixed cases without .NET. The latest eight cases verify
all five exact OS Probe object recipes, independent admission, destination
preservation, and closed selector/extension rejection; the Probe 40 cases preserve
the digest-bound normal object seed and resulting EFI image. Hostile
