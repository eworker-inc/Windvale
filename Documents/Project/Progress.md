...
The latest Phase 10 local increment is [Decision 0459](../Decisions/0459-Native-Wvb-1-11-Verifier-Admission.md). The Windvale-written verifier now admits and type-checks WVB 1.11 nominal variants plus the remaining frozen integer operations, while focused variant and record helpers keep its largest function at 1,683 locals under the unchanged ABI 22 limit. The native compiler reconstructs the exact verifier and publisher WVBs; temporary recovery-built verifier applications accept the native-compiled database probes. The qualified ordinary artifacts remain pinned because native build-driver self-convergence, native hosted packaging, native variant execution, independent Linux execution, and promotion are still pending. Decision 0458's changed-file front door continues to stop on those named gaps without invoking .NET. Next are native runner/lowerer support for the database probes, build-driver and hosted-package closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
