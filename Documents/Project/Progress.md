...
| 9. Shared native backend | 🎯 Current transfer | Implemented ABI 18 deterministically reuses exact-type physical cells across empty-stack blocks while retaining the 2,048-cell ceiling; exact compiler preflight now reaches `Bytesˉfromˉu8`. | Cross-host qualify the compact backend, then implement only the next opcode/runtime contract demanded by exact compiler execution. |
