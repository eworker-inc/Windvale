...
| WebAssembly interoperability | 🚧 In progress | The complete portable compiler now lowers to an 18,349,927-byte import-free ABI-4 direct WebAssembly artifact through a fixed 34-segment Windvale generator. The direct compiler preserves the exact 183-byte WVB and result `42`, reports 1,186,358 compiler instructions, and executes the pinned source in 991.3 ms on the measured Windows Node.js host instead of the preceding 64-to-105-second interpreter path. The normal Monaco playground, artifact publication, website verification, deployment, and complete current-Windows generation/verification command are .NET-free; the separate ABI-3 interpreter admits and executes compiler-produced WVB. Decision 0504's complete native verifier passes in 1,619.5 seconds, including the strict Node.js engine, record-arena, and compiler probes. Direct package verification passes under the optimizing WebAssembly tier, while independent Linux execution and real Chromium/cross-browser memory and latency evidence remain pending. Reconstructing the pinned segmented generator/backend packages still uses the explicit Stage 0 recovery seam. | Run the same native package and verification contract independently on Linux, measure the direct package in desktop and mobile Chromium, reduce its 18.35 MB transfer and fixed 2,497-page memory where useful, replace the segmented generator/backend recovery seams, and complete the grouped retirement gate. |

The latest implemented Phase 10 local increment is [Decision 0518](../Decisions/0518-Native-Source-Bindings-Wvir-And-Wvb-Build-Inspection-Transfer.md). The paired native helper now builds 97 exact artifacts and owns 156 build/verify/inspect/run cases. This slice reproduces the source-bindings, typed-WVIR, and source-WVB core/demo/tool products and natively inspects all three core surfaces. Its complete Windows contract passes in 903.3 seconds, and the focused bindings, extended typed-WVIR, and extended WVB-backend owners pass 3/3 in 48.504 test seconds. Six products use the generic native Project front door; the source-WVB family uses a bounded compiler-seed launcher because the pinned generic driver cannot compile the current WVB closure. All nine manifests are honest repository-root aggregates, and their dependency order remains the documented pinned-driver compatibility workaround. This removes twelve additional managed invocations from each broad host script, 162 cumulatively. The three demos, bindings/WVIR hosted tools, and complete source-WVB fixture/differential/oracle sequence remain managed because the native runner stops the demos with code `3004`, lacks those tool profiles, and does not yet provide an independent replacement oracle. The broad harness and later qualification phases also still use .NET, so T2 remains managed-normal. Independent Linux execution, the remaining broad transfers, grouped qualification, promotion, and the final Stage 0 recovery release remain open.

[Decision 0517](../Decisions/0517-Native-Source-Semantic-Build-And-Inspection-Transfer.md) previously transferred canonical source sets, import graphs, and declaration/signature symbols. Decision 0518 preserves that evidence while advancing through source bindings, typed WVIR, and executable WVB emission.
