...
The latest Phase 10 local increment is [Decision 0461](../Decisions/0461-Native-WVHV-Metadata-Ownership.md). A dedicated `WVVR 1` request now drives portable Windvale construction and independent admission of exact `WVHV 1` compiler-verifier metadata without redefining compiler-family `WVHB` profile 2. The 21,566-byte production WVB is built through the native compiler, executes identically under Windvale interpretation and native lowering, matches the frozen Windows/Linux C# metadata oracle byte for byte, and rejects fifteen malformed request families. The frozen C# source compiler is no longer used to compile this new source. Decision 0460's repinned 57-artifact toolset and exact package smoke remain stable. The qualified ordinary verifier artifacts remain pinned while native request/service-bundle composition, runtime header, verifier startup, layout/plan, platform bytes, publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next are the `WVHV` runtime header and native request producer, then verifier startup/layout composition, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
