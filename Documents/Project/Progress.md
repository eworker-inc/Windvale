...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact replacement source design, and [migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds its bundles and fixtures. Slices 2 and 3 cover the scalar/value surface, typed failure, generic Foundation `Option`/`Result`, canonical generic syntax and resolution, and bounded generic-function specialization. Slice 4 now includes generic nominal materialization, contextual fixed arrays, executable Vector/Sequence type and runtime operations, owned local transfer, exact Sequence reads, Vector length/freeze, exact freeze result contexts, and the canonical qualified owned `Memoryˉbudget` source/WIR identity. [Decision 0810](../Decisions/0810-Use-The-Split-Compiler-For-Compiler-Scale-Development.md) keeps immutable Seed recovery provenance while the evolving analyzer/emitter becomes the Windvale 1.0 compiler. The focused Language 1.0 owner declares 417 cases; WVB/runtime memory-budget representation is not yet implemented. | Add explicit budget transfer, WVB/runtime representation, allocation effects, and launcher accounting, then public fallible Vector construction and recoverable append. Continue remaining collections and ownership work without running broad storage, OS, paired-host, or complete Qualification gates unless their boundary changes or a promotion claim requires them. |
