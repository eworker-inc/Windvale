...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0169 package and directly run the exact ABI-22 compiler. Decisions 0185–0187 add paired standalone verifier applications and a project-aware format-5 native build driver. Decisions 0190 and 0194 transfer the first algorithmic ABI-22 selector path: Windvale verifies and exactly lowers bounded straight-line `i32` constants, locals, checked arithmetic, and return, with current-host native-tool and malformed-input preservation evidence. Cross-host qualification is pending. | Qualify the verifier/driver/lowering evidence on both hosts, add the first explicit basic-block/control-flow slice, and add a named atomic-replacement or external native publication step before moving native packaging and normal automation off Stage 0. |
