...
| Offline package lifecycle | 🎯 | [Decision 0568](../Decisions/0568-Offline-Package-Lifecycle-And-Generation-Activation-1.md) selects Milestone 4 without assigning `v0.2.0`. Portable generation semantics and durable activation/recovery pass; WVDB Query and the new exact WVB inspector package supply two distinct real package candidates. | Admit both packages through one offline directory and route commands through the active generation on both hosts. |
...
rollback transition. The WVB inspector now adds the second exact source-package
and lock with a distinct whole-file-read capability closure. Active-generation
command dispatch, shared offline-directory admission, approval/launch closure,
and the two-package end-to-end demonstration remain. Networking, OS-1, and
database breadth stay independent.
