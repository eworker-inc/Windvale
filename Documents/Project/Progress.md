...
The latest Phase 10 local increment is [Decision 0468](../Decisions/0468-Native-WVHV-Startup-Composition.md). Windvale now admits the paired native-assembled verifier startup objects, derives the complete format-4 Windows/Linux runtime and service placement, constructs the ordered `WVSI 1` relocations, and reuses the existing portable instantiator to reproduce both exact startup payloads. The new 63,636-byte WVB and its natively reconstructed platform applications extend the digest-bound hosted candidate to 66 artifacts; no managed product writer or target was added. Decisions 0461 through 0467 own admitted `WVHV` metadata, the exact runtime header, immutable hashing, exact `WVVR` projection, retained request containers, native reconstruction, and the complete six-service bundle. New C# remains test/differential evidence plus deletion-bound recovery identity wiring. The qualified ordinary verifier artifacts remain pinned while outer platform bytes, final publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next is verifier outer-container construction/publication, then native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
