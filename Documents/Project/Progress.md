...
| 7. Foundation modules | 🔵 Ongoing | Machine contracts, byte ordering, decimal parsing, and byte construction are shared by real Windvale tools. Decisions 0145 and 0153 qualify explicit transitive platform-library approval plus the first versioned, rights-limited immutable directory read. The unqualified `WVDB 1` experiment adds a portable typed page/B+tree reader, and implemented-candidate Decision 0210 composes it with the real hosted directory provider while preserving storage-versus-database failures. Probe 35 exercises resource semantics through an isolated guest service, and Probe 36 contains one deliberate failure of that service. | Select one product-facing read-only database consumer and specify the first pre-opened storage-object/resource contract with `u64` positions and exact lifetime, mutation, flush, fencing, and recovery semantics before implementing writes. |
...
- 🚧 Rights-limited immutable directory → at most six bounded chunks → complete `WVDB 1` validation → exact B+tree lookup with separate storage/database failures; durable format, storage authority, mutation, transactions, and service placement remain
