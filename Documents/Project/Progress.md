...
| 10. Native host tools and .NET retirement | 🚧 In progress | Exact commit `524e84afb6e5bab6bbd95ebc0b9eeaf886af834b` cross-host qualifies the Decision 0213 WVB 1.11 semantic-freeze baseline, paired standalone verifier applications, project-aware format-5 native build driver, direct no-.NET child execution, and current Windvale-owned bounded ABI-22 lowering evidence. Decision 0214 now has deterministic raw Windows/Linux publisher candidates over one shared Windvale verifier, transaction core, SHA-256, and transcript; current-host Windows direct execution proves replacement, malformed-input and alias preservation, cleanup, exact reporting, and no CLR loading. Forward language semantics advance only through `Compiler/Windvale`; C# retains its recovery and independent-evidence role. | Complete the independent Linux and dual-host fault/concurrency qualification, distribute pinned publisher artifacts and provenance, then move ordinary source-to-verified-WVB automation to the native front door. |
