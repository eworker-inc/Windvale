...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0169 package and directly run the exact ABI-22 compiler. Decisions 0185–0187 add paired standalone verifier applications and a project-aware format-5 native build driver. Decisions 0190, 0194, 0195, 0204, 0205, and 0206 transfer the bounded metered ABI-22 selector path through `i32`/`bool` control flow, loops, two functions, one direct helper edge, and up to four register scalar arguments, with current-host native-tool and malformed-input preservation evidence. Decision 0213 accepts the current WVB 1.11 state as the semantic-freeze candidate and ends forward C# frontend expansion once that exact baseline qualifies. | Qualify the freeze candidate, verifier, driver, and lowering evidence on both hosts; add a named atomic-replacement or external native publication step; then move ordinary source-to-verified-WVB automation to the native front door. |
