...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds all 16 bundles and 72 source fixtures and admits the exact `en@1` chain only through hash-pinned Project 3 inputs. Slice 2 now has vertical compiler/verifier/runtime checkpoints for named record update, WVB 1.12 fixed-width integers, WVB 1.13 exact runes, WVB 1.14 strict `f32`/`f64`, and WVB 1.15 ordinary `unit` plus return-only `never`. The focused owner includes deterministic positive compilation, source rejection, malformed-bytecode rejection, and execution returning `42`. Project 2 and descriptorless Seed retain their prior behavior; remaining aggregate/control semantics, localized token execution, paired-host evidence, and `zh-Hans@1` qualification remain open. | Continue Slice 2 with multi-field variants/destructuring and value-producing control flow over the same compiler. |
