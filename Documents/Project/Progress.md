...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration Slice 1 evidence](Windvale-Language-1.0-Migration-Evidence.md) now binds all 16 bundles and 72 source fixtures, runs 33 bounded descriptor assertions plus four profile-admission outcomes, and admits the exact `en@1` component chain only through explicit hash-pinned Project 3 lock/profile inputs. It exposes a no-copy post-descriptor view, parses the standalone Core header, and deterministically compiles, independently verifies, and executes one 221-byte edition-1 WVB. Project 2 and descriptorless Seed retain their prior behavior; localized token execution, most Language 1.0 semantics, Linux performance evidence, and `zh-Hans@1` qualification remain open. | Implement official Migration Slice 2's frozen values, binding, and control-flow subset on the same compiler architecture. |
