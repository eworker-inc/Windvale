...
| 8. Self-hosted compiler | ✅ Qualified | The committed 12-module inventory produces byte-identical 599,868-byte Stage 1 and Stage 2 compilers on Windows and Debian. Cross-host-qualified Decision 0168 packages the exact native compiler as independently verified PE/ELF executables and directly reproduces Stage 2 on both hosts without loading .NET; the Stage 0 recovery runbook records clean-checkout provenance. | Qualify the public atomic format-3 project/CLI route on both hosts. |
...
| 10. Native host tools and .NET retirement | 🚧 In progress | Cross-host-qualified Decisions 0160–0168 carry the exact ABI-22 compiler through bounded large-native transport, service manifests, RW/NX runtime state, deterministic PE/ELF containers, and direct Stage 2 reproduction on both hosts without loading .NET. Candidate Decision 0169 exposes the same pinned containers as atomically published public format-3 project/CLI targets, and the recovery runbook records their Stage 0 provenance. | Qualify the public format-3 route and clean dual-host recovery replay before changing normal Stage 0 automation. |
| 11. Boot path and kernel | 🚧 In progress | Probe 36 remains the cross-host baseline. Locally verified Probe 37 adds kernel-owned `WVENDP01`: process capabilities resolve through one generation-safe endpoint, rebind across client reuse, and close exactly once on service exit or fault. All 38 focused OS tests and five pinned Windows QEMU scenarios pass. | Qualify Probe 37 cross-host; add names, discovery, replacement, or a second endpoint only when a concrete consumer creates that pressure. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector reaches local profile 16 over execution ABI 3. The complete 722,837-byte verifier and separate 404,340-byte WVB interpreter are deterministic import-free Wasm artifacts. Node.js runs verify-then-interpret scalar/text/bytes/record/enum workloads, and a new three-artifact import-free verifier bundle admits the exact 599,868-byte, 328-function compiler through canonical, typed, and control proof. Profile 8's editable .NET-free page remains intact; profile 10 is cross-host qualified. | Add the portable in-memory compiler entry and expand interpreter frames, recursion, budgets, and dynamic-value lifetime until that admitted compiler produces canonical WVB; then package the complete static-worker pipeline. |
...
- 🚧 Exact ABI-22 compiler → measured large-native WVO → bounded link/runtime/service profiles → cross-host-qualified direct PE/ELF Stage 2 reproduction → candidate public atomic project/CLI route; public-route qualification remains
