...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds all 16 bundles and 72 source fixtures, admits the exact `en@1` chain only through hash-pinned Project 3 inputs, and deterministically compiles, verifies, and executes one 221-byte edition-1 WVB. Slice 2 has begun: appended primitive token/type identities and an edition preflight are covered by 23 front-end assertions, and a rebuilt compiler rejects Seed-only `void` without publication. Project 2 and descriptorless Seed retain their prior behavior; primitive literal/runtime execution, aggregate/control semantics, localized token execution, Linux evidence, and `zh-Hans@1` qualification remain open. | Complete Slice 2's exact primitive execution, named aggregate operations, destructuring, and value-producing control flow without widening WVB ahead of verified runtime support. |
