...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds all 16 bundles and 72 source fixtures and admits the exact `en@1` chain only through hash-pinned Project 3 inputs. Slice 2 now has vertical compiler/verifier/runtime checkpoints for storage-free `unit` return/control, named record update, WVB 1.12 fixed-width integers, and WVB 1.13 exact runes. The focused 64-case owner includes 20 rune cases; deterministic rune output is 1,148 bytes and the reconstructed runner returns `42`. Project 2 and descriptorless Seed retain their prior behavior; floating scalars, complete unit/`never`, remaining aggregate/control semantics, localized token execution, paired-host evidence, and `zh-Hans@1` qualification remain open. | Continue Slice 2 with the next frozen scalar/value boundary, preserving lowest-required WVB versions and one shared verifier/runtime path. |
