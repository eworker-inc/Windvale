...
| Development/stable installers | 🚧 | [Decision 0562](../Decisions/0562-First-Deterministic-Development-Installers.md) retains the paired exact `0.1.0-dev.1` archives. [Decision 0565](../Decisions/0565-First-Stable-Preview-Installers.md) pins separate `0.1.0` Windows/Linux archives, stable internal metadata, and one channel-neutral owner while reproducing the development bytes unchanged. | Pair the stable installer owner on both hosts, then select only the stable identities in the signed release envelope. |
...
remaining sequence is paired stable-installer evidence, project-owner root
custody, one selected-state complete qualification, reproducible envelope
