...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. [Migration evidence](Windvale-Language-1.0-Migration-Evidence.md) binds all 16 bundles and 72 source fixtures and admits the exact `en@1` chain only through hash-pinned Project 3 inputs. Slice 2 now has vertical compiler/verifier/runtime checkpoints for named record update, WVB 1.12 fixed-width integers, WVB 1.13 exact runes, WVB 1.14 strict `f32`/`f64`, and WVB 1.15 ordinary `unit` plus return-only `never`. Named zero-through-64-field variant construction and destructuring reach the compiler-aligned verifier through WVB 1.16 with deterministic positive, source-rejection, and malformed-bytecode evidence; current execution consumers remain explicitly narrower. Project 2 and descriptorless Seed retain their prior behavior; WVB 1.16 execution, value-producing control, localized token execution, paired-host evidence, and `zh-Hans@1` qualification remain open. | Complete the WVB 1.16 scalar execution boundary, then implement value-producing `if` and `match` over the same compiler. |
