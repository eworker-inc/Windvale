...
| 9. Shared native backend | 🎯 Current transfer | Cross-host-qualified Decision 0133 consumes the record maps in ABI 21 and runs the exact compiler fixture with zero record-arena use. Decision 0143 proves the full 1,852,773-allocation trace fits a concrete 16 MiB first-fit arena. Local Decision 0147 publishes and independently reconstructs 186,557 exact-compiler descriptor ownership actions without changing ABI 21 or generated bytes. | Consume the verified plan in a successor ABI with explicit owner tokens and bounded allocator operations, then independently verify the emitted mechanics. |
