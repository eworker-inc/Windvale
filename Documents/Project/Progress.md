...
| Unsafe Foundation slice | Candidate | Canonical unsafe value types and `Constructˉscratch` lower to typed WVIR and WVB 1.33 with explicit memory authority, size, alignment, ABI, effect, failure shape, complete compiler-aligned verification, and bounded scalar/native execution. [Decision 0905](../Decisions/0905-Transfer-Affine-Memory-Budgets-Through-Ordinary-Calls.md) transfers an entry or split child budget by value; [Decision 0906](../Decisions/0906-Represent-Immutable-Borrowed-Memory-Budget-Calls-In-Wvb-1.34.md) adds WVB 1.34 immutable borrowed-budget calls while preserving the caller's owner. | Add pointer and write-region borrowing, authenticated Foreign calls, a migrated real boundary, Linux reproduction, and paired-host qualification. |
