...
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale constructs live service leaves and owns image layout/lifecycle; the exact compiler produces the exact 815-byte Stage 0 WVB through Windows and Linux native W^X paths. Paired `windows-x64-console-v1` and `linux-x64-console-v1` slices package the same capability-free scalar fragment as deterministic import-free PE32+ and static-PIE ELF containers, share portable `0…255` process-result semantics, publish atomically, use WVA-authored startups, and now consume one digest-pinned portable Windvale plan for every live file/virtual placement after complete C# oracle agreement; Windows direct execution passes while fresh Linux evidence is pending. | Qualify both narrow containers cross-host, then transfer exact byte construction and untrusted-input verification into portable Windvale before adding measured hosted/runtime ownership and satisfying the larger .NET-retirement gate. |
| 11. Boot path and kernel | 🚧 In progress | Probe 32 through ABI 20 / `WVKMEM10` is cross-host qualified with the same `WVPROC11` behavior, a 134-page arena, and a 113-page client root; all four local pinned-QEMU scenarios pass. Implemented-candidate `WVRS 1` adds a deterministic three-resource image and Windvale-owned dynamic lookup without changing the guest ABI. | Define bounded request/reply IPC plus an independently lived resource-service capability before moving `WVRS 1` into the guest; let broader traps, Hyper-V, or a third runnable remain separate pressure. |
...
| WebAssembly interoperability | 🚧 In progress | The portable `.wv` selector now adds profile 9 over execution ABI 3: statically checked primitive/bytes locals, byte reads and slicing, widening, little-endian construction, concatenation, and a bounded monotonic 4 MiB arena with distinct range, value, narrowing, budget, and aggregate-allocation failures. Profile 8's strict UTF-8 transport and editable .NET-free page remain intact; profile 5 is the latest cross-host-qualified boundary. | Use the new byte-runtime foundation for a Windvale-native WVB envelope verifier, then complete measured scalar/control, text, record, and enum runtime needs; obtain cross-host profile-6–9 and cross-browser evidence. |
...
- 🚧 Three typed resources → canonical `WVRS 1` image → independent Stage 0 verification → Windvale-owned dynamic lookup; bounded guest IPC/resource-lifetime integration and cross-host qualification remain
