...
The latest Phase 10 local increment is [Decision 0466](../Decisions/0466-Native-WVHV-Request-Container-Reconstruction.md). The small `WVVE` to `WVVR` wrapper now has exact retained Windows and Linux format-3 containers, and Windvale's shared native hosted-container pipeline reconstructs both products byte-for-byte without a C# process. Its WVB and paired applications are part of the 60-artifact digest-bound candidate; the focused Windows smoke passes local-host and cross-target reconstruction plus its existing rejection and cleanup case. New C# remains limited to deletion-bound target and identity wiring. Decisions 0461 through 0465 own request evidence/projection, admitted `WVHV` metadata, the exact 4,096-byte runtime header, and retained recovery identities. The qualified ordinary verifier artifacts remain pinned while six-service bundle materialization/orchestration, verifier startup, layout/plan, platform bytes, publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next are the verifier service-bundle process and startup/layout composition, then native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
