...
The latest Phase 10 local increment is [Decision 0472](../Decisions/0472-Shared-WVHV-Startup-Targets.md). Windvale now admits the complete digest-bound verifier service bundle, joins the native-owned runtime, platform, startup, and bundle responses into exact format-4 PE/ELF applications, and directly executes the packaged constructor plus its generated verifier on Windows without loading .NET. Startup production and the forthcoming completed-application verifier now share one ordered target model instead of duplicating 45 Windows and 24 Linux relocation calculations. The hosted candidate remains 72 artifacts; no managed product writer or target was added. Decisions 0461 through 0471 own admitted `WVHV` metadata, the exact runtime header, immutable hashing, exact `WVVR` projection, retained request containers, native reconstruction, the complete six-service bundle, startup relocation, platform bytes, final composition, and direct Windows execution. New C# remains test/differential evidence plus deletion-bound recovery identity wiring. The qualified ordinary verifier artifacts remain pinned while format-4 admission into the existing durable publisher, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next is consuming the shared targets in format-4 application admission, then native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
