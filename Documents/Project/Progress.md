...
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale constructs live service leaves and owns image layout/lifecycle; the exact compiler produces the exact 815-byte Stage 0 WVB through Windows and Linux native W^X paths. Paired `windows-x64-console-v1` and `linux-x64-console-v1` slices share portable `0…255` process-result semantics, publish atomically, use WVA-authored startups, consume a portable Windvale layout, and now derive every final PE/ELF byte from fixed sparse Windvale recipes with complete C# recovery agreement; Windows direct execution passes while fresh Linux evidence is pending. | Transfer untrusted PE/ELF structural verification into portable Windvale, qualify both narrow containers cross-host, then add measured hosted/runtime ownership and satisfy the larger .NET-retirement gate. |
| 11. Boot path and kernel | 🚧 In progress | Probe 32 through ABI 20 / `WVKMEM10` is cross-host qualified with the same `WVPROC11` behavior, a 134-page arena, and a 113-page client root; all four local pinned-QEMU scenarios pass. Implemented-candidate `WVRS 1` plus one-page `WVRQ 1` / `WVRY 1` now prove strict dynamic lookup through a format-blind capacity-one lifecycle and a live hosted Windvale service without changing the guest ABI. | Version checked one-page user-buffer IPC and an immutable store capability into the guest, then prove service/client death cleanup and one configuration lookup in QEMU; keep block storage and broader traps separate. |
