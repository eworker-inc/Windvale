...
The latest implemented Phase 10 increment is [Decision 0523](../Decisions/0523-Grouped-Staging-And-Linux-Publisher-Closure.md). Staged-content admission now compares one manifest code chunk across consecutive bounded lowering values without joining a complete WVO. Linux durable publication preserves `0600` for WVB/WVO data, selects `0755` for an admitted ELF application, and keeps immutable-snapshot policy outside both complete `stat` buffers when a destination already exists. Construction candidate 20 coherently repins the affected request tools, paired host tools, publisher admission, promoter, and generic hosted-verifier/WVB/WVO/console publisher family. Focused Windows and independent Linux execution pass the exact file pipeline, object instantiation, real AOT chain, immutable segment sets, publisher rejection/preservation, digest-bound launchers, WVO read-only rejection, and WVO-inspector reconstruction. Both hosts then pass all 224 Standard Seed tests and all 39 OS tests, with exactly matching normalized conformance contracts. Decision 0522's 105-artifact/185-case paired helper and cumulative 193 managed-call removals remain unchanged. Capability-bearing managed execution, grouped repository Qualification, promotion, and the final recovery release remain explicit; T2 therefore remains `managed-normal`.
