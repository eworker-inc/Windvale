...
| Unsafe Foundation slice | Candidate | Canonical unsafe value types and `Constructˉscratch` lower to typed WVIR with explicit memory authority, size, alignment, ABI, effect, and failure shape. [Decision 0902](../Decisions/0902-Represent-Unsafe-Scratch-Construction-In-Candidate-Wvb-1.33.md) adds the exact non-executable WVB 1.33 serialization, and [Decision 0903](../Decisions/0903-Verify-Candidate-Wvb-1.33-Without-Opening-Execution.md) admits it through the complete compiler-aligned structural, semantic, typed-stack, and affine-ownership verifier. | Add bounded runtime/provider execution, exact failure and zeroing behavior, affine teardown, native containment, write-region borrowing, and authenticated Foreign calls. The operation intentionally remains non-executable today. |
...
- The evolving Language 1.0 compiler uses later versioned WVB contracts, with
  ...
  but non-executable unsafe-scratch candidate at WVB 1.33.
