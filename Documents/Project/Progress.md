...
| Offline package lifecycle | 🎯 | [Decision 0568](../Decisions/0568-Offline-Package-Lifecycle-And-Generation-Activation-1.md) selects Milestone 4 without assigning `v0.2.0`. All selected behaviors now pass locally, including the two-package stage, activation/recovery, dispatch, rollback, and recoverable uninstall that preserves application data. | Obtain the final paired Windows/Linux lifecycle and uninstall reports, then close the milestone without publishing a release. |
...
unrelated root entries. Local implementation is complete and final paired-host
evidence remains. Networking, signed revocation/minimum-version policy, OS-1,
and database breadth stay independent.
