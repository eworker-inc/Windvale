...
| Windvale Language 1.0 | 🚧 | [Decision 0767](../Decisions/0767-Freeze-Windvale-Language-1.0-Source.md) freezes the exact 250-input replacement source design. The first [migration evidence](Windvale-Language-1.0-Migration-Evidence.md) now binds all 16 bundles and 72 source fixtures, records compiler/WIR/memory/artifact baselines, and adds a bounded allocation-free descriptor reader with 34 executable boundary cases under one focused cross-host owner. Seed remains implemented; profile resolution, Language 1.0 parsing/semantics, Linux performance baseline, and `zh-Hans@1` native qualification remain open. | Connect the reader to one edition-dispatch front door, admit exact locked `en@1` data, and preserve honest unsupported failures and Seed recovery while Slice 1 advances. |
