...
| 7. Foundation modules | 🔵 Ongoing | Machine contracts, byte ordering, decimal parsing, and byte construction are shared by real Windvale tools. The reusable library inventory has native Project 2 ownership. Database Decisions 0534 through 0549 now compose checked `u64` geometry, one rights-limited mutable provider, dual superblocks, immutable pages/logs, four-action recovery-safe publication, variable-key `WVTN 1`, deterministic root-leaf split, and bounded provider-backed depth-two lookup. | Generalize depth-two mutation and child split propagation with explicit obsolete-page ownership, then define the first catalog/row codec over the typed engine rather than over raw pages. |
