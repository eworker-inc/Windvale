...
The latest Phase 10 local increment is [Decision 0490](../Decisions/0490-Indexed-Compiler-WVB-Verification.md). The shared compiler-WVB verifier now indexes typed-local shapes and control-flow boundaries per function, retains the exact 16-billion-instruction ceiling, and admits the current 1.1 MiB build driver in 8.4 seconds. The native role-2 construction pipeline reproduces the repinned Windows/Linux WVB-publisher applications byte-for-byte, and the current-host candidate publishes a canonical portable WVB without loading .NET. Independent Linux execution, native build-driver self-convergence, GitHub cutover, the grouped dual-host gate, promotion, and the final Stage 0 recovery release remain.
