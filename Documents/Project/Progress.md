...
The latest Phase 10 local increment is [Decision 0462](../Decisions/0462-Native-WVHV-Runtime-Header-Ownership.md). A verifier-specific Windvale module now consumes exact `WVHV 1` metadata and constructs the complete 4,096-byte compiler-verifier runtime header without confusing it with compiler-family `WVHB` profile 2. Its 17,941-byte native-built WVB executes identically under Windvale interpretation and native lowering, matches the frozen Windows/Linux C# runtime-header oracle byte for byte, and rejects ten malformed request families. Decision 0461's separate native `WVHV` metadata owner remains the admitted input boundary; neither new module uses the frozen C# source compiler. Decision 0460's repinned 57-artifact toolset and exact package smoke remain stable. The qualified ordinary verifier artifacts remain pinned while native request/service-bundle composition, verifier startup, layout/plan, platform bytes, publication, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion remain pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next is the native verifier request/service-bundle producer, then verifier startup/layout composition, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
