...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified ABI-21 Probe 32 retains `WVPROC11` behavior while `WVKMEM11` bounds a 141-page arena, 120-page client root, exact 24,240-byte stack path, and zero record-arena use. Implemented-candidate Probe 33 advances to `WVPROC12` and `WVCHAN02`; checked one-page RX-to-RW/NX request/reply copies carry and validate `boot:main.configuration` in both rebuilt clients, and all four Windows pinned-QEMU scenarios pass. | Give init an independently lived immutable `WVRS 1` capability, run dynamic store validation/lookup in the guest, and prove service/client death cleanup; keep paths, block storage, and broader traps separate. |
