...
The latest Phase 10 local increment is [Decision 0454](../Decisions/0454-Native-Probe-40-System-Kernel-Target.md), which replaces the frozen kernel object with a real Windvale source-to-WVB-to-WVO path and reduces the frozen seed to two objects. The retirement coordinator now owns 29 suites and 3,143 cases. Earlier transferred families remain indexed by their decisions and the native retirement-suite specification. Internal `WVL1011`, hostile unsafe-bytecode value limits, large-native segmented-object transfer, broader native host-container reconstruction, additional dynamic WVA vectors, complete Windows/Linux execution, promotion, and the grouped retirement gate remain.
...
owns 29 suites and 3,143 fixed cases without .NET. The latest seven cases verify
exact system-kernel source/WVB/WVO construction, independent admission,
destination preservation, and unsupported-valid-module rejection; the Probe 40 cases preserve
