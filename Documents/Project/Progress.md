...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 36 advances to `WVPROC15`/`WVCHAN04` and proves exact service peer-loss containment: the channel is scrubbed, one waiting client is awakened with `-1`, its resources are revoked, and shutdown continues. All 87 Seed and 38 OS tests pass on Windows and Debian; five pinned-QEMU scenarios pass on Windows. | Add endpoint discovery, restart policy, or another Windvale-owned machine-policy leaf only when a concrete next consumer requires it. |
