...
| 11. Boot path and kernel | 🚧 In progress | Cross-host-qualified Probe 34 retains `WVPROC13`, `WVCHAN03`, and `WVRES005`; the combined ABI-22 rebuild advances kernel memory to `WVKMEM13`, a 144-page arena, a 121-page client root, and 110 RX pages while preserving the six-page stack proof. | Select the next measured filesystem/provider operation without broadening the kernel's policy surface. |
...
- ✅ Three typed resources → canonical `WVRS 1` image → checked guest request/reply → independently mapped RO/NX init store → dynamic guest lookup → peer-death cleanup in cross-host-qualified Probe 34; real filesystem namespace/provider semantics remain
