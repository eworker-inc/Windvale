...
| 8. Self-hosted compiler | ✅ Qualified | The committed 12-module inventory produces byte-identical 599,868-byte Stage 1 and Stage 2 compilers on Windows and Debian; its exact native form now compiles a real fixture to byte-identical WVB in the local implementation candidate. | Cross-host qualify native execution, then require native Stage 1/Stage 2 equality. |
| 9. Shared native backend | 🎯 Current transfer | Qualified Decision 0111 admits the exact 4,556,121-byte compiler; implemented Decision 0112 measures 1,480,096 record bytes and completes execution under a bounded 2 MiB host arena without changing ABI 20. | Cross-host qualify the execution result, then resolve the separate 4 MiB WVO/object and flat-linker AOT boundaries. |
| 10. Native host tools and .NET retirement | 🚧 In progress | Windvale constructs live service leaves and owns image layout/lifecycle; the exact compiler now produces the exact 815-byte Stage 0 WVB through the local native W^X path. | Qualify native compiler reproduction and standalone Windows/Linux tools, then satisfy the documented .NET-retirement gate. |
...
| WebAssembly interoperability | 🚧 In progress | A cross-host-qualified portable `.wv` selector emits deterministic Wasm from a bounded straight-line WVB `i32` stream. The editable playground compares worker evidence with its .NET oracle, while a separate static route executes the pinned 432-byte artifact without loading .NET. | Move the remaining Stage 0 pipeline off the UI thread and add structured control flow before considering a broader target. |
...
- 🎯 Qualified compiler WVB → exact ABI-20 selection/verification → bounded W^X execution → exact 815-byte compiler output
