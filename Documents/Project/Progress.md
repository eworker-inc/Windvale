...
The latest Phase 10 local increment is [Decision 0460](../Decisions/0460-Native-Hosted-Enum-Directory-For-Variants.md). A separate Windvale hosted nominal-shape reader now admits WVB 1.11 records and variants for bounded package metadata without pretending that the ordinary native lowerer can execute variants. The Windvale request and service processes preserve records and variants as zero-member type-directory entries, including modules with no enum members, and the refreshed source-exact retained Windvale bridge passes focused process and artifact-identity checks. This closes Decision 0459's measured enum-request rejection. The qualified ordinary artifacts remain pinned because candidate-toolset repinning, complete hosted-package composition, native build-driver self-convergence, native variant execution, independent Linux execution, and promotion are still pending. Decision 0458's changed-file front door continues to stop on named gaps without invoking .NET. Next are candidate-toolset/package closure, native runner/lowerer support for the database probes, build-driver closure, non-Windows dispatch evidence, separate WebAssembly reconciliation, GitHub cutover, the grouped retirement gate, and the final Stage 0 recovery release.
