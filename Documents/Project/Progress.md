...
| Unsafe Foundation slice | Candidate | Canonical unsafe value types and `Constructˉscratch` lower to typed WVIR with explicit memory authority, size, alignment, ABI, effect, and failure shape. [Decision 0902: represent unsafe scratch construction in candidate WVB 1.33](../Decisions/0902-Represent-Unsafe-Scratch-Construction-In-Candidate-Wvb-1.33.md) adds an exact non-executable serialization with explicit budget, result, and ABI indexes. | Add complete WVB 1.33 verification, runtime/provider execution, native containment, write-region borrowing, and authenticated Foreign calls. The operation intentionally remains non-executable today. |
...
- The evolving Language 1.0 compiler uses later versioned WVB contracts, with
  the executable structured-task slice at WVB 1.32 and the non-executable
  unsafe-scratch serialization candidate at WVB 1.33.
