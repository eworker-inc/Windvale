...
This is bounded browser integration, not a general backend or replacement of the .NET playground path. It does not yet implement recursion, `break`, `continue`, reclaiming allocation, browser capability authorization/imports, general WVB nonempty stack joins, scalar formatting, text quoting, byte hashing, nominal aggregates in the interpreter, compiler self-hosting in WebAssembly, cross-browser qualification, or complete worker containment for the editable pipeline. The exact experimental contract is [`Specifications/Windvale-WebAssembly.md`](../../Specifications/Windvale-WebAssembly.md).
...
The surrounding page can use TypeScript or JavaScript, ordinary HTML, and an established browser editor. The generated Wasm subset now executes in a disposable worker, and the complete compiler-aligned WVB verifier plus the first scalar WVB interpreter execute as separate import-free Wasm modules under Node.js. Text/bytes and nominal interpreter values, source compilation, authorization, and their worker integration should follow so expensive or malicious input cannot freeze the page's user-interface thread. The Windvale program itself does not need to contain JavaScript.
