...
This is bounded browser integration, not a general backend or replacement of the .NET playground path. It does not yet implement recursion, nested control flow, `break`, `continue`, general text operations, descriptor-bearing calls/control, records, enums, reclaiming allocation, capabilities, a general WVB verifier, compiler self-hosting in WebAssembly, cross-browser qualification, or UI-thread containment for Stage 0. The exact experimental contract is [`Specifications/Windvale-WebAssembly.md`](../../Specifications/Windvale-WebAssembly.md).
