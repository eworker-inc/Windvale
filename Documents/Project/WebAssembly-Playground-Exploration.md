...
[Decision 0131](../Decisions/0131-Windvale-Native-WebAssembly-Wvb-Envelope-Verifier.md) adds local profile-10 evidence for compiler-produced nested control over the same bounded runtime values. Validated WVB basic blocks lower through a metered Wasm program-counter dispatch loop. The first real consumer is a Windvale-written WVB 1.6 envelope verifier: valid input returns `[1]`, hostile or incomplete envelopes return `[0]`, and exact budget exhaustion remains `WVR3011`. It verifies the outer envelope only; section payloads, executable semantics, and source compilation remain later gates. The static page remains on profile 8.

This is bounded browser integration, not a general backend or replacement of the .NET playground path. It does not yet implement recursion, `break`, `continue`, general text operations, descriptor-bearing calls, records, enums, reclaiming allocation, capabilities, complete WVB semantic verification, compiler self-hosting in WebAssembly, cross-browser qualification, or UI-thread containment for Stage 0. The exact experimental contract is [`Specifications/Windvale-WebAssembly.md`](../../Specifications/Windvale-WebAssembly.md).
...
2. Compile and run bounded portable programs in a Web Worker. Implemented locally through profile 7 by Decisions 0107, 0113, 0116, 0120, and 0121; profile 5 remains the latest cross-host-qualified backend boundary.
