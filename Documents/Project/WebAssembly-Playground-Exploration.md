...
This is bounded browser integration, not a general backend or replacement of the .NET playground path. It does not yet execute compiler-scale recursion, provide the portable in-memory compiler adapter, implement `break`, `continue`, reclaiming allocation, browser capability authorization/imports, general WVB nonempty stack joins, compiler self-hosting in WebAssembly, cross-browser qualification, or complete worker containment for the editable pipeline. The exact experimental contract is [`Specifications/Windvale-WebAssembly.md`](../../Specifications/Windvale-WebAssembly.md).
