...
| Bulk transaction branch partition | Focused portable Windows native implementation; cross-host qualification pending | [`WVCR 1`](../../Specifications/Windvale-Database-Transaction-Child-Replacements.md) carries one bounded tree level's old-to-new child mappings. [`WVBP 1`](../../Specifications/Windvale-Database-Transaction-Branch-Partition.md) applies every changed child to one shared parent exactly once and partitions the complete final branch with deterministic promoted separators. Bottom-up parent grouping, durable branch pages, root completion, and publication remain next. |
