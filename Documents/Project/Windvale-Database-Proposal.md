...
| Deterministic durable bootstrap | Focused native Windows implementation; cross-host qualification pending | The [database bootstrap](../../Specifications/Windvale-Database-Bootstrap.md) constructs the canonical generation-1 empty `WVDS 1`/`WVPG 1` image, publishes it through the existing four-action executor, and resumes only two byte-exact interruption states. Other nonempty storage fails closed without mutation. Storage-object creation, engine create-or-open orchestration, and server policy remain separate. |
