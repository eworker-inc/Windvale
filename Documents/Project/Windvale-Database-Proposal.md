...
| Canonical transaction mutations | Focused portable Windows native implementation; cross-host qualification pending | [`WVTM 1`](../../Specifications/Windvale-Database-Transaction-Mutations.md) describes 1 through 32 strictly sorted, duplicate-free puts/deletes in at most 256 KiB. It validates the complete request before returning owned bytes. Shared tree planning and one-batch durable publication remain the next transaction milestone. |
...
| Transaction leaf-group plan | Focused portable Windows native implementation; cross-host qualification pending | [`WVLG 1`](../../Specifications/Windvale-Database-Transaction-Leaf-Groups.md) groups sorted mutations by routed leaf, applies each leaf once, and returns explicit unchanged, rewritten, or split-required work without partial bytes on hard failure. Split handling, shared ancestor replacement, and one commit publication remain next. |
| Bulk transaction leaf partition | Focused portable Windows native implementation; cross-host qualification pending | [`WVLP 1`](../../Specifications/Windvale-Database-Transaction-Leaf-Partition.md) merges one leaf with all of its sorted mutations as a final state and emits one through 33 ordered leaves. It handles later deletes after temporary overflow and rejects an individually oversized entry without partial output. Group integration, page identities, shared ancestor replacement, and publication remain next. |
| Persistent comparison benchmark | Planned after persistent server and batch commit | Compare Windvale with the installed PostgreSQL service and SQLite using matched durable single-record, batched-write, point-read, range-read, and mixed workloads. Report throughput, latency percentiles, process/server memory, storage growth, durability settings, warmup, exact versions, and raw results; keep the existing cold-process restart comparison as a separate startup/recovery test. |
