...
| Transaction tree completion | Focused segmented Windows native implementation; cross-host qualification pending | [`WVTC 1`](../../Specifications/Windvale-Database-Transaction-Tree-Completion.md) composes changed leaves, immediate parents, every remaining ancestor, and split-root growth for input depths two through eight into at most 792 consecutive durable pages and exactly one root. It binds mutations, exact paths, and pages and replays the complete plan during validation. Commit composition is implemented; the persistent hosted writer remains next. |
| Transaction commit publication | Focused segmented Windows native implementation; cross-host qualification pending | The [transaction commit coordinator](../../Specifications/Windvale-Database-Transaction-Commit.md) validates up to 792 consecutive tree pages and unique obsolete ownership, appends one compact commit-log page, constructs the inactive superblock, and returns the exact four-action durable publication state. Logical no-ops publish nothing. The hosted persistent writer loop remains next. |
