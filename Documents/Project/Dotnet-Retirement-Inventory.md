...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer admits up to 512 scalar/control functions with arbitrary exported-Main order, fewer than 2,048 combined parameters/locals, 32,768 code bytes, and 8,192 instructions per function under the retained 2,048-cell frame check, bounded forward/self/cyclic calls, zero through 64 `i32`/`bool`/`text`/`u8`/`u32`/`bytes`, enum, or record helper parameters across ABI 22's four register positions and canonical 16-byte outgoing stack cells, checked `u32` add/subtract/multiply, complete bounded `u32`/`u8` comparisons, up to 64 immutable text/bytes/i32 declarations with borrowed descriptor locals, conversion, slicing, length, and little-endian reads, service-backed text concatenation, UTF-8 validation/conversion, quoting, and unsigned formatting, bounded generation-owned bytes concatenation plus one-byte `u8` and four-byte signed/unsigned little-endian construction, up to 64 nominal declarations with enum locals/constants/comparisons/name lookup including nonzero-first tables, and record-bearing functions with up to 256 compactly indexed declared record locals, 1,024 blocks, 8,192 instructions, and 128 record values per block. It reproduces Stage 0's exact general-call, descriptor-call, unsigned-arithmetic, large-module, large-function, large-record-planner, `Sum-Data.wv`, compiler-produced `Function-Only.wv`, `Data-And-Text.wv`, and `Nominal-Types.wv`, static-descriptor, text-service including maximum `u32` formatting, focused enum/signature, direct-record, record-call, multiple-record-call, process-input, file-read, file-write, console-output, and diagnostic-output WVO objects and composes through link/package/execute. Remaining scalar and multi-byte construction operations, other hosted calls, broader nominal shapes, required-service serialization, and the complete backend remain. |
