...
| T1 | Fixed native portable tests | `native-candidate` | The digest-bound [native retirement suite](../../Specifications/Windvale-Native-Retirement-Test-Suite.md) composes 24 focused Windows/Linux command lanes and 3,048 fixed cases without a normal .NET dependency. Its owned families include Seed behavior, twenty fixed unsafe WVB boundaries, hostile WVB/WVO admission, exact assembler golden objects, assembler/linker/lowerer/publisher rejection contracts, WVA/WVO differential corpora, source/WVB/WVO containment, version-1 and hosted format-2 PE/ELF containment and valid-shaped mutations, segmented console maximum-plus-one rejections and maximum-valid construction, exact console-packager WVB source reconstruction, hostile-size WVO consumers, and the fixed source-to-AOT chain. Each child owns exact reports plus applicable input, destination, and scratch preservation. Complete Linux execution of the current plan, remaining hostile value-limit transfer, additional WVA evidence, large-native segmented-object transfer, native host-container reconstruction, promotion, and the grouped gate remain. |
...
| T2 | Complete Seed, OS, golden, malformed, and differential suites | `managed-normal` | The C# harness owns the broad suite. A digest-bound native retirement coordinator now aggregates 3,048 transferred fixed cases, including twenty fixed unsafe WVB boundaries, three exact positive assembler products, the source-to-AOT chain, 200 hostile linker inputs, 256 hostile PE/ELF candidates, 19 canonical version-1 mutations, 15 valid/mutated hosted format-2 candidates, two segmented maximum-plus-one console rejections, two maximum-valid segmented constructions, two exact console-packager source reconstructions, four hostile-size WVO consumer cases, 256 frozen WVO differential inputs, 200 frozen WVA differential inputs, and the 2,000-case random-containment corpus independently of a live C# oracle; remaining reusable cases must move to versioned fixtures and manifests rather than being ported line for line. |
...
focused golden lane. Decisions 0430 and 0431 then add ten compact typed and
nominal WVB rejections without adding a lane. The current 24-suite coordinator
therefore owns 3,048 fixed cases. Hostile nominal/value-size limits and
additional dynamic WVA vectors remain.
