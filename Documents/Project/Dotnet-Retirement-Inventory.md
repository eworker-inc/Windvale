...
| O2 | Windvale OS probe-image construction | `native-candidate` | Portable Windvale owns exact UEFI v3 construction plus independent untrusted-byte verification. A hosted Windvale packager consumes the real digest-bound native linker's exact flat image and reported entry, matches the frozen Stage 0 writer byte for byte, preserves a destination on rejection, and rebuilds through its native Project 1 front door. Its paired 278,528-byte PE/ELF containers now reconstruct without .NET, are retained behind digest-bound launchers, and own three permanent native retirement cases. The normal recovery command now gives the real Probe 40 linked payload and entry to that retained native packager, so the managed writer is no longer executed there. Stage 0 exposes fifteen logical object components as fourteen ordered verified WVO containers and still owns their production and ordinary recovery link. The scale-safe native linker now reproduces that exact 681,913-byte link on the current Windows host. Independent Linux execution, recovery-command cutover, a UEFI-specific durable publication transaction, native Probe 40 object/scenario production, and reconstruction of all five images remain required before promotion. |
