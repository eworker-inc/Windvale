...
| T1 | Fixed native portable tests | `native-candidate` | The digest-bound [native retirement suite](../../Specifications/Windvale-Native-Retirement-Test-Suite.md) composes 24 focused Windows/Linux command lanes and 3,038 fixed cases without a normal .NET dependency. Its owned families include Seed behavior, unsafe and hostile WVB/WVO admission, exact assembler golden objects, assembler/linker/lowerer/publisher rejection contracts, WVA/WVO differential corpora, source/WVB/WVO containment, version-1 and hosted format-2 PE/ELF containment and valid-shaped mutations, segmented console maximum-plus-one rejections and maximum-valid construction, exact console-packager WVB source reconstruction, hostile-size WVO consumers, and the fixed source-to-AOT chain. Each child owns exact reports plus applicable input, destination, and scratch preservation. Complete Linux execution of the current plan, broader unsafe bytecode and additional WVA evidence, large-native segmented-object transfer, native host-container reconstruction, promotion, and the grouped gate remain. |
...
| T2 | Complete Seed, OS, golden, malformed, and differential suites | `managed-normal` | The C# harness owns the broad suite. A digest-bound native retirement coordinator now aggregates 3,038 transferred fixed cases, including three exact positive assembler products, the source-to-AOT chain, 200 hostile linker inputs, 256 hostile PE/ELF candidates, 19 canonical version-1 mutations, 15 valid/mutated hosted format-2 candidates, two segmented maximum-plus-one console rejections, two maximum-valid segmented constructions, two exact console-packager source reconstructions, four hostile-size WVO consumer cases, 256 frozen WVO differential inputs, 200 frozen WVA differential inputs, and the 2,000-case random-containment corpus independently of a live C# oracle; remaining reusable cases must move to versioned fixtures and manifests rather than being ported line for line. |
...
focused golden lane. The current 24-suite coordinator therefore owns 3,038 fixed
cases. Nominal count/value-size limits, other typed opcode families, and
