...
| T2 | Complete Seed, OS, golden, malformed, and differential suites | `managed-normal` | The C# harness owns the broad suite. A digest-bound native retirement coordinator now aggregates 3,005 transferred fixed cases, including the source-to-AOT chain, 200 hostile linker inputs, 256 hostile PE/ELF candidates, 19 canonical PE/ELF mutations, 256 frozen WVO differential inputs, 200 frozen WVA differential inputs, and the 2,000-case random-containment corpus independently of the live C# oracle; remaining reusable cases must move to versioned fixtures and manifests rather than being ported line for line. |
