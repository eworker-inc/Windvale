...
| O2 | Windvale OS probe-image construction | `native-candidate` | Portable Windvale now owns exact UEFI v3 construction plus independent untrusted-byte verification, and both Project 1 front doors rebuild through the native compiler. The focused constructor matches the frozen Stage 0 writer byte for byte and agrees across interpreter and native x64 execution. `Operating-System/Windvale.Bootstrap` still owns upstream Probe 40 object production and orchestration through the explicit recovery command; native composition and reconstruction of all five scenario images remain required before promotion. |
