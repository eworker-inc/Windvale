...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer handles one through eight scalar/control functions with arbitrary exported-Main order, bounded forward/self/cyclic calls, and one immutable `[i32]` declaration. It reproduces Stage 0's exact general-call and canonical `Sum-Data.wv` WVO objects and composes through link/package/execute. More functions, multiple/text/bytes data, nominal and descriptor values, capabilities, and the complete backend remain. |
