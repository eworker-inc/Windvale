...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer handles one through eight decreasing-ordinal scalar/control functions plus one bounded immutable `[i32]` declaration. It reproduces Stage 0's exact canonical `Sum-Data.wv` `.text`/`.rodata`/relocation WVO and composes through link/package/execute. Multiple/text/bytes data, nominal and descriptor values, capabilities, and the complete backend remain. |
