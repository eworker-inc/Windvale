...
| T1 | Fixed native portable tests | `native-candidate` | The digest-bound [native retirement suite](../../Specifications/Windvale-Native-Retirement-Test-Suite.md) composes 30 focused Windows/Linux command lanes and 3,145 fixed cases without a normal .NET dependency. Its owned families include Seed behavior, twenty fixed unsafe WVB boundaries, hostile WVB/WVO admission, four exact assembler golden objects, a 269-case WVA differential/positive corpus, assembler/linker/lowerer/publisher rejection contracts, the four-case verified WVO export-renaming contract, the nine-case OS Probe object-producer contract, the seven-case system-kernel target contract, the two-case process-policy source-object contract, WVO differential corpora, source/WVB/WVO containment, version-1 and hosted format-2 PE/ELF containment and valid-shaped mutations, segmented console maximum-plus-one rejections and maximum-valid construction, exact console-packager WVB source reconstruction, hostile-size WVO consumers, native-link-to-UEFI construction/repetition/rejection, the two-case native Probe 40 build lane, and the fixed source-to-AOT chain. Each child owns exact reports plus applicable input, destination, and scratch preservation. Complete Linux execution of the current plan, remaining hostile value-limit transfer, additional WVA evidence, large-native segmented-object transfer, broader native host-container reconstruction, promotion, and the grouped gate remain. |
...
| T2 | Complete Seed, OS, golden, malformed, and differential suites | `managed-normal` | The C# harness owns the broad suite. A digest-bound native retirement coordinator now aggregates 3,145 transferred fixed cases, including twenty fixed unsafe WVB boundaries, four exact positive assembler products, the four WVO export-renamer cases, nine OS Probe object-producer cases, seven system-kernel target cases, two process-policy source-object cases, the source-to-AOT chain, 200 hostile linker inputs, 256 hostile PE/ELF candidates, 19 canonical version-1 mutations, 15 valid/mutated hosted format-2 candidates, two segmented maximum-plus-one console rejections, two maximum-valid segmented constructions, two exact console-packager source reconstructions, four hostile-size WVO consumer cases, three native-link-to-UEFI cases, two native Probe 40 construction/preservation cases, 256 frozen WVO differential inputs, 200 frozen WVA mutations plus 69 positive register/control/relocation vectors, and the 2,000-case random-containment corpus independently of a live C# oracle; remaining reusable cases must move to versioned fixtures and manifests rather than being ported line for line. |
...
| O2 | Windvale OS probe-image construction | `native-candidate` | Portable Windvale owns exact UEFI v3 construction plus independent untrusted-byte verification. A hosted Windvale packager consumes the real digest-bound native linker's exact flat image and reported entry, matches the frozen Stage 0 writer byte for byte, preserves a destination on rejection, and rebuilds through its native Project 1 front door. Its paired 278,528-byte PE/ELF containers now reconstruct without .NET. A manifest-bound normal-scenario seed initially froze eleven Stage 0-produced WVOs from Decision 0444. Decisions 0446 through 0455 now construct the canonical native-probe, admission, x64 exception, paging, WVB admission-bridge, native bridge/support, normal memory, normal loader, system-kernel, and process-policy objects in the ordinary Windvale-native build, using verified native export renames, shared verified WVO construction for exact architecture recipes, a real source-to-WVB-to-WVO kernel path, and the general accepted-subset lowerer for portable policy source. The Windows/Linux launchers validate one frozen WVO, construct ten objects through native producers, assemble three top-level objects, link fourteen inputs, and package the exact EFI without invoking `.NET`. The current-host nine-case producer, seven-case kernel-target, two-case policy, and two-case image lanes pass exact construction, admission, malformed-input rejection, and existing-output preservation. Stage 0 remains the explicit regeneration/differential path, and only its managed process-object construction still requires transfer. Linux execution, native replacement of the final frozen producer, a UEFI-specific durable publication transaction, the other four scenarios, and final seed qualification remain required before promotion. |
...
current 30-suite coordinator therefore owns 3,145 fixed cases after Decision
0455's two process-policy source-object cases. Hostile
