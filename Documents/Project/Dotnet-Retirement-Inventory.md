...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer handles one through eight scalar/control functions with arbitrary exported-Main order, bounded forward/self/cyclic calls, `i32`/`bool`/`u8`/`u32` helper shapes, complete bounded `u32`/`u8` comparisons, and one immutable `[i32]` declaration. It reproduces Stage 0's exact general-call, `Sum-Data.wv`, and compiler-produced `Function-Only.wv` WVO objects and composes through link/package/execute. More functions, remaining scalar operations, multiple/text/bytes data, nominal and descriptor values, capabilities, and the complete backend remain. |
