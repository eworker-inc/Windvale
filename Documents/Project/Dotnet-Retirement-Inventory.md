...
> Inventory snapshot: 12 August 2026

This is the operational ledger for .NET's retirement from Windvale's normal Windows and Linux workflows under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md), [Decision 0526](../Decisions/0526-Dotnet-Retirement-Qualification-And-Stage0-Archive.md), and the [native-execution architecture](../Architecture/Native-Execution-And-Dotnet-Retirement.md). It records direct managed entry points, the replacement standing of each product surface, the accepted retirement subset, and the retained recovery owners.
...
The accepted dependency-retirement subset is complete. Rows that remain
`native-candidate` below track future feature or product breadth beyond that
subset; they do not represent a normal .NET dependency or reopen the eight-part
gate.
...
| E1 | Accepted-subset WVB execution | `native-qualified` | The source-reconstructed paired profile-5 runner and the fixed execution owners pass on Windows and Linux for the accepted capability-free WVB subset, including exact results, traps, diagnostics, and instruction budgets. Capability-bearing execution, the 4 MiB Byte Construction value shape, and per-function profiling remain future breadth rather than retirement blockers. |
| T1 | Fixed native portable tests | `native-qualified` | The digest-bound native retirement suite composes 45 focused owners and 3,206 fixed cases. Exact commit `e5a1a7473c57935c5dfcf09b78b18c3c099e70ef` passed all suites and cases independently on Windows and Debian in GitHub run 31608597009. Changed-file verification selects these owners without a managed or unfiltered fallback. |
...
| I2 | Accepted-subset WVO verification and inspection | `native-qualified` | Shared Windvale admission protects the read-only inspector, publisher, linker, and execution boundaries. Both digest-bound host applications passed the fixed rejection, mutation, containment, reconstruction, and positive-product owners in the final dual-host matrix. Broader future object-format features require their own admitted extensions. |
| L1 | Accepted standard WVO linking | `native-qualified` | The reconstructed standard flat linker, segmented construction toolset, digest-bound launchers, rejection/hostile/map-limit owners, and real fourteen-input Probe image passed on Windows and Debian. Previous-seed renewal and additional linker models remain future bootstrap or feature work. |
| P1 | Accepted version-1 and hosted verifier PE/ELF packaging | `native-qualified` | Windvale-owned construction, admission, durable publication, mutation/hostile-size coverage, maximum-valid segmented construction, publisher reconstruction, and exact Windows/Linux products all passed the final native matrix. New container profiles or platform features must extend this qualified boundary explicitly. |
| P2 | Accepted hosted format-2 WVB packaging | `native-qualified` | The digest-bound native compiler-family toolset constructs, admits, preserves, and publishes the accepted Windows/Linux hosted containers without .NET. Exact reconstruction, cross-target output, malformed-input preservation, and cleanup owners passed on both hosts. |
| N1 | Accepted-subset WVB to WVO lowering | `native-qualified` | The current Windvale lowerer and durable WVO publisher cover the bounded scalar, text, bytes, nominal, record, capability-call, compiler-scale, and segmented-publication subset exercised by the accepted repository products. Exact products, rejections, reconstruction, containment, link/package composition, and both host applications passed the final matrix. Additional operations and nominal shapes are forward language/backend breadth. |
| N2 | Accepted-subset native backend and baseline JIT | `native-qualified` | Deterministic WVO/AOT construction and the real source-to-AOT chain pass on both hosts. The six-case typed patch-plan and W^X publication owner qualifies the representative baseline-JIT path, including results `42` and `-1`, forced seal failure, and teardown. General optimization, cache policy, and broader JIT lowering remain later work. |
| T2 | Accepted retirement conformance and differential gate | `native-qualified` | The managed broad harness is retained only for recovery/differential questions. The ordinary gate is the 45-suite, 3,206-case native coordinator plus separate WebAssembly and bootstrap owners; all six Windows/Linux jobs passed at the exact release commit. Future semantic breadth must add focused native cases and does not silently fall back to the managed suite. |
...
| O2 | Accepted Windvale OS probe-image construction | `native-qualified` | Native Windows/Linux owners construct all eleven objects, assemble the WVA objects, link fourteen inputs, and package the exact normal, invalid-opcode, and general-protection EFI images. Producer, kernel-target, policy, process-object, image, rejection, and preservation owners passed the final matrix. Additional contained process-fault scenarios and a stronger UEFI publication transaction remain future OS breadth. |
| W1 | Complete accepted WebAssembly generation and verification | `native-qualified` | The paired native owner builds the source corpus and compiler WVBs, invokes the digest-bound native WVB-to-Wasm backend, and runs the strict Node.js engine and probe evidence without .NET. Both Windows and Linux jobs passed in the final six-job matrix. Cross-browser measurement and package-size work remain product improvements. |
| G1 | Independent dual-host qualification | `native-qualified` | Decision 0525's six parallel native owners passed at exact commit `e5a1a7473c57935c5dfcf09b78b18c3c099e70ef`; the required Verification gate succeeded only after both retirement suites, both WebAssembly owners, and both compiler-convergence owners passed. |
...
| C1 | Clean bootstrap from documented native seeds | `native-qualified` | Digest-bound native compiler seeds and provenance inventories reproduce byte-identical Stage 1 and Stage 2 compilers on Windows and Debian. Both exact bootstrap jobs passed at the release commit, and the final recovery archive independently reconstructs the historical seed before reaching the current fixed point. |
| C2 | Final digest-bound Stage 0 recovery archive | `native-qualified` | The immutable 13-asset release `stage0-recovery-e5a1a7473c57` contains complete history, source/artifact/dependency/license inventories, runbook, reports, and checksums. One selected bundle reconstructed on Windows and Linux and an independently held E-Worker copy matches all published assets. |

### Historical transfer record

The following Decision 0496 through 0525 notes preserve the standing observed
at each incremental transfer. Their then-current `managed-normal`, candidate,
pending, and open statements are superseded by the completion dashboard,
promoted ledger, and Decision 0526 above; they remain here to explain how the
broad managed work was decomposed into focused native owners.
...
compiler probes. The complete independent Linux verifier passes in 1,371.8
seconds with empty stderr and the exact final 183-byte compiler result. The
command remains the direct standalone owner rather than being added to the
fixed coordinator because it is long-running. Changed-file planning now
dispatches it once after cheaper fixed suites; all five former
`webassembly-native-verification` mappings are closed without a managed or
unfiltered fallback. This removes one normal direct managed entry point without
changing W1's `native-candidate` standing. Package reconstruction,
cross-browser evidence, grouped qualification, promotion, and recovery
retirement remain.

Current native front-door owner update: the fixed coordinator now contains 45
suites and 3,206 cases. The `seed-native-front-door` lane directly owns the
paired exact-report audit scripts. All paths formerly classified with the
`seed-native-front-door` evidence gap now select that focused native lane, so
changed-file verification needs neither a managed fallback nor an unfiltered
native gate for this boundary.
...
Current T2 console-AOT owner closure:
...
native front doors. The standalone focused owner constructs the exact input
WVB through the qualified native Project 1 front door, then passes the same
lower, admission, link, paired-package, identity, and current-host result
contract on Windows and Debian. This removes two managed invocations from each
host script, eleven cumulatively with Decision 0505, and closes all three
`seed-native-console-aot` planner gaps without changing the three normal plus
nine recovery direct-entry inventory. T2 remains `managed-normal` because the
broad scripts still use .NET for general execution, the harness, and later
qualification phases. Remaining broad-suite transfers, GitHub cutover, grouped
...
website, and deployment routes are already .NET-free. Decision 0458 removes the
indirect normal call from `Verify-Changed.ps1`; Decision 0504 removes the
...
The companion JSON records nine operational files, all in the explicit recovery
lane: paired native compiler-seed reconstruction, paired managed bootstrap
comparison, paired Seed recovery/differential verification, two WebAssembly
package reconstructors, and the OS Probe Stage 0 reconstructor. No normal or
release-gating workflow invokes them.
...
| Owner | Current retained responsibility | Retention boundary |
| --- | --- | --- |
| `Compiler/Reference` | Frozen source compiler oracle and recovery compiler | Archived and retained live only for explicit Stage 0 recovery, security, or differential work; forward semantics are prohibited. |
| `Runtime/Windvale.Bytecode` and `Runtime/Windvale.Runtime` | Frozen decoding, verification, and interpreter oracle | Recovery/differential-only. Removal requires a later explicit archival decision; it is not required for dependency retirement. |
| `Compiler/Native` and `Runtime/Windvale.Native` | Frozen Stage 0 native lowering, ABI, publication, and host-execution provenance | Recovery/differential-only and covered by the final archive. |
| `Assembler/Reference`, `Object-Model/Windvale.ObjectModel`, and `Linker/Reference` | Independent object/assembly/link oracles and historical managed CLIs | Recovery/differential-only; ordinary routes are native-qualified. |
| `Tools/Windvale.Project` and `Tools/Windvale.Tool` | Historical managed orchestration and project parsing | Recovery-only; ordinary project and tool launchers are native-qualified. |
| `Tests/Windvale.Seed.Tests` and `Tests/Windvale.Os.Tests` | Broad historical conformance, differential, malformed-input, OS, and artifact evidence | Optional named comparison/recovery evidence; not an ordinary or required release gate. |
| `Operating-System/Windvale.Bootstrap` | Host-side Stage 0 Probe image construction oracle | Retained only behind `Tools/Recovery/Rebuild-Os-Probe.ps1`; ordinary accepted image construction is native-qualified. |
| `Tools/Windvale.Playground` and `Tools/Windvale.Playground.Engine` | The static files are the normal browser product; the managed host and engine are historical comparison/recovery evidence | Keep the static application; the managed projects remain recovery/differential-only unless a later archival decision removes the live copies. |

Deleting these owners would weaken recovery or independent evidence. Every normal responsibility in the accepted subset now has a qualified native owner, and the managed projects are behind explicit recovery commands. Their frozen source remains preserved by the final Stage 0 release; retirement does not require erasing bootstrap history.

## Post-retirement verification rhythm

For each coherent post-retirement slice, run `Tools/Verify/Verify-Changed.ps1`
