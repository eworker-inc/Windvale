...
| T1 | Fixed native portable tests | `native-candidate` | `Tools/Native/Test-Seed.cmd` and `.sh` run an eight-case digest-bound result/failure plan covering scalar/control, static text/bytes, records, enums, and three exact runtime failures. Broader native fixture orchestration and malformed-module coverage remain. |
...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer handles one through eight decreasing-ordinal scalar/control functions, proves the existing three-function calls fixture, and composes through link/package/execute. It is not the complete backend. |
