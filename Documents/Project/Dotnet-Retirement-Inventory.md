...
adds an exact profile-5 WVHV construction path for the retained 90,009-byte
runner WVB, its 761,854-byte WVO, and paired 778,240-byte applications. The
normal digest-bound launchers now consume that current candidate, and its
focused Windows owner passes 3/3. This advances the fixed coordinator from 42
suites and 3,201 cases to 43 suites and 3,204 cases without changing the direct
managed-entry inventory. Source-to-WVB bindings, independent Linux execution,
grouped qualification, promotion, and recovery deletion remain.
