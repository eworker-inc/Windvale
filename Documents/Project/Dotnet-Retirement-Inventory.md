...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer handles one through eight scalar/control functions with arbitrary exported-Main order, bounded forward/self/cyclic calls, `i32`/`bool`/`u8`/`u32` helper shapes, complete bounded `u32`/`u8` comparisons, up to eight immutable text/bytes/i32 declarations with borrowed descriptor locals, conversion, slicing, length, and little-endian reads, service-backed text concatenation, UTF-8 validation/conversion and quoting, plus bounded generation-owned bytes concatenation. It reproduces Stage 0's exact general-call, `Sum-Data.wv`, compiler-produced `Function-Only.wv` and `Data-And-Text.wv`, static-descriptor, and text-service WVO objects and composes through link/package/execute. More functions, remaining scalar and byte-builder operations, descriptor calls/returns, nominal values, capabilities, and the complete backend remain. |
