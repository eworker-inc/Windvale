...
1,101,068-byte WVB in one current-host run. Linux execution, candidate-toolset
reconstruction, dual-host qualification, and front-door promotion remain open.
...
route remains pending current candidate-toolset reconstruction. Decision 0491
separately proves that the current Windows-hosted native build driver reproduces
its own byte-identical 1,101,068-byte WVB. Linux execution, dual-host promotion,
native reconstruction of the seed PE/ELF and current candidate toolset, remaining
accepted-tool reconstruction, and a later release's use of this candidate as its
previous seed remain before C1 is complete.
