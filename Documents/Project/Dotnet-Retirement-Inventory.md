...
| T1 | Fixed native portable tests | `native-candidate` | `Tools/Native/Test-Seed.cmd` and `.sh` run a five-case digest-bound plan covering scalar/control, static text/bytes, records, and enums. Broader native fixture orchestration and malformed-input coverage remain. |
