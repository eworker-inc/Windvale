...
| T1 | Fixed native portable tests | `native-candidate` | `Tools/Native/Test-Seed.cmd` and `.sh` run a thirteen-case digest-bound plan covering scalar/control, static text/bytes, records, enums, three exact runtime failures, and five malformed WVB envelope rejections. Typed/control corruptions, unsafe bytecode, and broader native fixture orchestration remain. |
