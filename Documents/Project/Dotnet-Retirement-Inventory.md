...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer admits up to 512 scalar/control functions with arbitrary exported-Main order, bounded forward/self/cyclic calls, `i32`/`bool`/`u8`/`u32` helper shapes, complete bounded `u32`/`u8` comparisons, up to 64 immutable text/bytes/i32 declarations with borrowed descriptor locals, conversion, slicing, length, and little-endian reads, service-backed text concatenation, UTF-8 validation/conversion and quoting, bounded generation-owned bytes concatenation, up to 64 nominal declarations with enum locals/constants/comparisons/name lookup including nonzero-first tables, multi-block direct records and calls over deterministic frame storage, exact hosted capability-table admission, and all six hosted calls declared by the real lowerer. It reproduces Stage 0's exact general-call, large-envelope, `Sum-Data.wv`, compiler-produced `Function-Only.wv`, `Data-And-Text.wv`, and `Nominal-Types.wv`, static-descriptor, text-service, focused enum, direct-record, record-call, process-input, file-read, file-write, console-output, and diagnostic-output WVO objects and composes through link/package/execute. Modules beyond the current envelope, remaining scalar and byte-builder operations, descriptor calls/returns, other hosted calls, broader nominal shapes, required-service serialization, and the complete backend remain. |
