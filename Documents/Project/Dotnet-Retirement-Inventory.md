...
| T1 | Fixed native portable tests | `native-candidate` | `Tools/Native/Test-Seed.cmd` and `.sh` run a twenty-six-case digest-bound plan covering scalar/control, static text/bytes, records, enums, three exact runtime failures, five malformed WVB envelope rejections, eight typed-execution corruptions, one control-reachability corruption, one canonical WVO acceptance, and three structural WVO rejections. `Test-Linker-Rejections.cmd` and `.sh` separately require exact reports for every externally driven `WVL1001` through `WVL1010` family plus preservation of an existing output without rebuilding the AOT chain; `Test-Linker-Hostile-Inputs.cmd` and `.sh` add 200 immutable zero-through-511-byte inputs with exact `WVL1002`, input, and destination preservation; `Test-Linker-Map-Limit.cmd` and `.sh` add the separate exact `WVL1012` boundary over 16,384 definitions from one compact generated-fixture archive, while internal `WVL1011` retains reconstruction/corruption evidence. `Test-Console-Packager-Rejections.cmd` and `.sh` separately require exact entry-at-end, non-decimal-entry, and empty-image reports plus preservation of an existing destination without building or linking. `Test-Console-Container-Hostile-Inputs.cmd` and `.sh` add 128 immutable PE and 128 immutable ELF candidates with exact native publisher rejection, input/destination preservation, and zero publication scratch; `Test-Console-Container-Mutations.cmd` and `.sh` separately freeze 10 canonical PE and 9 canonical ELF truncation/structural/trailing cases with detailed Stage 0 code provenance and the same preservation contract. `Test-Publisher-Rejections.cmd` and `.sh` separately require exact console-application and WVO admission reports, destination preservation, and zero publication scratch. `Test-Lowerer-Rejections.cmd` and `.sh` separately require exact malformed-WVB and valid-but-unsupported-function reports, destination preservation, and zero private lowerer work. `Test-Assembler-Rejections.cmd` and `.sh` separately require one exact output-preserving rejection for every stable `WVA1001` through `WVA1011` diagnostic family without rebuilding the qualified assembler; `Test-Wva-Differential.cmd` and `.sh` freeze 200 seeded mutations with one exact accepted WVO plus 199 Stage 0 rejection-code matches and complete input/destination preservation. `Test-Wvo-Read-Only-Rejections.cmd` and `.sh` separately require both digest-bound read-only launchers to agree on the exact report for every stable WVO status family while preserving the input; `Test-Wvo-Differential.cmd` and `.sh` add frozen Stage 0 acceptance for 128 valid-shaped mutations and 128 arbitrary values, including exact success reports for all 32 accepted mutations; `Test-Wvo-Hostile-Size.cmd` and `.sh` require verify, inspect, link, and publish to contain the exact 4,194,305-byte input at the bounded native file-snapshot boundary while preserving every applicable file. `Test-Source-Containment`, `Test-Wvb-Containment`, and `Test-Wvo-Containment` freeze the exact continued legacy random sequence as 500 source, 1,000 WVB, and 500 WVO cases; the import-free compiler adapter and digest-bound native tools require bounded rejection plus complete input and assembler-destination preservation without a live managed oracle. `Test-Aot-Chain.cmd` and `.sh` additionally build, lower, verify, link, package, and execute the fixed return-42 program through pinned native tools while checking every product identity. `Test-Retirement-Suite.cmd` and `.sh` now compose all nineteen lanes and 3,009 cases through one digest-bound direct coordinator with exact per-lane filters. Complete Windows/Linux execution, broader unsafe bytecode, segmented console-size rejection, hosted PE/ELF mutations, large-native segmented-object transfer, and other extended WVA vectors remain. |
...
| T2 | Complete Seed, OS, golden, malformed, and differential suites | `managed-normal` | The C# harness owns the broad suite. A digest-bound native retirement coordinator now aggregates 3,009 transferred fixed cases, including the source-to-AOT chain, 200 hostile linker inputs, 256 hostile PE/ELF candidates, 19 canonical PE/ELF mutations, four hostile-size WVO consumer cases, 256 frozen WVO differential inputs, 200 frozen WVA differential inputs, and the 2,000-case random-containment corpus independently of the live C# oracle; remaining reusable cases must move to versioned fixtures and manifests rather than being ported line for line. |
