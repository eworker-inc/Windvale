...
| N1 | Accepted-subset WVB to WVO lowering | `native-candidate` | The current Windvale lowerer handles one through eight scalar/control functions with arbitrary exported-Main order, bounded forward/self/cyclic calls, `i32`/`bool`/`u8`/`u32` helper shapes, complete bounded `u32`/`u8` comparisons, and up to eight immutable text/bytes/i32 declarations with borrowed descriptor locals, conversion, slicing, length, and little-endian reads. It reproduces Stage 0's exact general-call, `Sum-Data.wv`, compiler-produced `Function-Only.wv`, and static-descriptor WVO objects and composes through link/package/execute. More functions, remaining scalar operations, dynamic descriptor ownership, descriptor calls/returns, nominal values, capabilities, and the complete backend remain. |
