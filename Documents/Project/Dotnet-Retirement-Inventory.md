...
their private bridge functions no longer inflate the product candidate. The
baseline-JIT producer bridge now exposes a smaller concrete subset gap: its WVB
rebuilds natively, but descriptor-returning `Main() -> bytes` is rejected by the
current lowerer, so its verified WVO still has Stage 0 recovery provenance.
That return path, Linux execution, native host-package construction, remaining
backend subset work, promotion, and grouped qualification remain open.
