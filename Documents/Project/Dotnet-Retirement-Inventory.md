...
executes inside the Windows publisher without loading .NET. The pinned
native lowerer applications predate the current 409-function source closure, so
native self-reconstruction and tool promotion, Linux execution, native
host-package construction, remaining backend subset work, and grouped
...
5,792,768-byte Windows lowerer application. That process emits both the
descriptor-entry and retained baseline-JIT bridge WVOs byte for byte. Genuine
