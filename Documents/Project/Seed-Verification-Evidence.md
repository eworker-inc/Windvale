...
| Reproducible compiler bootstrap | The last qualified thirteen-module inventory is rebuilt by the versioned native seed into a verified 921,900-byte Stage 1 compiler. On Windows, the segmented native pipeline packages and executes that newly built compiler over the same inventory, independently verifies the 921,900-byte Stage 2 result, and requires complete byte equality at SHA-256 `fd96bd567d08a18107a9b149560ce9f2e38b49454250e934a4375f465d132556`. The matching Linux execution remains pending for the grouped qualification. The older managed proof remains frozen recovery/differential evidence. |
...
## Local native byte-from-i32-little evidence

[Decision 0267](../Decisions/0267-Native-Byte-From-I32-Little.md) admits
`bytes.from_i32_little` through the existing bounded dynamic-byte arena. Typed
analysis requires one `i32` operand and produces one bytes descriptor; lowering
checks four-byte arena growth, publishes the exact descriptor, stores all source
bits in little-endian order, and preserves the existing arena-exhaustion and
lifetime behavior.

The affected tests were reviewed before execution. The existing selectable
byte-construction fixture now also constructs `-7`, requires four bytes, reads
the exact signed little-endian value back, and retains the earlier maximum-`u8`
case. The final focused differential selection passes in 2.893 seconds and
compares the complete Stage 0 WVO with both Windvale adapters. Its 655-byte WVB
has SHA-256
`a7d05bf1057ba47defac824650c26342921a0179fe095645c825755d489fb877`;
the exact 4,884-byte WVO contains 4,811 code bytes and has SHA-256
`cd6ec0d5f132d90c44278c46c2080a9325ad3e29c25303d0e0647f7397871208`.
The separate pinned-package case passes in 8.451 seconds. Both Release builds
report zero warnings and errors.

Direct Stage 0 calculation pins the 333,205-byte core closure at SHA-256
`f0d93cc849e3f3246e695872cd61b91f9c8d8ee58285b5f2b9b2e92a04a786d2`.
The pinned Windows native source front door independently rebuilds the
328,294-byte memory adapter and 329,322-byte hosted tool in 32.2 seconds; both
are byte-identical to Stage 0 at SHA-256
`bb69b780a71d2510944edc38ec30aa828b3e4a9cd2dd242215c621114315f406`
and `2b0679507a01a5dd0c290f2c873c8360b52bf6b1699c4afb0b93cad3d25a267f`.
Current unpromoted packages are 4,554,240 Windows and 4,554,752 Linux bytes at
SHA-256
`b0e1e17198494397b646adcac37ce815622e6e194cb9cc0a3dcd322fc75bcc97`
...
## Local native byte-from-u16-little evidence

[Decision 0271](../Decisions/0271-Native-Byte-From-U16-Little.md) admits
`bytes.from_u16_little` through the existing ABI-22 dynamic-byte owner. Typed
analysis consumes a `u32` and produces an owned two-byte descriptor. Lowering
rejects values above 65,535 before allocation through the exact `WVR3016`
branch, publishes length two and reserved zero, and stores the low word without
a runtime service.

The affected test was reviewed before execution. The existing Stage 0 coverage
already owns explicit rejection above 65,535, while the focused differential
fixture now uses the maximum accepted input, checks its exact two-byte
round-trip, and compares the complete object containing that rejection branch.
The selection passes in 4.767 seconds. Its 1,107-byte WVB has SHA-256
`aa3736cee76c6aaf7e19e7eb36c715ce1614d8b007db03da7229b253784f305f`;
the exact 8,657-byte WVO contains 8,584 code bytes and has SHA-256
`973a428de734f5414c33c1f5f91dd3b3943110ea747eb7565b34f269382802f4`.
The separate pinned-package case passes in 10.082 seconds. Both Release builds
report zero warnings and errors.

Direct Stage 0 calculation pins the 336,925-byte core closure at SHA-256
`d9cbc92ab06f4e67ced19f85fd35e1a16d0a43a39a14ec9ac0dc59f4133d2a5d`.
The pinned Windows native source front door independently rebuilds the
331,926-byte memory adapter and 332,954-byte hosted tool in 33.0 seconds; both
are byte-identical to Stage 0 at SHA-256
`5a388b1684b6f8eb6c4d47ee95ad0d75c0bc3015f5b9093b10032ae70db71de3`
and `5b79bbd499c65a02a4d59dac28c4bfb897aeda66f95825f06c57aedb4c047bfd`.
Current unpromoted packages are 4,588,032 Windows and 4,587,520 Linux bytes at
SHA-256
`a2d1d452179e1305b365724d8b30c829e2a3b948d9651ce81f253df560dfbf8d`
and `a1857c475f336820962e2f064c98e540663c6feb5a2e6e8df37eef09881561ba`.

One direct self-lowering probe remains fail-closed as `Unsupportedˉcode` and
publishes no output. A complete ordinal scan of the first 1,917 instructions
identifies the next unsupported instruction as `u32.from_u8` in function 29,
`__WvM12F7(bytes, u32) -> u32`, at function offset `0x005D`.
...
## Local fixed native WVA positive-matrix evidence

[Decision 0433](../Decisions/0433-Fixed-Native-Wva-Positive-Matrix.md)
freezes sixteen paired 8/16-bit register sources and the typed narrow
immediate/shift source from the managed positive assembler assertion. Their 17
LF-terminated inputs total 4,123 bytes and their exact Stage 0 WVOs total 1,707
bytes. One deterministic compact archive avoids both loose fixtures and one
...
The decoded 3,576-byte archive has SHA-256
`ebb9e8e4ae5d90ace39f828996ebab9b75fc66d78c62ac7c58e86cf05ba9ba00`,
and its 4,769-byte LF-only base64 representation has SHA-256
`a2e6a55419d7b4aaa3d1dbb6f7101e3a02aefb27f7d1d7309280e3b73877970b`.

The focused managed assertion passed 1/1 in 527 ms while establishing source
and WVO provenance. Its temporary exporter was removed, the authoritative
`Program.cs` remained unchanged, and the staging directory was removed after
independent archive comparison.

The reviewed retirement plan remains 2,054 LF-only bytes and now has SHA-256
`521488bb63e001cccc673db3e41c6718b20313a11b28a9e9421d735c6b992f56`,
covering 24 suites and 3,066 fixed cases. The reviewed Windows command
...
## Local native Probe 40 native-bridge/support producer evidence

[Decision 0450](../Decisions/0450-Native-Probe-40-Native-Bridge-And-Support-Producer.md)
moves `13-native-bridge-and-support.wvo` from the frozen seed and adds its exact
two-section recipe to the selector-bound OS Probe object producer. The 40,025-byte
Project 1 WVB has SHA-256
`889da93916e6387e35d11502c432ee5a661b5eefdc9772a0692235e846bfccfe`.
Its retained Windows application is 434,688 bytes at SHA-256
`8e4c93a79584873b4cbf3884837f33f94e2909d0a8b99745739fb17dfed283cf`;
the Linux application is 434,176 bytes at SHA-256
`4ae292f582b309b0ce0fad9ccf90a5f115cf46f5f3ff489f587d313cb97231c5`.

Current-host execution reproduces all three exact former seed objects. The new
native bridge/support object is 461 bytes at SHA-256
`472a0fbe6497525e634a4785e92aa9ee62c3c7d70fff7510e45acbea644eea0b`
and passes independent native WVO admission.

After affected-test review, the combined Windows producer lane passes 6/6. The
ordinary `os-probe` lane then passes construction and existing-output
preservation 2/2 in 11.9 seconds. The final EFI remains 683,008 bytes at
SHA-256
`080b4d669e9a11fdc802bf7197ae5a044978b6ba39741b2b1c832296987f74d9`.

The retirement plan remains 2,331 LF-only bytes and now has SHA-256
`f272a2dd892df2ea0324e7920768cd51d85dccfa12f3f8321b52c0253c313420`.
It owns 28 suites with 3,133 fixed cases. The seed retains six frozen WVOs
...
## Local native Probe 40 memory-object producer evidence

[Decision 0452](../Decisions/0452-Native-Probe-40-Memory-Object-Producer.md)
moves normal `08-memory.wvo` from the frozen seed into a separate focused
Windvale producer behind the unified object launcher. The 37,769-byte Project 1
WVB has SHA-256
`2ae5f3a2f108b74a86150854c78f7f2dc0335cff2cb1e071be7718fce40e17e7`.
Its retained Windows application is 399,872 bytes at SHA-256
`79461480b72cc1865278ea6f06170b8f4e9f4e849898d7b3c06aa3d36ff70032`;
the Linux application is 401,408 bytes at SHA-256
`02280b115ead806f8b6e2f1dd066d7d06a85ae571d790c66d05daecf2acc6554`.

Current-host execution reproduces the former 1,529-byte seed object byte for
byte at SHA-256
`2668e17c3181e168415fb7bdee530873e2ddc8fa2d100af94bcc7b74909df3ed`
and passes independent native WVO admission.

After affected-test review, the combined Windows producer lane passes 8/8 in
1.7 seconds and the ordinary `os-probe` lane passes construction and
existing-output preservation 2/2 in 11.4 seconds. The final EFI remains 683,008
bytes at SHA-256
`080b4d669e9a11fdc802bf7197ae5a044978b6ba39741b2b1c832296987f74d9`.

The retirement plan remains 2,331 LF-only bytes and now has SHA-256
`b9fddc319c4185393e255917c9b22d426101336cb3e2901d6f8043c6a043faab`.
It owns 28 suites with 3,135 fixed cases. The seed retains four frozen WVOs
