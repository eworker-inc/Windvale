...
| Windvale-written WVA frontend | `Wvaˉscannerˉcore` validates source size, strict UTF-8, physical-line bytes, LF/CRLF/CR, spaces, tabs, comments, word boundaries, and the exact WVA 1 header over immutable bytes without host text parsing. Its multi-pass semantic inspector validates declarations, sections, definitions, the complete initial statement set, names, canonical order, exact integer widths, contexts, aggregate limits, definition ownership, and references against the Stage 0 diagnostic families. |
| Representative programs | The portable sum returns `29`; the hosted example prints `Hello from Windvale`; the Foundation header returns `1`; WvDump emits the complete golden Sum report; the WVO core writes its exact object; the Windvale WVA frontend reports exact structural and semantic counts for the real assembly source; and `Hello-Object.wva` assembles into exact code/data, three symbols, and two relocations. |
| Malformed-input coverage | Structured adversarial cases plus deterministic bounded random Windvale source, WVA source, module, and object input exercise diagnostic and rejection boundaries. The Windvale frontend covers malformed UTF-8, wrong/extra/missing headers, exact and exceeded line/source bounds, all accepted line endings, all eleven WVA diagnostic families, cross-binding/cross-kind duplicates, invalid ownership/contexts, missing/wrong-kind references, unclosed structures, integer overflows, aggregate-size overflow, and 200 deterministic source mutations compared with Stage 0 acceptance. |
| Cross-host coverage | The same 28-test suite passed on Windows and Debian Linux. Its normalized contracts compare equal for WVB/WVO/WVA versions, all module and object hashes, results, hosted output, every WvDump and WVA frontend report line, both Windvale-written binary modules, and assembled WVO bytes. |
| Scope control | The Windvale WVA frontend validates but does not yet derive encoded definition records, construct instruction/data bytes, or emit WVO. The Stage 0 WVA assembler emits objects but does not link. No Windvale-written WVA encoder, linker, native backend, executable image, self-hosted compiler, firmware, kernel, driver, or OS implementation is included. |
...
The verifier performs a Release build, runs the 28-test conformance suite, produces a Windows report, publishes the CLI as framework-dependent `linux-x64`, and exercises the real module, WVA semantic inspection, assembly, and object CLI paths including unauthorized-capability refusal and native file writer failures. The Windows report SHA-256 was `5b928d9ef9627a8f9309e153ca409385737145933fabdebcf6478b8d0f5e9e6f`.

## Linux QA qualification

Commit `cc57bf9` was archived and transferred with matching SHA-256 `77820f7730a5fc588d8f1ca44838a47acb7fa2d2594c406404cb2b1936f7a279`, then verified in the uniquely named disposable directory `/tmp/windvale-wva-semantics-cc57bf9-20260730` on the isolated E-Worker QA host. The host ran Debian GNU/Linux 12 x64 with .NET SDK `10.0.302`. The verification did not use E-Worker release, configuration, service, or durable-data paths. The Debian report was retrieved with SHA-256 `11469ee740e2c99bdb3e26cbd6fe776d6756c6c69497b1af3d3da7053456a5fa`; the frontend module was retrieved separately and matched the Windows module byte for byte; then the resolved exact temporary directory and transferred archive were removed.
...
The Release build completed with zero warnings and errors. All 28 conformance tests passed, including bounded hosted resources, immutable values, strict UTF-8, WvDump decoding, WVO validation, the complete WVA 1 instruction/data subset, Windvale-written WVA scanning and semantic inspection, deterministic assembly, stable assembly diagnostics, runtime limits, bounded random input, and golden hashes. The semantic test exercises exact numeric boundaries, every WVA diagnostic family, targeted cross-pass failures, and 200 deterministic character mutations against the Stage 0 oracle. The real CLI flow compiled and verified `Wvaˉscannerˉcore`, refused its ungranted capabilities, ran its embedded adversarial checks, semantically inspected `Hello-Object.wva`, rejected a non-WVA source as `WVA1001`, assembled the canonical source, verified and inspected its exact WVO, and preserved every prior module/object/native-host case on both systems.

The Windows and Linux reports were then compared with the test runner:

```powershell
dotnet run --project Tests/Windvale.Seed.Tests/Windvale.Seed.Tests.csproj --configuration Release --no-build -- --compare-reports artifacts/seed-conformance-windows-x64.json artifacts/seed-conformance-debian-x86_64-cc57bf9.json
```
...
- `Wva-Scanner-Core.wvb`: `bfc29da9a58ed2ff9130bada231ad2b7ab006c6e5f021926a2a9c8d7a9314e31`, result `0`; the directly retrieved Windows and Debian modules were byte-for-byte equal.
- Hosted frontend report: `wvascan 1`, then `status=valid bytes=403 lines=21 meaningful-lines=17 tokens=52 offset=403 line=22 column=1`, then `semantics status=valid sections=2 symbols=3 definitions=2 relocations=2 data-bytes=18 memory-bytes=18 offset=403 line=22 column=1` on both hosts.

This qualifies roadmap gate 6C, the Windvale-written WVA semantic inspector, and advances Phase 6 without completing it. The lexical scanner remains qualified at `e5fd109`, the WVA 1 Stage 0 oracle at `3bfc6bb`, Phase 5 at `f87a5fa`, Phase 4 at `a829fc8`, Phase 3 at `1f4b48a`, the Phase 2 WVB 1.3 slice at `e6c51c6`, WVB 1.2 at `a4b0f5d`, and WVB 1.1 at `60fd261`; none is a compatibility promise or supported obsolete input format. The next Phase 6 gate is exact Windvale-written instruction/data and WVO encoding, followed by the hosted write boundary and separately owned linker. Future bytecode, assembly, object, compiler, verifier, runtime, host-adapter, or report-contract changes must regenerate both native reports rather than inheriting this evidence automatically.
