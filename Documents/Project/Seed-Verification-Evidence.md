...
| Scope control | The qualified Stage 0 linker produces one bounded raw flat memory image and canonical map. The Windvale-written linker currently stops after constructing, independently reconstructing, comparing, and hashing the relocated candidate image. No Windvale-written canonical map or output path, executable-container adapter, native backend, directly executable host program, self-hosted compiler, firmware, kernel, driver, or OS implementation is included. |
...
The verifier performs a Release build, runs the 34-test conformance suite, produces a Windows report, publishes the CLI as framework-dependent `linux-x64`, and exercises the real module, Windvale/Stage 0 assembly, Windvale WVO scan, candidate-image construction and independent reconstruction, object, and Stage 0 link CLI paths. It checks capability refusal, deterministic file snapshots, SHA-256 sequence/slice behavior, byte-for-byte assembler equality, exact candidate-image identity, maximum-size complete-image comparison, independent WVO and flat-image verification, exact map output, no file for rejected source/link or a missing parent, and preservation of existing object/image outputs when validation fails. The independent-reconstruction-qualified WVB 1.6 Windows report SHA-256 is `58748521198bcefa71ec75d785ed097a860eb8badd0e2386ee1c65414a3d1b0c`.
