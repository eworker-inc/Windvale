...
The accepted first boot environment is x86-64 with UEFI 2.11, QEMU as the primary automated VM, and Hyper-V Generation 2 as the later Windows compatibility target. [Decisions 0044](../Decisions/0044-First-X64-Uefi-Boot-Environment.md) through [0092](../Decisions/0092-First-Windvale-Init-Resource-Service.md) establish deterministic PE32+, firmware exit, kernel handoff, owned memory/stack/page tables, WVA shutdown and normalized faults, the shared WVB-derived AOT path, fixed in-guest WVB admission, protected processes, and the first Windvale init/resource service. Candidate Decision 0092 adds two CPL3 roots, reduced endpoints, cross-process IPC, deterministic wait/wake, and contained client general protection. This is substantial vertical source-to-machine evidence, not yet a general WVB loader/verifier, scheduler, resource namespace, general trap system, or complete kernel runtime.
