...
The accepted first boot environment is x86-64 with UEFI 2.11, QEMU as the primary automated VM, and Hyper-V Generation 2 as the later Windows compatibility target. [Decisions 0044](../Decisions/0044-First-X64-Uefi-Boot-Environment.md) through qualified [0095](../Decisions/0095-First-Runtime-Supplied-Wvb-Boot-Resource.md) establish deterministic PE32+, firmware exit, kernel handoff, owned memory/stack/page tables, WVA shutdown and normalized faults, fixed in-guest WVB admission, protected processes, a Windvale init service, and Windvale-written bytecode interpretation at CPL3. Candidate [Decision 0096](../Decisions/0096-First-Windvale-Init-Owned-Boot-Resource-Grant.md) makes init own and select the admitted module, then publishes one immutable client alias through a checked grant. Probe 26 remains the cross-host baseline; probe 27 has focused Windows and pinned-QEMU evidence pending cross-host qualification. This is substantial vertical source-to-machine evidence, not yet an arbitrary or general WVB loader/verifier, resource namespace, ownership-transfer system, JIT, scheduler, general trap system, or complete kernel runtime.
