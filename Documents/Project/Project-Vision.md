...
The accepted first boot environment is x86-64 with UEFI 2.11, QEMU as the primary automated VM, and Hyper-V Generation 2 as the later Windows compatibility target. [Decisions 0044](../Decisions/0044-First-X64-Uefi-Boot-Environment.md) through qualified [0097](../Decisions/0097-First-Terminal-Resource-Borrow-Revocation.md) establish deterministic PE32+, firmware exit, kernel handoff, owned memory/stack/page tables, WVA shutdown and normalized faults, fixed in-guest WVB admission, protected processes, a Windvale init service, Windvale-written bytecode interpretation at CPL3, an init-selected immutable resource grant, and automatic terminal alias/publication cleanup. Probe 28 is the cross-host baseline, with live QEMU evidence on Windows. Candidate [Decision 0098](../Decisions/0098-First-Typed-Two-Resource-Lookup.md) adds an atomic typed WVB/execution-budget pair, WVA lookup, Windvale opcode charging, and complete pair cleanup. This is substantial vertical source-to-machine evidence, not yet an arbitrary or general WVB loader/verifier, dynamic resource namespace, ownership-transfer/reclamation system, JIT, scheduler, general trap system, or complete kernel runtime.
