...
The accepted first boot environment is x86-64 with UEFI 2.11, QEMU as the primary automated VM, and Hyper-V Generation 2 as the later Windows compatibility target. [Decisions 0044](../Decisions/0044-First-X64-Uefi-Boot-Environment.md) through [0093](../Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) establish deterministic PE32+, firmware exit, kernel handoff, owned memory/stack/page tables, WVA shutdown and normalized faults, fixed in-guest WVB admission, protected processes, a Windvale init service, and the first Windvale-written bytecode interpreter at CPL3. Probe 21 remains the cross-host-qualified OS baseline; candidate probe 24 executes the exact admitted WVB through the interpreter instead of the program's host-built AOT derivative. This is substantial vertical source-to-machine evidence, not yet a general WVB loader/verifier, JIT, scheduler, resource namespace, general trap system, or complete kernel runtime.
