...
The accepted first boot environment is x86-64 with UEFI 2.11, QEMU as the primary automated VM, and Hyper-V Generation 2 as the later Windows compatibility target. [Decision 0044](../Decisions/0044-First-X64-Uefi-Boot-Environment.md), [Decision 0045](../Decisions/0045-First-Uefi-Application-And-Boot-Probe.md), and [Decision 0046](../Decisions/0046-Bounded-Uefi-Memory-Map-Probe.md) now fix the environment, deterministic PE32+ application boundary, firmware-entry evidence, and bounded memory-map acquisition. These probes are vertical evidence and do not claim a kernel or `.wv` execution.
