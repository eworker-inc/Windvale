...
The accepted first boot environment is x86-64 with UEFI 2.11, QEMU as the primary automated VM, and Hyper-V Generation 2 as the later Windows compatibility target. [Decisions 0044](../Decisions/0044-First-X64-Uefi-Boot-Environment.md) through [0091](../Decisions/0091-First-Protected-Windvale-Process.md) establish deterministic PE32+, firmware exit, kernel handoff, owned memory/stack/page tables, WVA shutdown and normalized faults, the shared WVB-derived AOT path, fixed in-guest WVB admission, and the first protected process/capability/IPC experiment. Candidate Decision 0091 adds a real CPL3 process root/thread, fixed capability/resource state, register IPC, syscall exit, and contained user general protection. This is substantial vertical source-to-machine evidence, not yet a general WVB loader/verifier, scheduler, service environment, general trap system, or complete kernel runtime.
