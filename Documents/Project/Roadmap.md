...
| Reproducible release artifacts and installers | ✅ Infrastructure / 🚧 Official envelope | Decision 0562 retains the exact qualified `0.1.0-dev.1` artifacts. Decision 0565 pins distinct internally consistent `0.1.0` stable identities without duplicating the builder; the combined owner passes on both hosts in Verify run 31885759856. Decision 0563's bounded creator, independent verifier, eleven-artifact profile, and thirteen-case format core pass in Verify run 31883543587. Decision 0566 expands the owner to sixteen cases for protected key custody; paired evidence and the owner-custodied official envelope remain. |
