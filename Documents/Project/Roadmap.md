...
Decision 0044 keeps public `WVSD 1.0` unchanged and advances the private `WVSI` index to 1.1 with deterministic mappings between source-order directory entries and canonical nominal ordinals. Binding and typed-WVIR consumers use those mappings directly, packed directory scans avoid unsuccessful match materialization, and equality paths reject unequal byte lengths before comparison. The real nine-module binding closure falls from 2,972,056,275 to 2,600,859,185 instructions despite a larger source-derived workload. The candidate passes all 48 Windows Standard tests. Exact cross-host Qualification remains pending, and the ten-module typed-WVIR input still exceeds four billion; repeated lexical/parser traversal is the next measured performance slice.
