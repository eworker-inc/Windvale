...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Multi-module backend candidate implemented; qualification, performance, and compiler closure next |
...
Decision 0040's candidate lowers a complete validated WVSS graph to one ordinary WVB without adding runtime linkage. It resolves every global WVSD identity through its owner source, internalizes dependency functions and nominal types, preserves root data/profile/capabilities/exports, and discovers text literals across canonical global function order. Its three-module fixture is byte-identical to Stage 0, verifies, exposes only `Main`, and returns `42`; noncanonical dependency order produces no output. Cross-host qualification is pending, while source-envelope/performance closure and full compiler bootstrap closure remain later work.
