...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Lexer/declarations qualified; body parser candidate implemented |
...
The declaration pass is cross-host qualified at `fc87a3e` under Decision 0026. It parses module headers and complete top-level declaration shapes into streaming immutable source views, then identifies balanced function-body spans for the later statement pass. It parses both the real lexer and its own declaration source without a token/declaration collection. The next slice parses statements and expressions inside those bounded body views and uses the resulting pressure to decide whether Windvale needs a packed collection or diagnostic sequence.

That body-parser candidate is implemented under Decision 0027. It reproduces the complete Stage 0 statement/expression grammar as flat parent/child source views, validates the lexer, declaration parser, and itself, and still retains no syntax collection. Windows qualification is complete; exact-archive Debian qualification is pending. Its evidence will decide whether semantic binding can continue with bounded rescanning or needs a packed node/index facility.
