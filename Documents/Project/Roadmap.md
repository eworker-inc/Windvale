...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 10 runs the complete checked-in Windvale `wvdump` through interpreter/JIT/AOT on Windows; cross-host qualification is pending |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Firmware probe 12 passes focused OS tests through ABI 10/context 2; cross-host/QEMU qualification, guest loading, traps, runtime, shutdown, and Hyper-V remain |
...
[Decision 0069](../Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md) implements ABI 10. Service-table version 4 plus a 16 MiB execution-owned text arena supply bounded enum names, integer formatting, concatenation, quoting, and diagnostics. Hidden verified result cells admit descriptor returns while preserving packed status; void calls use the same propagation path. The complete 1,441-line checked-in `Wv-Dump-Core.wv` produces byte-identical reports under the interpreter, Windows W^X JIT, and linked WVO/AOT. Malformed UTF-8, aggregate text exhaustion, and corrupted result conventions have deterministic negative coverage. Firmware probe 12 and all 15 focused OS tests pass. Cross-host Release, portable-artifact, and pinned-QEMU qualification remain the immediate gate; after that, native implementations of the closed runtime services are the next .NET-retirement slice.
