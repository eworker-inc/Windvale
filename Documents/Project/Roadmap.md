...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Syntax, WVSS, and import graph qualified; declaration/signature candidate implemented |
...
Decision 0033's Windvale-written declaration/signature phase is implemented and passes the complete Windows candidate gate: a zero-warning Release build, all 45 conformance tests, and the native CLI verifier. It enforces global namespace and capability policy, binds visible nominal signature types, assigns canonical nominal indices, and publishes an independently validated `WVSD 1` declaration directory plus a bounded transitive-visibility matrix. A repeated-rescan prototype exhausted 4,000,000,000 instructions on the real closure; the retained packed evidence removes that impractical path. Exact-archive Debian qualification remains required. Body/local/call binding and typed expression/control-flow semantics are next.
