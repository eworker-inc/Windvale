...
Phase 1 is qualified at commit `60fd261` after the exact source passed the Windows verifier, Debian QA verifier, and portable report comparison. Phase 2 is the current implementation focus. Its first slice is qualified at commit `a4b0f5d`: immutable nominal records flow through source, WIR, WVB 1.2, verification, inspection, and runtime execution. Its second candidate slice adds nominal status enums and deterministic bounded formatting in WVB 1.3. `Wvˉdumpˉcore` now returns named status, section count, and failure offset and can format their portable summary. Cross-host qualification is required before Phase 2 is marked complete.
