...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Syntax and WVSS qualified; portable import graph implemented; symbol binding next |
...
Decision 0030's Windvale-written import-graph candidate now resolves exact module names, rejects repeated and missing imports, computes the complete root closure, and proves acyclicity over WVSS without host collections. Windows passes all 44 tests, including an exact 64-module/63-edge chain and the real seven-module compiler closure; exact-archive Debian qualification is pending. Declaration namespaces and signature/body binding remain the next semantic slice.
