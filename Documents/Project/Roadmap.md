...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Body/local/call binding implemented; cross-host qualification next |
...
Decision 0034's Windvale-written body/local/call phase is implemented. It assigns stable parameter/local slots and scopes, binds reads and assignments, resolves visible constructors/functions/capabilities and Foundation intrinsics, checks arity, and publishes an independently validated `WVLB 1` directory. Measured temporary-directory and per-candidate source-slicing variants exceeded the fixed 4,000,000,000-instruction ceiling; the retained packed-span design binds the real nine-module closure within that ceiling. Exact-archive Windows/Debian qualification remains the gate before this slice is called qualified. Complete expression types, field/operator validation, control-flow proof, and typed WIR are next.
