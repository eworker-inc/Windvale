...
[Decision 0550](../Decisions/0550-Measured-Native-Retirement-Sharding.md) keeps that database and compiler evidence cold while reducing its feedback critical path. The sharding decision measured 52 suites and 3,287 cases; the current manifest contains 53 suites and 3,329 cases after the expanded native owners. GitHub runs all four shards on both hosts, preserves the sequential oracle and focused filters, reports non-semantic owner timing, and retains the unchanged aggregate Verification gate. [Decision 0553](../Decisions/0553-Content-Addressed-Project-Object-Development-Checkpoints.md) adds a source/producer-keyed WVB/WVO cache for eligible local database feedback, [Decision 0554](../Decisions/0554-Content-Addressed-Hosted-Application-Development-Checkpoints.md) reuses the exact validated current-host application while rerunning behavior, and [Decision 0555](../Decisions/0555-Content-Addressed-Project-Wvb-Development-Checkpoints.md) reuses the verified compiler build-driver input WVB. The former two-case warm owner took 71.048 seconds instead of the 1,111.135-second clean owner; the new eight-case owner takes 402.638 seconds with compiler and hosted-application caches hit. Its six portable tree targets still perform fresh lower/link/package construction and execution. Broad changes, GitHub shards, and qualification remain cold; caching or avoiding that repeated unchanged work is the next measured boundary.
