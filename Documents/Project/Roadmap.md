...
| 6G. Windvale linker | A Windvale-written verified-bytecode linker implementing the accepted contract. | WVB 1.6 SHA-256 identity and deterministic bounded file snapshots are qualified at `348c82a`; balanced persistent bytes are qualified at `89ce80b`; the complete Windvale WVO scanner and immutable object views are qualified at `3eb331a`; multi-object resolution and deterministic layout are qualified at `709ccb3`; immutable image construction and checked relocation are qualified at `ec9c980`; independent complete-image reconstruction is qualified at `d8008e3`; canonical map construction and publish-after-success output are implemented with cross-host qualification pending. |
