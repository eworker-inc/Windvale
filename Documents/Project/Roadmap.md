...
| 6F. Linker contract and oracle | A separate link specification covering inputs, duplicate/undefined symbols, layout, alignment, relocation arithmetic, limits, map output, and the first flat-image target. | Stage 0 linker passes deterministic, overflow, hostile-object, and layout tests without absorbing assembler duties. |
| 6G. Windvale linker | A Windvale-written verified-bytecode linker implementing the accepted contract. | Windows and Debian produce identical images and maps from identical verified objects; oracle and Windvale results agree. |
...
Phase 6's WVA 1 Stage 0 contract is qualified at `3bfc6bb`, its Windvale-written bounded scanner at `e5fd109`, and the complete semantic inspector at `cc57bf9`. The exact semantic-inspector archive passed 28 tests and real CLI validation on Windows and Debian; the Windvale module, classifications, aggregate counts, and hosted reports agree across hosts. The current implementation focus is gate 6D: derive definition ranges, encode the complete initial instruction/data set, construct canonical sections/symbols/relocations, and emit WVO 1.0 byte-for-byte identically to Stage 0. Phase 6 remains incomplete until that encoder and hosted write boundary qualify and a separately owned linker implements resolution, layout, relocation application, and final image output.
