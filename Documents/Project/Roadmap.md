...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 8 borrowed argument/file input and the first native Windvale-written WVB header inspector are cross-host qualified |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Firmware probe 10 is qualified through ABI 8 under pinned QEMU; guest loading, traps, runtime, shutdown, and Hyper-V remain |
