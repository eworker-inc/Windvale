...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 5 calls, budgets, recursion, and immutable i32 data are cross-host qualified; first OS AOT consumer is implemented and pending exact-candidate qualification |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Special system `.wv` and one portable-WVB-derived ABI-5 module run on a bounded kernel-owned stack; guest loading, traps, runtime, shutdown, and Hyper-V remain |
