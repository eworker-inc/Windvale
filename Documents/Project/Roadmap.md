...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 10 is cross-host qualified through the complete checked-in Windvale `wvdump` |
| 10. Native host tools and .NET retirement | Produce and qualify native Windvale tools, runtime, JIT/AOT execution, and bootstrap recovery on Windows and Linux without a normal .NET dependency. | First platform-neutral runtime-native UTF-8 service implemented; cross-host qualification pending |
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Firmware probe 12 is cross-host/pinned-QEMU qualified through ABI 10/context 2; guest loading, traps, runtime, shutdown, and Hyper-V remain |
...
[Decision 0070](../Decisions/0070-First-Runtime-Native-Utf8-Service.md) implements the first such leaf without changing ABI 10: strict UTF-8 validation uses one exact 800-byte x86-64 implementation on Windows and Linux instead of a C# delegate and platform thunk. The managed decoder remains the oracle, and the complete Windows Standard pass covers its W^X boundary cases. Cross-host qualification is the immediate gate; native text-arena ownership is then required before formatting and concatenation can follow.
