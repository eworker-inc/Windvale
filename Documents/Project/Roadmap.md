...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 7 borrowed immutable bytes plus `u8`/`u32` operations are implemented over the qualified ABI-6 context/service seam; cross-host qualification, then narrow file/argument input and a real tool, are next |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Development firmware probe 9 runs special system `.wv` plus a portable ABI-7 byte-decoding module on a bounded kernel-owned stack; guest loading, traps, runtime, shutdown, and Hyper-V remain |
