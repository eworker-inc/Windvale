...
Phase 2 is qualified at commit `e6c51c6` after the exact source passed the Windows verifier, Debian QA verifier, and portable report comparison. Its first slice introduced immutable nominal records at commit `a4b0f5d`; its second added nominal status enums and deterministic bounded formatting in WVB 1.3. `Wvˉdumpˉcore` now returns named status, section count, and failure offset and formats their portable summary. Phase 3 is the current implementation focus: define the smallest explicit hosted boundary for arguments, file-byte input, diagnostics, and output while keeping parsing and inspection portable.
