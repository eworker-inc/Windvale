...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 11/context 3 is cross-host qualified with shared text-arena ownership |
| 10. Native host tools and .NET retirement | Produce and qualify native Windvale tools, runtime, JIT/AOT execution, and bootstrap recovery on Windows and Linux without a normal .NET dependency. | UTF-8 validation, concatenation, and integer formatting have exact native leaves; enum naming and quoting are next |
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Probe 13 is cross-host/pinned-QEMU qualified through ABI 11/context 3; guest loading, traps, runtime, shutdown, and Hyper-V remain |
