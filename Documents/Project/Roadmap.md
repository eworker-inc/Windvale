...
Phase 1 is qualified at commit `60fd261` after the exact source passed the Windows verifier, Debian QA verifier, and portable report comparison. Phase 2 is the current implementation focus. Its first slice is qualified at commit `a4b0f5d`: immutable nominal records now flow through source, WIR, WVB 1.2, verification, inspection, and runtime execution. `Wvˉdumpˉcore` returns a structured result with section count and failure offset and uses a structured section descriptor. Compact enum/status naming and bounded formatting remain next within Phase 2.
