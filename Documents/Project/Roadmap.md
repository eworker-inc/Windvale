...
- Compiler folder roles wait for qualified self-hosting. After the Windvale-written compiler can compile itself reproducibly, name it as the compiler (for example `Compiler/Windvale`), retain the C# implementation as an independent reference/recovery compiler, and reserve `Bootstrap` for the staged transition, provenance, and recovery process rather than the primary compiler product.
