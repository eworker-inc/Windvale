...
The [experimental Windvale Database reader](../../Specifications/Windvale-Database-Reader.md) first applied those facilities to immutable bytes. The durable successor now has checked `u64` geometry, bounded random-access mutation, dual superblocks, checksummed immutable pages and compact logs, recovery-safe publication, variable-key tree nodes, provider-backed depth-two lookup, repeated routed-leaf replacement, deterministic internal splitting, first depth-three root growth, and unique obsolete-page ownership. Compiler/lowerer/tool latency and generated-code size remain the immediate measured priority; the next database engine gate is updating an existing depth-three generation and cascading a split from a non-root internal branch. Concurrency, group commit, native path replacement, and directory durability remain separate interfaces.

[Decision 0550](../Decisions/0550-Measured-Native-Retirement-Sharding.md) keeps that database and compiler evidence cold while reducing its feedback critical path. The sharding decision measured 52 suites and 3,287 cases; the current manifest contains 53 suites and 3,350 cases after the expanded database and package owners. Explicit qualification runs all four shards on both hosts, preserves the sequential oracle and focused filters, reports non-semantic owner timing, and retains the aggregate Verification gate. [Decision 0553](../Decisions/0553-Content-Addressed-Project-Object-Development-Checkpoints.md) adds a source/producer-keyed WVB/WVO cache for eligible local database feedback, [Decision 0554](../Decisions/0554-Content-Addressed-Hosted-Application-Development-Checkpoints.md) reuses the exact validated current-host application while rerunning behavior, and [Decision 0555](../Decisions/0555-Content-Addressed-Project-Wvb-Development-Checkpoints.md) reuses the verified compiler build-driver input WVB. [Decision 0557](../Decisions/0557-Separate-Development-Verification-From-Qualification.md) runs only affected owners on Windows and Linux for ordinary pushes and pull requests and reserves the complete cold six-profile matrix for an explicit milestone dispatch. The former two-case warm database owner took 71.048 seconds instead of the 1,111.135-second clean owner; the new eight-case owner takes 402.638 seconds with compiler and hosted-application caches hit. Its six portable tree targets still perform fresh lower/link/package construction and execution; caching or avoiding that repeated unchanged work is the next measured boundary.
