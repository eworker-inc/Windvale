...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Body/local/call binding qualified; typed expression/control-flow semantics next |
...
Decision 0036's first Windvale-written WVB backend is implemented as a qualification candidate. It emits a complete WVB 1.6 module for one portable, ordinal, function-only primitive source module; assigns WVIR temporaries to locals; computes branch offsets and maximum stack in a separate pass; and rejects unsupported metadata and operations explicitly. Its four-function fixture is exactly byte-identical to Stage 0, passes the mandatory verifier, and executes with result `6`. Cross-host qualification remains pending; data, nominal types, capabilities, imports, and full bootstrap closure are the next backend expansions.
