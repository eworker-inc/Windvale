...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 7 is cross-host qualified; ABI 8 borrowed argument/file input and the first Windvale-written WVB header inspector are implemented with qualification pending |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Qualified firmware probe 9 remains evidence; implemented probe 10 rebuilds its portable consumer through ABI 8, while guest loading, traps, runtime, shutdown, and Hyper-V remain |
...
[Decision 0067](../Decisions/0067-Borrowed-Hosted-Input-And-First-Native-Wvb-Inspector.md) implements ABI 8's first hosted input boundary. Borrowed text and bytes share an execution-bounded descriptor shape; service-table version 2 admits explicitly authorized argument count, argument text, file snapshot input, and console output. The checked-in `Wvb-Header-Inspector.wv` now reads a real compiler-produced WVB and validates `WVB1`/version `1.6` identically under the reference interpreter and Windows W^X path. Full `wvdump` still needs nominal records/enums, dynamic text formatting, and diagnostic policy. Cross-host qualification remains open.
...
Decisions 0044 through 0049 complete the environment, image, firmware-exit, handoff, and first compiler-generated boot slices. Decision 0052 completes the first memory part of step 3: firmware probe version 6 claims only one 64 KiB conventional-memory arena, exercises a zeroing page allocator, copies the handoff, and runs compiler-generated Main on an 8 KiB owned stack under exact QEMU evidence. Decisions 0054 and 0056 establish the bidirectional WVA/WV execution seam, move memory-through-Hello evidence into `.wv`, and assign future machine mechanics to WVA and kernel policy to Windvale source. Decision 0064 advances the qualified probe to version 7 and executes one ordinary portable-WVB-derived ABI-5 module with internal calls, a bounded loop, and immutable data on that stack before the special Main succeeds. Decision 0065 advances the qualified probe to version 8 and supplies that module through the ABI-6 context. Decision 0066 advances qualified probe 9 to ABI 7 and makes the same portable source pass, slice, and read immutable bytes before accepting result 29. Decision 0067 assigns firmware probe 10 to the ABI-8 rebuild without exposing host services in the guest. This is host-built AOT evidence; in-guest WVB verification/loading, general reclamation, paging, traps, shutdown, and Hyper-V remain open.
