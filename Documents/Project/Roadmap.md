...
- Compiler folder names describe implementation roles rather than lifecycle status. Move the Windvale-written implementation to `Compiler/Windvale` and the C# implementation to `Compiler/Reference` in a separate qualified refactor; reserve `Bootstrap` for the staged transition, provenance, and recovery process. Calling the Windvale implementation a compiler does not claim that it is already self-hosting.
