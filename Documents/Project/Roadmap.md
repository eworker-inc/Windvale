...
| 8. Self-hosted compiler | Windvale-written lexer, parser, semantics, and code generation for a meaningful subset, followed by a reproducible bootstrap closure. | Function-only WVIR-to-WVB backend qualified; metadata and bootstrap closure next |
...
Decision 0036's first Windvale-written WVB backend is cross-host qualified and published at `d65d286`; its tree is byte-identical to exact qualified candidate `ca56996`. It emits a complete WVB 1.6 module for one portable, ordinal, function-only primitive source module; assigns WVIR temporaries to locals; computes branch offsets and maximum stack in a separate pass; and rejects unsupported metadata and operations explicitly. Its four-function fixture is exactly byte-identical to Stage 0, passes the mandatory verifier, and executes with result `6`. Windows and Debian pass all 48 tests and the complete native verifier with matching normalized reports and 53 byte-identical retrieved portable artifacts. Data, nominal types, capabilities, imports, canonical remapping, and full bootstrap closure are the next backend expansions.
