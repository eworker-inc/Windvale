...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 9 is cross-host qualified through enums, immutable arena records, strict UTF-8, and the native Windvale `wvdump` structural parser |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Firmware probe 11 is cross-host/pinned-QEMU qualified through ABI 9/context 2; guest loading, traps, runtime, shutdown, and Hyper-V remain |
