...
| 9. Shared native backend | Native WIR/WVB lowering, x86-64 ABI, WVO/AOT output, baseline JIT, and interpreter/JIT/AOT differential tests. | ABI 6 versioned context and the first authorized static-text console service are implemented; cross-host/OS requalification and bytes/file services are next |
...
| 11. Boot path and kernel | x86-64 UEFI/QEMU boot, diagnostics, memory foundation, minimal kernel boundary, and Hyper-V qualification. | Special system `.wv` and one portable-WVB-derived ABI-6 module use a versioned context on a bounded kernel-owned stack; version-8 QEMU requalification, guest loading, traps, runtime, shutdown, and Hyper-V remain |
...
[Decision 0065](../Decisions/0065-Versioned-Native-Execution-Context-And-Console-Service.md) advances the development target to ABI 6. One 32-byte versioned context replaces positional resource arguments and carries an optional 16-byte service table. The first closed service lowers immutable static UTF-8 through `console.write_line`, requires explicit authorization and implementation before W^X publication, uses identical generated bytes plus tiny runtime-owned Windows/System V thunks, and contains service failure as a packed trap. The OS bridge constructs the same context with an empty service table. All regular Windows and OS tests plus a real pinned development-QEMU boot pass; exact Windows/Debian qualification is pending.
