...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0063 cross-host qualify the shared Stage 0 seam through constants, checked arithmetic/traps, typed control, dynamic instruction budgets, backward control, internal calls, bounded recursion, and immutable i32 data through WVO/AOT and Windows/Linux W^X paths. Decision 0064 qualifies the first downstream Windvale OS AOT consumer of that same ABI at exact candidate `708242e`. Decisions 0065 through 0068 cross-host qualify the versioned context, borrowed values, hosted input, enums, execution-owned immutable records, pure UTF-8 validation, the native `wvdump` structural parser through ABI 9, and firmware probe 11. [Decision 0069](../Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md) cross-host qualifies ABI 10's complete native `wvdump` and firmware probe 12 at exact commit `7979933`; [Decision 0070](../Decisions/0070-First-Runtime-Native-Utf8-Service.md) qualifies its first platform-neutral native service leaf. [Decision 0071](../Decisions/0071-Native-Text-Arena-And-Core-Text-Services.md) cross-host qualifies ABI 11/context 3, three more runtime-native leaves, and firmware probe 13 at exact commit `8888951`. [Decision 0072](../Decisions/0072-Final-Pure-Runtime-Native-Services.md) implements exact enum-name and quote leaves without advancing those contracts; cross-host qualification is pending. This document defines the larger native destination and migration boundaries; it does not claim an in-guest WVB loader, a general native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
...
Decision 0072 implements the final two pure runtime services without advancing ABI 11, context 3, service-table 4, or firmware probe 13. Enum naming uses one exact native leaf plus an adjacent bounded, independently verified runtime-private `WVEN` metadata block reconstructed from canonical fragment types. Deterministic quoting uses one exact two-pass strict-UTF-8 native leaf and preserves the existing UTF-16-code-unit escape contract. All six deterministic pure services are therefore native in the development candidate; cross-host qualification remains pending. The five hosted/capability adapters and Stage 0 construction, verification, W^X publication, arena ownership, and execution remain managed.

## Native runtime ABI

Generated code targets a Windvale-owned internal ABI rather than emitting host calls throughout ordinary functions. The ABI-11 entry receives a pointer in `RDX` to the exact [native execution context](../../Specifications/Windvale-Native-Execution-Context.md). One identical `Main` preserves that pointer in `R15` and loads the versioned instruction/depth budgets into reserved `R11` and `R10`. Internal functions accept as many as four scalar, enum, record-offset, or borrowed-text/byte parameters. Scalars, enums, and records use `R8D`, `R9D`, `ECX`, and `EDX`; borrowed descriptors use the corresponding 64-bit register as a pointer to a caller descriptor that the callee copies. Scalar/enum/record results and packed statuses return in `RAX`.

The context's optional service-table pointer is the only generated-code route to host or runtime support. ABI 11 retains the closed version-4 table for console/diagnostic output, bounded argument/file input, UTF-8 validation, enum naming, integer formatting, concatenation, and quoting; only capability services require authorization. All six deterministic pure services use exact platform-neutral native leaves. Hosted services retain runtime-owned managed platform thunks. WVO 1.0 does not serialize fragment service or nominal requirements, so a linked service-bearing image is not independently loadable without its verified fragment metadata. Host-returned text/bytes, text-arena values, and record-arena values are execution-owned and cannot escape the run. This remains a bounded experimental convention, not the final stack-argument, general-allocation, asynchronous-I/O, or safe-point ABI.
