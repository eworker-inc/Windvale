...
[Decision 0248](../Decisions/0248-Measured-Native-Lowering-Module-Envelope.md) replaced the lowerer's three eight-entry prototype guards with a measured bounded envelope. Decision 0561 later raises only the function directory to 1,024 entries so the 532-function variant-capable segmented compiler remains self-hosting; the 64 immutable-data, 128 total nominal-type, per-function, aggregate-code, and relocation boundaries remain explicit. Canonical D4 WVO helper and data names are generated through the focused layout module.
