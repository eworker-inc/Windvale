...
Decision 0070 implements the first runtime-native service without advancing ABI 10. Strict UTF-8 validation now executes as one exact platform-neutral x86-64 leaf instead of a managed delegate plus Windows/System V adapter. The managed decoder remains the oracle, while exhaustive encoding boundaries and exact service bytes are covered in the focused native loop. Cross-host qualification is pending; every allocation-bearing and hosted service remains managed.
...
The context's optional service-table pointer is the only generated-code route to host or runtime support. ABI 10 uses one closed version-4 table for console/diagnostic output, bounded argument/file input, UTF-8 validation, enum naming, integer formatting, concatenation, and quoting with runtime-owned platform thunks; only capability services require authorization. WVO 1.0 does not serialize fragment service or nominal requirements, so a linked service-bearing image is not independently loadable without its verified fragment metadata. Host-returned text/bytes, text-arena values, and record-arena values are execution-owned and cannot escape the run. This remains a bounded experimental convention, not the final stack-argument, general-allocation, asynchronous-I/O, or safe-point ABI.
