...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0074 cross-host qualify the shared Stage 0 and first OS consumer seams through ABI 13. [Decision 0076](../Decisions/0076-Native-Windows-And-Linux-File-Input.md) advances the current candidate to ABI 14, native leaves for all eleven service slots, and firmware probe 16; cross-host qualification is pending. This document defines the larger native destination and migration boundaries; it does not claim an in-guest WVB loader, a general Windvale-owned native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
...
Decision 0076 advances the current candidate to ABI 14 and context version 6. A runtime-private 136-byte `WVFI` table owns bounded path scratch, 64 canonical name/data snapshot slots, and the narrow Windows function-pointer boundary. Exact Windows and Linux leaves perform file input without a managed delegate, preserve first-success immutable snapshots, and map native failures back to stable `WVR302x` details. All eleven service-table slots now have native leaves. Windows development and OS tests pass; exact-commit cross-host and pinned-QEMU evidence remain pending. C# Stage 0 still constructs, verifies, allocates, publishes, invokes, maps, and releases the runtime artifacts.
