...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0068 cross-host qualify the shared Stage 0 and first OS consumer seams through ABI 9. [Decision 0069](../Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md) through [Decision 0072](../Decisions/0072-Final-Pure-Runtime-Native-Services.md) cross-host qualify ABI 10 and ABI 11, the complete native `wvdump`, all six deterministic pure native-service leaves, and firmware probe 13. [Decision 0073](../Decisions/0073-Native-Argument-Table-And-Process-Input-Services.md) advances the current Windows-tested candidate to ABI 12/context 4 with exact immutable argument services and firmware probe 14. This document defines the larger native destination and migration boundaries; it does not claim an in-guest WVB loader, a general native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
...
Decision 0073 advances the current candidate to ABI 12 and context version 4. The context appends an execution-owned immutable argument-table pointer and count. The Stage 0 owner packs at most 67 already validated strict-UTF-8 arguments, independently rereads every descriptor/range/byte before publication, and releases the snapshot after execution. Exact 5-byte count and 70-byte checked descriptor-copy leaves replace both managed argument delegates and platform adapters while retaining capability preflight and generated call shapes. The focused maximum/empty/Unicode/out-of-range case, every runtime-tagged test, and all 15 OS tests pass on Windows; exact cross-host qualification remains pending. Console, diagnostic, and file input are now the three managed runtime callbacks.

## Native runtime ABI

Generated code targets a Windvale-owned internal ABI rather than emitting host calls throughout ordinary functions. The ABI-12 entry receives a pointer in `RDX` to the exact [native execution context](../../Specifications/Windvale-Native-Execution-Context.md). One identical `Main` preserves that pointer in `R15` and loads the versioned instruction/depth budgets into reserved `R11` and `R10`. Internal functions accept as many as four scalar, enum, record-offset, or borrowed-text/byte parameters. Scalars, enums, and records use `R8D`, `R9D`, `ECX`, and `EDX`; borrowed descriptors use the corresponding 64-bit register as a pointer to a caller descriptor that the callee copies. Scalar/enum/record results and packed statuses return in `RAX`.

The context's optional service-table pointer is the only generated-code route to host or runtime support. ABI 12 retains the closed version-4 table for console/diagnostic output, bounded argument/file input, UTF-8 validation, enum naming, integer formatting, concatenation, and quoting; only capability services require authorization. All six deterministic pure services plus argument count and argument selection use exact platform-neutral native leaves. Console, diagnostics, and file input retain runtime-owned managed platform thunks. WVO 1.0 does not serialize fragment service or nominal requirements, so a linked service-bearing image is not independently loadable without its verified fragment metadata. Host-returned text/bytes, text-arena values, argument-table values, and record-arena values are execution-owned and cannot escape the run. This remains a bounded experimental convention, not the final stack-argument, general-allocation, asynchronous-I/O, or safe-point ABI.
