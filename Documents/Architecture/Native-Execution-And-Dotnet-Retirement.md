...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0080 cross-host qualify the shared Stage 0 and first OS consumer seams through ABI 14, native leaves for all eleven service slots, two Windvale-owned stencil consumers, a bounded byte-result entry, and firmware probe 16. Decision 0081 separately implements the qualification-pending terminal CPU-exception candidate for probe 17. Decision 0082 implements the first Windvale-owned executable-image layout and awaits exact cross-host qualification. This document defines the larger native destination and migration boundaries; it does not claim an in-guest WVB loader, a general Windvale-owned native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
