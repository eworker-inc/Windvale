...
Decision 0089 advances the current candidate to ABI 16 without changing context 7 or the service table. The first four parameters retain the shared volatile registers; up to 60 later positions use exact 16-byte outgoing cells, matching the language's bounded 64-parameter limit. The strict fragment decoder reconstructs stack reservation, scalar/descriptor cells, adjusted hidden result, direct call, release, and caller/callee agreement. Maximum-width scalar, descriptor-returning, and void calls agree across interpreter, W^X JIT, and linked WVO/AOT, while targeted corruption is rejected. Exact compiler preflight passes all functions with five through 23 parameters and advances to its sole 1,049-local function against the current 1,024-slot native frame bound. Cross-host qualification remains pending.
