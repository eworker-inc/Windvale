...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0082 cross-host qualify the shared Stage 0 and first OS consumer seams through ABI 14, native leaves for all eleven service slots, two Windvale-owned stencil consumers, a bounded byte-result entry, Windvale-owned executable-image layout, and firmware probe 17's terminal invalid-opcode boundary. Decision 0083 implements the qualification-pending Windvale-owned publication-lifetime graph and internal host state owner. This document defines the larger native destination and migration boundaries; it does not claim an in-guest WVB loader, a general Windvale-owned native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
...
Decision 0083 implements the next ownership candidate through the separate [`WVLQ 1`/`WVLT 1`](../../Specifications/Windvale-Native-Publication-Lifetime.md) contract. Windvale emits the complete allowed state/action graph once per accepted image extent. C# independently reconstructs every transition, then one internal executable-image owner holds the raw address and actual state, gates allocate/copy/seal/invoke/release operations, and admits release from every post-allocation partial state. The larger executor no longer imports executable-memory platform calls. Windows/Linux P/Invoke authority, context/service/arena/result-cell lifetime, and native compilation/verification remain Stage 0 responsibilities pending exact cross-host qualification and later measured transfers.
