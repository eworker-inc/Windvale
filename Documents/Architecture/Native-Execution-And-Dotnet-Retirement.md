...
now qualifies this architecture's accepted repository subset on Windows and
Linux and retires .NET from its normal workflows. The exact six-job native
matrix, 45-suite/3,206-case coordinators, native compiler convergence,
WebAssembly owners, and paired-host Stage 0 release reconstruction all pass.
The broader interpreter, optimizer, capability, browser, package, and OS work
described below remains forward product breadth, not an open dependency gate.
Earlier incremental paragraphs retain their then-current open statements as a
...
[Decision 0248](../Decisions/0248-Measured-Native-Lowering-Module-Envelope.md) replaced the lowerer's three eight-entry prototype guards with a measured bounded envelope. Decision 0561 later raises only the function directory to 1,024 entries so the 532-function variant-capable segmented compiler remains self-hosting. Decision 0724 retains 128 total nominal types while assigning unused internal scalar tags to record ordinals 64 through 115; the 64 immutable-data, per-function, aggregate-code, and relocation boundaries remain explicit. Canonical D4 WVO helper and data names are generated through the focused layout module.
...
Decision 0365 advances those historical seams without changing either serialized contract. Both publication bridges are now capability-free portable `Main(bytes) -> bytes` modules, and normal evaluation uses verified native fragments rather than the managed reference interpreter and synthetic file capability. One focused service-free bootstrap publishes only the planners themselves and is checked against their Windvale results. Retained-WVB loading and lowering, final platform W^X authority, service-bundle assembly, contexts, arenas, and invocation remain managed pending later retirement slices.

Decision 0366 applies the direct-artifact boundary to the two fixed process-input service leaves. The exact Windvale stencil consumer, bridge, WVOs, and WVB remain source-reproducible recovery and differential evidence, but the normal runtime embeds only the separately digest-bound 5-byte and 70-byte machine leaves. Process-input service assembly therefore no longer performs managed WVB decoding, native lowering, temporary publication, invocation, result copying, or splitting. Three variable-input retained WVB loaders and the final managed host boundaries remain.

Decision 0367 defines `WVNF 1.0` for those variable-input loaders. The target-specific derived artifact preserves final base-independent code plus every target, ABI, symbol, patch, nominal-type, and required-service field consumed by the existing fragment verifier. It avoids treating raw code as trusted and avoids rebuilding a fragment from WVO plus hardcoded metadata. The first managed codec and malformed corpus are a migration seam: production consumers and a later native decoder must both call the same verifier contract, while canonical WVB remains portable recovery and differential evidence.

Decision 0368 promotes that seam for segmented enum metadata and both publication planners. Their exact source WVBs stay in the repository, but the normal native runtime embeds only the three digest-bound WVNF fragments and contains no WVB helper/generator resource. First use still performs complete format and native-fragment verification, then executes the same variable-input contracts. This removes managed helper WVB decoding, semantic verification, and x86-64 lowering from normal startup without removing the application WVB trust boundary or final managed host ownership.

Decision 0369 removes the next live C# machine-code generator from service-bundle construction. Two focused portable Windvale modules now construct the exact Windows and Linux console/diagnostic output leaves through the shared native service-code builder; one source-reproducible bridge returns all four. The normal runtime embeds only their unchanged digest-bound `.bin` artifacts and selects them by platform and service. C# no longer owns output-leaf instruction emission or branch patching, while platform output-table binding, bundle assembly, W^X publication, and invocation remain host-adapter work.

Decision 0370 applies the same ownership boundary to the larger `file.write_bytes` adapters. Shared, Windows, and Linux Windvale modules now construct the unchanged 787-byte Windows and 823-byte Linux leaves, and a native-front-door-reproducible bridge returns both. Cohesive entry-validation and open-emission helpers keep individual generated functions inside the bounded native frame instead of growing one monolithic source function. The normal runtime embeds only the two digest-bound `.bin` leaves; C# retains platform `WVFO` table binding and exact artifact selection but no longer emits or patches file-output instructions.

Decision 0371 completes platform-leaf source transfer for `file.read_bytes`. Shared and platform-focused Windvale modules construct the unchanged 1,218-byte Windows and 996-byte Linux adapters, including cache lookup, snapshot publication, bounded reads, and stable error mapping. Named emission phases keep individual functions reviewable while each platform module retains its complete label/patch invariant. The normal runtime embeds only the two digest-bound `.bin` leaves; C# retains `WVFI` table binding and exact artifact selection but no longer emits or patches file-input instructions.

Decision 0372 moves the next layer above those leaves. One capability-free portable Windvale fragment consumes an exact `WVPQ` plus the verified fragment/leaf payload and constructs every service bundle whose complete request and response fit the ordinary 4 MiB byte-value contract. It owns unchanged fragment and leaf copying, zero-filled initial alignment, and NOP-filled later alignment; the host independently checks every output byte. The narrow bootstrap is renamed for its actual service-free role and publishes only internally selected digest-bound fragments. Compiler-scale images remain on an explicitly named Stage 0 fallback until segmented construction removes it; adapter metadata, service tables, W^X authority, contexts, arenas, invocation, and teardown remain host work.

Decision 0373 removes that compiler-scale fallback without widening Windvale's 4 MiB byte values. Canonical 4,194,104-byte image segments each carry the complete publication plan plus only their intersecting fragment/leaf source ranges. Windvale derives and constructs every zero/NOP region, while a focused managed session invokes segments in order, independently validates their bytes, and concatenates them. Small and large bundles now use the same Windvale-owned constructor; adapter metadata, service tables, W^X authority, contexts, arenas, invocation, and teardown remain host work.

Decision 0374 moves the first dynamic binding table. A capability-free Windvale constructor accepts already acquired console/diagnostic targets plus the Windows writer as opaque byte ranges, validates platform/flag/range rules, and returns the unchanged 48-byte `WVIO` table. The host still pins handles, resolves `WriteFile`, independently verifies and copies the result, and owns teardown. `WVFI`/`WVFO`, the service table, execution context, W^X authority, arenas, invocation, and teardown remain host work.

Decision 0375 applies the same bounded pattern to the 80-byte `WVFO` table. Windvale validates the scratch target/capacity and six Windows-function presence rules, then constructs the exact table from opaque target bytes. The host retains scratch allocation, library/export acquisition, independent verification/copy, and teardown. `WVFI`, the service table, execution context, W^X authority, arenas, invocation, and teardown remain host work.

Decision 0376 completes the three specialized binding-table constructors with the 136-byte initial `WVFI` table. Windvale validates four opaque arena targets, exact snapshot/name/data/scratch limits, zero initial mutable state, and seven Windows-function presence rules before returning the unchanged table. The host retains sparse arena allocation, library/export acquisition, independent verification/copy, post-execution snapshot count/record/name validation, and teardown. The shared service table, execution context, W^X authority, arenas, invocation, and teardown remain host work.

Decision 0377 moves the shared 104-byte service-table version 5 constructor. Windvale validates a nonempty closed twelve-service mask and the exact required-target/absent-zero relation, then emits version, size, and all slots from opaque published targets. The managed executor no longer writes table fields or maps services to byte offsets; it retains executable publication, placement-to-address calculation, independent response verification/copy, invocation, and teardown. The execution context, W^X authority, arenas, invocation adapter, result admission, and teardown remain host work.

Decision 0378 moves the ordinary 112-byte execution-context version 7 constructor. Windvale validates exact budgets, arena bounds, zero mutable state, arguments, and optional table-presence relationships before returning the complete initial context. A focused owner independently verifies/copies those bytes and admits only the three specified post-call mutations. Because every retained Windvale constructor itself needs a context, the explicitly service-free bootstrap lane keeps one frozen Stage 0 byte oracle and compares it with Windvale; normal application execution no longer uses the C# writer. Replacing that bootstrap oracle, W^X allocation authority, arena allocation, invocation, result admission, and teardown remain open host slices.

Decision 0379 returns to ABI 12's immutable argument snapshot and moves its remaining descriptor writer. A bounded Windvale constructor consumes prevalidated packed-payload evidence plus opaque per-entry targets, validates count/length/offset/coverage rules, and constructs every exact 16-byte pointer/length/reserved descriptor. The host retains strict UTF-8 validation, packing and address projection, independently rereads every descriptor/range/byte, and owns lifetime. C# no longer writes argument-table fields; arenas, entry/result bridge cells, invocation, W^X platform authority, and teardown remain host work.

Decision 0380 moves the native function-entry bridge shared by parameterless and byte-input descriptor results. Windvale validates one opaque optional input target and bounded length, then constructs the exact zero result cell plus immutable input descriptor. A focused host owner verifies and copies the bytes, permits only the result cell to change, and returns the parsed descriptor to the existing independent range admission step. Normal application execution no longer writes or directly decodes bridge fields in the large executor. Service-free constructor execution retains one explicit frozen Stage 0 bridge oracle to avoid recursion; entry-input allocation, result copying, arenas, invocation, W^X platform authority, and teardown remain host work.

Decision 0381 moves that independent result-range admission policy. A bounded Windvale constructor receives the untrusted descriptor plus verified live arena, optional entry-input, and immutable static-data ranges, then performs exact unsigned containment using two `u32` limbs. The host verifies the response and unchanged descriptor before copying the admitted bytes. The large executor no longer contains range arithmetic. Service-free constructor execution retains the frozen former algorithm only as a bootstrap oracle; real-memory copying, arena and input allocation, invocation, W^X platform authority, and teardown remain host work.
...
Decision 0526 completes that promotion. Exact commit
`e5a1a7473c57935c5dfcf09b78b18c3c099e70ef` passes all six jobs and the
required gate, one selected Stage 0 bundle reconstructs on both hosts, and the
direct-entry inventory reports zero normal plus nine recovery-only managed
owners. T2 and G1 are `native-qualified` for the accepted subset; later feature
breadth must add focused native evidence without restoring a managed normal
fallback.
