...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0076 cross-host qualify the shared Stage 0 and first OS consumer seams through ABI 14, native leaves for all eleven service slots, and firmware probe 16. Decision 0077 cross-host qualifies the first active WVA-authored, Windvale-assembled copy-and-patch runtime artifact. This document defines the larger native destination and migration boundaries; it does not claim an in-guest WVB loader, a general Windvale-owned native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
...
Decision 0078's measured extension adds the exact 70-byte `process.argument` template with eight strictly ordered one-byte locations and six named ABI meanings. Repeated meanings share one checked value, while the WVO header fixes patch count and template size. The live runtime consumes the Windvale-assembled object without changing the leaf digest. This establishes the minimum multi-patch model but still deliberately excludes branch-target patching, calls, relocations, data references, template selection, native Windvale validation, and executable-memory ownership.
...
Decision 0076 cross-host qualifies ABI 14 and context version 6 at exact commit `ef08619`. A runtime-private 136-byte `WVFI` table owns bounded path scratch, 64 canonical name/data snapshot slots, and the narrow Windows function-pointer boundary. Exact Windows and Linux leaves perform file input without a managed delegate, preserve first-success immutable snapshots, and map native failures back to stable `WVR302x` details. All eleven service-table slots now have qualified native leaves. Windows and Debian reports and all 61 portable artifacts agree, both OS suites pass, GitHub passes independently, and pinned-QEMU probe 16 passes on Windows. Decision 0077 then qualifies the move of the smallest live leaf's template and typed patch description into a Windvale-assembled WVO while preserving its exact final bytes. C# Stage 0 still validates and patches that artifact, constructs the remaining leaves and tables, allocates, publishes, invokes, maps, and releases the runtime artifacts.

A post-qualification Windows candidate admits borrowed text/byte descriptors inside the existing 16-byte record cells and directly lowers the bounded `Textˉtoˉutf8`, `Bytesˉfromˉu32ˉlittle`, and `Bytesˉconcat` operations needed by the Windvale-written Project 1 parser. Fragments containing descriptor records add independently checked nominal-type tags; scalar-only record fragments retain their previous bytes. Dynamic byte construction shares the existing execution arena and failure channel, so ABI 14, context version 6, and service-table version 4 do not change. The project shell agrees across interpreter, JIT, and linked WVO/AOT over real files on Windows, but this extension remains candidate evidence until the proportional Windows/Linux gates are reported from one exact commit.

## Native runtime ABI

Generated code targets a Windvale-owned internal ABI rather than emitting host calls throughout ordinary functions. The ABI-13 entry receives a pointer in `RDX` to the exact [native execution context](../../Specifications/Windvale-Native-Execution-Context.md). One identical `Main` preserves that pointer in `R15` and loads the versioned instruction/depth budgets into reserved `R11` and `R10`. Internal functions accept as many as four scalar, enum, record-offset, or borrowed-text/byte parameters. Scalars, enums, and records use `R8D`, `R9D`, `ECX`, and `EDX`; borrowed descriptors use the corresponding 64-bit register as a pointer to a caller descriptor that the callee copies. Scalar/enum/record results and packed statuses return in `RAX`.

The context's optional service-table pointer is the only generated-code route to host or runtime support. ABI 14 retains the closed version-4 table for console/diagnostic output, bounded argument/file input, UTF-8 validation, enum naming, integer formatting, concatenation, and quoting; only capability services require authorization. All eleven slots use exact native leaves. Platform-specific output and file input read independently verified runtime-private tables, while the remaining leaves are platform-neutral. WVO 1.0 does not serialize fragment service or nominal requirements, so a linked service-bearing image is not independently loadable without its verified fragment metadata. Host-returned text/bytes, text-arena values, argument-table values, file snapshots, and record-arena values are execution-owned and cannot escape the run. This remains a bounded experimental convention, not the final stack-argument, general-allocation, asynchronous-I/O, or safe-point ABI.
