...
The context's optional service-table pointer is the only generated-code route to host or runtime support. ABI 11 retains the closed version-4 table for console/diagnostic output, bounded argument/file input, UTF-8 validation, enum naming, integer formatting, concatenation, and quoting; only capability services require authorization. UTF-8 validation, concatenation, and integer formatting use exact platform-neutral native leaves. Managed enum/quote services share the context-owned text cursor, while hosted services retain runtime-owned platform thunks. WVO 1.0 does not serialize fragment service or nominal requirements, so a linked service-bearing image is not independently loadable without its verified fragment metadata. Host-returned text/bytes, text-arena values, and record-arena values are execution-owned and cannot escape the run. This remains a bounded experimental convention, not the final stack-argument, general-allocation, asynchronous-I/O, or safe-point ABI.
