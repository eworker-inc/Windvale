...
Locally implemented [Decisions 0136](../Decisions/0136-Exact-Compiler-Dynamic-Value-Pressure.md) and [0141](../Decisions/0141-Exact-Compiler-Dynamic-Value-Lifetime.md) measure the next boundary without changing ABI 21. The successful exact compiler constructs 902,262,268 flat dynamic bytes cumulatively but reaches an ideal peak of 9,030,829 bytes across 17 typed backing identities, including input/output copy overlap, and releases every root by completion. This fits the retained 16 MiB capacity with 7,746,387 bytes before metadata and fragmentation. The next ABI candidate must first replay a concrete bounded reclaiming allocator and then independently verify generated ownership operations; the measurement alone does not qualify native self-hosting or a permanent collection mechanism.
