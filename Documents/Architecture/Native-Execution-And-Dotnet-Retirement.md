...
Decision 0213 deliberately separates the earlier semantic-freeze checkpoint from final retirement. Once the current WVB 1.11 candidate is qualified, the C# source compiler stops receiving new language features while remaining correctable recovery evidence. The qualified native build driver and verifier then replace the ordinary source-to-verified-WVB entry point after exact artifacts, provenance, and atomic publication are available. Native backend, runtime, test, packaging, and final archive work continue under the complete Decision 0057 gate.
