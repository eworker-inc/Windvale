...
Accepted architectural direction under [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md). Decision 0058 qualifies bytecode compiler self-reproduction. Decisions 0059 through 0061 cross-host qualify the first shared Stage 0 constant, checked-arithmetic/trap, typed-block, comparison, and forward-control slices through WVO/AOT and Windows/Linux W^X paths. Decision 0062 adds dynamic instruction budgets and backward control on Windows; exact-commit Debian qualification is pending. This document defines the larger native destination and migration boundaries; it does not claim calls, static data, capabilities, a general native runtime, broad JIT or AOT compiler, PE host, ELF host, garbage collector, or native self-hosting chain.
