...
Decision 0080 adds one focused boundary case for the capability-free descriptor entry and live stencil consumption. The case recompiles and matches the retained WVB, compares reference/JIT/linked-WVO bytes, corrupts the entry-result prologue, rejects wrong executor kinds and hosted byte entries, covers static and arena result provenance, and observes the live 5/70 service split. The retained consumer executes once per process through a thread-safe lazy owner; later native executions reuse only its exact immutable 75-byte result, avoiding a new per-call compiler/JIT cost. The first complete Windows Release Development gate passes all 61 regular tests in 57.456 seconds; the warm bridge case takes 0.223 seconds. Exact cross-host Qualification remains the milestone gate and will be recorded after the candidate is committed.
