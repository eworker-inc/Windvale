...
The qualified probe now boots through firmware shutdown, owns a 64 KiB arena and 8 KiB stack, and runs one ordinary portable WVB module through the shared ABI-14 AOT path before compiler-generated system source completes. [Decision 0081](../Decisions/0081-First-Terminal-X64-Cpu-Exception-Boundary.md) adds an implemented, qualification-pending probe-17 candidate: the first allocated page owns one vector-6 gate, the ordinary image reports `cpu-exceptions=armed`, and an explicit `UD2` image reaches a fixed terminal panic handler. The handler's QEMU exit is negative test transport, not a platform lifecycle API.
