...
Accepted architecture direction, refined by [Decision 0057](../Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md), [Decision 0140](../Decisions/0140-Per-Module-Platform-Scope-And-Filesystem-Capabilities.md), future virtualization/accelerator [Decision 0171](../Decisions/0171-Future-Virtualization-And-Accelerator-Architecture.md), process/service/driver [Decision 0173](../Decisions/0173-Windvale-Process-Service-And-Driver-Architecture.md), language/metadata [Decision 0179](../Decisions/0179-Language-Application-And-Capability-Metadata-Direction.md), OS-mechanism [Decision 0181](../Decisions/0181-Next-Windvale-Os-Mechanism-Contracts.md), and product-evolution [Decision 0183](../Decisions/0183-Product-Packaging-Trust-And-Evolution.md). Current implementation status remains governed by the corresponding format, runtime, native-target, and OS specifications.
...
This is now a functioning but deliberately tiny service and interpreter proof, not a complete kernel or general runtime/trap/process system. Cross-host-qualified Probe 39 has three fixed protected processes, separate resource and directory endpoints, a bounded ready/wait dispatcher, and one private pinned-Q35 HPET/local-APIC four-interrupt preemption proof. Probe 40 adds the implemented candidate for releasing and reusing one generation-safe client memory object while a later directory object remains live. It does not establish a general timer scheduler or physical-memory manager. Capability transfer, general loading, JIT publication, resource domains, dynamic launch, a resource namespace, page-fault policy, double-fault containment, IST, general interrupt-controller policy, supervision, and platform lifecycle coordination remain absent. Windvale `WVR` runtime traps remain packed semantic statuses and are not redefined as CPU faults.
...
The first accepted hosted boundary supplies an immutable launcher argument snapshot, a bounded opaque-name-to-bytes file read, deterministic standard output, and a separate diagnostic sink. Native adapters own path resolution and native errors. See [Hosted-Resources.md](../../Specifications/Hosted-Resources.md). The first platform library uses that read leaf to acquire an immutable `WVRS 1` image while keeping format policy in capability-free Foundation. These current tool-oriented leaves are bootstrap evidence, not the complete future application or filesystem API.
...
The first common core is limited to operations whose signatures and semantics can be stated exactly across providers: opening relative to a granted directory, bounded reads and writes at explicit offsets, length change, and close. Source-facing members follow Windvale naming, such as `Readˉat`, `Writeˉat`, and `Setˉlength`; machine-facing capability and protocol names remain ASCII-safe. Open and create dispositions, link-following policy, access rights, and collision behavior must be explicit. Implicit shared file cursors, native sharing flags, advisory locks, and mandatory locks are not assumed common semantics. Queries, directory operations, enumeration, flush strength, rename, removal, atomic replacement, watching, links, permissions, mapping, sparse storage, and transactions are separate interfaces until their exact guarantees are selected. Package resources, mutable application storage, and native host files remain different concepts even when one host adapter uses files to implement more than one of them.
