...
This is now a functioning but deliberately tiny protection proof, not a complete kernel or general trap/process system. It has no scheduler, second process, capability transfer, general loader, init/resource service, page-fault policy, double-fault containment, IST, interrupt controller, recovery, or general platform lifecycle coordination. Windvale `WVR` runtime traps remain packed semantic statuses and are not redefined as CPU faults.
