...
This is now a functioning but deliberately tiny service and interpreter proof, not a complete kernel or general runtime/trap/process system. Cross-host-qualified Probe 38 has three fixed protected processes, separate resource and directory endpoints, and a bounded ready/wait dispatcher. It still has no timer preemption, general scheduling, capability transfer, general loader, JIT publication, resource domain, dynamic launch, resource namespace, page-fault policy, double-fault containment, IST, interrupt controller, supervision, or general platform lifecycle coordination. Windvale `WVR` runtime traps remain packed semantic statuses and are not redefined as CPU faults.
