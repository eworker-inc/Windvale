...
The qualified probe now boots through firmware shutdown, owns a 64 KiB arena and 8 KiB stack, and runs one ordinary portable WVB module through the shared AOT path before compiler-generated system source completes. [Decision 0081](../Decisions/0081-First-Terminal-X64-Cpu-Exception-Boundary.md) cross-host qualifies probe 17 and ABI 14/context 6 at exact commit `ba2cf69`: the first allocated page owns one vector-6 gate, the ordinary image reports `cpu-exceptions=armed`, and an explicit `UD2` image reaches a fixed terminal panic handler. Both exact images pass pinned QEMU. Candidate [Decision 0085](../Decisions/0085-First-Wva-Owned-Q35-Clean-Shutdown.md) adds the WVA-owned Q35 shutdown path in probe 18; candidate [Decision 0086](../Decisions/0086-First-Wva-Owned-Normalized-X64-Trap-Entries.md) adds normalized vector-6/vector-13 entries in probe 19. [Decision 0087](../Decisions/0087-Native-Windows-And-Linux-File-Output.md) composes probe 20 for ABI 15/context 7 with a zero file-output-table pointer; its exact committed-archive and pinned-QEMU qualification remains pending. The handler's QEMU exit is negative test transport, not a platform lifecycle API.
