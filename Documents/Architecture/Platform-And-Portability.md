...
This is now a functioning but deliberately tiny service and interpreter proof, not a complete kernel or general runtime/trap/process system. Cross-host-qualified Probe 38 has three fixed protected processes, separate resource and directory endpoints, and a bounded ready/wait dispatcher. Implemented-candidate Probe 39 adds only a private pinned-Q35 HPET/local-APIC four-interrupt preemption proof; it does not yet establish a general or qualified timer scheduler. Capability transfer, general loading, JIT publication, resource domains, dynamic launch, a resource namespace, page-fault policy, double-fault containment, IST, general interrupt-controller policy, supervision, and platform lifecycle coordination remain absent. Windvale `WVR` runtime traps remain packed semantic statuses and are not redefined as CPU faults.
