...
This conceptual model is accepted. Candidate [protected process version 7](../../Specifications/Windvale-Protected-Process.md) implements one deliberately fixed internal representation: separate init and interpreter roots, role-specific W^X extents, explicit module/resource identities and budgets, one init-owned RO/NX page, one checked immutable client alias with automatic terminal cleanup, reduced rights, one kernel-owned capacity-one result channel, waiting state, and closed lifecycle/fault/result evidence. That representation is implementation evidence, not yet a stable public process ABI. Qualified version 6 remains the cross-host baseline.
...
Protected-process version 7 retains the first measured mechanics: `EBX` numbers 1/2/3 select send/receive/exit and number 4 selects the single boot-resource grant; `ESI` carries reference 65536; `EAX` carries the message/result or resource identifier. It preserves the ABI-16 context pointer in `RDX`, records role-specific extents and identities, validates one `WVRES002` owned-to-borrowed transition, and automatically removes the alias/private publication when the fixed borrower becomes terminal. This assignment is explicitly experimental and internal. It is not general capability transfer or a public revocation API. Public user-ABI stability, larger-message encoding, copy-versus-map thresholds, ownership migration, root/page reclamation, peer lifecycle, scheduling, and backpressure remain deferred.
...
The first x86-64 path remains UEFI-based and evidence-driven. Qualified probe 24 implements steps 1 through 5 plus the first bounded part of step 6 by interpreting one admitted WVB in a CPL3 Windvale runtime process. Qualified probe 25 derives and validates the admitted module's section payloads. Qualified probe 26 separates the WVB from the interpreter RX image. Qualified probe 27 makes Windvale init select resource `1` and grants one immutable alias before client entry. Candidate probe 28 removes that alias and private publication after client exit or contained fault:
