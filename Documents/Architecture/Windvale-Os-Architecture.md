...
7. Generalize one measured process boundary at a time. The independent endpoints, statically constructed third service, state-driven single-CPU ready/wait dispatcher, and Probe 39's private timer-preemption experiment are qualified; Probe 40 adds the implemented candidate for the first independently lived memory objects.
