...
The first x86-64 path remains UEFI-based and evidence-driven. Qualified probes 24 through 31 establish interpretation, section-derived validation, runtime-supplied WVB, init ownership/grant, terminal cleanup, typed two-resource publication, generation-safe root reuse, and one exact nontrivial WVB across Windows, Linux, and Windvale OS. Implemented Probe 32 adds a second existing compiler fixture, broader scalar/control-flow interpretation, and build-time native-stack proof:
