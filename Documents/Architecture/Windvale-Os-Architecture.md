...
Implemented-candidate [Decision 0165](../Decisions/0165-Contained-Windvale-Service-Failure.md) retains ABI 22 and `WVKMEM14` while advancing to Probe 36, `WVPROC15`, and `WVCHAN04`. All 38 focused OS tests and five pinned Windows QEMU scenarios pass locally. The fifth scenario proves that a malformed live request can terminate init, close and scrub the channel, wake its blocked client with exact peer-loss status, revoke the client's resources, and still reach deterministic shutdown. Cross-host qualification remains pending; no general service supervision or compiler change is claimed.
