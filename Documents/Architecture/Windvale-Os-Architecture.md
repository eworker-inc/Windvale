...
Implemented-candidate [Decisions 0126](../Decisions/0126-First-Read-Only-Resource-Store.md) and [0129](../Decisions/0129-Bounded-Resource-Service-Request-Reply.md) add the first post-Probe-32 resource pressure without changing that qualified guest. `WVRS 1` owns deterministic typed lookup; one-page `WVRQ 1` / `WVRY 1` owns correlation, limits, canonical failures, and copied inline results; a format-blind exchange oracle owns directional rights and terminal cleanup. Portable Windvale and an independent Stage 0 handler agree through a live hosted lookup. Moving this path into the guest still requires a versioned channel/process ABI, checked user-buffer copies, an immutable store capability, service-death behavior, and QEMU evidence.

Implemented [Decision 0133](../Decisions/0133-Frame-Owned-Direct-Native-Records.md) rebuilds Probe 32 through the single ABI-21 backend. The exact WVB, 199-instruction result, `WVPROC11`, resource lifecycle, fault behavior, and six-page stack envelope remain unchanged. The process builder now consumes the compiler's projected frame plan: `Executeˉmain` needs 755 cells and the deepest call path needs 24,240 bytes. Explicit direct-record copies grow the linked client to 445,085 bytes and 109 RX pages, while record-arena use falls from 528 to zero. `WVKMEM11` therefore supplies a 120-page client root inside a 141-page arena. All 31 integrated OS tests and four pinned Windows QEMU scenarios pass; cross-host qualification is pending.
