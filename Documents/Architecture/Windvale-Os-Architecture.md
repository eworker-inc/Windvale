...
Implemented [Decision 0105](../Decisions/0105-Typed-Block-Scoped-Native-Value-Slots.md) is the ABI-18 qualification candidate selected from that measured stack and image pressure. It preserves Probe 32's WVB, interpreter behavior, process format, paging, resources, and serial contracts while reusing exact-type physical value cells across verified empty-stack blocks. `Executeˉmain` now needs 745 actual frame cells, the complete call graph needs 23,824 bytes in a minimal six-page stack, and the client needs 102 code pages. `WVKMEM10` therefore shrinks the reclaimable client extent from 161 to 113 pages and the bounded kernel arena from 182 to 134 pages. All local OS and pinned-QEMU scenarios pass; cross-host qualification remains pending.
