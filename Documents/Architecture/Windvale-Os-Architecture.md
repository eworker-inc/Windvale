...
[Decision 0108](../Decisions/0108-Native-One-Byte-Construction.md) implements the shared backend's ABI-19 one-byte-construction candidate without changing Probe 32's reachable operation set, WVB, process/memory formats, stack proof, or firmware bytes. All 25 local OS tests and all four exact pinned-QEMU scenarios retain their qualified identities. Cross-host qualification remains pending, so ABI 18 / `WVKMEM10` remains the latest qualified OS baseline.
