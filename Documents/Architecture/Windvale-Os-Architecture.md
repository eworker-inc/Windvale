...
Locally verified [Decision 0172](../Decisions/0172-First-Kernel-Owned-Service-Endpoint.md) retains the same arena, ABI, WVA wire values, resources, and five scenarios while advancing to Probe 37, `WVPROC16`, and `WVENDP01`. All 38 OS tests and all five pinned Windows QEMU scenarios pass. The candidate replaces raw per-process channel pointers with exact endpoint resolution, rebinds the second client generation, and closes provider availability on normal exit or fault. Cross-host qualification is pending.
