...
The next physical-memory and accounting direction is detailed in [Memory objects and resource domains](Memory-Objects-And-Resource-Domains.md). Qualified Probe 40 supplies the fixed deterministic page bitmap, ownership, generation, committed-object, zero-before-reuse, and non-tail-reuse baseline. The current native candidate integrates portable resource-domain policy 1 and immutable launch transactions for both sequential generations of one fixed known child, including distinct checked machine layouts and unpublished rollback. It does not yet provide a public encoding, arbitrary image admission, runtime allocation/mapping, or general supervision. The guide retains separate rights-reduced mappings and recovery capacity outside ordinary budgets as successor contracts. The staged launch and supervision boundary is detailed in [Process launch and supervision](Process-Launch-And-Supervision.md).
...
Cross-host-qualified protected-process version 17 retains internal syscall numbers 1 through 7 for the fixed send, receive, exit, grant, service-receive, call, and reply experiment. It keeps ABI 22/context 7, three fixed process records, two capacity-one channels, four fixed resources, and two exact client generations. Separate resource and directory capability references resolve through two `WVENDP01` records before channel or resource mutation. The retained dispatcher scans the three validated records from a persistent cursor and selects only ready work. Qualified Probe 39 precedes that workload with a separate fixed involuntary-preemption experiment and does not change these syscall or `WVPROC17` contracts. Qualified Probe 40 replaces tail-order reclamation with three fixed generation-safe memory objects without changing the process record or syscall contract. Its current Windvale policy admits a versioned start request from init and a generation-safe executable-publication reference before deriving each child identity. The architecture-neutral user-copy policy now bounds and snapshots the exact request and checks its encoded caller against a separately supplied current-caller identity, but no x86-64 live-user-page adapter or start syscall reaches that policy yet. These register assignments and record encodings are replaceable internal evidence. They are not a public ABI, dynamic object table, capability-transfer interface, process manager, general scheduler, or backpressure design.
...
Implemented-candidate [Decision 0615](../Decisions/0615-First-Boot-Service-Composition-Admission.md) adds a small portable selection gate to the current process-policy object. Probe 40 token 97 now depends on exact filesystem, bounded-operation queue, and network authority version/limit values whose complete semantics remain independently owned. This records the boot dependency without moving paths, filesystems, packet processing, transport, or service policy into the kernel; dynamic checked provider launch and IPC/resource binding remain required.

Implemented-candidate [Decision 0619](../Decisions/0619-Boot-Embedded-Os-Provider-Process-Images.md) adds the separately linked filesystem and network user images to the Windvale-owned process object as immutable read-only launch inputs. The expanded normal EFI completes the pinned QEMU/OVMF boot contract, but the established architecture fixture does not yet allocate, map, launch, bind, or supervise either provider. Embedding is not authority and does not make their service protocols live.

Implemented-candidate [Decision 0620](../Decisions/0620-First-Checked-Os-Provider-Launch-Transaction.md) composes the exact `WVPR 1` filesystem/network profiles with generation-one resource domains, exact linked-image geometry, W^X/private construction evidence, endpoint binding, readiness-only publication, stale rejection, active-work drain, rollback, and zero-charge teardown. This is the portable policy gate for the next machine change; at that checkpoint the three-process architecture fixture did not instantiate either provider.

Implemented-candidate [Decision 0729](../Decisions/0729-Privately-Construct-The-Filesystem-Machine.md) executes the first checked machine half of that policy. After terminal generation-two client cleanup, WVA releases its 122-page allocation, first-fits an 85-page generation-three object at the same root, and calls the source-owned image, W^X-paging, record, and context constructors. Live QEMU exposed and closed two integration defects: raw constructor fragments needed explicit callable returns, and the record needed process generation 3. The resulting machine remains private and non-runnable; endpoint generation advance, resource-domain charge, fresh thread publication, provider entry, one FAT32 read, and rollback/teardown remain the next boundary.

Implemented-candidate [Decision 0730](../Decisions/0730-Publish-Provider-Side-Filesystem-State.md) advances that machine without inventing a consumer. The record now starts private and binds the exact admitted launch-request digest. WVA advances endpoint slot 0 to generation 2 with provider `196610` and client 0, reconstructs its empty capacity-one channel, creates a fresh generation-three thread, and commits endpoint, thread, then process ready state. Client 0 conveys no application authority, the boot path does not dispatch the provider, and no durable domain ledger or FAT32 media identity is claimed. Consumer/domain binding, first entry and request, complete rollback, and teardown remain one later transaction.

Implemented-candidate [Decision 0731](../Decisions/0731-Publish-Durable-Filesystem-Domain-Ledger.md) adds the missing live accounting without widening the fixed state page. After validating every terminal directory-endpoint field, WVA reuses only that closed 64-byte slot as `WVDOM001`, binds domain `65538` to filesystem process `196610`, and commits exact use `1 process / 81 user pages / 1 endpoint` before endpoint, thread, and process readiness. The retained directory channel is untouched. This fixed ledger grants no capability: client 0, provider entry, FAT32 media binding, the first request, full rollback, and teardown remain pending.

Accepted [Decision 0732](../Decisions/0732-Select-The-First-Filesystem-Consumer-Machine.md) selects the next exact reuse without claiming it is live. Exited init is rejected as stale; terminal directory process/memory identifier 3 advances to generation 2 (`131075`) as a ten-page consumer in separate domain `65540`. The application receives rights 17 to provider-owned endpoint `131072` and is charged `1 process / 6 user pages / 0 endpoints`. Exact retired ledger storage, private construction, atomic binding, entry, and the first read remain pending.

Implemented-candidate [Decision 0621](../Decisions/0621-First-Windvale-Owned-Process-Machine-Code-Emission.md) begins that machine change at the recovered source boundary. A bounded Windvale module now owns exact x86-64 byte emission, numeric labels, relative fixups, placeholders, and alignment for one 65,536-byte construction. Its first consumer source-owns the exact 1,119-byte coordinator entry and fixed three-record ready/wait dispatcher. Decisions 0622 through 0635 advance coordinator state, retained IPC records, checked allocation, init and provider/client records, private W^X paging, measured images, and native contexts through byte 9,682. Decisions 0636 through 0639 then construct separate program, budget, immutable-store, and read-only-directory resources through byte 10,637. Decisions [0640](../Decisions/0640-Windvale-Owned-Client-Store-Validation.md) and [0641](../Decisions/0641-Windvale-Owned-Client-Directory-Validation.md) validate both descriptors through byte 11,441. [Decision 0642](../Decisions/0642-Windvale-Owned-Privileged-Entry-Setup.md) adds exact GDT/TSS construction, four IDT gate relocations, processor admission, and syscall-MSR setup through byte 12,082. Decisions 0643 through 0646 add private thread/timer state, fail-closed timer activation, guarded provider entry, and checked provider-return/init transfer. [Decision 0649](../Decisions/0649-Windvale-Owned-Init-Return-And-Program-Validation.md) adds init-return program-resource validation, [Decision 0650](../Decisions/0650-Windvale-Owned-Remaining-Client-Resource-Validation.md) closes budget and retained store/directory backing validation, [Decision 0652](../Decisions/0652-Windvale-Owned-Client-User-Context-Transfer.md) enters the admitted client context, [Decision 0653](../Decisions/0653-Windvale-Owned-Client-Return-And-Init-Resume.md) returns it to init, [Decision 0654](../Decisions/0654-Windvale-Owned-Init-Reply-Publication-Resume.md) completes init's 116-byte reply publication, [Decision 0656](../Decisions/0656-Windvale-Owned-Client-Reply-Delivery.md) delivers it to the client, [Decision 0657](../Decisions/0657-Windvale-Owned-Client-Directory-Request-Delivery.md) transfers the client's exact 37-byte request to the isolated directory provider, [Decision 0659](../Decisions/0659-Windvale-Owned-Directory-Reply-Publication-Resume.md) publishes the exact 3,096-byte reply and resumes that provider, [Decision 0660](../Decisions/0660-Windvale-Owned-Client-Directory-Reply-Delivery.md) delivers it to the client, [Decision 0662](../Decisions/0662-Windvale-Owned-First-Client-Completion-Cleanup.md) validates and scrubs its terminal IPC state, [Decision 0663](../Decisions/0663-Windvale-Owned-Client-Reclamation-Preflight.md) revalidates retained state before release, [Decision 0665](../Decisions/0665-Windvale-Owned-Client-Memory-Recycle.md) releases generation 1 and checks same-root generation-2 allocation, [Decision 0666](../Decisions/0666-Windvale-Owned-Generation-Two-Client-Record.md) privately reconstructs the recycled client record, [Decision 0667](../Decisions/0667-Reuse-Checked-Paging-For-Generation-Two-Client.md) reuses its exact private paging constructor, [Decision 0669](../Decisions/0669-Reuse-Checked-Image-For-Generation-Two-Client.md) reuses its interpreter-copy/context-seed constructor, [Decision 0670](../Decisions/0670-Windvale-Owned-Generation-Two-Endpoint-Rebind.md) validates and rebinds both retained endpoints, [Decision 0671](../Decisions/0671-Windvale-Owned-Generation-Two-Reentry.md) validates recycled memory and returned generation state, binds the resume context, completes resource accounting, restores user registers, and executes the first generation-2 `sysretq`, [Decision 0672](../Decisions/0672-Windvale-Owned-Generation-Two-Return-Validation.md) validates the resumed processor, resource, mapping, alias, and context-record state, [Decision 0675](../Decisions/0675-Adapt-Checked-User-Transfer-For-Generation-Two.md) reuses the checked user transfer with only the selected generation changed to 2 and executes the second generation-2 `sysretq`, [Decision 0676](../Decisions/0676-Adapt-Checked-Client-Return-For-Generation-Two.md) reuses the checked return-to-init contract with only the returning generation changed to 2 and delivers result 55, [Decision 0677](../Decisions/0677-Adapt-Checked-Init-Reply-For-Generation-Two.md) adapts reply publication for operation 7 and retained generation 2 and returns zero to the client, [Decision 0678](../Decisions/0678-Adapt-Checked-Reply-Delivery-For-Generation-Two.md) adapts operation 8 plus retained, thread, and selected client generation 2 while preserving result 116 and the final `sysretq`, and [Decision 0680](../Decisions/0680-Adapt-Checked-Directory-Request-Delivery-For-Generation-Two.md) selects directory-provider generation 2 while preserving exact 37-byte request delivery and the provider `sysretq`, bringing source ownership to byte 28,467 with 290 relocation fields. These slices do not yet replace the 46,678-byte process fixture; provider receive/reply, later lifecycle, remaining syscall and exception handlers, context switching, and live QEMU application execution remain separate migration slices.

[Decision 0681](../Decisions/0681-Adapt-Checked-Directory-Reply-For-Generation-Two.md)
then adapts operation 4 and selected provider generation 2 while preserving the
exact 3,096-byte reply, zero resume, and provider `sysretq`. Current source
