...
[Decision 0153](../Decisions/0153-First-Versioned-Read-Only-Directory-Capability.md) defines that first typed filesystem instance above the kernel: one authorization binds one immutable directory snapshot, a Windvale platform library validates a single segment and decodes exact 3 KiB read-at chunks, and ordinary provider/lifecycle failures remain typed. Stage 0 currently owns strict provider invocation and `WVDR 1` envelope validation. The next Windvale OS slice must bind the same semantics to a checked runtime adapter and isolated service endpoint while keeping the kernel format-blind; it must not reinterpret Probe 34's package-resource `WVRS 1` store as a directory.
