...
This implementation is the qualified Windvale bytecode compiler: Stage 0 builds Stage 1, and Stage 1 reproduces the exact Stage 2 WVB from the committed 12-module inventory on Windows and Debian. Cross-host-qualified ABI 22 also runs Stage 1 as verified native x86-64 and reproduces the same Stage 2 bytes under a bounded host arena. The accepted standalone native compiler/tool packaging and ordinary front doors are qualified; later operations and profiles remain consumer-driven breadth.
...
The C# object verifier returns a `Verifiedˉobject` and remains an independent recovery/differential oracle. Digest-bound native WVO verification and inspection launchers are the qualified ordinary entry points after the completed Windows/Linux retirement gate.
