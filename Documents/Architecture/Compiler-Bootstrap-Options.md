...
[Decisions 0136](../Decisions/0136-Exact-Compiler-Dynamic-Value-Pressure.md) and [0141](../Decisions/0141-Exact-Compiler-Dynamic-Value-Lifetime.md) measure the remaining dynamic boundary on the successful reference bootstrap. Although flat immutable construction totals 902,262,268 bytes, only 17 unique backings and 9,030,829 bytes are simultaneously required when typed roots and allocation copy overlap are preserved. [Decision 0143](../Decisions/0143-Bounded-First-Fit-Dynamic-Arena-Replay.md) then places the complete 1,852,773-allocation trace through a concrete 16-byte-header, 16-byte-aligned first-fit arena. It peaks at 9,031,216 charged bytes, reaches address 10,700,368, fails no request, and fully coalesces at completion inside the unchanged 16 MiB capacity. [Decision 0147](../Decisions/0147-Native-Descriptor-Ownership-Plan.md) publishes 186,557 deterministic exact-compiler ownership actions and requires a separately implemented reconstruction before selection. [Decision 0148](../Decisions/0148-First-Wva-Native-Descriptor-Allocator-Leaf.md) now implements the reclaiming mechanics as a digest-bound 2,989-byte WVA leaf, validates it through live W^X execution against an independent model, and projects the exact plan to 180,190 allocator calls. ABI 21 and its machine bytes remain unchanged. Owner-token integration and selected-operation verification in a successor ABI are the next compiler step; standalone host containers and native self-hosting remain later work.
