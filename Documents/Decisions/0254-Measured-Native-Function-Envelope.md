...
- The next real rejection is the independently bounded record-storage planner used by ordinal 88, not another general function guard.
...
Measure the exact record-planner pressure of the first remaining real rejection before changing any record-specific ceiling. Revisit the general envelope only if a qualified accepted module reaches one of these bounds, and preserve the fixed native frame limit unless a separate ABI decision changes it.
