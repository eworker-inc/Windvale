...
- Build that closure from `Projects/Compiler/Windvale-Compiler-Memory.wvproj`. Keep the hosted `Projects/Examples/Windvale-Compiler.wvproj` as the CLI and bootstrap artifact; the two adapters share the same compiler core instead of introducing parallel compilers.
