...
- Retain an independently implemented Stage 0 codec, provider oracle, and hostile-input verifier, now preserved by the [managed recovery archive](../../Bootstrap/Stage0/README.md).
- Extract the existing copied-message state machine into a shared format-blind transport oracle, now preserved by the [managed recovery archive](../../Bootstrap/Stage0/README.md). Resource and directory services demonstrably share one 4 KiB transport oracle rather than giving a resource-named class accidental format ownership.
