...
| Artifact or bound | ABI 17 / Probe 32 | ABI 18 candidate | Change |
| --- | ---: | ---: | ---: |
...
Focused Windows verification passes all ten native compiler cases and all four pinned-QEMU 11.0/Q35/TCG scenarios. The complete Standard gate passes a zero-warning Release build, all 68 Seed tests including the golden cross-host contract, and all 25 OS tests. Exact compiler preflight now clears the former slot-2,049 failure and reaches `WVN2003` in `Compilerˉcompileˉsourceˉwvb` for the next unsupported opcode, `Bytesˉfromˉu8`. Cross-host qualification remains pending.
