...
**Current focus:** define explicit publication-lifetime state and move the next measured part of the Windows/Linux W^X adapter into Windvale. The qualified planner already owns bounded executable-image extent and canonical runtime-service placement, while C# independently validates the response and still owns OS allocation/protection, instruction-cache publication, invocation, arenas, and teardown.
