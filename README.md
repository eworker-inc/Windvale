...
| Windvale operating system | 🚧 In progress | Probe 21 is cross-host qualified; candidate probe 24 executes the admitted WVB through a Windvale-written interpreter at CPL3 and preserves probe 23's separate init service | Cross-host-qualify the current OS contracts, then generalize one boot-resource/loader boundary or measure a third runnable |
...
**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies special system-profile diagnostics, while portable Windvale modules compile through canonical WVB and the shared native path. Exact commit `22e350b` implements probe 23's admission gate, two CPL3 roots, receive-only [`Init-Resource-Service.wv`](Operating-System/Kernel/Init-Resource-Service.wv), and send-only client. Candidate [Decision 0093](Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) replaces that client's admitted-program AOT derivative with an AOT-built [`Bytecode-Interpreter.wv`](Operating-System/Runtime/Bytecode-Interpreter.wv). The interpreter decodes the exact admitted WVB at CPL3, sends result 29 or faults after sending, and the coordinator wakes init to finish. This is real verify-before-execute, user-space interpretation, hardware isolation, reduced authority, and cross-process IPC, but not yet a general loader, JIT, or scheduler. Version 17 remains useful historical evidence:
...
Candidate Decision 0091 advances the firmware probe to version 22. One exact process receives four table pages plus user RX code and RW/NX stack/data, enters CPL3 with a private GDT/TSS and syscall MSRs, checks capability slot/generation/rights, and returns to the kernel after either send/receive/exit or the explicitly selected contained user fault. All 25 focused OS tests and all six focused assembler tests pass on Windows. All four exact pinned-QEMU scenarios pass: normal `a8a9bebac3fba95d964187e43d67791c36cb0d541aa222150bf71790526c7f03`, invalid opcode `5c7d924c4bb336137213b28c7c5876d85c8f46272e4cb114494044a146b574e7`, general protection `4db4aa08f3b8d736a04a4d46fb4810d9a5b9e19b2f953cded520822ad2b64d6a`, and contained user fault `ed2c7cf51ef9a9e5c93e163f94b56283afb717f7f8541d3f7ee9ab8c75cb5a0a`. Cross-host qualification remains pending.

Decision 0092 advances firmware probe 23 to two roots and the first user-space service written in Windvale. Init has receive-only authority, the client has send-only authority, and one kernel-owned message drives a real wait/wake transition. All four exact pinned-QEMU scenarios pass: normal `5e2314ad4f3bbc3809c3027e56cf955b398d40d82657be597f8ca822ad6cdec8`, invalid opcode `a5929f97d1ef7d1152c8f4783f8f5a0bf40ba7f6f87dda529519a6b76327a2bb`, general protection `205b4dfc88f73f9ecec41f91242642528387e0ae0c55d1273cb50a46f14d2847`, and contained client fault `b2ed520486199104cad227f0bcbc863b428c9484400116dc88c2a55c159d2951`. Exact commit `22e350b` passes 67/67 Seed qualification tests and compiles the OS projects on both Windows and Linux; Linux OS-test execution remains pending.

Candidate Decision 0093 advances firmware probe 24 through the first user-space Windvale bytecode interpreter. Its 3,211-byte WVB is AOT-built into process 2, which interprets the exact admitted 174-byte WVB subset rather than executing the program's host-built AOT derivative. All 25 focused OS tests pass. All four exact pinned-QEMU scenarios pass: normal `4248f3402a0abfe9eda531109460fb37a7c1f3f907ded162a49a785bab38fbb7`, invalid opcode `465ac8d095160e169187860413b0b347148df4acc2f24802f7a5e166981f6c59`, general protection `0a65ba2982f551434553054fc92d82a1a6e727a73527c85a79526f328105b58c`, and contained interpreter fault `8cd17f693ae088eefaccb2b8449fd47f34e345352bb46beebd4a2a32fe8fad5d`. Cross-host qualification remains pending.

The exact ownership and evidence limits are recorded through candidate [Decision 0093](Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md), the [interpreter specification](Specifications/Windvale-Os-Bytecode-Interpreter.md), [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, memory, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel OS terms, cross-host-qualify probe 24, then replace one fixed interpreter assumption with a bounded boot-resource/loader contract or let a third runnable measure scheduling pressure. Raw Windows/Linux memory authority plus OS page/descriptor/MSR construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest fully cross-host-qualified OS evidence:** exact commit `860c69c` passes Windows and Debian Qualification through probe 21 with all 67 Seed tests and all 21 OS tests. Commit `22e350b` later passes GitHub's Windows/Linux Seed qualification and OS-project build, while probe 23's OS-test cross-host promotion remains pending. Pinned Windows QEMU supplies its exact four-scenario machine evidence. C# remains the native host bootstrap, OS page/descriptor/MSR constructor, platform memory authority, packaging adapter, and independent recovery implementation. See [Decisions 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md) through [0092](Documents/Decisions/0092-First-Windvale-Init-Resource-Service.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
