...
| Native compiler and host programs | 🚧 In progress | The bounded kernel target remains, and a first general verified-WVB constant function shares explicit machine IR, WVO/AOT output, and a W^X JIT fragment on qualified Windows and Debian paths | Add checked arithmetic and traps, then control flow, calls, and data |
...
**Current focus:** extend the qualified shared interpreter/JIT/AOT fragment path through checked integer arithmetic and traps, then control flow, calls, and static data.
