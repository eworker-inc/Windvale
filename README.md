...
**Current focus:** qualify and expand the first active Windvale-assembled native stencil, then move bounded verification, W^X publication, arenas, and execution into Windvale while retaining C# as the independent Stage 0/reference/recovery implementation.

**Latest qualified evidence:** exact commit `ef08619` passes zero-warning Windows and Debian Qualification with all 57 tests. ABI 14/context 6 replaces the final managed native-service callback with exact Windows and Linux file-input leaves plus a checked execution-owned `WVFI` snapshot table. All eleven service-table slots execute without C# delegates or platform argument thunks. Interpreter, Windows/Linux W^X JIT, and linked WVO/AOT agree over real files, first-success caching, bounded failures, and the complete 1,441-line Windvale `wvdump`; normalized contracts match and all 61 portable artifacts (7,752,647 bytes) are byte-identical. Both hosts pass all 15 OS tests. The exact 15,872-byte Windvale OS image has SHA-256 `206a036f8cbe3198544b6878bf52c80ef8d489c14d5437c6c7004ff1d6599504` and emits the complete version-16 success transcript under pinned QEMU 11.0 on Windows. C# still constructs, verifies, and publishes leaves/tables, owns arenas and W^X execution, maps traps, packages the OS image, and remains the independent recovery implementation; this is an important boundary completion, not .NET retirement. See [Decision 0076](Documents/Decisions/0076-Native-Windows-And-Linux-File-Input.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete qualified scope.
...
- A first active WVA-authored native-runtime stencil whose Windvale-assembled WVO is strictly validated and typed-patched into the unchanged `process.argument_count` leaf
