...
| Native compiler and host programs | 🚧 In progress | ABI 14/context 6 has qualified native leaves for all 11 service slots on Windows and Linux | Move bounded construction/publication ownership into Windvale |
...
- Two active WVA-authored native-runtime stencils whose Windvale-assembled WVOs are strictly validated and typed-patched into the unchanged `process.argument_count` and `process.argument` leaves; the first is qualified and the multi-patch second is the current candidate
