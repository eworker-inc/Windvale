...
| Windvale operating system | 🚧 In progress | Qualified Probe 31 interprets the exact canonical `Sum-Data.wv` WVB—data, loop, calls, and branches—in two protected client generations | Let a second real program, independent resource lifetime, or third runnable choose the next measured boundary |
...
**Current focus:** let measured pressure from another real program, an independently lived resource, or a third runnable determine whether semantic expansion, allocator/lifecycle work, or scheduling comes next. The compiler remains at ABI 17: decomposing the interpreter kept its largest frame at 1,883 slots and avoided an unmeasured ceiling increase. Raw Windows/Linux memory authority plus OS page/descriptor/MSR/resource construction, dispatcher mechanics, packaging, and linking remain narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest fully cross-host-qualified OS evidence:** exact implementation commit `f3eca7c` passes GitHub's complete verifier through Probe 31 with all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12. All four pinned-QEMU scenarios pass on Windows and emit the complete same-WVB/process-reuse evidence; no Debian QEMU execution is claimed. C# remains the native host bootstrap, OS page/descriptor/MSR/resource constructor, platform memory authority, packaging adapter, and independent recovery implementation. See [Decisions 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md) through [0101](Documents/Decisions/0101-First-Exact-Wvb-Across-Three-Environments.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
