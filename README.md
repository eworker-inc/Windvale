...
| Native compiler and host programs | 🚧 In progress | ABI 11/context 3 now gives one arena to managed and native services; UTF-8 validation, concatenation, and integer formatting have exact native leaves | Cross-host qualify ABI 11, then migrate enum naming and quoting |
| Windvale operating system | 🚧 In progress | Candidate probe 13 carries the service-free AOT consumer through ABI 11/context 3; probe 12 remains cross-host/pinned-QEMU qualified | Qualify probe 13, then add traps, in-guest verification/runtime, clean shutdown, and Hyper-V evidence |
...
Portable Native-Wvb-Probe.wv -> verified WVB -> ABI-11 WVO -> linked UEFI image -> kernel-owned execution
Hosted Wv-Dump-Core.wv -> ABI-11 W^X/WVO execution -> complete deterministic real-WVB report
```

**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies the special system-profile diagnostics, while ordinary portable [`Native-Wvb-Probe.wv`](Operating-System/Kernel/Native-Wvb-Probe.wv) compiles to canonical verified WVB and then to the current ABI-11 path. After the loader exits UEFI boot services, claims and clears a 64 KiB arena, exercises its allocator, copies the handoff, and switches to an 8 KiB kernel stack, the bridge constructs context version 3. The portable module loops over immutable i32 data, passes borrowed bytes into an internal function, slices and reads them, checks `u8`/`u32` results, and must return exact result 29 before boot continues. This is host-built AOT evidence; the guest does not yet load or verify WVB. The current version-13 transcript passes pinned QEMU on Windows; exact cross-host qualification is pending:
...
The exact ownership, allocator, implementation seam, target, and evidence limits are recorded in [Decision 0052](Documents/Decisions/0052-First-Kernel-Owned-Memory-Foundation.md), [Decision 0056](Documents/Decisions/0056-Windvale-Owned-Post-Memory-Evidence.md), [Decision 0064](Documents/Decisions/0064-First-Shared-Native-Wvb-In-Windvale-Os.md), [Decision 0065](Documents/Decisions/0065-Versioned-Native-Execution-Context-And-Console-Service.md), [Decision 0066](Documents/Decisions/0066-Borrowed-Bytes-And-Unsigned-Native-Values.md), [Decision 0067](Documents/Decisions/0067-Borrowed-Hosted-Input-And-First-Native-Wvb-Inspector.md), [Decision 0068](Documents/Decisions/0068-Bounded-Native-Nominal-Values-And-Wvdump-Structural-Core.md), [Decision 0069](Documents/Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md), [Decision 0070](Documents/Decisions/0070-First-Runtime-Native-Utf8-Service.md), [Decision 0071](Documents/Decisions/0071-Native-Text-Arena-And-Core-Text-Services.md), the [native execution-context specification](Specifications/Windvale-Native-Execution-Context.md), the [kernel-memory specification](Specifications/Windvale-Kernel-Memory.md), and the [kernel native-seam specification](Specifications/Windvale-Kernel-Native-Seam.md).

**Current focus:** cross-host qualify ABI 11's shared text-arena ownership and exact concatenation/integer-format leaves, then migrate native enum metadata and deterministic quoting while retaining the independent .NET reference/recovery implementation through the retirement gate.

**Latest qualified evidence:** exact commit `53cee69` passes zero-warning Windows and Debian Qualification with all 56 tests and replaces strict UTF-8's managed callback/platform adapters with one exact 800-byte runtime-native x86-64 leaf. Interpreter, Windows/Linux W^X JIT, and linked WVO/AOT agree over ABI 10's bounded dynamic text, descriptor/void calls, diagnostics, and the complete 1,441-line Windvale `wvdump`; normalized contracts match and all 61 portable artifacts (7,752,647 bytes) are byte-identical. Both hosts pass all 15 OS tests. The exact 15,872-byte qualified Windvale OS image has SHA-256 `3010bc72b9c26386f062f78481c900cac841321b040b41447a0bbb65a9e392fe` and emits the complete version-12 success transcript under pinned QEMU 11.0 on Windows. The current ABI-11/probe-13 candidate passes all 56 Windows Standard tests, all 15 OS tests, and pinned QEMU, but is not yet cross-host qualified. The Windvale bytecode compiler still reproduces its exact 599,868-byte artifact in 6,700,562,174 verified VM instructions. C# still constructs and publishes the native leaves and supplies the remaining runtime services, so this is not yet a Windvale-written native runtime or .NET retirement. See the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md) and [development roadmap](Documents/Project/Roadmap.md) for the complete scope and remaining gates.
...
The accepted native destination keeps canonical WVB as the portable program identity while a Windvale-written execution stack supplies a verified interpreter, low-latency baseline JIT, optional measured optimizing tier, deterministic AOT, native memory management, and narrow Windows, Linux, and Windvale OS adapters. JIT and AOT share one native ABI, backend, and relocation model. [Decision 0059](Documents/Decisions/0059-First-Shared-Native-Wvb-Slice.md) through [Decision 0070](Documents/Decisions/0070-First-Runtime-Native-Utf8-Service.md) cross-host qualify the Stage 0 seam through ABI 10 and its first runtime-native leaf. [Decision 0071](Documents/Decisions/0071-Native-Text-Arena-And-Core-Text-Services.md) implements the ABI-11 text-arena/native-service candidate. The larger direction, safety boundary, proposed WVA copy-and-patch tier, and exact .NET retirement conditions remain defined by [Decision 0057](Documents/Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md) and the [native-execution architecture](Documents/Architecture/Native-Execution-And-Dotnet-Retirement.md).
