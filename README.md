...
| Windvale operating system | 🚧 In progress | Probe 21 remains cross-host qualified; candidate probe 22 adds one real separate CPL3 process root/thread, fixed capability/budgets, register IPC, syscall exit, and contained user fault | Cross-host-qualify probe 22, then start a minimal Windvale init/resource service |
...
Exact WVB admission -> separate CPL3 process root -> capability-checked send/receive/exit
Deliberate CPL3 privileged fault -> process fault record -> continued kernel execution
```

**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies the special system-profile diagnostics, while ordinary portable [`Native-Wvb-Probe.wv`](Operating-System/Kernel/Native-Wvb-Probe.wv) compiles to canonical verified WVB and then to the shared native path. Exact commit `860c69c` qualifies integrated [Decisions 0088](Documents/Decisions/0088-First-Kernel-Owned-X64-Page-Tables.md) through [0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md): context 7 under ABI 16, a kernel-owned page-table root, and a deliberately fixed first admission profile. Candidate [Decision 0091](Documents/Decisions/0091-First-Protected-Windvale-Process.md) keeps token 73 as the gate, then runs Windvale [`Process-Foundation.wv`](Operating-System/Kernel/Process-Foundation.wv), constructs a separate user root, and executes the derived AOT program at CPL3 behind a WVA entry. Its fixed capability sends and receives result 29 before syscall exit; a second image proves that privileged `CLI` at CPL3 faults the process while the kernel continues. This is real verify-before-execute plus hardware isolation, but not yet a general WVB loader, scheduler, or user service runtime. Version 17 remains useful historical evidence:
...
The exact ownership and evidence limits are recorded in the decisions through candidate [Decision 0091](Documents/Decisions/0091-First-Protected-Windvale-Process.md), the [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, memory, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel OS terms, cross-host-qualify probe 22, then use its protected boundary for the first minimal Windvale init/resource service. Raw Windows/Linux memory authority plus OS page/descriptor/MSR construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest qualified evidence:** exact commit `860c69c` passes zero-warning Windows and Debian Qualification with all 67 Seed tests, the complete CLI/reproduction gate, and all 21 OS tests. ABI 16/context 7 supplies all 12 current native service leaves and bounded internal calls through 64 parameters; exact compiler preflight now reaches the one 1,049-local frame blocker. Normalized contracts and all 69 portable artifacts (7,851,187 bytes) match byte for byte. Pinned QEMU qualifies probe 21's clean exit, normalized vector-6/vector-13 terminal paths, kernel-owned W^X root, and first fixed Windvale-owned WVB admission. C# remains the native WVB loader/compiler/verifier, platform memory and arena authority, invocation adapter, and independent recovery implementation. See [Decisions 0088](Documents/Decisions/0088-First-Kernel-Owned-X64-Page-Tables.md) through [0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
