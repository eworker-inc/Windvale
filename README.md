...
| Native compiler and host programs | 🚧 In progress | ABI 10 is cross-host qualified through complete `wvdump`; its first platform-neutral runtime-native UTF-8 leaf now replaces one managed callback | Cross-host qualify the leaf, then give allocation-bearing text services native arena ownership |
...
**Current focus:** cross-host qualify the first callback-free ABI-10 UTF-8 service leaf, then define native text-arena ownership for formatting and concatenation while retaining the independent .NET reference/recovery implementation through the retirement gate.

**Latest qualified evidence:** exact commit `7979933` passes zero-warning Windows and Debian Qualification with all 56 tests. Interpreter, Windows/Linux W^X JIT, and linked WVO/AOT agree over ABI 10's bounded dynamic text, descriptor/void calls, diagnostics, and the complete 1,441-line Windvale `wvdump`; normalized contracts match and all 61 portable artifacts (7,752,647 bytes) are byte-identical. Both hosts pass all 15 OS tests. The exact 15,872-byte Windvale OS image has SHA-256 `3010bc72b9c26386f062f78481c900cac841321b040b41447a0bbb65a9e392fe` and emits the complete version-12 success transcript under pinned QEMU 11.0 on Windows. The Windvale bytecode compiler still reproduces its exact 599,868-byte artifact in 6,700,562,174 verified VM instructions. See the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md) and [development roadmap](Documents/Project/Roadmap.md) for the complete scope and remaining gates.
