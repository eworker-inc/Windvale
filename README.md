...
| Native compiler and host programs | 🚧 In progress | Calls, bounded recursion, shared instruction/depth budgets, immutable i32 data, loops, and traps now share WVO/AOT and W^X JIT paths on Windows | Cross-host qualify ABI 5, then use it to advance the first native Windvale tool |
...
**Current focus:** cross-host qualify the ABI-5 shared-budget call and static-data slice, then select the smallest native Windvale tool that exercises it.
