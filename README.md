...
| Native compiler and host programs | 🚧 In progress | Constants and checked i32 add/subtract/multiply/negate share verified WVO/AOT and W^X JIT paths with recoverable overflow traps on qualified Windows and Debian x64 hosts | Add comparisons and structured control flow, then calls and data |
...
**Current focus:** extend the cross-host-qualified native fragment path through boolean comparisons and structured control flow while preserving the shared interpreter/JIT/AOT semantics and strict byte decoder.
