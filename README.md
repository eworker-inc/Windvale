...
| Native compiler and host programs | 🚧 In progress | ABI 10 now runs the complete checked-in Windvale `wvdump` through interpreter, JIT, and linked WVO/AOT on Windows; ABI 9 remains the latest cross-host-qualified baseline | Cross-host qualify ABI 10, then replace its C# runtime services with native Windvale implementations |
| Windvale operating system | 🚧 In progress | Probe 12 rebuilds the portable byte-decoding consumer through ABI 10/context 2 and passes all focused OS tests; probe 11 remains the latest pinned-QEMU-qualified image | Qualify probe 12, then add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
...
Portable Native-Wvb-Probe.wv -> verified WVB -> ABI-10 WVO -> linked UEFI image -> kernel-owned execution
Hosted Wv-Dump-Core.wv -> ABI-10 W^X/WVO execution -> complete deterministic real-WVB report
```

**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies the special system-profile diagnostics, while ordinary portable [`Native-Wvb-Probe.wv`](Operating-System/Kernel/Native-Wvb-Probe.wv) compiles to canonical verified WVB and then to the current ABI-10 candidate path. After the loader exits UEFI boot services, claims and clears a 64 KiB arena, exercises its allocator, copies the handoff, and switches to an 8 KiB kernel stack, the bridge constructs context version 2. The portable module loops over immutable i32 data, passes borrowed bytes into an internal function, slices and reads them, checks `u8`/`u32` results, and must return exact result 29 before boot continues. This is host-built AOT evidence; the guest does not yet load or verify WVB. The candidate version-12 transcript is:
...
The accepted native destination keeps canonical WVB as the portable program identity while a Windvale-written execution stack supplies a verified interpreter, low-latency baseline JIT, optional measured optimizing tier, deterministic AOT, native memory management, and narrow Windows, Linux, and Windvale OS adapters. JIT and AOT share one native ABI, backend, and relocation model. [Decision 0059](Documents/Decisions/0059-First-Shared-Native-Wvb-Slice.md) through [Decision 0068](Documents/Decisions/0068-Bounded-Native-Nominal-Values-And-Wvdump-Structural-Core.md) qualify the Stage 0 seam through ABI 9, execution-owned nominal values, the native Windvale `wvdump` structural parser, and firmware probe 11. [Decision 0069](Documents/Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md) implements ABI 10's bounded dynamic text, descriptor/void calls, diagnostics, complete native `wvdump`, and probe 12; cross-host and pinned-QEMU qualification are pending. The larger direction, safety boundary, proposed WVA copy-and-patch tier, and exact .NET retirement conditions remain defined by [Decision 0057](Documents/Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md) and the [native-execution architecture](Documents/Architecture/Native-Execution-And-Dotnet-Retirement.md).
