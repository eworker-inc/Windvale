...
| Native compiler and host programs | 🚧 In progress | ABI 14/context 6 has all 11 native service leaves; Windvale supplies the live process-input leaves, plans every image layout, and now defines the allowed executable-image lifetime graph | Cross-host qualify the lifetime owner, then select the next measured execution-owner boundary |
