...
| Native compiler and host programs | 🚧 In progress | ABI 14/context 6 has all 11 current native service leaves; Windvale supplies live process-input leaves and owns executable-image layout plus lifetime policy | Preflight the qualified compiler WVB; its first explicit native blocker is `file.write_bytes` |
| Windvale operating system | 🚧 In progress | Qualified probe 17 owns vector 6; candidate probe 19 adds WVA-normalized vectors 6/13 and clean Q35 poweroff | Cross-host qualify probe 19, then own page tables and add in-guest verification/runtime |
...
Portable Native-Wvb-Probe.wv -> verified WVB -> ABI-14 WVO -> linked UEFI image -> kernel-owned execution
Hosted Wv-Dump-Core.wv -> ABI-14 W^X/WVO execution -> complete deterministic real-WVB report
...
**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies the special system-profile diagnostics, while ordinary portable [`Native-Wvb-Probe.wv`](Operating-System/Kernel/Native-Wvb-Probe.wv) compiles to canonical verified WVB and then to the qualified ABI-14 path. After the loader exits UEFI boot services, claims and clears a 64 KiB arena, exercises its allocator, copies the handoff, and switches to an 8 KiB kernel stack, bridge 9 constructs context version 6 with zero argument, output-table, and file-input-table fields. The portable module loops over immutable i32 data, passes borrowed bytes into an internal function, slices and reads them, checks `u8`/`u32` results, and must return exact result 29 before boot continues. This is host-built AOT evidence; the guest does not yet load or verify WVB. Version 17 is the qualified normal transcript:
