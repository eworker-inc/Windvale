...
| Windvale operating system | 🚧 In progress | Probe 21 is cross-host qualified with context 7 under ABI 16, the first kernel-owned W^X page-table root, and Windvale-owned admission of one embedded WVB before its AOT form executes | Add one protected process/thread/capability/IPC boundary |
...
**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies the special system-profile diagnostics, while ordinary portable [`Native-Wvb-Probe.wv`](Operating-System/Kernel/Native-Wvb-Probe.wv) compiles to canonical verified WVB and then to the shared native path. Exact commit `860c69c` qualifies integrated [Decisions 0088](Documents/Decisions/0088-First-Kernel-Owned-X64-Page-Tables.md) through [0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md): context 7 under ABI 16, a kernel-owned page-table root, and a deliberately fixed first admission profile. AOT [`Wvb-Admission.wv`](Operating-System/Kernel/Wvb-Admission.wv) validates the exact canonical bytes produced from [`Embedded-Wvb-Program.wv`](Operating-System/Kernel/Embedded-Wvb-Program.wv) inside the guest, and its bridge can call the derived AOT program only after token 73. The retained portable ABI probe then runs unchanged. This is real verify-before-execute ordering, but not yet a general WVB loader, semantic verifier, process boundary, or user-mode runtime. Version 17 remains useful historical evidence:
...
The exact ownership and evidence limits are recorded in the decisions through qualified [Decision 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md), the [native file-output specification](Specifications/Windvale-Native-File-Output.md), the [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, memory, trap-frame, CPU-exception, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel architecture terms, the next OS slice is one protected process/thread/capability/IPC boundary. Raw Windows/Linux memory authority plus OS table construction, descriptor publication, terminal policy, admission bridging, packaging, and linking remain inside narrow C# Stage 0 adapters.
