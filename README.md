...
| Native compiler and host programs | 🚧 In progress | ABI 17's verified 2,048-cell frame and all 12 native service leaves are cross-host qualified; exact compiler preflight clears the former 1,049-local blocker | Reduce the next measured lowered-value pressure at slot 2,049, then continue exact native preflight |
| Windvale operating system | 🚧 In progress | Probe 31 interprets the exact canonical `Sum-Data.wv` WVB—data, loop, calls, and branches—in two protected client generations | Qualify the same bytes on Windows, Linux, and pinned QEMU, then let a second real program choose the next boundary |
...
Portable Native-Wvb-Probe.wv -> verified WVB -> ABI-17 WVO -> linked UEFI image -> kernel-owned execution
Hosted Wv-Dump-Core.wv -> ABI-17 W^X/WVO execution -> complete deterministic real-WVB report
...
Windvale init selection -> immutable grant -> client read -> terminal alias/publication cleanup
Windvale typed resource set -> canonical Sum-Data WVB plus budget -> 203 charged opcodes -> atomic cleanup
116-page client extent -> exact tail zero/release -> generation-2 same-root rebuild -> second execution
```

**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies special system-profile diagnostics, while portable Windvale modules compile through canonical WVB and the shared native path. Qualified [Decisions 0093](Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) through [0100](Documents/Decisions/0100-First-Reclaimed-And-Reused-Process-Root.md) establish protected interpretation, typed runtime input, terminal cleanup, and generation-safe root reuse. Implemented [Decision 0101](Documents/Decisions/0101-First-Exact-Wvb-Across-Three-Environments.md) replaces the four-opcode fixture with the exact 493-byte canonical `Examples/Seed/Sum-Data.wv` WVB. Interpreter profile 5 executes its immutable data, loop, branches, locals, and internal calls for exactly 203 guest instructions in both protected generations. This is real runtime input, Windvale-owned lifecycle policy, verify-before-execute, user-space interpretation, hardware isolation, reclamation, and cross-process IPC, but not yet arbitrary loading, a dynamic namespace, general capability transfer, a general allocator/interpreter, JIT, or scheduler. Version 17 remains useful historical evidence:
...
Qualified Decision 0097 advances firmware probe 28 to the first terminal borrow cleanup. Protected-process version 7 accepts ordinary exit or contained fault, revalidates the live `WVRES002` borrow while allowing only the x86 hardware-set accessed bit, clears the client PTE plus its complete service/`WVBR` publication, and records owned/no-borrower/grant-one/mapping-zero state before init resumes. Exact commit `b2197fa` passes all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12 in GitHub [Verify run 30741650532](https://github.com/eworker-inc/Windvale/actions/runs/30741650532). All four Windows pinned-QEMU scenarios pass; the normal 230,912-byte image is `bc5f04c0e75fb217c9339bcc2a391bbe68f9f79ad97c18a93e35e310dab62d46`.

Qualified Decision 0098 advances firmware probe 29 to the first typed two-resource lookup. Windvale init selects ordered set `(1,2)` containing the admitted WVB and a separate four-byte execution budget; the kernel publishes two distinct RO/NX aliases and `WVBR002` atomically; WVA validates the exact typed name/entry; and the Windvale interpreter charges the supplied budget once per opcode. Ordinary exit and contained client fault both clean the full pair. Exact implementation commit `3fd9ef7` passes all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12 in GitHub [Verify run 30745623111](https://github.com/eworker-inc/Windvale/actions/runs/30745623111); all four Windows pinned-QEMU scenarios also pass.

Qualified Decision 0100 advances firmware probe 30 to the first reclaimed and reused process root. After generation 1 executes and loses both aliases, kernel memory 7 accepts only its exact 42-page allocator tail, zeroes/releases it, and reallocates the same physical root. `WVPROC09` and `WVRES004` stamp the rebuilt client as generation 2, preserve historical grant count `2`, and reject stale generation-1 evidence. Init grants and receives twice; normal and contained-user-fault paths execute both client generations. Exact implementation commit `4a077ab` passes all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12 in GitHub [Verify run 30749304867](https://github.com/eworker-inc/Windvale/actions/runs/30749304867). All four Windows pinned-QEMU scenarios pass; the normal 261,120-byte image is `5034c01a98f20344d96fa091fd9a55a303e72669d746a4b83df2900eed93992f`.

Implemented Decision 0101 advances candidate firmware Probe 31 toward the first exact-WVB proof across Windows, Linux, and Windvale OS. The admitted artifact is the canonical 493-byte `Sum-Data.wv` WVB, SHA-256 `6f3a272d37dd8893995c7f85c236414ed2864bf59de2f3775c08afd426013f8c`; both OS client generations interpret its data, loop, branches, locals, and calls for 203 guest instructions and return `29`. Its largest native interpreter frame is 1,883 slots, below unchanged ABI 17's 2,048-slot limit. A 256-byte arena inside the existing client data page supplies the five execution-result records without changing the compiler ABI or process page count. All 25 focused Windows OS tests and all four local Windows pinned-QEMU scenarios pass; complete committed Windows/Debian qualification remains pending. The normal 531,456-byte EFI candidate is `30acb028e44b6d12bc4d0e4d34232d86a43b83b40f070d3a48b7c56e505bc0bc`.

The exact ownership and evidence limits are recorded through qualified [Decisions 0096](Documents/Decisions/0096-First-Windvale-Init-Owned-Boot-Resource-Grant.md), [0097](Documents/Decisions/0097-First-Terminal-Resource-Borrow-Revocation.md), [0098](Documents/Decisions/0098-First-Typed-Two-Resource-Lookup.md), and [0100](Documents/Decisions/0100-First-Reclaimed-And-Reused-Process-Root.md), the [interpreter specification](Specifications/Windvale-Os-Bytecode-Interpreter.md), [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-memory specification](Specifications/Windvale-Kernel-Memory.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** qualify Probe 31's exact canonical WVB on both permanent hosts; its pinned-QEMU gate is already green. The compiler remains at ABI 17: decomposing the interpreter kept its largest frame at 1,883 slots and avoided an unmeasured ceiling increase. After qualification, another real program, independent resource lifetime, or third runnable should determine whether semantic expansion, allocator/lifecycle work, or scheduling comes next. Raw Windows/Linux memory authority plus OS page/descriptor/MSR/resource construction, dispatcher mechanics, packaging, and linking remain narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest fully cross-host-qualified OS evidence:** exact implementation commit `4a077ab` passes GitHub's complete non-Fast verifier through probe 30 with all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12. All four pinned-QEMU scenarios pass on Windows and emit `process-reuse=pass`; no Debian QEMU execution is claimed. C# remains the native host bootstrap, OS page/descriptor/MSR/resource constructor, platform memory authority, packaging adapter, and independent recovery implementation. See [Decisions 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md) through [0100](Documents/Decisions/0100-First-Reclaimed-And-Reused-Process-Root.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
