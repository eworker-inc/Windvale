...
| Windvale operating system | 🚧 In progress | Candidate probe 27 makes Windvale init select its owned `boot:main.wvb` and installs one checked RO/NX client alias before the unchanged CPL3 interpreter runs; probe 26 remains cross-host qualified | Qualify probe 27, then add typed lookup/lifetime, broader input, or scheduling only when measured |
...
Candidate Decision 0096 advances firmware probe 27 to the first Windvale-selected resource grant. Init's 273-byte WVB returns fixed resource identifier `1`; its WVA shim requests the grant and then waits for the unchanged result channel. Process `2` begins without the resource mapping or service pointers. Stage 0 constructs one exact kernel-owned `WVRES001` record; the syscall revalidates its security-critical state and bounds, installs a RO/NX alias of init's page, publishes the existing ABI-16 tables, and records one borrow while init remains owner. The coordinator checks the exact result before entering the client. All 25 focused Windows OS tests and all four Windows pinned-QEMU scenarios pass; the normal 224,768-byte image (`709ebd7f643f2f9d9c7cf4eb4042977c675a3ff19d7a34da4d7e26e0526a29b7`) completes the grant, interpretation to `29`, IPC, and shutdown. Cross-host qualification is pending.

The exact ownership and evidence limits are recorded through candidate [Decision 0096](Documents/Decisions/0096-First-Windvale-Init-Owned-Boot-Resource-Grant.md), the [interpreter specification](Specifications/Windvale-Os-Bytecode-Interpreter.md), [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-memory specification](Specifications/Windvale-Kernel-Memory.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel OS terms, qualify probe 27, then let a second resource, teardown, a broader real module, or a third runnable determine whether typed lookup, revocation, semantic expansion, or scheduling comes next. Raw Windows/Linux memory authority plus OS page/descriptor/MSR/resource construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest fully cross-host-qualified OS evidence:** exact commit `6bb34bb` passes GitHub's complete non-Fast verifier through probe 26 with all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12. Candidate probe 27 adds the Windows evidence above but does not supersede that cross-host claim until both verifier jobs pass. C# remains the native host bootstrap, OS page/descriptor/MSR/resource constructor, platform memory authority, packaging adapter, and independent recovery implementation. See [Decisions 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md) through [0096](Documents/Decisions/0096-First-Windvale-Init-Owned-Boot-Resource-Grant.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
