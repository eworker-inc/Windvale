...
The current Stage 0 toolchain includes the typed language, compiler, bytecode verifier, portable reference runtime, assembler, object model, linker, CLI, editor support, and focused Foundation modules. A Windvale-written compiler reproduces its canonical bytecode compiler exactly; independently verified native PE/ELF compiler packages now reproduce it directly on Windows and Debian without loading .NET. Probe 36 remains the cross-host-qualified OS baseline. The Probe-37 candidate adds one kernel-owned, generation-safe service endpoint between process capabilities and the retained channel; all 38 focused OS tests and five pinned Windows QEMU scenarios pass. C# remains the bootstrap, host adapter, public packaging path, and recovery implementation while the remaining retirement gates are completed.
