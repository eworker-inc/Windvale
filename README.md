...
| Windvale operating system | 🚧 In progress | Probe 30 reclaims, scrubs, and reuses one 42-page client root under a new generation, then executes the same typed WVB/resource pair again | Let non-tail lifetime pressure, broader input, or a third runnable determine the next boundary |
...
Windvale typed resource set -> WVB plus execution budget -> four charged opcodes -> atomic cleanup
Terminal client extent -> exact tail zero/release -> generation-2 same-root rebuild -> second execution
```

**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies special system-profile diagnostics, while portable Windvale modules compile through canonical WVB and the shared native path. Qualified [Decision 0093](Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) replaces the admitted client's AOT derivative with an AOT-built [`Bytecode-Interpreter.wv`](Operating-System/Runtime/Bytecode-Interpreter.wv). Qualified [Decision 0094](Documents/Decisions/0094-First-Section-Derived-User-Space-Wvb-Profile.md) removes fixed section offsets. Qualified [Decision 0095](Documents/Decisions/0095-First-Runtime-Supplied-Wvb-Boot-Resource.md) removes the embedded program from the interpreter WVB/RX image. Qualified [Decision 0096](Documents/Decisions/0096-First-Windvale-Init-Owned-Boot-Resource-Grant.md) gives init the RO/NX page and lets Windvale source grant one checked immutable alias. Qualified [Decision 0097](Documents/Decisions/0097-First-Terminal-Resource-Borrow-Revocation.md) removes that alias and its complete private publication after ordinary exit or contained fault. Qualified [Decision 0098](Documents/Decisions/0098-First-Typed-Two-Resource-Lookup.md) adds a separately owned execution-budget page, an atomic typed pair, a WVA lookup leaf, per-opcode Windvale enforcement, and cleanup of both aliases. Qualified [Decision 0100](Documents/Decisions/0100-First-Reclaimed-And-Reused-Process-Root.md) then zeroes/releases the exact terminal 42-page tail, rebuilds generation 2 at the identical physical root, regrants the pair, and executes it again while stale generation-1 identities fail closed. This is real runtime input, Windvale-owned lifecycle policy, verify-before-execute, user-space interpretation, hardware isolation, reduced authority, reclamation, and cross-process IPC, but not yet arbitrary loading, a dynamic resource namespace, general capability transfer, a general allocator/interpreter, JIT, or scheduler. Version 17 remains useful historical evidence:
...
**Current focus:** ABI 17's verified 2,048-cell frame admits the exact compiler's former 1,049-local blocker. Exact preflight now reaches lowered value slot 2,049 in `Compilerˉbodyˉparseˉprimary`; the next native slice should reduce measured value pressure rather than merely grow the ceiling again. In OS terms, Probe 30 resolves the first actual root-reuse pressure, so a non-tail lifetime, broader real module, independently lived resource, or third runnable should determine whether allocator generalization, semantic expansion, lifecycle separation, or scheduling comes next. Raw Windows/Linux memory authority plus OS page/descriptor/MSR/resource construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.
