...
The current Stage 0 toolchain includes the typed language, compiler, bytecode verifier, portable reference runtime, assembler, object model, linker, CLI, editor support, and focused Foundation modules. A Windvale-written compiler reproduces its canonical bytecode compiler exactly; independently verified native PE/ELF compiler packages and public format-3 project/CLI routes now reproduce it directly on Windows and Debian without loading .NET. A paired fixed-authority application profile now packages the Windvale-written compiler-aligned WVB verifier as deterministic Windows and Linux executables; local qualification is in progress. Cross-host-qualified Probe 38 advances Windvale OS to three protected processes, separate resource and directory endpoints, and a bounded ready/wait dispatcher; all 87 Seed and 38 OS tests pass on both hosts, and five pinned Windows QEMU scenarios pass. C# remains the bootstrap, host adapter, package constructor, and recovery implementation while the remaining retirement gates are completed.
