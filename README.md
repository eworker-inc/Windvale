...
The separate [experimental WebAssembly target](Specifications/Windvale-WebAssembly.md) proves the first lower layer in Windvale source: portable `.wv` code revalidates canonical WVB and lowers bounded scalar streams, loops, sequential `if`/`if/else` regions, acyclic scalar and descriptor-bearing call graphs, fixed UTF-8/byte buffers, and a primitive/bytes runtime to deterministic import-free Wasm. Execution ABI 1 preserves checked arithmetic, ABI 2 carries one exact instruction budget through scalar control flow and callees, and ABI 3 adds fixed 4 MiB input/output regions plus one shared budget and arena across private `bytes -> bytes` functions. An eight-function import-free verifier now covers complete WVB structure plus canonical metadata, catalog/declaration/nominal identities, operands, strict text UTF-8, exact branch targets, exports, fields, and enum uniqueness. Typed executable flow remains before it can replace the complete Stage 0 oracle. The playground executes selected output in a disposable worker, and its separate direct demo needs no .NET runtime; artifact production and the general editable compiler path still use Stage 0.
