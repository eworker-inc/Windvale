...
| Native compiler and host programs | 🚧 In progress | ABI 14/context 6 has qualified native leaves for all 11 service slots plus two Windvale-assembled process-input stencils; a Windvale-written consumer now validates and patches both exact contracts | Add a bounded native byte-result bridge, then move live consumption and publication ownership into Windvale |
| Windvale operating system | 🚧 In progress | Service-free ABI-14/context-6 probe 16 passes both OS suites and pinned QEMU | Add traps, in-guest verification/runtime, clean shutdown, and Hyper-V evidence |
