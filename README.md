...
| Native compiler and host programs | 🚧 In progress | ABI 10 is cross-host qualified through the complete checked-in Windvale `wvdump` under interpreter, JIT, and linked WVO/AOT | Replace its C# runtime services with native Windvale implementations |
...
**Current focus:** replace selected ABI-10 C# runtime-service callbacks with native Windvale implementations while retaining the independent .NET reference/recovery implementation through the retirement gate.
