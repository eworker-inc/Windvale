...
| Native compiler and host programs | 🚧 In progress | Arithmetic/traps are cross-host qualified; typed blocks, all signed i32 comparisons, bool conditions, forward branches, and early returns now share WVO/AOT and W^X JIT paths on Windows | Qualify control flow on Debian, then add execution-budgeted loops, calls, and data |
...
**Current focus:** qualify dynamic native instruction budgets and backward branches on Debian, then add shared-ABI calls and static data.

**Latest qualified evidence:** the Windvale bytecode compiler reproduces its exact 599,868-byte artifact on Windows and Debian in 6,700,562,174 verified VM instructions; constants, checked i32 arithmetic, typed locals, all signed i32 comparisons, Boolean operations, forward branches, and early returns now agree across the interpreter, W^X JIT, and WVO-linked AOT paths on both hosts. All 49 tests pass and all 61 portable artifacts remain byte-identical across hosts. See the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md) and [development roadmap](Documents/Project/Roadmap.md) for the complete scope and remaining gates.
...
The accepted native destination keeps canonical WVB as the portable program identity while a Windvale-written execution stack supplies a verified interpreter, low-latency baseline JIT, optional measured optimizing tier, deterministic AOT, native memory management, and narrow Windows, Linux, and Windvale OS adapters. JIT and AOT share one native ABI, backend, and relocation model. [Decision 0059](Documents/Decisions/0059-First-Shared-Native-Wvb-Slice.md) cross-host qualifies that shared seam for one constant-return function in the C# Stage 0. [Decision 0060](Documents/Decisions/0060-Checked-Native-I32-Arithmetic-And-Traps.md) cross-host qualifies checked straight-line i32 computation and a packed recoverable overflow status. [Decision 0061](Documents/Decisions/0061-Typed-Native-Blocks-And-Forward-Control-Flow.md) cross-host qualifies typed blocks, comparisons, Boolean operations, forward control flow, mutable locals, and early returns at exact commit `f0a53a9`. [Decision 0062](Documents/Decisions/0062-Dynamic-Native-Instruction-Budgets-And-Backward-Control-Flow.md) adds dynamic per-run instruction budgets and backward control flow on Windows while exact Debian qualification is pending. The larger direction, safety boundary, proposed WVA copy-and-patch tier, and exact .NET retirement conditions remain defined by [Decision 0057](Documents/Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md) and the [native-execution architecture](Documents/Architecture/Native-Execution-And-Dotnet-Retirement.md); calls, static data, and broader native components are not claimed as implemented.
