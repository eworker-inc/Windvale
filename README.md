...
| Native compiler and host programs | 🚧 In progress | ABI 14/context 6 has all 11 native service leaves; Windvale now assembles, validates, patches, returns, and supplies the two live process-input leaves | Qualify the byte-result/live path cross-host, then move W^X publication and lifetime ownership into Windvale |
