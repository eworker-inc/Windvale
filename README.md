...
| Windvale operating system | 🚧 In progress | Probe 25 is cross-host qualified; candidate probe 26 supplies `boot:main.wvb` to the CPL3 Windvale interpreter through a separate RO/NX page and an exact ABI-16 service leaf | Qualify probe 26, then move resource ownership toward init/package service or measure scheduling pressure |
...
**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies special system-profile diagnostics, while portable Windvale modules compile through canonical WVB and the shared native path. Qualified [Decision 0093](Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) replaces the admitted client's AOT derivative with an AOT-built [`Bytecode-Interpreter.wv`](Operating-System/Runtime/Bytecode-Interpreter.wv). Qualified [Decision 0094](Documents/Decisions/0094-First-Section-Derived-User-Space-Wvb-Profile.md) removes fixed section offsets. Candidate [Decision 0095](Documents/Decisions/0095-First-Runtime-Supplied-Wvb-Boot-Resource.md) removes the embedded program from the interpreter WVB/RX image: the hosted Windvale interpreter fetches `boot:main.wvb` through one exact WVA-owned ABI-16 leaf, and the OS supplies a separate RO/NX page. This is real runtime-owned input, verify-before-execute, user-space interpretation, hardware isolation, reduced authority, and cross-process IPC, but not yet arbitrary loading, a resource namespace, general interpreter, JIT, or scheduler. Version 17 remains useful historical evidence:
...
Candidate Decision 0095 advances firmware probe 26 to the first runtime-supplied WVB boundary. The 12,265-byte hosted interpreter WVB declares only `file.read_bytes`; it no longer contains the admitted 174-byte WVB. Process `2` receives one additional RO/NX resource page and an exact 199-byte WVA-owned service leaf under unchanged ABI 16. All 25 focused OS tests and all four pinned-QEMU scenarios pass on Windows; the normal 215,552-byte image (`ab6b818beee3ac7419d48ad7ac5d04f06bab0ec67ab3909de64ed1b88c2e1170`) completes the real CPL3 resource call and returns result `29`. Cross-host qualification is still required.

The exact ownership and evidence limits are recorded through candidate [Decision 0095](Documents/Decisions/0095-First-Runtime-Supplied-Wvb-Boot-Resource.md), the [interpreter specification](Specifications/Windvale-Os-Bytecode-Interpreter.md), [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-memory specification](Specifications/Windvale-Kernel-Memory.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel OS terms, finish probe-26 qualification, then decide whether the next evidence should move the fixed boot-resource ownership/transfer toward init or package service, broaden the interpreter for a real module, or introduce a third runnable when it measures scheduling pressure. Raw Windows/Linux memory authority plus OS page/descriptor/MSR/resource construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest fully cross-host-qualified OS evidence:** exact commit `33555fd` passes GitHub's complete non-Fast verifier through probe 25 with all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12. Pinned Windows QEMU supplies the exact four-scenario machine evidence; no Debian QEMU run is claimed. C# remains the native host bootstrap, OS page/descriptor/MSR constructor, platform memory authority, packaging adapter, and independent recovery implementation. See [Decisions 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md) through [0094](Documents/Decisions/0094-First-Section-Derived-User-Space-Wvb-Profile.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
