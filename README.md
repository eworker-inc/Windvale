...
| Windvale operating system | 🚧 In progress | Probe 21 remains cross-host qualified; candidate probe 23 adds two CPL3 roots, a Windvale init/resource service, reduced endpoints, cross-process IPC, wait/wake, and client-fault containment | Cross-host-qualify probe 23, then measure the smallest loader/runtime or third-runnable case |
...
**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies special system-profile diagnostics, while portable Windvale modules compile through canonical WVB and the shared native path. Exact commit `860c69c` qualifies context 7 under ABI 16, a kernel-owned page-table root, and fixed in-guest admission. Candidate [Decision 0092](Documents/Decisions/0092-First-Windvale-Init-Resource-Service.md) retains that gate, then runs a receive-only [`Init-Resource-Service.wv`](Operating-System/Kernel/Init-Resource-Service.wv) and send-only admitted client under separate CPL3 roots. The service blocks, the client sends result 29 or faults after sending, and the fixed coordinator wakes the service to finish. This is real verify-before-execute, hardware isolation, reduced authority, and cross-process IPC, but not yet a general loader or scheduler. Version 17 remains useful historical evidence:
...
Candidate Decision 0092 advances firmware probe 23 to two roots and the first user-space service written in Windvale. Init has receive-only authority, the client has send-only authority, and one kernel-owned message drives a real wait/wake transition. All 25 focused OS tests pass. All four exact pinned-QEMU scenarios pass: normal `5e2314ad4f3bbc3809c3027e56cf955b398d40d82657be597f8ca822ad6cdec8`, invalid opcode `a5929f97d1ef7d1152c8f4783f8f5a0bf40ba7f6f87dda529519a6b76327a2bb`, general protection `205b4dfc88f73f9ecec41f91242642528387e0ae0c55d1273cb50a46f14d2847`, and contained client fault `b2ed520486199104cad227f0bcbc863b428c9484400116dc88c2a55c159d2951`. Cross-host qualification remains pending.

The exact ownership and evidence limits are recorded through candidate [Decision 0092](Documents/Decisions/0092-First-Windvale-Init-Resource-Service.md), the [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, memory, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel OS terms, cross-host-qualify probe 23, then let the smallest general loader/runtime or third-runnable case measure the next boundary. Raw Windows/Linux memory authority plus OS page/descriptor/MSR construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.
