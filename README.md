...
| Native compiler and host programs | 🚧 In progress | ABI 9 candidate runs enums, immutable arena records, strict UTF-8, and the existing Windvale `wvdump` structural parser through JIT and WVO/AOT | Cross-host qualify ABI 9, then add bounded dynamic text, descriptor returns, void calls, and diagnostics for full `wvdump` output |
| Windvale operating system | 🚧 In progress | Probe 11 candidate rebuilds the portable byte-decoding consumer through ABI 9/context 2 on the bounded kernel-owned stack | Qualify probe 11, then add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
...
**Current focus:** cross-host qualify ABI 9's native nominal values and structural `wvdump` parser, then add the bounded dynamic-text, descriptor-return, void-call, and diagnostic-output slice needed for the complete hosted report path without retiring the independent .NET reference/recovery implementation prematurely.
...
The accepted native destination keeps canonical WVB as the portable program identity while a Windvale-written execution stack supplies a verified interpreter, low-latency baseline JIT, optional measured optimizing tier, deterministic AOT, native memory management, and narrow Windows, Linux, and Windvale OS adapters. JIT and AOT share one native ABI, backend, and relocation model. [Decision 0059](Documents/Decisions/0059-First-Shared-Native-Wvb-Slice.md) through [Decision 0067](Documents/Decisions/0067-Borrowed-Hosted-Input-And-First-Native-Wvb-Inspector.md) qualify the Stage 0 seam through ABI 8, hosted input, the first native WVB inspector, and firmware probe 10. [Decision 0068](Documents/Decisions/0068-Bounded-Native-Nominal-Values-And-Wvdump-Structural-Core.md) defines the ABI-9 candidate: enums, immutable execution-arena records, pure strict UTF-8 validation, the existing Windvale-written `wvdump` structural parser under interpreter/JIT/AOT, and firmware probe 11. Cross-host qualification is pending. The larger direction, safety boundary, proposed WVA copy-and-patch tier, and exact .NET retirement conditions remain defined by [Decision 0057](Documents/Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md) and the [native-execution architecture](Documents/Architecture/Native-Execution-And-Dotnet-Retirement.md); dynamic text and the complete hosted `wvdump` report path are not yet claimed as native.
