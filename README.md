...
| Windvale operating system | 🚧 In progress | Probe 24 is cross-host qualified; candidate probe 25 discovers and validates real WVB sections in a Windvale-written CPL3 interpreter while preserving the separate init service | Cross-host-qualify probe 25, then introduce one bounded runtime-supplied module boundary or measure a third runnable |
...
**Shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies special system-profile diagnostics, while portable Windvale modules compile through canonical WVB and the shared native path. Qualified [Decision 0093](Documents/Decisions/0093-First-User-Space-Windvale-Bytecode-Interpreter.md) replaces the admitted client's AOT derivative with an AOT-built [`Bytecode-Interpreter.wv`](Operating-System/Runtime/Bytecode-Interpreter.wv). Candidate [Decision 0094](Documents/Decisions/0094-First-Section-Derived-User-Space-Wvb-Profile.md) removes the interpreter's fixed-offset assumption: it validates the WVB envelope, discovers all seven section payloads, checks the bounded function/export shape, executes result 29 at CPL3, and sends it to the separate init service. This is real verify-before-execute, user-space interpretation, hardware isolation, reduced authority, and cross-process IPC, but not yet runtime-supplied loading, a general interpreter, JIT, or scheduler. Version 17 remains useful historical evidence:
...
Candidate Decision 0094 advances firmware probe 25 to section-derived interpretation. Its 12,359-byte interpreter WVB validates the module envelope and seven ordered sections, derives the code payload rather than relying on one serialized address map, and still returns `29` for a second compiler-produced module whose longer name moves the code section. The 32-page RX image and observed 16 KiB stack make its current cost explicit. All 25 focused OS tests and four Windows pinned-QEMU scenarios pass; the normal 214,528-byte image has SHA-256 `99686e2b2484de156a01bbec0ece68e6a15af7229a477d9245ee4f3068ed4f5b`. Cross-host qualification remains pending.

The exact ownership and evidence limits are recorded through candidate [Decision 0094](Documents/Decisions/0094-First-Section-Derived-User-Space-Wvb-Profile.md), the [interpreter specification](Specifications/Windvale-Os-Bytecode-Interpreter.md), [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, memory, trap-frame, CPU-exception, admission, and shutdown specifications.

**Current focus:** admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes under qualified ABI 16; records were already supported. In parallel OS terms, cross-host-qualify probe 25, then move the now section-derived interpreter from an embedded byte array to one bounded runtime-supplied boot-resource or init-service input; a third runnable should be added only when it measures real scheduling pressure. Raw Windows/Linux memory authority plus OS page/descriptor/MSR construction, dispatcher mechanics, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters with named Windvale/WVA replacement seams.

**Latest fully cross-host-qualified OS evidence:** exact commit `190174a` passes GitHub's complete non-Fast verifier through probe 24 with all 67 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12. Pinned Windows QEMU supplies the exact four-scenario machine evidence; no Debian QEMU run is claimed. C# remains the native host bootstrap, OS page/descriptor/MSR constructor, platform memory authority, packaging adapter, and independent recovery implementation. See [Decisions 0090](Documents/Decisions/0090-First-In-Guest-Wvb-Admission.md) through [0094](Documents/Decisions/0094-First-Section-Derived-User-Space-Wvb-Profile.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
