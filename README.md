...
| Native compiler and host programs | 🚧 In progress | The constant slice is cross-host qualified; checked i32 add/subtract/multiply/negate and recoverable overflow traps share WVO/AOT and W^X JIT paths on Windows | Qualify arithmetic/traps on Debian, then add comparisons and control flow |
...
**Current focus:** qualify the checked native arithmetic/trap slice on Debian, then extend the same shared fragment path through comparisons and structured control flow.

**Latest qualified evidence:** the Windvale bytecode compiler reproduces its exact 599,868-byte artifact on Windows and Debian in 6,700,562,174 verified VM instructions; the first general constant-return native slice now agrees across interpreter, W^X JIT, and WVO-linked AOT on both hosts. All 49 tests pass and all 61 portable artifacts remain byte-identical across hosts. See the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md) and [development roadmap](Documents/Project/Roadmap.md) for the complete scope and remaining gates.
