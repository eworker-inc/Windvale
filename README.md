...
| Native compiler and host programs | 🚧 In progress | Qualified ABI 7 remains evidence; ABI 8 borrowed argument/file input and the checked-in Windvale WVB header inspector are implemented with cross-host qualification pending | Add the nominal values and dynamic text needed to advance full `wvdump` |
| Windvale operating system | 🚧 In progress | Qualified probe 9 remains evidence; probe 10 rebuilds the portable byte-decoding consumer through ABI 8 on the bounded kernel-owned stack | Add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
...
**Current focus:** qualify ABI 8's narrow hosted-input boundary and first Windvale-written WVB header inspector, then add the smallest nominal-value and dynamic-text slice needed to move full `wvdump` without retiring .NET prematurely.

**Latest qualified evidence:** exact candidate `8d375bf` passes zero-warning Windows and Debian Qualification with all 51 tests. Interpreter, W^X JIT, and linked WVO/AOT agree over ABI 7 borrowed bytes and unsigned operations; normalized contracts match and all 61 portable artifacts (7,752,612 bytes) remain byte-identical. Its 15,360-byte Windvale OS image has SHA-256 `ac92cd4759961c7a046ede49af8dce7626016fbcf8bb46e7d90027f5974bffa4` and emits the complete version-9 success transcript under pinned QEMU 11.0 after decoding borrowed bytes. The Windvale bytecode compiler still reproduces its exact 599,868-byte artifact in 6,700,562,174 verified VM instructions. See the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md) and [development roadmap](Documents/Project/Roadmap.md) for the complete scope and remaining gates.
