...
| Native compiler and host programs | 🚧 In progress | Qualified ABI 7 adds borrowed immutable bytes, checked slicing/reads, and `u8`/`u32` operations to the versioned context, authorized console, shared WVO/AOT, and W^X paths | Add narrow file/argument input, then advance a real Windvale tool |
| Windvale operating system | 🚧 In progress | Qualified probe 9 exits firmware, claims a bounded physical arena, and runs both special system `.wv` and ordinary portable-WVB-derived ABI-7 byte-decoding code on a kernel-owned stack | Add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
...
**Current focus:** add narrow file/argument input over ABI 7's qualified borrowed-byte representation, then move the smallest useful existing Windvale-written inspection tool through native execution without retiring .NET prematurely.
