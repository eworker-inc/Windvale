...
| Native compiler and host programs | 🚧 In progress | ABI 17 implements a verified 2,048-cell frame and clears the exact compiler's former 1,049-local blocker; ABI 16/context 7 and all 12 native service leaves remain cross-host qualified | Reduce the next measured lowered-value pressure at slot 2,049, then continue exact native preflight |
...
Implemented Decision 0100 advances firmware probe 30 to the first reclaimed and reused process root. After generation 1 executes and loses both aliases, kernel memory 7 accepts only its exact 42-page allocator tail, zeroes/releases it, and reallocates the same physical root. `WVPROC09` and `WVRES004` stamp the rebuilt client as generation 2, preserve historical grant count `2`, and reject stale generation-1 evidence. Init grants and receives twice; normal and contained-user-fault paths execute both client generations. All 25 focused OS tests and all four Windows pinned-QEMU scenarios pass; the normal 261,120-byte image is `5034c01a98f20344d96fa091fd9a55a303e72669d746a4b83df2900eed93992f`. Cross-host qualification is pending.

The exact ownership and evidence limits are recorded through qualified [Decisions 0096](Documents/Decisions/0096-First-Windvale-Init-Owned-Boot-Resource-Grant.md), [0097](Documents/Decisions/0097-First-Terminal-Resource-Borrow-Revocation.md), and [0098](Documents/Decisions/0098-First-Typed-Two-Resource-Lookup.md), implemented [Decision 0100](Documents/Decisions/0100-First-Reclaimed-And-Reused-Process-Root.md), the [interpreter specification](Specifications/Windvale-Os-Bytecode-Interpreter.md), [protected-process specification](Specifications/Windvale-Protected-Process.md), [kernel-memory specification](Specifications/Windvale-Kernel-Memory.md), [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, trap-frame, CPU-exception, admission, and shutdown specifications.
