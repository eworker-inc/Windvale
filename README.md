...
| Native compiler and host programs | 🚧 In progress | ABI 6 adds a versioned execution context and explicitly authorized static UTF-8 `console.write_line` service to the shared WVO/AOT and W^X paths | Add bounded bytes plus narrow file/argument services, then advance a real Windvale tool |
| Windvale operating system | 🚧 In progress | A deterministic UEFI/QEMU probe exits firmware, claims a bounded physical arena, and runs both special system `.wv` and ordinary portable-WVB-derived ABI-6 code on a kernel-owned stack | Add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
...
Portable Native-Wvb-Probe.wv -> verified WVB -> ABI-6 WVO -> linked UEFI image -> kernel-owned execution
```

**First shared native WVB in the OS:** [`Hello-World.wv`](Operating-System/Kernel/Hello-World.wv) still supplies the special system-profile diagnostics, while ordinary portable [`Native-Wvb-Probe.wv`](Operating-System/Kernel/Native-Wvb-Probe.wv) compiles to canonical verified WVB and then to the qualified ABI-6 WVO path. After the loader exits UEFI boot services, claims and clears a 64 KiB arena, exercises its allocator, copies the handoff, and switches to an 8 KiB kernel stack, the bridge constructs the versioned execution context and the portable module loops over immutable data, calls an internal function, and must return exact result 29 before boot continues. This is host-built AOT evidence; the guest does not yet load or verify WVB. The accepted version-8 transcript is:
