...
| Windvale operating system | 🚧 In progress | The pre-paging probe-20 baseline is qualified; current candidates retain context 7 under ABI 16, own the first W^X page-table root, and admit one embedded WVB through AOT Windvale code before its native form executes | Cross-host qualify the integrated candidates, then add one protected process/thread/capability/IPC boundary |
...
Candidate Decision 0090 advances the firmware probe to version 21. Its 174-byte WVB is checked by a 2,786-byte Windvale admission WVB before the separately compiled module may return 29. All 21 local OS tests and all three pinned-QEMU scenarios pass; the exact 47,104-byte images are `c3a07e1a6c8f162720a3dcd690fdb945bd862b360b26665c53e5be0642a87c38` (normal), `0bbc0b6eedbd21aef853a2233d3fa3dbaa9564eca36f3344067b1c9b240237fc` (invalid opcode), and `724907ffd0963f015003c91431b79b728109ed861de73f3c1b0bf5e7b58568b6` (general protection). Cross-host qualification remains pending.
...
**Current focus:** qualify [Decision 0089](Documents/Decisions/0089-Bounded-Native-Stack-Arguments.md)'s ABI-16 call convention, then admit the exact compiler's next measured blocker: `Compilerˉsourceˉwirˉcompileˉblock` has 1,049 locals against the current 1,024-slot native frame bound. The former eight-parameter blocker now passes; records were already supported. Native file output and the pre-paging probe-20 baseline remain qualified at `12e9e2e`; the integrated page-table and first in-guest WVB-admission extensions require exact cross-host qualification. The next OS architecture slice is one protected process/thread/capability/IPC boundary. Raw Windows/Linux memory authority plus OS table construction, descriptor publication, terminal policy, admission bridging, packaging, and linking remain inside narrow C# Stage 0 adapters.
