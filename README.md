...
The current Stage 0 toolchain includes the typed language, compiler, bytecode verifier, portable reference runtime, assembler, object model, linker, CLI, editor support, and focused Foundation modules. A Windvale-written compiler reproduces its canonical bytecode compiler exactly. Qualified Probe 32 runs the exact 815-byte cross-compiler `Function-Only.wv` fixture in protected user-space client generations: four functions and four scalar families execute 199 guest instructions to result `6`. Qualified [Decision 0105](Documents/Decisions/0105-Typed-Block-Scoped-Native-Value-Slots.md) advances the shared backend to ABI 18, compacting typed physical values across verified block boundaries while retaining canonical semantic IDs and the 2,048-cell limit. Exact implementation commit `484c228` passes all 68 Seed tests and all 25 OS tests on Windows and digest-pinned Debian 12 in GitHub [Verify run 30762156220](https://github.com/eworker-inc/Windvale/actions/runs/30762156220); all four Windows pinned-QEMU scenarios pass. [Decision 0108](Documents/Decisions/0108-Native-One-Byte-Construction.md) implements an ABI-19 candidate for exact one-byte construction and advances native compiler preflight to `Bytesˉfromˉu16ˉlittle`; cross-host qualification is pending. C# remains the bootstrap, host adapter, packaging path, and recovery implementation.
