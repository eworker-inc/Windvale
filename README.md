...
| Native compiler and host programs | 🚧 In progress | ABI 7 adds borrowed immutable bytes, checked slicing/reads, and `u8`/`u32` operations to the versioned context, authorized console, shared WVO/AOT, and W^X paths | Cross-host qualify ABI 7, add narrow file/argument input, then advance a real Windvale tool |
| Windvale operating system | 🚧 In progress | Development probe 9 exits firmware, claims a bounded physical arena, and runs both special system `.wv` and ordinary portable-WVB-derived ABI-7 byte-decoding code on a kernel-owned stack | Qualify probe 9, then add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
