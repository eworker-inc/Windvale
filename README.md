...
| Native compiler and host programs | 🚧 In progress | Qualified ABI 9 runs enums, immutable arena records, strict UTF-8, and the existing Windvale `wvdump` structural parser through JIT and WVO/AOT on Windows and Linux | Add bounded dynamic text, descriptor returns, void calls, and diagnostics for full native `wvdump` output |
| Windvale operating system | 🚧 In progress | Qualified probe 11 rebuilds the portable byte-decoding consumer through ABI 9/context 2 on the bounded kernel-owned stack | Add traps, an in-guest WVB verifier/runtime, clean shutdown, and Hyper-V evidence |
...
The accepted native destination keeps canonical WVB as the portable program identity while a Windvale-written execution stack supplies a verified interpreter, low-latency baseline JIT, optional measured optimizing tier, deterministic AOT, native memory management, and narrow Windows, Linux, and Windvale OS adapters. JIT and AOT share one native ABI, backend, and relocation model. [Decision 0059](Documents/Decisions/0059-First-Shared-Native-Wvb-Slice.md) through [Decision 0068](Documents/Decisions/0068-Bounded-Native-Nominal-Values-And-Wvdump-Structural-Core.md) qualify the Stage 0 seam through ABI 9, execution-owned nominal values, the native Windvale `wvdump` structural parser, and firmware probe 11. The larger direction, safety boundary, proposed WVA copy-and-patch tier, and exact .NET retirement conditions remain defined by [Decision 0057](Documents/Decisions/0057-Windvale-Native-Execution-And-Dotnet-Retirement.md) and the [native-execution architecture](Documents/Architecture/Native-Execution-And-Dotnet-Retirement.md); dynamic text and the complete hosted `wvdump` report path are not yet claimed as native.
