...
The current Stage 0 toolchain includes the typed language, compiler, bytecode verifier, portable reference runtime, assembler, object model, linker, CLI, editor support, and focused Foundation modules. A Windvale-written compiler reproduces its canonical bytecode compiler exactly. Cross-host-qualified [Decision 0165](Documents/Decisions/0165-Contained-Windvale-Service-Failure.md) adds Probe 36's fifth boot scenario: one malformed request faults the isolated Windvale init service, the kernel closes and scrubs its channel, wakes the waiting client with explicit peer loss, revokes its resources, and still shuts down cleanly. All 87 Seed and 38 OS tests pass on Windows and digest-pinned Debian; five pinned Windows QEMU scenarios pass. C# remains the bootstrap, host adapter, packaging path, and recovery implementation.
...
The separate [experimental WebAssembly target](Specifications/Windvale-WebAssembly.md) proves the first lower layer in Windvale source: portable `.wv` code revalidates canonical WVB and emits deterministic import-free Wasm with checked scalar/control, fixed-memory value, and bounded call support. A complete compiler-aligned Windvale verifier now proves structure, identities, typed executable flow, reachability, and exact stack contracts. A separate 404,340-byte Wasm-hosted Windvale interpreter executes verified scalar WVB plus bounded static data and descriptors, immutable text/bytes operations, strict UTF-8, invariant formatting, deterministic text quoting, SHA-256, records, enums, and typed defaults with dual instruction budgets, call-depth limits, and explicit allocation failures under Node.js without .NET. Measured compiler execution, browser-worker packaging, capability authorization, and cross-browser qualification remain before the editable playground can switch. The existing direct demo already runs without a .NET runtime.
