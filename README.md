...
| Windvale operating system | 🚧 In progress | Qualified probe 17 owns vector 6; candidate probe 20 composes ABI 15/context 7, normalized vectors 6/13, clean Q35 poweroff, and a kernel-owned W^X page-table root | Cross-host qualify probe 20, then boot the AOT Windvale verifier in-guest |
...
Candidate probe 20 composes Decision 0087's ABI-15/context-7 bridge with the WVA-authored Q35 shutdown and normalized vector-6/vector-13 entries, then allocates, constructs, and activates the first kernel-owned page-table root. Page zero is absent, ordinary low-1-GiB pages are writable/NX, and a fixed 64 KiB boot window is read-only/executable under `CR0.WP`; compiler-generated Windvale prints `paging=owned` only after CR3 readback succeeds. Three exact 22,016-byte images pass 20 local OS tests, 6 focused assembler tests, and real pinned QEMU: normal SHA-256 `392a2801bd8d8895bd9c34213336a69057c1ae81675269056c60b8c3e974ab01` exits cleanly with code 0, invalid opcode SHA-256 `aa610e6ac00ed43466a87521bb4cebb2934d0885acb960db8913f025ced9cce9` reports `(6, 0)`, and general protection SHA-256 `74632fcde4873f2d46e18b1b77c5cc8b495e83f0f750930e039da27dd67cd0ee` reports `(13, 0)`; both faults terminate with host code 3. Cross-host qualification is pending. This is one ring-0 identity map, not process isolation, page-fault recovery, or a general virtual-memory manager.

The exact ownership, allocator, implementation seam, target, and evidence limits are recorded in the decisions through candidate [Decision 0088](Documents/Decisions/0088-First-Kernel-Owned-X64-Page-Tables.md), the [native file-output specification](Specifications/Windvale-Native-File-Output.md), the [kernel-paging specification](Specifications/Windvale-Kernel-Paging.md), and the linked native, memory, trap-frame, CPU-exception, and shutdown specifications.

**Current focus:** qualify Decision 0087's bounded native Windows/Linux `file.write_bytes` path together with probe 20's composed Decisions 0085, 0086, and 0088 evidence, then admit the next exact compiler blocker: record-shaped parameters or locals in `Compilerˉbodyˉblockˉstepˉvalid` (`WVN2002`). File output is no longer the blocker in the exact 599,868-byte compiler WVB. The next OS step is an AOT Windvale decoder/verifier that admits one embedded canonical WVB inside the guest. Raw Windows/Linux memory authority plus OS table construction, descriptor publication, terminal policy, packaging, and linking remain inside narrow C# Stage 0 adapters and are not claimed as Windvale-native yet.
