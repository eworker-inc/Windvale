...
**Current focus:** qualify typed native blocks, comparisons, and forward control flow on Debian, then design execution-budgeted loops before shared-ABI calls and static data.

**Latest qualified evidence:** the Windvale bytecode compiler reproduces its exact 599,868-byte artifact on Windows and Debian in 6,700,562,174 verified VM instructions; constants and checked straight-line i32 arithmetic now agree across the interpreter, W^X JIT, and WVO-linked AOT paths on both hosts, including recoverable overflow traps. All 49 tests pass and all 61 portable artifacts remain byte-identical across hosts. See the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md) and [development roadmap](Documents/Project/Roadmap.md) for the complete scope and remaining gates.
