...
| Native compiler and host programs | 🚧 In progress | ABI 14/context 6 has all 11 native service leaves; Windvale assembles and supplies the live process-input leaves and now plans executable-image extent and service placement | Cross-host qualify the Windvale publication layout, then transfer explicit W^X lifetime ownership |
| Windvale operating system | 🚧 In progress | Qualified probe 16 runs ABI-14 WVB; probe 17 adds a candidate kernel-owned terminal invalid-opcode boundary | Qualify probe 17, then add in-guest verification/runtime, clean shutdown, and Hyper-V evidence |
...
Probe 17 is the implemented next candidate. It uses the first allocated 4 KiB page for a vector-6-only IDT, disables maskable interrupts, installs the gate from live `CS` and the complete terminal-handler address on the kernel stack, and adds `cpu-exceptions=armed` after Hello World on the normal path. A separately selected `invalid-opcode` image runs the same Main chain, executes `UD2`, and must terminate with exact `panic=invalid-opcode`, `vector=6`, `error-code=none`, and `status=panic` lines plus QEMU host code 3. This is a terminal CPU exception proof, not recovery, a general interrupt system, or a mapping of Windvale runtime traps to processor faults. Cross-host and final pinned-QEMU qualification remain pending; the transcript above remains the latest qualified image.

The exact ownership, allocator, implementation seam, target, and evidence limits are recorded in [Decision 0052](Documents/Decisions/0052-First-Kernel-Owned-Memory-Foundation.md), [Decision 0056](Documents/Decisions/0056-Windvale-Owned-Post-Memory-Evidence.md), [Decision 0064](Documents/Decisions/0064-First-Shared-Native-Wvb-In-Windvale-Os.md), [Decision 0065](Documents/Decisions/0065-Versioned-Native-Execution-Context-And-Console-Service.md), [Decision 0066](Documents/Decisions/0066-Borrowed-Bytes-And-Unsigned-Native-Values.md), [Decision 0067](Documents/Decisions/0067-Borrowed-Hosted-Input-And-First-Native-Wvb-Inspector.md), [Decision 0068](Documents/Decisions/0068-Bounded-Native-Nominal-Values-And-Wvdump-Structural-Core.md), [Decision 0069](Documents/Decisions/0069-Dynamic-Native-Text-And-Complete-Wvdump.md), [Decision 0070](Documents/Decisions/0070-First-Runtime-Native-Utf8-Service.md), [Decision 0071](Documents/Decisions/0071-Native-Text-Arena-And-Core-Text-Services.md), [Decision 0072](Documents/Decisions/0072-Final-Pure-Runtime-Native-Services.md), [Decision 0073](Documents/Decisions/0073-Native-Argument-Table-And-Process-Input-Services.md), [Decision 0074](Documents/Decisions/0074-Native-Windows-And-Linux-Output-Services.md), [Decision 0076](Documents/Decisions/0076-Native-Windows-And-Linux-File-Input.md), [Decision 0077](Documents/Decisions/0077-First-Windvale-Owned-Native-Stencil.md), [Decision 0078](Documents/Decisions/0078-Multi-Patch-Windvale-Native-Stencil.md), [Decision 0079](Documents/Decisions/0079-First-Windvale-Native-Stencil-Consumer.md), [Decision 0080](Documents/Decisions/0080-Native-Byte-Result-And-Live-Stencil-Consumption.md), candidate [Decision 0081](Documents/Decisions/0081-First-Terminal-X64-Cpu-Exception-Boundary.md), and [Decision 0082](Documents/Decisions/0082-Windvale-Owned-Native-Publication-Layout.md), the [native execution-context specification](Specifications/Windvale-Native-Execution-Context.md), the [native publication-plan specification](Specifications/Windvale-Native-Publication-Plan.md), the [WVA stencil specification](Specifications/Wva-Native-Stencil.md), the [kernel-memory specification](Specifications/Windvale-Kernel-Memory.md), the [kernel CPU-exception specification](Specifications/Windvale-Kernel-Exceptions.md), and the [kernel native-seam specification](Specifications/Windvale-Kernel-Native-Seam.md).

**Current focus:** cross-host qualify Decision 0082's retained Windvale publication planner. The implemented planner owns bounded executable-image extent and canonical runtime-service placement, while C# independently validates the response and retains the narrow Windows/Linux memory-protection, invocation, and lifetime adapter.

**Latest qualified evidence:** exact commit `f547af8` passes zero-warning Windows and Debian Qualification with all 62 tests and the complete CLI/reproduction gate. A bounded native descriptor entry returns the 75-byte Windvale-produced stencil bundle through exact static-data or committed-arena ownership, and the live process-input service path consumes its unchanged 5-byte and 70-byte leaves. The reference interpreter, W^X JIT, and linked WVO/AOT agree; normalized contracts and all 65 portable artifacts (7,822,433 bytes) match byte for byte; both hosts pass all 15 OS tests; and GitHub passes independent Windows/Linux verification. The unchanged 15,872-byte Windvale OS image has SHA-256 `206a036f8cbe3198544b6878bf52c80ef8d489c14d5437c6c7004ff1d6599504` and emits the complete version-16 success transcript under pinned QEMU. C# remains the native WVB loader/compiler/verifier, W^X and arena owner, invocation adapter, and independent recovery implementation. See [Decision 0080](Documents/Decisions/0080-Native-Byte-Result-And-Live-Stencil-Consumption.md), the [qualification evidence](Documents/Project/Seed-Verification-Evidence.md), and the [development roadmap](Documents/Project/Roadmap.md) for the complete scope.
